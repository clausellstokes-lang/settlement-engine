#!/bin/sh
# run-gate-901.sh — the §901 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base 04bb92d19. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
...
BASE=04bb92d19735594718e61b271d5bdf4ddfd4a2cc   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse 04bb92d19735594718e61b271d5bdf4ddfd4a2cc)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base 04bb92d19 — land the previous consist first"; exit 7; }
...
[ "$CARS" = "24" ] || { echo "REFUSED: expected 24 cars over $BASE, found $CARS"; exit 7; }
