...
const all = [...await C.loadStateLeaves(), ...await C.loadCausalLeaf(), ...await C.loadCrierVoice(), ...C.loadInFunctionNarratives()];
for (const e of all) for (const f of walkEntry(e, withEntryContext(base,{})).fails)
  if (f.klass==='C2') console.log(`${f.arm.padEnd(34)} ${e.id.slice(0,46).padEnd(46)} val=${f.value} :: ${e.text.slice(0,80)}`);
