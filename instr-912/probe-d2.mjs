...
const all = [...await C.loadCrierVoice(), ...C.loadInFunctionNarratives()];
for (const e of all) for (const f of walkEntry(e, withEntryContext(base,{})).fails)
  if (f.klass==='D') console.log(`${e.register} ${e.id.slice(0,44).padEnd(44)} names={${f.clause}} declared={${f.value}} :: ${e.text.slice(0,64)}`);
