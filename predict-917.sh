#!/bin/sh
# predict-917.sh <dock> — the READ-ONLY prediction pass at the §917 dock tip BEFORE any register door (E4: every register figure is derived,
...
echo "PREDICT_HEAD=$(git rev-parse HEAD) cars_over_8961388ce=$(git rev-list --count 8961388ce..HEAD) start $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')"
