#!/bin/sh
# predict-910.sh <dock> — the READ-ONLY prediction pass at the §910 dock tip BEFORE any register door (E4: every register figure is derived,
...
echo "PREDICT_HEAD=$(git rev-parse HEAD) cars_over_6ebe0ef3b=$(git rev-list --count 6ebe0ef3b..HEAD) start $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')"
...
echo "--- negative-assertion frozen rows for the touched files:"; grep -n -E "'tests/(soak-harness/[A-Za-z]+|domain/demographicsEnvelope)\.test\.jsx?'" tests/lint/negativeAssertionAnchor.walker.test.js | cut -c1-120 || echo "  (none — at 0 / never frozen)"
