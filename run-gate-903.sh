#!/bin/sh
# run-gate-903.sh — the §903 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base fd36f0298. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
# a background task reports its LAST command's exit, and a trailing printf once reported 0 over a red gate.
D=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad/laneLDEFAULT
BASE=fd36f0298b1ead15f2a80eff83dafb92114a7ea4   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse fd36f0298b1ead15f2a80eff83dafb92114a7ea4)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base fd36f0298 — land the previous consist first"; exit 7; }
...
[ "$CARS" = "4" ] || { echo "REFUSED: expected 4 cars over $BASE, found $CARS"; exit 7; }
