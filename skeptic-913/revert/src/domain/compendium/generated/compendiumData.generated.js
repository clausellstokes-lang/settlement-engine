...
export const COMPENDIUM_DATA = Object.freeze({
  ...
  "operations": {
    ...
    "entries": [
      ...
      {"opType":"assignNpc","label":"Settle a wanderer","description":"Settles a wandering named figure into one of the realm's settlements. The realm's rulers can overrule a standing banishment when the DM says so explicitly, and the receipt names the order it set aside. It can be undone with Undo the last ruling.","klass":"macro","slice":"npcVerbsSlice","targetScope":"campaign","receiptRef":"npcRulings-entry(npc_assignment)","undoToken":"undoLastNpcVerb"},
      {"opType":"killNpc","label":"Record a death","description":"Records that a named figure in the realm's register of wanderers has died, striking them from it and ending any order standing against them. This is the only death the system ever writes, and it can be undone with Undo the last ruling.","klass":"macro","slice":"npcVerbsSlice","targetScope":"campaign","receiptRef":"npcRulings-entry(npc_death)","undoToken":"undoLastNpcVerb"},
  ...
  "faith": {
    ...
    "axes": [
      {"id":"alignment","label":"Alignment","lines":["Good-aligned worship exposes corruption and favors incorruptible successors","Evil-aligned worship lets corruption take root even without organized crime"]},
      {"id":"law","label":"Law","lines":["A lawful creed strengthens law and order","A chaotic creed erodes order and tolerates corruption"]},
      {"id":"rank","label":"Rank","lines":["A major orthodoxy anchors religious authority (a lift of 18)","Minor worship lends modest religious authority (a lift of 10)","A cult remains a fringe following with little authority (a lift of 5)"]},
      {"id":"temperament","label":"Temperament","derived":true,"lines":["A warlike creed raises the realm's aggression","A peacelike creed tempers the realm's aggression"]}
    ]
  },
  "terrain": [
  ...
  "presets": [
    ...
    {"id":"realistic_regional","label":"Realistic Regional","isDefault":true,"summary":"The default. The region evolves at a measured, realistic pace.","intensity":"conservative","autonomy":"routine","autonomyLabel":"routine acts run, big moves come to you","lights":["supplyWebWarfareEnabled","momentumEnabled","upswingArcsEnabled","resourceDynamicsEnabled","constructiveFlowsEnabled","peaceEngineEnabled","navalEnabled","interventionEnabled","settlementLifecycleEnabled","distancePricedNewsEnabled","reframeEnabled","spatialConsequenceEnabled","provenanceLedgerEnabled","urbanFabricEnabled","npcGrowthEnabled","npcLadderEnabled","traditionsEnabled","roadsEnabled","disastersEnabled"]},
