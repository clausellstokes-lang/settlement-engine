...
const FLAG_DESCRIPTIONS = Object.freeze({
  ...
  generationWorker: "Run settlement generation in a Web Worker so the tab never freezes while a town is built. The worker and the in-thread fallback call ONE core over cloned inputs, so the world is byte-identical either way; set false (the default) to keep generating on the main thread.",
