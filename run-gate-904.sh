#!/bin/sh
# run-gate-904.sh — the §904 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base dd5f13218. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
...
BASE=dd5f1321825b58fed2db54e9473440a310195eed   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse dd5f1321825b58fed2db54e9473440a310195eed)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base dd5f13218 — land the previous consist first"; exit 7; }
