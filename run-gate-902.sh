#!/bin/sh
# run-gate-902.sh — the §902 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base b0cbc67a1. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
...
BASE=b0cbc67a15baf0997028ab501c7ee09565f93980   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse b0cbc67a15baf0997028ab501c7ee09565f93980)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base b0cbc67a1 — land the previous consist first"; exit 7; }
