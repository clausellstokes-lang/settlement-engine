#!/bin/sh
# run-gate-910.sh — the §910 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base 6ebe0ef3b. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
# a background task reports its LAST command's exit, and a trailing printf once reported 0 over a red gate.
D=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad/laneB6
BASE=6ebe0ef3bd2e7fa0f5d0ede5c93780ecfbf3aada   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse 6ebe0ef3bd2e7fa0f5d0ede5c93780ecfbf3aada)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base 6ebe0ef3b — land the previous consist first"; exit 7; }
