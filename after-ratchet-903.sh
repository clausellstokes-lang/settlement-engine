#!/bin/sh
# after-ratchet-903.sh <expected-totalFiles> <expected-entries> — the §903 version (derived from 901; the CAPSULE step is ACTIVE again — the OSR CLI is open at rung 18): the totals car (guarded on TRUE_EXIT,
...
# re-stamp of the kit at the FINAL car count, then the train seal. No owed-ledger car at §903 (entries predicted 3). The gate is
# launched SEPARATELY (run-gate-903.sh). Every exit captured; refuses on anything unexpected. POSIX sh.
SC=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad
D=$SC/laneLDEFAULT; LOG=$SC/ratchet-903.run.log; BASE=fd36f0298b1ead15f2a80eff83dafb92114a7ea4; SEAL=ldefault-2026-09-06
...
echo "--- capsule car (ACTIVE at rung 18 — the OSR CLI is open):"; sh scripts/gate-mutex.sh --run -- node scripts/base-state-capsule.mjs --runtime-tests=$TT > $SC/capsule-903.log 2>&1; CE=$?; echo "CAPSULE_EXIT=$CE"; tail -3 $SC/capsule-903.log | cut -c1-160
[ "$CE" = "0" ] || { echo "REFUSED: the capsule did not regenerate — read capsule-903.log"; exit 7; }
CP=$(git status --porcelain -uall); echo "capsule porcelain=[$(echo "$CP" | tr '\n' ' ')]"
if [ -n "$CP" ]; then git add docs/implementation/BASE_STATE.json && git commit -q -m "Register (capsule car): the base-state capsule regenerates at the §903 tip (rung 18 still; the OSR input drift re-frozen by the register car)
...
echo "--- re-stamp the kit at $N cars:"; python3 $SC/mk-landing-kit.py 903 laneLDEFAULT $BASE landing-ldefault-2026-09-06 $N 2>&1 | tail -2
grep -q "^GATE_CARS=\|cars=$N\|EXPECT.*$N" $SC/run-gate-903.sh || grep -n "$N" $SC/run-gate-903.sh | head -2
git update-ref refs/preserve/$SEAL "$(git rev-parse HEAD)" && echo "SEALED $SEAL = $(git rev-parse --short refs/preserve/$SEAL)"
echo "NEXT: launch run-gate-903.sh in the background (quiet law; ~20 min; NO lane vitest meanwhile), then after-cas-903.sh, then fill texts-903 (__CARS__=$N __ENTRIES__ __TESTS__ __TOTALS__) and collect."
exit 0
