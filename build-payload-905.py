# -*- coding: utf-8 -*-
# build-payload-905.py — fills payload-905.template.json's ROW/CARD/TAIL placeholders and msg-905.txt from texts-905.draft.md
...
# Leaves __CAS_SHA__ and __TESTS__ for after-cas-905.sh (the CAS step). Refuses on any other surviving placeholder.
import io,json,re
SC='/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad/'
raw=io.open(SC+'texts-905.draft.md',encoding='utf-8').read(); assert '\u27e6' not in raw, 'unfilled token in texts-905.draft.md: '+str(sorted(set(re.findall('\u27e6[A-Z]+\u27e7',raw)))); d=raw.split('\n')
...
assert row[0].startswith('§905 · **'), row[0][:60]
m=re.match(r'§905 · \*\*(.+?)\*\*\s*(.*)$', row[0]); assert m, row[0][:80]
ROW='**'+m.group(1).strip()+'**'
ROW_BODY=(m.group(2).strip()+'\n\n' if m.group(2).strip() else '')+'\n\n'.join(row[1:])
card=[l for l in section('## CARD','## TAIL')]
h=[l for l in card if l.startswith('## ⭐⭐⭐⭐⭐ PICKUP AT §905')]; assert len(h)==1
m=re.match(r'## ⭐⭐⭐⭐⭐ PICKUP AT §905 — \*\*(.+?)\*\*\s*(.*)$', h[0]); assert m, h[0][:80]
CARD=m.group(1).strip(); rest=m.group(2).strip()
body=card[card.index(h[0])+1:]; CARD_BODY=(rest+'\n\n' if rest else '')+'\n'.join(body).strip()+'\n'
tail=section('## TAIL','## NEVER-MATCH')
th=[l for l in tail if l.startswith('## §905 — ')]; assert len(th)==1
m=re.match(r'## §905 — (.+?) \(SEAT: Fable 5\.1 — validated\)\s*$', th[0]); assert m, th[0]
TAIL=m.group(1).strip(); TAIL_BODY='\n'.join(tail[tail.index(th[0])+1:]).strip()
t=io.open(SC+'payload-905.template.json',encoding='utf-8').read()
...
io.open(SC+'payload-905.template.json','w',encoding='utf-8').write(t)
print('payload-905.template.json filled; row %d chars, card %d, tail %d; remaining placeholders %s'%(len(p['odq_rows'][0]),len(p['card_new_block']),len(p['frq_tail']),left))
msg=io.open(SC+'msg-905.txt',encoding='utf-8').read()
m=io.open(SC+'texts-905.msg.md',encoding='utf-8').read(); assert '\u27e6' not in m, 'unfilled token in texts-905.msg.md'
def part(name):
...
io.open(SC+'msg-905.txt','w',encoding='utf-8').write(msg)
print('msg-905.txt filled: subject %d chars, body %d chars'%(len(subject),len(bodytxt)))
