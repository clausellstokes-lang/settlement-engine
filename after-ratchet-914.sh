#!/bin/sh
# after-ratchet-914.sh <expected-totalFiles> <expected-entries> — the §914 version (from after-ratchet-908.sh by substitution 2026-09-07 03:08; the CAPSULE step is ACTIVE again — the OSR CLI is open at rung 18): the totals car (guarded on TRUE_EXIT,
...
# re-stamp of the kit at the FINAL car count, then the train seal. No owed-ledger car at §914 (entries predicted 3). The gate is
# launched SEPARATELY (run-gate-914.sh). Every exit captured; refuses on anything unexpected. POSIX sh.
SC=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/4e3d2f70-f45f-4e14-b571-514c339cfa17/scratchpad/kit
D=$SC/laneINSTR2; LOG=$SC/ratchet-914.run.log; BASE=f3ab08f5194201d19f99f8905068783b62fbfaf8; SEAL=instr-2026-09-08
...
echo "--- capsule car (ACTIVE at rung 18 — the OSR CLI is open):"; sh scripts/gate-mutex.sh --run -- node scripts/base-state-capsule.mjs --runtime-tests=$TT > $SC/capsule-914.log 2>&1; CE=$?; echo "CAPSULE_EXIT=$CE"; tail -3 $SC/capsule-914.log | cut -c1-160
[ "$CE" = "0" ] || { echo "REFUSED: the capsule did not regenerate — read capsule-914.log"; exit 7; }
CP=$(git status --porcelain -uall); echo "capsule porcelain=[$(echo "$CP" | tr '\n' ' ')]"
if [ -n "$CP" ]; then git add docs/implementation/BASE_STATE.json && git commit -q -m "Register (capsule car): the base-state capsule regenerates at the §914 tip (rung 18 still; no OSR movement at this landing)
...
echo "--- re-stamp the kit at $N cars:"; python3 $SC/mk-landing-kit.py 914 laneINSTR2 $BASE landing-instr-2026-09-08 $N 2>&1 | tail -2
grep -q "^GATE_CARS=\|cars=$N\|EXPECT.*$N" $SC/run-gate-914.sh || grep -n "$N" $SC/run-gate-914.sh | head -2
git update-ref refs/preserve/$SEAL "$(git rev-parse HEAD)" && echo "SEALED $SEAL = $(git rev-parse --short refs/preserve/$SEAL)"
echo "NEXT: launch run-gate-914.sh in the background (quiet law; ~20 min; NO lane vitest meanwhile), then after-cas-914.sh, then fill texts-914 (__CARS__=$N __ENTRIES__ __TESTS__ __TOTALS__) and collect."
exit 0
