...
console.log('run'.padEnd(20),'yrs setts meanRealmPop ms/settYear  us/soul-year');
for(const [k,f] of list){const j=JSON.parse(fs.readFileSync(f,'utf8'));
 ...
 if(Array.isArray(j.yearlyPopulations)&&j.yearlyPopulations.length){
   ...
   const yl=j.behavioral?.yearly||[]; const vals=[];
   for(const e of yl){ const sv=e?.stateVectors; if(Array.isArray(sv)) vals.push(sv.reduce((a,s)=>a+(Number(s?.population)||0),0)); }
 ...
 console.log(k.padEnd(20),String(j.years).padStart(4),String(j.settlements).padStart(4),String(mean?mean.toFixed(0):'n/a').padStart(11),mspsy.toFixed(1).padStart(11),(perSoul?perSoul.toFixed(2):'n/a').padStart(13));
}
