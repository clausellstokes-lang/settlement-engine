...
// Labels + descriptions only; polarity comes from bands.js DIM_POLARITY (the
// single source the band word is oriented by), so print and screen cannot drift.
const DIM_META = {
  resilience:       { label: 'Resilience',        desc: 'Can the settlement absorb shocks?' },
      ...
      {rows.map(row => {
        ...
        return (
          <View key={row.name} style={{ flexDirection: 'row', alignItems: 'baseline', marginBottom: 2 }}>
            ...
            {/* The POLARITY-CORRECT word, not the raw band. A lower-is-better
                variable bands off the inverted score, so criminal_opportunity at
                maximal crime carries band 'collapsed' — which a reader takes to
