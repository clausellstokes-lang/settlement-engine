/**
 * scripts/generate-k2.mjs -- K-2 EXHIBIT generator: the condition-mapping layer, made visible.
 *
 * Emits, from the pure dormant K-2 drift kernel (src/domain/townMap/arch/conditionGrammar.js +
 * settlementDress.js over the frozen K-4 contract -- imported by NOTHING shipped, closure Delta 0),
 * into public/landing-maps/k2-exhibit/:
 *   1. THE DRIFT MATRIX -- the SAME cathedral rendered under FOUR condition vectors (prosperous-good,
 *      war-scarred, corrupt-decayed, evil-drift) as plate-cathedral-{scene}.png. The GEOMETRY is
 *      invariant (K-2 selects the dress; K-4 is the single writer of drift into geometry); the drift
 *      shows through the DRESS -- dressRoleAlbedo lowers each scene's building dress into the plate's
 *      role->albedo read (the reskin seam), so marble/perpendicular prosperity, stone/ruin war-scar,
 *      and neglect-weathered corruption all read on one shape without touching a vertex.
 *   2. cathedral.glb -- the shared cathedral geometry (one GLB; the 4 scenes differ only in dress).
 *   3. THE LEGIBLE TONE GATE (owed from K-3, owner veto) -- tone-{macabre,beneficent,neutral}.png +
 *      matching GLBs: the statuary vocabulary each alignment band selects, rendered CLOSE UP at a scale
 *      where the grotesque / gargoyle / skull (evil drift) and the robed-figure / finial / crocket
 *      (good drift) forms are actually READABLE -- the tone judgement the whole-building plate could not
 *      show. Every form is an ABSTRACT architectural piece (K-3 kit), never a named figure.
 *   4. evil-chapel.glb + plate-evil-chapel.png -- the fixed macabre chapel in context (the tone-gate
 *      building, with the roof-underside striping cured: spire embedded below the tower top).
 *   5. index.html -- a self-contained raw-WebGL2 viewer (orbit/zoom) over the samplers + chapel + shape.
 *
 * DETERMINISM: every mesh/GLB/plate is pure (no clock, no rng); the script double-builds + asserts byte
 * identity before writing. Live WebGL pixels are device-dependent + NON-golden. Usage:
 *   node scripts/generate-k2.mjs [--check]
 */
...
import { STATUARY_VOCAB } from '../src/domain/townMap/arch/conditionGrammar.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const OUT_DIR = join(__dirname, '..', 'public', 'landing-maps', 'k2-exhibit');
...
// ── THE FOUR CONDITION SCENES (raw state -> validated conditionVector) ────────────────────────────
// Each is a corner of the drift space the owner must taste-check. corruptionCovert is omitted (the
// contract forces it to EXACTLY 0 -- the security invariant; a covert value would fail-close the build).
const SCENES = Object.freeze([
  ['prosperous-good', { prosperity: 0.90, patronAlignGood: 0.85, patronAlignLaw: 0.80, moral: 0.85, terrain: 0.75, warScar: 0.00, corruptionRevealed: 0.00, legitimacy: 0.85, patronEmblem: 3, historyMark: 1 }],
  ['war-scarred', { prosperity: 0.45, patronAlignGood: 0.60, patronAlignLaw: 0.50, moral: 0.50, terrain: 0.50, warScar: 0.80, corruptionRevealed: 0.10, legitimacy: 0.40, patronEmblem: 12, historyMark: 7 }],
  ['corrupt-decayed', { prosperity: 0.35, patronAlignGood: 0.45, patronAlignLaw: 0.40, moral: 0.35, terrain: 0.40, warScar: 0.15, corruptionRevealed: 0.80, legitimacy: 0.25, patronEmblem: 20, historyMark: 4 }],
  ['evil-drift', { prosperity: 0.60, patronAlignGood: 0.15, patronAlignLaw: 0.20, moral: 0.20, terrain: 0.55, warScar: 0.10, corruptionRevealed: 0.40, legitimacy: 0.30, patronEmblem: 33, historyMark: 9 }],
]);

const SEED = 'k2-demo', ANCHOR = 'cathedral-1';

/** double-build a ruleset+tier -> { mesh, glb }, asserting byte identity. */
function tierGlb(ruleset, tier, gen) {
  const a = buildArchMesh(ruleset, { seedId: 'k2', tier });
  const b = buildArchMesh(ruleset, { seedId: 'k2', tier });
  if (!eqF(a.positions, b.positions) || !eqF(a.normals, b.normals) || !eqF(a.indices, b.indices) || !eqF(a.ao, b.ao)) {
    console.error(`[k2] NON-DETERMINISTIC mesh: ${gen} tier ${tier}. Aborting.`); process.exit(1);
  }
  const geo = { positions: a.positions, normals: a.normals, indices: a.indices, vertexCount: a.vertexCount, min: a.min, max: a.max };
  return { mesh: a, glb: encodeGlb(geo, { ao: a.ao, generator: gen }) };
}

/**
 * A STATUARY SAMPLER ruleset: place each kit asset in a large box in a row, so a close-up plate reads
 * the iconography vocabulary a band selects. Exhibit-only (not a shipped ruleset). Roles are chosen so
 * every piece reads as carved stone. @param {ReadonlyArray<string>} assets @returns {object}
 */
function samplerRuleset(assets) {
  const CW = 130, CH = 230, CD = 130, GAP = 40;               // per-cell box + gap
  const roleOf = { grotesque: 'relief', gargoyle: 'corbel', skull: 'relief', robedFigure: 'relief', finial: 'pinnacleStone', crocket: 'pinnacleStone' };
  const symbols = ['row'];
  const rules = { row: [{ op: 'defer', byTier: { default: [] } }] };
  const slots = [];
  for (let i = 0; i < assets.length; i++) {
    const sym = `slot${i}`; symbols.push(sym);
    const x0 = i * (CW + GAP);
    slots.push({ sym, scope: { origin: [x0, 0, 0], frameRef: { frameIndex: 0, reflect: 0 }, size: [CW, CH, CD] } });
    rules[sym] = [{ op: 'instance', asset: assets[i], role: roleOf[assets[i]] || 'relief' }];
  }
  rules.row[0].byTier.default = slots;
  const totalW = assets.length * (CW + GAP);
  return Object.freeze({
    name: 'statuary-sampler', symbols,
    axiom: { sym: 'row', scope: { origin: [0, 0, 0], frameRef: { frameIndex: 0, reflect: 0 }, size: [totalW, CH, CD] }, attrs: { materialRole: 'relief', params: {} } },
    rules,
  });
}
...
<title>K-2 &mdash; condition drift + the legible tone gate</title>
<style>
...
<canvas id="c"></canvas>
<div id="info" class="panel"><h1>K-2 condition drift &mdash; the legible tone gate</h1>
<p>The statuary vocabulary each alignment band selects, close up: macabre (grotesque / gargoyle / skull) for evil drift, beneficent (robed figure / finial / crocket) for good. Every form is an abstract architectural piece, never a face. Drag to orbit &middot; wheel to zoom.</p>
<p id="stats"></p></div>
...
function build() {
  ...
  // (1) THE DRIFT MATRIX -- one cathedral, four dresses (geometry invariant; drift shows through the skin)
  const cath = buildArchMesh(cathedralRuleset(), { seedId: 'k2', tier: 2 });
  const cath2 = buildArchMesh(cathedralRuleset(), { seedId: 'k2', tier: 2 });
  if (!eqF(cath.positions, cath2.positions)) { console.error('[k2] cathedral non-deterministic. Aborting.'); process.exit(1); }
  for (const [name, raw] of SCENES) {
    const cv = makeConditionVector(raw);
    const dress = resolveSettlementDress(SEED, ANCHOR, cv);
    const bldg = driftBuildingDress(dress, 'sacred', cv);
    dressLog[name] = { settlement: { materialTier: dress.materialTier, traceryFamily: dress.traceryFamily, ornamentOrder: dress.ornamentOrder, statuaryMode: dress.statuaryMode, weathering: dress.weathering, damageState: dress.damageState, reliefMotif: dress.reliefMotif }, cathedral: { ornamentDensity: bldg.ornamentDensity, materialTier: bldg.materialTier, weathering: bldg.weathering, damageState: bldg.damageState, statuaryVocab: bldg.statuaryVocab } };
    files.push({ file: `plate-cathedral-${name}.png`, data: plate(cath, 'axonNW', { roleAlbedo: dressRoleAlbedo(bldg) }) });
  }
  files.push({ file: 'cathedral.glb', data: tierGlb(cathedralRuleset(), 2, 'k2-cathedral').glb });

  // (3) THE LEGIBLE TONE GATE -- the statuary vocabularies, close up + as GLBs
  const samplers = [
    ['macabre', STATUARY_VOCAB.macabre],
    ['beneficent', STATUARY_VOCAB.beneficent],
    ['neutral', STATUARY_VOCAB.neutral],
  ];
  const sampMeshes = {};
  for (const [name, vocab] of samplers) {
    const g = tierGlb(samplerRuleset(vocab), 2, `k2-tone-${name}`);
    sampMeshes[name] = g.mesh;
    files.push({ file: `tone-${name}.png`, data: plate(g.mesh, 'axonNW') });
    files.push({ file: `statuary-${name}.glb`, data: g.glb });
  }

  // (4) the fixed macabre chapel in context (striping cured)
  const chapel = tierGlb(evilChapelRuleset(), 2, 'k2-chapel');
  files.push({ file: 'evil-chapel.glb', data: chapel.glb });
  files.push({ file: 'plate-evil-chapel.png', data: plate(chapel.mesh, 'axonNW') });

  // (5) the viewer
  files.push({ file: 'index.html', data: viewerHtml([
    { name: 'macabre (evil drift)', glb: sampMeshesGlb(sampMeshes.macabre, 'macabre'), mesh: sampMeshes.macabre },
    { name: 'beneficent (good drift)', glb: sampMeshesGlb(sampMeshes.beneficent, 'beneficent'), mesh: sampMeshes.beneficent },
    { name: 'neutral', glb: sampMeshesGlb(sampMeshes.neutral, 'neutral'), mesh: sampMeshes.neutral },
    { name: 'evil chapel', glb: chapel.glb, mesh: chapel.mesh },
    { name: 'cathedral', glb: encodeGlb({ positions: cath.positions, normals: cath.normals, indices: cath.indices, vertexCount: cath.vertexCount, min: cath.min, max: cath.max }, { ao: cath.ao, generator: 'k2-cathedral' }), mesh: cath },
  ]) });

  return { files, stats: { scenes: SCENES.length, samplers: samplers.length, dress: dressLog } };
}

/** encode a sampler mesh to a GLB (for the viewer). @param {object} mesh @param {string} name @returns {Uint8Array} */
function sampMeshesGlb(mesh, name) {
  return encodeGlb({ positions: mesh.positions, normals: mesh.normals, indices: mesh.indices, vertexCount: mesh.vertexCount, min: mesh.min, max: mesh.max }, { ao: mesh.ao, generator: `k2-tone-${name}` });
}
...
function main() {
  ...
  if (checkOnly) {
    ...
    for (const em of files) {
      ...
      if (!existsSync(path)) { console.error(`[k2] MISSING: ${path}`); stale++; continue; }
      if (typeof em.data === 'string') { if (readFileSync(path, 'utf8') !== em.data) { console.error(`[k2] STALE: ${path}`); stale++; } }
      else if (!bytesEqual(readFileSync(path), em.data)) { console.error(`[k2] STALE: ${path}`); stale++; }
    }
    if (stale > 0) process.exit(1);
    console.log('[k2] check OK -- committed exhibit matches a fresh deterministic build.');
    return;
  }
  ...
  for (const em of files) {
    ...
    console.log(`[k2] emitted -> ${path} (${n} bytes)`);
  }
