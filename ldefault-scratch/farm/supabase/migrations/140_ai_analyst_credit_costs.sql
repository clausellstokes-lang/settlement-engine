-- ────────────────────────────────────────────────────────────────────────────
-- 140_ai_analyst_credit_costs.sql — Surveyor S1 commercial: task-priced managed
-- credits for the analyst + the AI-prose brief layer.
--
-- WHY THIS EXISTS
--   DESIGN_AI_CONTROL_SURFACE §4 (recorded decision): managed credits are TASK-PRICED
--   (interpret-session, construct-settlement, realm, brief, analysis, …). The analyst
--   spends the 'analysis' feature; the AI-prose brief layer (later) spends 'brief'. This
--   extends the spend_credits CASE with those two features so the ai-analyst edge
--   function's spend_credits({feature:'analysis'}) resolves a cost instead of raising
--   'unknown feature'.
--
--   PRICING IS PROVISIONAL (vetoable). Final Surveyor pricing is an OWNER-QUEUED
--   decision (design §6). analysis=3 (≈ narrative), brief=4 (≈ dailyLife) mirror the
--   existing repriced schedule; the owner may re-tune. Kept in lockstep with
--   src/config/pricing.js NEW_AI_COSTS by the pricing contract test.
--
-- WHAT
--   Re-defines spend_credits verbatim from 131 (the current definition) with TWO new
--   CASE arms. All other behaviour — config-first per-profile cost, the elevated
--   bypass, grant allocation, the 057 active-account gate — is unchanged.
--   CREATE OR REPLACE preserves the existing grants (authenticated).
--
-- Depends on: 131 (current spend_credits), 114 (ai_credit_costs config), 057
--   (account_is_active), 007 (credit_ledger, get_credit_balance). Re-runnable.
-- @rollback: re-apply 131's spend_credits definition (drop the two CASE arms).
-- ────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  if cfg_cost is not null then
    ...
    -- 057 CASE block + the Surveyor S1 task-priced features (analysis, brief).
    cost := case feature
