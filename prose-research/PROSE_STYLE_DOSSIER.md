...
Chair: Fable 5.1, 2026-09-05. Status: FIRST PASS ONLY — SUPERSEDED BY THE EXHAUSTIVE SWEEP (owner directive 09-05). Citations verified against sources by an Opus verifier (`citation-verification.md`; nineteen corrections applied). ⛔ THE RULES R1–R12 (§4) ARE UNSAFE AS WRITTEN — the taste sample built on them FAILED adversarial verification (`taste-sample-refutation.md`: 27 of 31 changes fail; R4 is two tests under one name and was inverted; R1 contradicts the corpus register law §0b; R-DST-B is missing; §0d's count/duration vocabularies are in no rule). §3's measurement covers THREE homes only — the Herald was NOT measured. Nothing in the corpus changed.
...
1. REGISTER IS EARNED, NOT WORN. Passages open in plain modern syntax and modulate upward only when the matter turns ancient or magical (Rosebury: "begins with essentially plain syntax"); Rosebury finds plain modern English predominant; Kullmann & Siepmann applied corpus linguistics to the question quantitatively. Rosebury's defence against Stimpson: the disputed sentences are ones "Ernest Hemingway could have written".
2. ARCHAISM IS RARE AND LOAD-BEARING. "alas", "thou", "whither" sparingly; the stronger device is a familiar word carrying an old overtone ("deem", "moot", "wight") (Walker). Walker: Tolkien goes furthest with Old English syntax for the Rohirrim; Dwarves and Elves carry archaism too, the hobbits almost none.
3. REGISTER BELONGS TO THE SPEAKER. At the Council: Elrond formal and archaic ("hither") and Glóin terse apposition are Shippey's observations; Bilbo modern and clipped ("Hullo") and Frodo one notch formal ("should like") are the "Tolkien: Medieval and Modern" blogger's own reading, which notes Shippey does not treat those two. The hobbit voice mediates between the modern reader and the heroic register.
...
6. The recorded criticisms name the risk of the elevated register: "wrenches syntax" (Stimpson), comforting and unchallenging (Wikipedia's paraphrase of Moorcock, whose own quoted phrase is "conservative misanthropism").
...
3. THE SENSORY CONCRETE NOUN carries atmosphere ("word paintings": orange trees, purple olives, cool linen) (Books & Boots). The line often attributed to Martin, books "richly textured and full of sensory detail", is found only on aggregator pages (Goodreads marks its quotes unverified); treat it as unconfirmed.
4. ONE TIGHT VIEWPOINT: "a limited but very tight third person" (Martin, Adria's News interview 2012, read via the Wayback Machine because the live domain is dead) — what is noticed is what that person would notice.
5. TERSE CHARGED DIALOGUE; deliberate repetition for atmosphere.
6. THE FLAWS TO AVOID: stock phrases repeated to tic ("mummer's farce", "jape" — the Erstwhile Philistine); the medievalisms that become mannerisms ("much and more", "oft", "whilst", dropped -ly) and the tonal breaks when modern idiom sits beside archaism (both Books & Boots); over-description and sentimental cliché (the Philistine).
...
1. THERE IS NO PUBLISHED WOTC 5E STYLE GUIDE (Half-Cover); conventions are inferred from the books.
2. LENGTH IS THE LAW. Jesse Decker and David Noonan (WotC, a GenCon 2005 seminar as Dyson reports it) watched tables and prescribed about two sentences of boxed text — an observation and a house rule, not a measured threshold; a paragraph break in boxed text is "already too long by half" (DM's Workshop); "1-2 sentences is the sweet spot", three the maximum, said of an NPC's first-sight description (Alexandrian); cut a quarter of what you wrote (Sly Flourish).
3. FACT ORDER IS THE READER'S EYE ORDER: what is seen first, exits, counts, the obvious object. Players missed room dimensions, exits, monster counts, and "the lady in the room is a corpse?" (Dyson).
4. NEVER PRESUME THE READER'S ACTION ("upon approaching…"); second person present for read-aloud; conversational not novelistic.
5. ONE MAIN THING per passage, decided before writing (Sly Flourish); sensory detail is allowed but "it is the facts that should drive the rest of your prose".
...
"A plain language is the noblest of all. It is also the most difficult." The Poughkeepsie style is "not really simple, but flat … not really clear, but inexact". Her test: she changed four words in a passage to relocate its scene to Washington and showed that nothing else had to move — the fantasy was never in the language. She praises plainness with rhythmic care and distance without costume; she condemns archaism as costume, purple ornament, borrowed diction. Steering the Craft: "the sound of the language is where it all begins" (LitHub excerpt); vary sentence length; adjectives and adverbs "rich and good and fattening", do not overindulge (a quotation page, secondary), and chapter five's "chastity" exercise (a reader's blog, secondary).
...
1. THE STRONGEST TELL IS STRUCTURAL, NOT LEXICAL. StoryScope (Russell et al. 2026) reaches 93.2% macro-F1 from discourse features alone; stripping "tapestry" and em dashes leaves the discriminating signal intact. Chakrabarty, Laban & Wu (CHI 2025): eighteen professional writers' edits to 1,057 LLM paragraphs were awkward word choice 28%, poor sentence structure 20%, redundant exposition 18%, cliché 17%; purple prose, LACK OF SPECIFICITY and tense inconsistency are the remaining categories, unranked in the paper.
2. THE TASTE INVERSION. Experts rated human stories higher (+1.25); non-experts and LLM judges rated AI stories higher (two estimates, +1.19 and +1.85) (Ismayilzada et al.). A gate built on a model's self-assessment grades in the wrong direction; the owner's eye and a measured rubric are the only honest judges.
3. BETTER MODELS ARE MORE UNIFORM, NOT LESS (O'Sullivan, Burrows' Delta: GPT-4 clusters more tightly than GPT-3.5; this claim rests on that one study). Separately, Doshi & Hauser (Science Advances 2024) show that giving human writers AI ideas made their stories more similar to one another (by roughly a tenth of the human-only similarity range; more shared ideas, more similarity) — homogenisation of a POOL, not a claim about model quality. A corpus written by one hand in one session will converge on one cadence unless the rubric forces spread.
Lexical studies (Kobak et al. 2025; Juzek & Ward 2025; "Word Overuse and Alignment" 2025): of 379 excess style words 66% are VERBS and 14% adjectives (Kobak); Juzek & Ward's 21 focal items are INFLECTED FORMS, not lemmas — delves, delved, delving, delve, showcasing, boasts, underscores, comprehending, intricacies, surpassing, intricate, underscoring, garnered, showcases, emphasizing, underscore, realm, surpasses, groundbreaking, advancements, aligns — and their evidence is only "consistent with RLHF playing a role"; the 2025 "Word Overuse and Alignment" paper is the one that experimentally links the overuse to learning-from-human-feedback rather than the training corpus. Jakesch et al. (PNAS 2023): 4,600 people could not detect AI self-presentation; their heuristics were exploitable. Gorrie (Dead Language Society): "What the LLM lacks is not technical ability, but taste" — the devices are legitimate; the failure is not knowing WHEN.
...
R5 POOL SPREAD. Within a pool, variants differ in sentence COUNT and OPENER as well as angle; no two share their first two words; where the band allows, one variant is ten words or fewer. (O'Sullivan on model uniformity; Doshi & Hauser on a pool converging when its writers share ideas; and our own measure — 407 of 708 pools uniform in shape.)
...
Per corpus file, per variant: lexical tells (Juzek & Ward's 21 inflected focal forms, listed in §2, + tapestry/testament/beacon/nestled/vibrant) · "rather than" rate · gloss-tail rate · semicolons per sentence · summarising second sentences · abstract closers · pet-word rates · words-per-sentence sd per pool (burstiness) · uniform-shape pools · shared-opener pools · causal over-thirty share. Baselines are the table in §3. Direction: every column shrinks or holds; the walker prints the sentence, never a score alone (the taste inversion says a score is not a judge).
