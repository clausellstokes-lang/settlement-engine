...
async function getMapURL(
  ...
  if (fontsToLoad.length) {
    ...
    // SettlementForge fork patch: a loaded .map controls the font family/unicode-range/variant. Strip the
    // CSS-structural characters (quotes, braces, semicolons, angle brackets, newlines) so a crafted family
    // like `"; } * { background:url(//evil) } .x{` cannot break out of the @font-face rule. `src` is a data:
    // URI regenerated by loadFontsAsDataURI (its `;base64,` must survive), so it is not stripped.
    const cleanCss = v => String(v).replace(/["'\\;{}<>\r\n]/g, "");
    const fontFaces = dataURLfonts
      .map(({family, src, unicodeRange = "", variant = "normal"}) => {
        return `@font-face {font-family: "${cleanCss(family)}"; src: ${src}; unicode-range: ${cleanCss(
          unicodeRange
        )}; font-variant: ${cleanCss(variant)};}`;
      })
      .join("\n");
