...
| B9 | Typecheck posture | `npm run typecheck:ratchet` then `npm run typecheck:domain:strict` | **UNMEASURED** — exit 0 both. `typecheck:ratchet` runs `tsconfig.full.json`; `typecheck:domain:strict` runs `tsconfig.domain-strict.json`, which declares no `include` of its own and inherits `tsconfig.json`'s `src/domain/**/*.js` + `src/domain/**/*.ts`. ⚠ An unbaselined new domain file's error allowance is **ZERO**, so the display leaf is **strict-clean or the packet stops** |
