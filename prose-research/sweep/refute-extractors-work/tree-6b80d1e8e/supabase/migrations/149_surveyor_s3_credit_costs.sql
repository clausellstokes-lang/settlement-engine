-- ────────────────────────────────────────────────────────────────────────────
-- 149_surveyor_s3_credit_costs.sql — Surveyor S3 commercial: task-priced managed
-- credits for the INTENT COMPILER (interpret-session) + THE PARLEY (parley).
--
-- WHY THIS EXISTS
--   DESIGN_AI_CONTROL_SURFACE §2 stage 3 / §2d / §4: the S3 wave adds two new
--   task classes — 'interpret' (session text → proposed canon events + party
--   impacts) and 'parley' (in-character consultation, musings-register only). Both
--   are TASK-PRICED managed credits (users buy OUTCOMES, not tokens). This extends
--   the spend_credits CASE with those two features so the two new edge functions'
--   spend_credits({feature:'interpret'|'parley'}) resolve a cost instead of raising
--   'unknown feature'.
--
--   PRICING IS PROVISIONAL (vetoable — final Surveyor pricing is an OWNER-QUEUED
--   decision, design §6). interpret=5 (a full session compile — long input, a
--   structured multi-op interpretation out; ≈ the 'progression' bucket), parley=3
--   (one in-character response; ≈ 'analysis'). Kept in lockstep with
...
--
-- WHAT
--   Re-defines spend_credits verbatim from 140 (the current definition) with TWO
--   new CASE arms (interpret, parley). All other behaviour — config-first per-profile
--   cost, the elevated bypass, grant allocation, the 057 active-account gate — is
--   unchanged. CREATE OR REPLACE preserves the existing grants (authenticated).
--
-- Depends on: 140 (current spend_credits), 114 (ai_credit_costs config), 057
--   (account_is_active), 007 (credit_ledger, get_credit_balance). Re-runnable.
-- @rollback: re-apply 140's spend_credits definition (drop the two new CASE arms).
-- ────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  if cfg_cost is not null then
    ...
    -- 057 CASE block + the Surveyor S1 (analysis, brief) + S3 (interpret, parley) features.
    cost := case feature
