...
C = [
 ...
 ("slow pace, journeys", "Alexander (via Wikipedia) says Kay never races to a destination; the joy is in the journeys; the slow pace is typical.", "wiki-ceas", "joy of his novels is invariably in the journeys"),
...
def squash(s):
    s=re.sub(r"\s+"," ",norm(s))
    s=re.sub(r"(\w) ?- (\w)", r"\1\2", s)   # PDF soft-hyphen artefacts: "giv -\ning" -> "giving"
    return s.lower()
...
json.dump(out,open(os.path.join(BASE,"found-kay-academic.v1.json"),"w"),ensure_ascii=False,indent=1)
...
open(os.path.join(BASE,"find-kay-academic.v1.md"),"w").write("".join(md))
