...
if (!NAME) throw new Error('args.name is required (tolkien | martin | dnd | ai | kay | leguin | probe-wolfe | probe-hobb)')
...
const RESUME = !!(args && args.chunks)
// ---------------------------------------------------------------- Find (fresh runs only)
phase('Find')
let freshClaims = []
if (!RESUME) {
  const ex = EXEMPLARS.find(e => e.key === NAME)
  const probe = PROBES.find(p => 'probe-' + p.key === NAME)
  let found = []
  if (ex) found = await batched(ANGLES, (an) => agent(
  ...
  const seen = new Set()
  for (const c of all) { const k = (c.source + '|' + c.feature + '|' + c.claim.slice(0, 60)).toLowerCase(); if (!seen.has(k)) { seen.add(k); freshClaims.push(c) } }
  log(`found ${all.length} claims, ${freshClaims.length} after dedupe, from ${found.filter(Boolean).reduce((n, r) => n + (r.sourcesRead || []).length, 0)} sources read`)
}
// ---------------------------------------------------------------- Verify
phase('Verify')
...
const chunks = RESUME
  ? args.chunks.map((c, k) => ({ k, file: c.file, indices: c.indices, inline: null }))
  : (() => { const out = []; for (let i = 0; i < freshClaims.length; i += CHUNK) { const k = out.length; out.push({ k, file: null, indices: freshClaims.slice(i, i + CHUNK).map((_, j) => i + j), inline: freshClaims.slice(i, i + CHUNK).map((c, j) => ({ index: i + j, ...c })) }) } return out })()
log(`verification: ${chunks.reduce((n, c) => n + c.indices.length, 0)} claims to verify in ${chunks.length} chunks of up to ${CHUNK} (${RESUME ? 'resumed from state chunks; ' + (args.verifiedCount || 0) + ' already verified' : 'fresh'})`)
...
if (unwritten.length) {
  ...
  await batched(unwritten, (r) => agent(`Write the file ${OUT}/verdicts-${NAME}-chunk-${String(r.ch.k).padStart(2, '0')}.json with EXACTLY this JSON content (create the directory if needed) and return the byte count you wrote:\n${JSON.stringify({ name: NAME, chunk: r.ch.k, claims: r.ch.inline || null, claimsFile: r.ch.file, verdicts: r.v.verdicts })}`, { label: `checkpoint:${NAME}:chunk${r.ch.k}`, phase: 'Verify', effort: 'low' }))
}
...
// ---------------------------------------------------------------- Synthesize
phase('Synthesize')
const ABOUT = NAME === 'ai' ? 'the catalogue of where generated prose FAILS against skilled human fiction and game writing' : NAME.startsWith('probe-') ? `the single craft device of ${NAME.slice(6)} (see the claims)` : `the prose style of ${(EXEMPLARS.find(e => e.key === NAME) || {}).name || NAME}`
const section = await agent(
  `First run, in a shell: \`cd ${KIT} && node sweep-state.mjs merge ${NAME} --update-state\` — it merges the state file${STATE_FILE ? ` (${STATE_FILE})` : ''} with every verdicts-${NAME}-chunk-*.json the verifiers wrote and writes ${OUT}/kept-${NAME}.json (the VERIFIED_VERBATIM / VERIFIED_SUBSTANCE claims only, each with its index, source, url, quote and verdict.trueWording). Read that file (with Read, in pieces if it is long). Then write the dossier section for "${NAME}" — ${ABOUT} — from those VERIFIED claims only. Structure: numbered concrete features, each with the critics/sources that support it (count them; cite by index), the true wording of any quotation (under twelve words), and the reconstruction rule it implies for a settlement dossier written as a calm archivist (present tense, concrete civic nouns, no digits, no em dash). Mark disagreements between sources explicitly. A feature resting on ONE source is flagged as such. End with a coverage table: sources per angle, and the verdict counts (verified / not found / contradicted / blocked). Write it to ${OUT}/section-${NAME}.md and return the markdown.`,
  { label: `synth:${NAME}`, phase: 'Synthesize' })
const critic = await agent(`You are the completeness critic for the sweep "${NAME}". Read ${OUT}/section-${NAME}.md, ${OUT}/kept-${NAME}.json and the find-${NAME.replace('probe-', 'probe-')}*.md notes in ${OUT} (if present). List what is MISSING: an angle not run, a well-known critic or study absent (name them), a claim that rests on one source, a copyright risk (a quotation over twelve words), a verdict that looks wrong on its face (a VERIFIED claim whose quote is not on its page, a NOT_FOUND that a second route would find), and any feature the section contradicts itself on. Return a markdown list with a recommended next round of searches. Write it to ${OUT}/critic-${NAME}.md.`, { label: `critic:${NAME}`, phase: 'Synthesize' })
return { name: NAME, resumed: RESUME, chunks: chunks.length, chunksReturned: results.filter(Boolean).length, verifiedThisRun: Object.keys(verdictMap).length, keptThisRun: kept, missingChunks: missing.map(c => c.k), sectionChars: (section || '').length, critic }
