...
const RULE = 'For each claim report its index and: VERIFIED_VERBATIM only if the quotation appears on the page word for word AND every limb of the claim sentence is supported by the page; VERIFIED_SUBSTANCE if the page supports the whole claim in other words (give the true wording, under twelve words); PARTIAL if the quotation is on the page but some limb of the claim is NOT (name that limb in unsupportedLimb — a PARTIAL is a visible grade, never a caveat buried in a note); NOT_FOUND if the page does not support it; CONTRADICTED if it says otherwise; BLOCKED only if the page cannot be fetched by any route — and a claim that carries routeHints must be tried by every hint (OpenAlex/Crossref abstract routes for paywalled DOIs, scribe.rip for Medium, archive.ph and the Wayback Machine, Internet Archive full text) before it is BLOCKED. Default to NOT_FOUND when uncertain. Return one verdict per index, every index.'
...
return { name: NAME, resumed: RESUME, regraded: !!(args && args.regrade), findAngles: FIND_ANGLES, extraAngles: EXTRA.map(a => a.key), freshClaims: freshClaims.length, chunks: chunks.length, chunksReturned: results.filter(Boolean).length, verifiedThisRun: Object.keys(verdictMap).length, keptThisRun: kept, missingChunks: missing.map(c => c.k), sectionChars: (section || '').length, critic }
