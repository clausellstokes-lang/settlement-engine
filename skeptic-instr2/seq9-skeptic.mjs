...
const D = '/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/4e3d2f70-f45f-4e14-b571-514c339cfa17/scratchpad/kit/skepINSTR2';
...
writeFileSync(`/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/4e3d2f70-f45f-4e14-b571-514c339cfa17/scratchpad/kit/skeptic-instr2/seq9-skeptic${MATERIALISE ? '-materialised' : ''}.json`, JSON.stringify({
...
// ── SKEPTIC ADDENDUM: the run rate is sensitive to the ORDER the lines are concatenated in.
const perTown = towns.map((t) => [...t.rungs.map((f) => f.text), ...t.bare].filter((x) => x && x.length > 3));
const ordersPerTown = perTown.map((ls) => ls.map((t) => orderIdOf(classifyMoves(t)) || classifyMoves(t).join('→')));
let rIn = 0, pIn = 0;
for (const os of ordersPerTown) { for (let i = 1; i < os.length; i++) { pIn += 1; if (os[i] === os[i-1]) rIn += 1; } }
const bareOrders = towns.flatMap((t) => t.bare).filter((x)=>x&&x.length>3).map((t)=>orderIdOf(classifyMoves(t))||classifyMoves(t).join('→'));
const rungOrders = towns.flatMap((t) => t.rungs.map((f)=>f.text)).filter((x)=>x&&x.length>3).map((t)=>orderIdOf(classifyMoves(t))||classifyMoves(t).join('→'));
const rr=(os)=>{let r=0;for(let i=1;i<os.length;i++) if(os[i]===os[i-1]) r+=1; return [r, os.length-1, r/Math.max(os.length-1,1)];};
const [rb,pb,vb]=rr(bareOrders); const [rg,pg,vg]=rr(rungOrders);
console.log(`SKEPTIC · run rate WITHIN a town (rungs then that town's bare): ${(rIn/Math.max(pIn,1)).toFixed(4)} (${rIn} of ${pIn} pairs)`);
console.log(`SKEPTIC · run rate over the BARE block alone: ${vb.toFixed(4)} (${rb} of ${pb})`);
console.log(`SKEPTIC · run rate over the RUNG block alone: ${vg.toFixed(4)} (${rg} of ${pg})`);
const v1bare = bareOrders.filter((o)=>String(o).split('|').includes('V1')).length;
const v1rung = rungOrders.filter((o)=>String(o).split('|').includes('V1')).length;
console.log(`SKEPTIC · V1 share bare ${(v1bare/Math.max(bareOrders.length,1)).toFixed(4)} (${v1bare}/${bareOrders.length}) · rungs ${(v1rung/Math.max(rungOrders.length,1)).toFixed(4)} (${v1rung}/${rungOrders.length})`);
console.log(`SKEPTIC · distinct orders: bare ${new Set(bareOrders).size} · rungs ${new Set(rungOrders).size}`);
