# -*- coding: utf-8 -*-
# fill-template-914.py — fills __ROW__/__ROW_BODY__/__CARD__/__CARD_BODY__/__TAIL__/__TAIL_BODY__ in payload-914.template.json and
# __SUBJECT__/__BODY__/__ENROLS__ in msg-914.txt from texts-914.draft.md, leaving __CAS_SHA__/__TESTS__ for after-cas-914.sh.
...
raw=io.open(SC+'texts-914.draft.md',encoding='utf-8').read()
...
assert '\n' not in CARD and '\n' not in TAIL
for f in ('payload-914.template.json','msg-914.txt'):
    if not os.path.exists(SC+f+'.pristine'): shutil.copy(SC+f, SC+f+'.pristine')
t=io.open(SC+'payload-914.template.json.pristine',encoding='utf-8').read()
...
io.open(SC+'payload-914.template.json','w',encoding='utf-8').write(t)
m=io.open(SC+'msg-914.txt.pristine',encoding='utf-8').read()
...
assert not re.search(r'__[A-Z_]+__',m); io.open(SC+'msg-914.txt','w',encoding='utf-8').write(m)
