#!/bin/sh
# run-gate-915.sh — the §915 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base 8522a17b2. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
# a background task reports its LAST command's exit, and a trailing printf once reported 0 over a red gate.
D=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/4e3d2f70-f45f-4e14-b571-514c339cfa17/scratchpad/kit/laneMEASURE
BASE=8522a17b20febb07ebe16bc9d739b9ebc54411c7   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse 8522a17b20febb07ebe16bc9d739b9ebc54411c7)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base 8522a17b2 — land the previous consist first"; exit 7; }
...
[ "$CARS" = "20" ] || { echo "REFUSED: expected 20 cars over $BASE, found $CARS"; exit 7; }
