 ...
 *
 * WHY
 *   The soak proves the world against a committed set of RATIFIED distribution bands.
 *   A malformed band (a flipped min/max, a missing governing constant, a band silently
 *   moved out of 'RATIFIED', a duplicate metric) would corrupt the soak's diagnosis
 *   or quietly undo the owner's signature. This gate makes the manifest's
 ...
 *     non-empty constant list, a rationale, and status EXACTLY 'RATIFIED'; no duplicate
 *     metric.
 ...
 *   - the RATIFIED law: every checked-in band carries the owner's signature.
 *
...
export function checkManifest() {
  assertValidSoakBands(RATIFIED_SOAK_BANDS);

  // The RATIFIED law, asserted independently of validateSoakBand so a future
  // relaxation cannot silently strip the owner's signature.
  const unsigned = RATIFIED_SOAK_BANDS.filter((b) => b.status !== 'RATIFIED');
  if (unsigned.length) {
    throw new Error(
      `[check-tuning-bands] ${unsigned.length} band(s) are not 'RATIFIED': ` +
      `${unsigned.map((b) => b.metric).join(', ')}. Every checked-in soak band must remain owner-signed.`,
    );
  }
  ...
  if (envelopes.length !== RATIFIED_SOAK_BANDS.length) {
  ...
  return { bandCount: RATIFIED_SOAK_BANDS.length, version: SOAK_BAND_MANIFEST_VERSION };
}

function main() {
  if (process.argv.includes('--json')) {
    process.stdout.write(JSON.stringify({ version: SOAK_BAND_MANIFEST_VERSION, bands: RATIFIED_SOAK_BANDS }, null, 2) + '\n');
    return;
  }
  try {
    const { bandCount, version } = checkManifest();
    console.log(`[check-tuning-bands] manifest v${version} OK: ${bandCount} ratified soak bands, all valid.`);
  } catch (err) {
