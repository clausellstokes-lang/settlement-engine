...
echo "--- 2. the full build (dist/):"; sh scripts/gate-mutex.sh --run -- npm run build > $SC/build-901.log 2>&1; echo "BUILD_EXIT=$?"; tail -3 $SC/build-901.log | cut -c1-140
echo "--- the WORKER bundle(s) in dist/ (bytes):"; find dist -name '*.js' | grep -iE 'worker' | while read f; do printf '  %8d  %s\n' "$(wc -c < "$f" | tr -d ' ')" "$f"; done
echo "--- hashed chunk listing → $SC/listing-901.txt:"; find dist -name '*.js' -exec sh -c 'printf "%s %s\n" "$(wc -c < "$1" | tr -d " ")" "$(basename "$1")"' _ {} \; | sort -k2 > $SC/listing-901.txt; wc -l < $SC/listing-901.txt | xargs echo "  chunks:"
...
echo "NEXT: write the WORKER_BUNDLE_CEILING_BYTES arm from the printed worker bytes (monotone-down; cite this log) as a chair car; diff listing-901.txt by filename against HORIZON-B1's prediction; then the whole suite, the registers (plan §3 + f2), the kit, the gate."
