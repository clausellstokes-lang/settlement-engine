...
function refinerPrompt(s, arm, who) {
  const p = s.p
  return "You are the " + who + " REFINER for arm " + arm + " of the TASTE, working for the Fable chair. Your pool: block " + p.block + ", pool key '" + p.pool + "'. " + VOICE + " " + FENCES +
    " THE JOB: the pool's wording set is already LAWFUL - every face inside every band (read the lawful draft at " + PK + "/" + p.dir + "/draft-round-" + s.rounds + ".md and its measured position at " + PK + "/measure-draft-base.json). Lawful is the floor. Refine every face once, one for one (never add or drop a face or a variant; never trim), toward NAMED targets beyond the gate: closer to the interior of each band rather than its edge; the four faces farther apart from each other in vocabulary and rhythm (sibling distance); the fact stated more sharply and more specifically within what the card licenses; the read-aloud ear of a clerk who was there. Re-read the licence card (node scripts/prose-licence-card.mjs " + p.block + " '" + p.pool + "' in " + DOCK_A + ") - a refinement that adds an unlicensed claim is a regression and the gate will revert it. Write the refined rows to " + PK + "/" + p.dir + "/refine-" + arm + ".md in the same grammar, then under '--- NOTES' say for each face what you changed and why, in one line each. You are a DIFFERENT author than the drafter, on purpose. Return the packet path and the per-face change notes."
}

const refineA = await parallel(lawful.map((s) => () => agent(refinerPrompt(s, 'A', 'Opus'), { label: 'refine:A:' + s.p.dir, phase: 'Refine', model: 'opus', effort: 'high' }).then((t) => ({ dir: s.p.dir, path: PK + '/' + s.p.dir + '/refine-A.md', text: t }))))
const spentAfterA = budget.spent()
const refineB = await parallel(lawful.map((s) => () => agent(refinerPrompt(s, 'B', 'Fable'), { label: 'refine:B:' + s.p.dir, phase: 'Refine', model: 'fable', effort: 'high' }).then((t) => ({ dir: s.p.dir, path: PK + '/' + s.p.dir + '/refine-B.md', text: t }))))
