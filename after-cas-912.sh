#!/bin/sh
# after-cas-912.sh — ONE command from a green gate log to the §912 ledger act (stamped by mk-landing-kit.py). POSIX sh. Guard + consequence together.
set -e
SC=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad
D=$SC/laneLMAT; REPO=/Users/cstokes/Desktop/settlement-engine
BASE=3b1c0eaa51f77561a036ae7ec54682c39856192c; LOG=$SC/gate-912.log
...
python3 $SC/chair-verify.py "$D" "$BASE" "$LOG" 6 > $SC/chair-verify-912.out 2>&1 || { tail -6 $SC/chair-verify-912.out; echo "⛔ chair-verify RED — stop"; exit 1; }
tail -3 $SC/chair-verify-912.out
...
SHORT=$(git rev-parse --short "$NEW"); printf '%s' "$SHORT" > $SC/cas-912.sha
...
sed -e "s/__CAS_SHA__/$SHORT/g" -e "s/__TESTS__/$TESTS/g" $SC/payload-912.template.json > $SC/payload-912.json
if grep -qE '__[A-Z0-9_]+__' $SC/payload-912.json; then echo "⛔ unfilled placeholder in payload-912"; exit 1; fi
sh $SC/collect-912.sh > $SC/collect-912.out 2>&1 || { tail -15 $SC/collect-912.out; echo "⛔ collect-912 failed — stop"; exit 1; }
tail -8 $SC/collect-912.out
git -C "$REPO" log -1 --format=%s review-fixes-2026-07-08 | grep -q '^§912:' || { echo "⛔ §912 did not land"; exit 1; }
