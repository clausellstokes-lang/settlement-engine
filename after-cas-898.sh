...
grep -q '^GATE_CARS=10$' "$LOG" || { echo "⛔ gate did not run over 10 cars: $(grep '^GATE_CARS=' "$LOG")"; exit 1; }
...
python3 $SC/chair-verify.py "$D" "$BASE" "$LOG" 10 > $SC/chair-verify-898.out 2>&1 || { tail -6 $SC/chair-verify-898.out; echo "⛔ chair-verify RED — stop"; exit 1; }
