#!/bin/sh
# run-gate-891.sh — the §891 bare gate (npm run check) on the KERNELMARK dock, quiet-window law, exits captured.
...
BASE=ca651d54bab1fc68d5c417226d6a5f719562cc5b
cd $D || exit 9
STREAK=0; WAITED=0
...
echo "GATE_DISK_FREE_PRE=$(df -h /private/tmp | tail -1 | awk '{print $4}')"
...
echo "GATE_DISK_FREE_POST=$(df -h /private/tmp | tail -1 | awk '{print $4}')"
