...
while true; do
  ...
  STATE=$(cd $SC/prose-research && node export-sweep-state.mjs 2>/dev/null | cut -c1-300)
  ...
  GATE=$(ls -t $SC/gate-90*.log 2>/dev/null | head -1); GATES=$( [ -n "$GATE" ] && grep -E "GATE_DONE=|TRUE_EXIT=" "$GATE" | tail -1 || echo none)
  PROCS=$(pgrep -fl "run-gate-90|run-ratchet-90|run-registers-90|build-90" 2>/dev/null | cut -c1-60 | tr '\n' ';')
...
<!-- /AUTOSTATUS -->"
  python3 - "$BLOCK" <<'PY'
import sys,io,re
p='/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/d5b9a39f-b0b2-4d9c-a1a0-08b291896f89/scratchpad/RESUME-NOTE.md'
...
PY
  ...
  for f in RESUME-NOTE.md prose-research/research-workflow.js prose-research/export-sweep-state.mjs; do echo "$f" >> .kit-files.auto.txt; done
  ls prose-research/sweep/state-*.json 2>/dev/null >> .kit-files.auto.txt; ls receipt-*.md 2>/dev/null >> .kit-files.auto.txt
