...
7. **Dead threat literals.** Normalize legacy `embattled/high/low/safe` branches (`safetyProfile.js:234`, `powerGenerator.js` — ADDRESS STRUCK 2026-09-20: the module is a 13-line barrel over `./power/*` and carries no threat branch; the last `embattled` arm is recorded dead at `settlementThreat.js:26`, defaults in capacity/system-state) to canonical `heartland/frontier/plagued` via one shared helper.
