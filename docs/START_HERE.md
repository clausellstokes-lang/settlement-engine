...
the lanes in flight and how to collect rather than re-dispatch them; the two attributed
gate reds nobody should chase; the owner's open decisions; and the laws that cost the most
to relearn.

⚠ This file remains accurate on WHAT THE PROJECT IS, the owner's doctrine, and the
constitutional laws. It is stale on build state, dispatch, and process — **`RESUME_STATE.md`
and `docs/implementation/INDEX.md` own those.**
...
## 3k. ⭐⭐ THE GATE HANDOFF (2026-08-07, ~16:00 EDT — written IN ADVANCE of a
## weekly-credit exhaustion, at the owner's instruction "prepare for that".
...
behind it; typecheck errors grew 14 → 351 invisibly at ~0.92/commit. As of this
writing **steps 1-13 all exit 0** and only step 14 `verify:dist` is red, on three
INHERITED budget failures. `npm run smoke:boot` **exits 0 — the dist boots.**

### WHAT CHANGED, AND THE ONE IDEA BEHIND ALL OF IT
A boolean gate demanding zero that **nobody can pass gets ignored, and then
tolerates infinity while blinding every step behind it.** The cure is a RATCHET:
freeze current debt as a **per-file / per-test ceiling**, fail only on GROWTH, let
it only shrink. Two were added this day, both mirroring the mature
`scripts/check-domain-strict.mjs` (READ IT before touching either):
- **`scripts/check-full-typecheck.mjs`** + `.full-typecheck-baseline.json` —
  wired as `typecheck:ratchet` at step 9. `npm run typecheck` KEPT RAW for burn lanes.
- **`scripts/check-test-ratchet.mjs`** + `.test-ratchet-baseline.json` — wired as
  `test:ratchet` at step 12. **49 attributed failures** frozen. ⚠ Its laws:
  per-TEST identity never per-file; the tests RUN (a baselined test that turns up
  SKIPPED reds; skip count frozen at 105); `--update` is **REMOVE-ONLY**; every
  entry carries an attribution or the meta-test refuses it.
⚠⚠ **A GREEN RATCHET IS EVIDENCE ABOUT ITS OWN CLASS ONLY, NEVER ABOUT THE COMMIT.**
Proven twice this day on the chair's own work: a commit whose TOTAL typecheck count
was unchanged (177→177) had 2 new errors caught only by the PER-FILE rule; and a
commit the typecheck ratchet passed cleanly broke **13 tests** in four families.

### ⛔ THE THREE STEP-14 FAILURES ARE INHERITED — DO NOT READ THEM AS NEW
`firstPaintNonJs` render-blocking CSS **19,813 B vs 19,800** (over by 13 bytes) ·
`vendorPdfLazy` first-paint static closure **1,168,130 vs 1,040,000** · gzip
**367,507 vs 337,000**. Measured at BOTH ends: at `f707a905` it was 5 files / 8
tests failing; the layering repair `67f8a58e` **cured five and shrank the rest**
(first-paint ~1.71 MB → 1.17 MB). ⚠ Any doc or task saying "65% over" is STALE.

### ⛔⛔ WHAT WAS IN FLIGHT WHEN THIS WAS WRITTEN — AND HOW TO RECOVER IT
Build HEAD `1e4c493b`. **THREE independent writers were live in the ONE worktree**
(the concurrency law allows TWO): a determinism lane, a pre-mortem lane, and an
owner-started background session. Dirty at write time:
`src/domain/townMap/{asymmetrySources,townLayoutV2}.js` (the owner's separate
session), `tests/lint/{observedShapeReaders.walker,testRatchet}.test.js`
(determinism lane), and untracked `scripts/premortem.mjs` +
`scripts/lib/premortem-triggers.mjs` (pre-mortem lane).

⭐ **THE ORPHANED-WORK RECOVERY PROCEDURE, PROVEN ON A REAL INCIDENT THIS DAY**
(a lane finished 26 files of verified work and died before committing; all of it
was recovered). **Do this, in this order, before anything else:**
1. `git status` — treat every dirty file as possibly-valuable foreign work.
2. **BACK IT UP FIRST**: copy every dirty/untracked file to a scratch dir AND
   `git diff HEAD > FULL.patch`. Never `git stash` (FORBIDDEN by owner order),
   never `git checkout --`.
3. **RE-VERIFY INDEPENDENTLY — do not trust the dead lane's report.** Run the
   walkers/tests the work claims to fix and confirm they are green.
4. **EARN any "pre-existing" claim**: build an integrity-counted `git archive` of
   HEAD and reproduce the failure there before believing it is not yours.
5. Commit with EXPLICIT PATHSPECS and **verify the staged set BY NAME AND COUNT**.
   ⚠ `git add` aborts ATOMICALLY on one bad pathspec — a stale path silently
   stages 1 file instead of 26. This bit the chair; the count check caught it.
6. After committing, confirm untracked files survived the pre-commit hook.

### THE OTHER INSTRUMENTS BUILT THIS DAY
- **`scripts/check-observed-shape-readers.mjs`** — the READER-WITHOUT-A-WRITER
  walker. Derives ground truth by EXECUTING the real generator and histogramming
  observed keys; types are consulted nowhere (they were wrong or silent in all four
  known instances). ⚠ Its steading leg is LOAD-BEARING — 40 driven intervals mint
  ZERO steadings, so a shape the corpus never produces looks CLEAN. Inventory
  ~3,261 findings; it found **31 unknown instances** of one class on first run.
- **`scripts/check-hazard-registry.mjs`** + `scripts/hazard-registry.json` — ⭐⭐
  **THE HAZARD CONVERSION LAW (owner, 2026-08-07):** every confirmed hazard class
  becomes MACHINERY or is EXPLICITLY ACCEPTED with a stated reason. A recorded
  hazard is a DOCUMENT, and documents do not prevent recurrence — three classes bit
  the same day they were already written down with their cures. Statuses:
  MACHINERY / **PARTIAL (a cure that is OPTIONAL — the status that hides)** /
  DOCUMENT (shrink-only) / ACCEPTED. **The metric is INSTANCES PER CLASS, never
  errors-per-round.**
- **`scripts/premortem.mjs`** (in flight at write time) — the only instrument that
  runs BEFORE the error. Advisory, never blocking.
- **`scripts/gate-mutex.sh`** (the gate slot) and **`scripts/ratchet-inventory.sh`**
  (a red ratchet's inventory can GROW behind a byte-identical failing row).

### ⛔ OWED TO THE OWNER — DO NOT SELF-RULE
- **Migration `195_civility_guard_and_public_identity.sql` landed past the
  rehearsal manifest head of 194**, with no `supabase/rollback/195_*.down.sql`, no
  `-- @rollback:` note, and `DEPLOY.md` naming an older head. Three test reds, one
  cause. **Migrations are owner-gated; this is a live deploy-safety gap.**
- ~~**Two docs are not IP and should leave the repo**~~ — **DONE 2026-08-10** under the
  owner's delegation grant. `docs/PRICING_MARGIN_SHEET.md` (margins, per-action credit
  prices, provider rate cards), `docs/REVIEW_FINDINGS.md` (a 133-finding security
  register, incl. a PostgREST `.or()` filter-injection seam), and its
  `docs/.review_findings.json` sidecar (the machine-readable superset, tracked on THIS
  branch only) were preserved byte-exact to the owner's out-of-repo design-handoff
  folder under `repo-extracted-2026-08-10/` and removed from the repo.
  - ⚠⚠ **THE "9 OPEN MARKERS" FIGURE WAS WRONG — and the truth is worse.** That number
    came from a case-insensitive `grep -c open` that matched incidental prose ("leaves
    the abuse vector open", "left the Stripe tab open"). **Neither the markdown nor the
    JSON ever had a per-finding open/closed field at all.** Only the 15 critical+high
    were ever re-verified (2026-06-16: 14 fixed, 1 partial); the **65 medium + 53 low
    were never individually re-triaged**. So **118 findings carry no remediation status**,
    not 9. Whether each is still live IN CODE remains UNVERIFIED and is owed. The
    extracted `.review_findings.json` is the worklist — every row has `file` and `line`.
- Adding `foundingTier` to the satellite mint (persisted-shape widening).

### ⭐⭐ THE LAW THAT CAME OUT OF ALL OF IT — LEARN THIS ONE IF YOU LEARN NOTHING ELSE
**A FAILING TEST IS DEBT. A FAILING WALKER IS A DISABLED GUARD. THEY MUST NEVER BE
FROZEN BY THE SAME MECHANISM.** The step-12 census froze four rows belonging to
enforcement walkers, so a brand-new un-anchored negative assertion stopped redding
anything — **proven by planting one and watching the census absorb it in silence.**
Worse, the inventories BEHIND those tolerated rows had grown unseen: un-anchored
negatives 1,303 → **1,565** sites, bare seed loops 13 → **26, doubled.** Cured at
`af8815e9` by relocating each walker's debt into its OWN shrink-only inventory,
with generation-facing re-admissions quarantined and audited by EXACT IDENTITY both
directions — so a new file reds, growth reds, AND an un-banked shrink reds.
⚠⚠ **AND THE SEQUEL, WHICH IS THE WHOLE POINT: that commit stated the new law in
`CONTRIBUTING.md` and `testRatchet.test.js` as COMMENTS — and violated it TEN times
in the same census.** Ten of the 40 remaining entries are walker rows across seven
files. ⛔ A `.walker.` FILENAME CHECK IS THE WRONG CURE: `mechanismLitCoverage.test.js`
is two of the ten and carries no such name, so a filename guard catches 8 of 10 and
reports success — converting an open problem into a solved-looking one. A lane was
in flight on the derived identifier when this was written.

### ⚠ KNOWN-OPEN, NOT LOST
Step 12 was measured **NON-DETERMINISTIC under load** — three runs at the same sha
gave 0, 2 and 5 out-of-census failures, all **TIMEOUTS not assertion failures**
(22-26 s against `testTimeout: 20000`; the same tests cost 6.6 s idle). A repair
lane was mid-flight. ⛔ **DO NOT cure it by raising the global `testTimeout`** —
that hides the next one. And ⚠ **any test-ratchet census entry frozen because of a
timeout is a PHANTOM** that can never be burned down.
...
- **FP: 75 declared** (60 numbered + 8 ES + 7 WY-engine, matching its own §5
  header). Landed whole: SP-A · CW-0w · SP-B · SP-B2 · SP-C · GR-0 · GR-1 ·
  TR-1 · ES-0 · **SP-D · GR-2** (the last two at cycle 4).
- ⚠ **THE PROGRAM-WIDE TOTAL IS 113, NOT THE 108 THIS SECTION FIRST RECORDED.**
  Cycle 4's close re-derived it and the larger figure is right: **75 (FP §5) + 5
  (WY LANE S surface waves, which §5's own clause EXCLUDES from its 75 but which
  are real work) + 33 (the three unfolded volumes) = 113 declared.** My earlier
  108 simply omitted the five WY surfaces.
  ⭐⭐ **RE-DECOMPOSED 2026-08-07, SAME TOTAL: THE 33 ARE NO LONGER "UNFOLDED".**
  The HB + WC + EP integration fold landed (below), so FP §5 now carries **108**
  waves and the arithmetic reads **108 (FP §5) + 5 (WY LANE S) = 113 declared** —
  the same figure, decomposed correctly. ⚠ **DO NOT ADD 33 TO 113.** The three
  volumes' waves moved INSIDE FP §5's count; they were never outside the 113. **11 whole landed → 102 REMAINING**,
  two of them PART-BUILT: IN-0 (slice 0a of four) and TR-9 (slice 9c only).
- ⛔ **DO NOT RESTATE A FLAGGED/UNFLAGGED SPLIT.** The remembered "49 flagged /
  45 not" is struck and cycle 4's close deliberately declined to replace it: a
  text scan classifies only **69 of 75** wave blocks cleanly, so any split is
  hand-maintained — the exact shape derive-don't-restate forbids. What IS
  cleanly derivable, and therefore what is recorded: **FP §3 holds exactly 52
  flag rows, of which 10 are live in `ENGINE_GATED_VIRTUAL_RULE_KEYS`** (cycle 4
  minted two). Derive any scheduling split from those, at the moment you need it.
...
Three architected programs plus two verification instruments and the
diagnostic-soak design lived ONLY in a session scratchpad under `/private/tmp/`
until commit `82f06898`. Read that directory's README before touching them. Each
...
⛔⛔ **THIS ROW CARRIED THREE FIGURES AND ALL THREE WERE WRONG — struck here,
not adjusted.** ~~"EPOCH 16 waves SEALED"~~: EPOCH declares **SIX** (EP-0..EP-5)
and is **NOT SEALED** — it is a DRAFT AT ROUND SEVEN, and the word "SEALED" came
from `scripts/refresh-archive.sh`'s copy DESTINATION, never from the volume.
~~"WAR-CIRCULATION ~12 waves"~~: it declares **SEVENTEEN** (WC-0..WC-16). Only
"HABIT 10 waves" was right. The correct figures are **EP 6 · HB 10 · WC 17 = 33**,
and the directory's own README carries them with the address that declares each.
The lesson this row is now the second demonstration of: **a restated count goes
stale, and an aggregate built from two wrong figures is internally consistent,
...
## ⭐⭐ 2026-08-07 — THE HB + WC + EP INTEGRATION FOLD IS **LANDED AND
## DISCHARGED.** (Recorded BY HAND, because the ES/WY precedent's own fold item
## appears in none of its commits and its discharge was lost out-of-band.)

**33 BUILD WAVES ARE UNBLOCKED — the largest single blocker in the program.**
The three volumes took canonical homes on `claude/composite-r4` as
`docs/DESIGN_FP_ARCH_{HB,WC,EP}.md`, promoted whole from the snapshots in
`docs/architected-volumes-pending-fold/` on THIS branch. Four commits from
`eca65c8a`: **`dd0cc340`** (the volumes + every parent site + the cross-volume
reconciliation) → **`6ee83e3e`** (the prefix admission) → **`5a44b5db`** (the
queue rows + the ledger row) → **`bdb647f8`** (the sweep). The full judgment
record, with every receipt and the four things a Fable-capable account should
re-examine, is the dated block at the tail of
`docs/FABLE_VALIDATION_QUEUE.md` on the build branch.

**THE COMPILED VOLUME IS NOW THIRTEEN PROGRAMS: 63 flags · 108 waves · 112
seams** (50 physical §9 rows — a folded volume contributes ONE physical row and
+N logical seams). Every figure was EXECUTED against committed blobs, never
inherited; EP's VERIFY-AT-FOLD banner came due and was discharged, and its
STOP-and-report condition did NOT fire.

⛔ **WHAT IS STILL OWED, AND IT IS DELIBERATE.** The snapshots in
`docs/architected-volumes-pending-fold/` are **NOT deleted** — that is a second
commit on THIS branch and it lands after the fold. Until it does, five stale
snapshots sit beside three landed volumes, which is the derive-don't-restate
hazard this program keeps being bitten by. ⚠⚠ **THE DELETION IS THREE PARTS AND
A `git rm` ALONE IS SILENTLY UNDONE:** (1) `git rm` the snapshot · (2) delete its
`cp` line in `scripts/refresh-archive.sh`, which otherwise copies it back and
`git add`s the whole directory on the next refresh · (3) update the README's
Contents row. ⚠ `HABIT_countsweep.py` and `HABIT_VERIFY_oddsratio.py` are
INSTRUMENTS, not snapshots — `seams()` at its L327 is the rule the estate's fold
arithmetic uses for every volume's §8.1, and deleting it would take the ruler
away with the thing it measured.

⛔ **LANDING IS NOT SEALING.** EP is a DRAFT AT ROUND SEVEN; HB's round four has
NO RECORDED CLOSE; WC is ARCHITECTED, NOT STARTED. Owner gates still standing:
CR-WC-9 (WC's persisted field batch), WY F9 `supplyCargo` UNSIGNED (WC-10
hard-gates on it), EP's four §7a parked rows, HB Q1's two persisted fields.
