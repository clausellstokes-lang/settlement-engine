...
### 6.4b ⛔ ONE LANE PER DOCK — the chair's own violation, 2026-09-14
The owner's law of 2026-09-11 is ONE LANE AT A TIME. The chair broke it by dispatching an implementer car into `laneRW-DEF2` while a prose workflow was running there. What happened: the implementer landed three commits and held seven dirty files; the workflow's CURE gate arrived, found HEAD three commits ahead and the porcelain dirty and GROWING (3 files → 7 during its own read-only check), and REFUSED — correctly, because a cure must rebuild `wiring-census.json` and re-freeze `prose-shift-register.json` and the other lane held uncommitted work in both. Six drafted pools were left mid-pipeline and an untracked file at the dock root vanished in the window (this repo's recorded lint-staged hazard).
THE RULE: a workflow OWNS its dock from launch to completion. An implementer car that must run at the same time gets ITS OWN WORKTREE — `git worktree add $SC/kit/lane-<name> <sha>` from the same repo — and its brief names that path, never the workflow's dock. A car that only READS may share, and must say in its brief that it reads only and must verify porcelain before and after.
THE RECOVERY when it happens anyway: the refusing gate is right; do not override it. Let the foreign lane finish or stop it by message, wait for a clean dock, then re-run the refused phase — the packets are on disk and a cure's target rows are usually byte-identical at the new HEAD, so nothing is lost but time.
...
- ⛔ THE LEDGER COMMIT IN THE MAIN TREE (learned 2026-09-14, the hard way): the main tree's INDEX is PRE-STAGED with the owner's WIP (~991 `D ` entries, visible in the first column of `git status`), so `git add <mine>` + a bare `git commit` commits ALL of it (it happened: 993 files / 367k deletions; repaired with `git reset --soft HEAD~1` + a pathspec commit). ALWAYS commit ledger docs as `git commit -F <msg> -- docs/A.md docs/B.md` (the pathspec form commits only those paths and leaves the owner's staged set where it was), and run `git diff --cached --stat | tail -1` BEFORE the commit — if it names anything but yours, stop. Never reset the index to 'clean' it; the staged set is the owner's.

### 6.8a ⛔⛔ THE LEDGER COMMIT GOES THROUGH THE TEMPORARY INDEX, NEVER THE PATHSPEC FORM (found 2026-09-15 ~03:5x)
§6.8's `git commit -F msg -- <paths>` is RETIRED for the main tree. It does keep the OTHER staged entries out of the commit, but git ALSO STAGES THE COMMITTED PATHS into the real index — so every pathspec ledger commit replaced the owner's staged entries for those docs with HEAD's content (measured: the owner's 223,179 B staged fragment of OWNER_DECISION_QUEUE.md is gone from the index, replaced by HEAD's 3.46 MB blob; not recoverable). The estate's own tool never touches the real index: `$SC/kit/chair-tools/chair-commit.sh <expected-tip> <message-file> [src:repo/path ...]` — a private `GIT_INDEX_FILE`, `read-tree HEAD`, `update-index --cacheinfo` per mapped file (the ODQ from the working tree is always included), `write-tree`, `commit-tree`, `update-ref` on the branch. The message file MUST carry exactly one seat trailer (`Seat: Fable 5.1 — validated` or `Seat: Opus 5 — Fable-unvalidated`); an Opus-seat commit must map `docs/FABLE_RETROVALIDATION_QUEUE.md` and its blob must differ from the parent's (§688.7); `--require-ref <name> <sha>` binds a declared seal to the act. Regenerate every file you edit from `git show HEAD:<path>` first (the checkout copies are stale). After the tool runs, the real index's cached stat must be UNCHANGED — check it.
