...
## 14. Edit at the source, never at the derivation (the owner, ODQ §934.44; thought through whole at the owner's word, 2026-09-19)

**The rule.** The world is a derivation graph rooted in the seed. A fact is editable only where it exists first. A derivation is never editable, on any card.

**Root means CHOSEN, not computed.** A strict reading — "a writer that reads no other fact" — would leave almost nothing editable: an NPC's role is drawn from the institution roster, a faction's archetype from the economy. What makes a fact a root is that the generator CHOSE it from a candidate set, however that set was gated by other facts. A derived fact is COMPUTED from other facts with no choice in it. The estate registers every seeded chooser in its decision-fork classification registry, so the root set is that registry's rows for the editable card kinds — a measurement, never a judgment.

**Four kinds of fact, four doors, no overlap.**
1. **Seed-level** — tier, terrain, culture, trade access, the pre-generation dials, the stress and constraint grids. The wizard's door: changing one is a regeneration. A DM who wants a famine at the start sets it there. The editor never touches them.
2. **Root** — a registered chooser's output: an NPC's name, role and disposition; an institution's name, class and standing; a faction's name, archetype, stance and power share; a power seat's holder; the rosters themselves (which institutions, NPCs and factions exist — additions and removals are roster roots). The editor's pencils, pluses and pools live here, and a root's pool is the candidate set its chooser drew from.
3. **Derived** — the economy, the supply chains, the defense scores, food security, the system states, the history. Never editable, on any card. A derived card shows PROVENANCE where a pencil would be — "follows from X — change X" — from a static producer map admitted by a walker that checks every named producer is live. The DM's lever on derived state is a DECREE at the tick. History is the chronicle's and immutable (THE PROMISE).
4. **Annotation** — the free notes. They exist only in the DM's layer; the flavor census proves nothing reads them (§12.3's readers proof, by property reads and value joins).

**The layer acts AT THE CHOOSER, not on the record (this supersedes the mechanism of §12.5; its intent stands).** If a root edit were patched onto the saved record, every derived fact the pipeline STORED at generation time — the power structure, the economy summary, the desk prose's inputs — would go stale, which is exactly the contradiction this rule exists to prevent. So for a DRAFT, an edit re-runs generation with the layer consulted at each root chooser: where the DM overrode the value the chooser takes it and STILL CONSUMES ITS DRAW, so every untouched root keeps its value; the record then carries the edited values and every derivation follows from them. THE PROMISE extends: same seed, same layer, same world. Generation is milliseconds, so an edit is instant. `dmLayer` records which roots are the DM's; the goldens are generation with an empty layer and cannot move.

**Canon stays events.** A canonized settlement cannot regenerate — history is lived. There an edit is a DECREE the simulation applies at the tick, and a rename runs the existing cascade on the record (`free-cascade`, §12.3). Drafts regenerate with the layer; canon simulates its decrees. Both paths end with one coherent record.

**Three things this makes simpler.** Phantom promotion (§2.8, §13) is "forge the seed with the phantom's traits as its layer" — the same mechanism. Additions and removals are roster roots, so the plus and the minus need no second machinery. The Surveyor proposes only root ops and decrees, which keeps the AI a clerk (FINITE-SEMANTICS).

**What leaves the editable set.** The system-state cards named in ARCH §9 are derived and are struck. The first card list is NPC, institution, faction, power seat; neighbours and phantoms are their own records.

**One edge, ruled by default.** A faction's power share is a chosen share the guard renormalizes to one hundred. It stays a ROOT under the totality guard (`renormalizeFactionPower` as the guard's `fulfil`): who is strong is a first fact; the rebalance is the guard's. Vetoable.

**What this changes in the train.** EM-A1's declarations are read from the decision-fork registry (a chooser without a registry row is registered first or its field is dropped); EM-A2's pools are the choosers' candidate sets; EM-B2 becomes the load-bearing packet — the layer hook at the root choosers plus the regenerate-with-layer orchestration — and splits in two if its budget demands; EM-F2's promotion is a forge with the phantom's layer.
