...
| Regional impact **resolve**: condition removed ↔ graph `resolved` ↔ cloud | Optimistic dual write + **fire-and-forget** persist (the F2 treatment was NOT applied here) | ⚠️ partial | `campaignRegionalSlice.js:525-589` |
...
|---|---|---|---|---|
| **R1** | Med | `resolveRegionalImpact` uses fire-and-forget `persistSaveUpdate` while its sibling `applyQueuedRegionalImpact` got the F2 ordered-await. A failed settlement save after the graph flips to `resolved` leaves a reload showing "resolved" with the condition still present. Lower severity than apply (resolve *removes* a condition). | `campaignRegionalSlice.js:525-589` | Apply the F2 phase-1/2/3 ordered-write pattern (await the settlement save before marking `resolved`). |
| **R2** | Low | External-mill export lockout + grain `banalité` note is scoped by `/\bmill/i` on `processingInstitutions`, which over-matches: it still suppresses exports + leaks the grain-specific note onto `floodplain_agriculture` (a non-grain chain). The worst over-reach (leather/fishing/pottery) is fixed. | `computeActiveChains.js:239-246` | Scope on chain id/resource (or require flour/grain outputs), not any processing-institution name matching `/mill/`. |
...
| **R6** | Low | No performance-regression budget in the gate; chunk sizes are hand-tuned with a raised warning limit. | `vite.config.js` (manualChunks + `chunkSizeWarningLimit`) | Optional: assert a first-paint/entry-chunk ceiling in CI. |
