...
| `src/domain/edit/dmLayer.js` | the DM's layer, applied ON WRITE (§2.4 as ruled by §12.5): the saved record carries the edited values and `dmLayer` records which fields are the DM's, so every reader (PDF view model, prose readers, world book) sees one record | `applyEdit(record, layer, op) → { record, layer }`, `reapplyLayer(regenerated, layer)` (regeneration re-applies; goldens ignore), `DM_ID_NS`, `mintDmId(seed, kind, n)` |
...
| `src/domain/edit/phantoms.js` | phantoms as MINIMAL SAVE RECORDS (§2.8 as ruled by §12.6): `kind: 'phantom'`, a seed, the pooled traits, in the owner's library and hidden from the shelf, so every neighbour derivation that resolves by save works unchanged | `mintPhantom(seed, name, kind) → saveRecord`, `promotePhantom(record) → seed` (forging the seed replaces the record in place), `isPhantomRecord(record)` |
