...
2. `flavorFields.census` — every `free` field has zero GROUNDED readers under `src/generators`, `src/domain/worldPulse` and the FILE `src/domain/causalState.js` (the fourth root the earlier wording named, `src/domain/edit/guards*`, matches ZERO files anywhere under `src/` and is dropped rather than walked as an empty set), measured by PROPERTY READS (the receiver-shape scan over `scripts/lib/writer-reach-scan.mjs :: scanSurfaceReads`, keyed by `writerIdentity(key, shape)` over a corpus carrying an EXPLICIT `rootShapes`: an empty `R` proves zero GROUNDED reads and never zero reads, so the census's own key-level site roster of declared unrelated receivers carries the rest, and the two register baselines carry neither half) AND by VALUE JOINS (the rename cascade's join list): a field the cascade joins on may only be `free-cascade` (§12.3); a proof string per row.
