...
| `src/domain/edit/dmLayer.js` | the DM's layer, applied AT THE CHOOSER (§14, superseding §12.5's mechanism): for a draft, an edit re-runs generation with the layer consulted at each root chooser — an overridden root takes the layer's value and still consumes its draw, so untouched roots keep theirs; the record then carries the edited values and every stored derivation follows; `dmLayer` records which roots are the DM's; the goldens are generation with an empty layer | `layerRead(layer, key) → value \| undefined` (the chooser hook, one call site per registered root chooser through the pipeline's `drawRoot(rng, key, pool, layer)`), `applyEdit(layer, op) → layer`, `regenerateWithLayer(seed, config, layer) → record`, `DM_ID_NS`, `mintDmId(seed, kind, n)` |
