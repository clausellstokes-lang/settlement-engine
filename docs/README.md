...
| [`ENGINE_DEFECT_DISPOSITIONS.md`](./ENGINE_DEFECT_DISPOSITIONS.md) | LONG TAIL #40 — the code-facing disposition for each engine defect measured and deliberately NOT cured: what was measured, at which line, and what a lane must not do about it. Append-only. Read it before "fixing" anything that looks like a live defect in the generation or display layers. |
