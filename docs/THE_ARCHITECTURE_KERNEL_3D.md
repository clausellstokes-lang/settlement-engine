...
> **STATUS — RETIRED FROM THE SETTLEMENT-MAP CRITICAL PATH (ODQ §287).** The map is a
> deterministic zero-tilt 2.5D survey plate driven by canonical `MassPart[]`; it does not depend on
> facade meshes, oblique views, WebGL or this cathedral-detail kernel. Keep this as research for a
> separately authorised future per-building inspector/detail provider only after canonical massing
> exists. It may never become a second town-map authority.
