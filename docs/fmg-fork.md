...
|---|---|---|
| `~8–30` | The diagnostic channels `INFO` / `TIME` / `WARN` are gated on FMG's own `PRODUCTION` constant (`Boolean(DEBUG.<ch>) \|\| !PRODUCTION`) instead of the flat `true` they were vendored with; `ERROR` stays unconditional. | REVIEW-P F15 (ODQ §934.63) walked the public path and found `/realm` the **only** route with anything in its console — `Generate random map`, `Unresolved depressions: N. Edit heightmap to fix`, `TOTAL: 0.31s` — while every other route was clean on both viewports. All three are `WARN &&` guards (two in `main.js`, one in the vendor bundle `index-*.js`, which is **not** touched). `PRODUCTION` was declared at the original vendoring (`f386f48d96`) and read **nowhere** in `public/map/**`; this reads it. `import.meta.env.DEV` is unavailable here — `public/` is served verbatim and Vite never transforms it — so the hostname test is the DEV signal, with the same meaning (localhost keeps every diagnostic, a deployed host prints none). All 64 `console.time` calls are paired with 64 `console.timeEnd`, both `TIME`-guarded, so muting `TIME` cannot orphan a timer into a new warning. The `debug` localStorage key re-opens any channel on a deployed host (the existing `DEBUG.cloud` idiom). Pinned by `tests/lint/embeddedMapConsoleGate.walker.test.js`. |
