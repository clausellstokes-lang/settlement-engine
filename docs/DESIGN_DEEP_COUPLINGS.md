...
| D-3 | beliefs (triggers) · bonds/obligations · trade edges · credibility stocks | generosity mints/ledger (gift, consideration) AND the transfer record — generosity OWNS intelTransfers + intelCooldown (mint + prune, generosityKernel; the build inverted the draft's ownership) · statecraft ledgers (injection, credibility deltas; intelTransfers READ-ONLY — informationStatecraft only consumes) · newsEntries |
