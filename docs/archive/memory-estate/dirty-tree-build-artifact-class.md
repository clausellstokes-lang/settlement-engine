...
metadata:
  ...
  modified: 2026-08-07T16:57:31.751Z
---
...
## ⚠⚠ A PURE `git mv` OF A BUNDLED INPUT IS A CONTENT CHANGE (bit the CHAIR, 2026-08-07)

The law says "a lane TOUCHING bundled inputs must rebuild edge-shared in the same commit," and
a rename reads as not touching — no line changed, no diff hunk in the file itself. **It breaks
the bundles anyway**, because the meta records input PATHS and `aiGroundingBundle.freshness`
asserts *"every input path in the meta still exists on disk."* A `git mv` of
`src/generators/hookThemes.js` → `src/domain/hookThemes.js` reddened three edgeFunctions tests
at `67f8a58e` with the file's bytes untouched.

⭐ **RESTATE THE TRIGGER AS: any commit that changes a bundled input's PATH OR ITS BYTES owes a
rebuild in the same commit.** A layering/relocation lane is exactly the kind that will not think
of itself as a "bundle lane".
⭐ **AND THE INSTRUMENT LESSON:** the per-file TYPECHECK ratchet was green on that commit; the
TEST ratchet caught it. Different instruments catch different classes, and a green ratchet is
evidence about its own class only — never about the commit.
