...
metadata: 
  ...
  modified: 2026-08-10T17:59:59.717Z
---
...
## ⚠ THE TWO-BRANCH DOC LAYOUT, AND THE `git show` TRAP (folded from the index 2026-08-10)

There are TWO live branches and the files are NOT where the names suggest:

- **The LEDGER branch** `review-fixes-2026-07-08` (checked out in the MAIN tree) holds
  `docs/archive`, `docs/architected-volumes-pending-fold`, and `OWNER_DECISION_QUEUE.md`.
- **The BUILD branch** `claude/composite-r4` (worktree `.claude/worktrees/minifold`) holds
  `docs/FABLE_VALIDATION_QUEUE.md` and `docs/SOL_QUEUE.md` — despite the ledger-sounding
  names, neither is on the ledger branch.

⚠⚠ **`git show` resolves an object from EITHER tree**, because both trees share one object
database. So a successful `git show <branch>:<path>` does **NOT** mean the file is on your
branch or in your working tree. Check `git ls-files <path>` (or the branch name explicitly)
before assuming you can read or edit it in place.
