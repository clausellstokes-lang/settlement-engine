...
metadata:
  ...
  modified: 2026-08-07T12:47:33.577Z
---
...
## ⚠⚠ AMENDMENT 2026-08-07 — THE LAW SERIALIZES BUILD LANES AND FORGOT VERIFIERS

**MEASURED BY A VERIFIER MID-ROUND: the single vitest gate slot was held by TWO OTHER VERIFIER
LANES for ~30 minutes**, confirmed by `gate-mutex` pid lines, with two `--wait` cycles timing
out at exit 3 before acquiring. The law counts BUILD lanes and says nothing about verifiers —
but an adversarial verifier re-runs every gate it is checking, so **a fan-out of N verifiers is
a fan-out of N gate consumers**, and they contend with each other AND with the build lanes.

⭐ **THE FIX IS ORCHESTRATION, NOT THE LAW'S SHAPE: fan verifiers out on ANALYSIS, serialize
them on EXECUTION.** Give each lens a distinct read-only slice that does NOT need the suite
(diffs, censuses, source reading, archive-vs-archive greps), and let at most ONE verifier hold
a full-suite run at a time. A three-lens parallel fan-out where every lens runs `npm run check`
costs three serialized gate runs of wall-clock and buys one gate's worth of information.

⚠ Second-order effect, observed: **a walker can PASS IN ISOLATION AND FAIL THE REAL GATE ON A
TIMEOUT** when the machine is contended — `observedShapeReaders.walker.test.js` did exactly
this. That failure is a STRUCTURAL TIMEOUT, not a ratchet red, and the two must be labelled
separately or a lane "repairs" a defect that does not exist.
