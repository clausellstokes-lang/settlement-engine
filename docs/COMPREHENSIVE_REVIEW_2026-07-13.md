...
# PHASE V VERDICTS — ROUND 2 (Opus adversarial verification, 2026-07-13)
19 CONFIRMED / 2 PARTIAL(downgraded low) / 0 REFUTED. test-quality-6 verifier died (structured-output cap) — MANAGER-ACCEPTED as probable pending direct check.

**[build-tooling-docs-3] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — Self-contained ~6-line script change, safe direction (fails where it now passes green; cannot make a passing gate fail on legitimate clean/regression runs). Build tooling only — no shipped bytes, no seed output, no migration/schema/public-API/security surface. The register's proposed condition ('out
- Fix: Distinguish 'ran clean' from 'did not run'. Set a boolean in the catch (`threw=true`), and after parsing fail loudly (exit 1) when `threw && total===0` — i.e. tsc exited non-zero/was killed yet produced zero parseable domain-error lines (config-resolution failure, broken install, OOM all land here, including the empty-output kill case the register's error-TS-substring condition misses). This is st

**[build-tooling-docs-4] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — Test/lint-only. sf-bridge.js lives in public/map (iframe), not the ratcheted app bundle, so no first-paint bytes. No golden/seed impact. One tension: extracting pure helpers OUT of sf-bridge.js complicates the fmg-fork.md 'reapply as-is' runbook — the jsdom-eval-the-IIFE variant avoids editing the f
- Fix: Confirmed structural-prevention gap on a byte-producing constitutional seam. Prefer the fork-preserving variant: eval sf-bridge.js's IIFE in jsdom with stubbed FMG globals (pack/cells) and pin the getSpatialPack contract — copy-not-alias, field set {h,biome,r,p,c}, TypedArray→plain conversion, null-on-missing-cells. This keeps fmg-fork.md's 'leave sf-bridge.js alone' runbook intact (no source spli

**[code-quality-1] → CONFIRMED/CONFIRMED (final severity high)**
- Blast: goldens_shift=False eager_bytes=True persistence=False owner_gated=False — No same-seed byte golden covers AI narrative prose (edge, non-deterministic), so gating or wiring the wrapper shifts no golden; a parity test (tests/lib/entityRefWrapper.parity.test.js) covers id-parity only. eager_bytes_risk: porting the renderer trio + wiring tokenizeProse into DossierNarrativeBan
- Corrected: CONFIRMED as written. The producer (wrapEntityRefsInProse) runs unconditionally on this branch's edge function while the token consumer (tokenizeProse renderers, commit 6d95adc7) is not an ancestor of HEAD, so this tree ships a producer with no consumer. The wrapped prose is authoritative (ai.js:267) and persisted, and is rendered verbatim at DossierNarrativeBanner.jsx:46 and npcComponents.jsx:244
- Fix: Prefer the bold-but-contained port over the gate: bring master's renderer trio (ProseParagraph.jsx, pdf/primitives/ProseText.jsx, the OutputContainer provider hoist) onto this branch and wire tokenizeProse at the four raw-render sites (DossierNarrativeBanner thesis + per-tab note, npcComponents goal.short, pdf Overview) so the already-shipped producer resolves instead of leaking. Add a producer⇒co

**[code-quality-2] → CONFIRMED (final severity low)**
- Blast: goldens_shift=False eager_bytes=True persistence=False owner_gated=True — Eager-bytes risk applies ONLY to the owner 'mount the gallery pair' disposition — adding Realm gallery components to the first-paint closure risks CLOSURE_BUDGET_BYTES (law 5, the +363B Wave-2 feed bust is precedent). The delete/test-add/inventory-test paths are byte-neutral or byte-reducing. owner_
- Corrected: The census holds: 12 modules, ~1,124 lines, zero PRODUCTION (src) importers, additive to the register's six, untracked in any ledger. Two corrections to the evidence: (1) "zero importers plus tests/scripts/api/tools" overstates — 4 of 12 have test importers, so they are dead PRODUCT code but not literally importerless (this is the code-quality-3 "tested-but-unconsumed" class); (2) data/categoryVoc
- Fix: Fold into an only-shrinks inventory test that walks the import graph FROM ENTRY POINTS over all of src — not "any import," or the 3 test-only-consumed orphans stay hidden — with a small allowlist for JSDoc-typedef and worker-URL modules. Owner triages per file: the 8 fully-orphaned are delete candidates; the 3 tested-but-unconsumed (Disclosure/debounce/StatTile) join code-quality-3's triage; categ

**[code-quality-3] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=True — Zero importers means these modules are tree-shaken out entirely — no first-paint bytes, no generation/same-seed impact. The minimum fix (correcting the two false headers, or deleting dead modules+tests) is byte-neutral and NOT gated. The wire-vs-delete triage per module IS owner-gated: it is new-cap
- Corrected: Accurate. Minor: actual line total ≈1,769 (register says ~1,780; per-file counts run ~1 high) — negligible. Two headers falsely claim consumers, not one: pipelineRail.js:5 AND provenance.js:5.
- Fix: Two tiers. Immediate/byte-neutral: correct pipelineRail.js:5-7 and provenance.js:5-6 so they stop asserting consumers that do not exist (the successor-AI trap, in a doctrine where docs must match code same-commit). Structural: fold into code-quality-2's proposed only-shrinks all-of-src unused-module inventory ratchet, baselining these 8 with an explicit "roadmap read-model, no consumer — owner: wi

**[code-quality-4] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=True eager_bytes=True persistence=False owner_gated=False — goldens_shift: swapping a passthrough clamp for a finite-guarding one changes NaN/non-numeric output, so ANY site where such a value currently rides through would shift a same-seed golden — the finding's per-site byte-identical gate keeps the intended migration byte-neutral, but the risk is real and
- Corrected: Confirmed as a duplication/latent-inconsistency finding, not an active bug: the ~66 (not ~30) clamp-family helpers all agree on finite inputs, so no current byte-identity break is demonstrated; the hazard is that malformed/non-finite input diverges three ways (NaN→lo, NaN→0, raw passthrough) and every new mover inherits whichever neighbor's variant it sat next to. MEDIUM is well-calibrated — above
- Fix: Bold structural-prevention first: add a lint nudge (deepCloneHotPath-style) NOW — zero bytes, and it halts the still-live propagation (4 movers copied the passthrough variant in the last 48h). Then add src/kernel/math.js exporting clamp/clamp01 with an EXPLICIT documented NaN policy — pick NaN→lo/0 (a non-finite value must never silently ride through a determinism engine). Migrate existing sites o

**[code-quality-5] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=True persistence=True owner_gated=False — A correct parametrized-primitive extraction is byte-identical per site (goldens stay green — that is the fix's own gate; if any golden shifts the refactor is wrong, not the goldens). schema_or_persistence flagged because these functions MINT persisted ids (institution id-joins, npc ids, prebuilt ref
- Corrected: Confirmed as written. One calibration note: of the 8 variants, only the two domain/events copies are accidental duplication; the dash-vs-underscore differences are deliberate per-namespace id conventions. The finding already reflects this ("the underscore family happens to agree") and its parametrized-primitive fix correctly preserves the intentional differences rather than collapsing outputs.
- Fix: Land the bold shape: one kernel/leaf slugify(value,{sep,max,fallback}) with each id-minting site importing it and passing its own params — byte-identical output per site, goldens + save round-trip prove no id drift. This preserves the intentional dash (foundry/dossier) vs underscore (engine) namespaces while removing the copy-discipline hazard. Minimum acceptable: merge the two byte-identical doma

**[code-quality-6] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=True persistence=False owner_gated=False — Pure import re-routing: `export *`→curated named re-exports and repointing consumers at simulationRules.js leaf changes module boundaries, not runtime values, so same-seed engine output is byte-identical (goldens unaffected). eager_bytes_risk flagged because trimming the region barrel so wizardNews 
- Fix: Adopt the house's own known-good pattern (spatial/index.js): curate region/index.js to explicit named re-exports (or split a display-safe leaf for REGIONAL_CHANNEL_TYPES/summarizeWizardNews) with the "must NEVER enter the entry static closure" law comment; repoint BOTH worldPulse-barrel consumers (LivingWorldGates.jsx:39 and useRealmInspector.js:182) at simulationRules.js directly, then delete/shr

**[code-quality-7] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — The ratchet itself is eslint-config-only: byte-neutral, zero first-paint impact, no persisted shape touched, aligned with already-established house doctrine (F31 generator ratchet + components 600 ratchet) so not owner-gated. Grandfather-list selection is decide-within-scope/record-vetoably. The SEP
- Corrected: Finding stands exactly as written. One fix-shape gap: the named grandfather list (warDeployment, pulseKernel, settlement.schema, causalState, explanation, capacityModel) is INCOMPLETE. If the ratchet ceiling is set "just above today's worst offenders," the actual worst offender — src/domain/display/causeConjunctionRoleContent.js at 3933 lines — is unnamed and would fail the gate; if set lower to c
- Fix: Add a src/domain/**/*.js max-lines rule mirroring F31, but set the ceiling to exert real pressure (e.g. 800-1000, matching generators) rather than "just above 2024" which ratchets nothing. Pair it with a COMPLETE shrink-only grandfather list covering every current offender — critically causeConjunctionRoleContent.js (3933) and the eight other >1000-line files — or the gate breaks on first run. Shi

**[content-immersion-1] → CONFIRMED/CONFIRMED (final severity high)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — Pure lazy-display read-model change, byte-inert to the engine and persisted state; the rumor surface is a lazy dossier tab (dossierLazyTabs), so no first-paint budget impact; aspatial goldens are marker-gated/dormant and untouched. No same-seed shift.
- Fix: Add a what-token → in-world phrase vocabulary in the display layer (the IMPACT_LABELS pattern), keyed over the candidateType families (war/coup/faith/flow/institution/npc/stressor), with a humanized fallback that strips 'npc_'/'stressor_birth_' prefixes and maps bare transition kinds ('queued'/'applied') to a neutral word ('trouble'/'unrest') rather than rendering them. Pin with a register-guard t

**[content-immersion-2] → CONFIRMED/CONFIRMED (final severity high)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — newsVoice.js is a lazy-display read-model imported only by the lazy WizardNewsPanel (per its own docstring: ZERO first-paint bytes, byte-inert to engine, never imported by generation/kernel). Both a guard fix and new content categories stay in the lazy chunk — no eager bytes, no golden shift, no per
- Fix: Bold fix (owner's bold-over-safe preference, and calamity currently gets NO line): author a fourth 'pestilence' and fifth 'calamity' VoiceCategory with onset/impact/relief/fade cells, and classify impactKind 'plague_arrival'→pestilence and 'calamity'→calamity ahead of the channelType fallback, so the sim's most dramatic emergent events get correct voice. Minimum safe floor if content isn't authore

**[content-immersion-4] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=True eager_bytes=False persistence=False owner_gated=True — Pure data edit but generation-facing: removing/replacing entries reindexes maleNames/femaleNames/surnames/settlementSuffixes ⇒ same-seed NPC and settlement names shift for east_asian/mesoamerican (and 'mixed' resolving to them), colliding with the constitutional same-seed byte-identity law. Must lan
- Corrected: Confirmed. One precision refinement to the surveyor's example set: the given-name and settlement-suffix contamination ('burg', 'Kayla', JP surnames-as-given-names, Gemini/Pleiades/Zero/Venus, cross-gender 'Ixchel') is reachable at ALL tiers because firstName and settlementSuffix pools are consumed unsliced. The colonial-surname examples ('Zero Valladolid', 'Gemini Yucatan') are reachable only at m
- Fix: Data-hygiene sweep of NAMING_DATA: drop/replace the confirmed contaminants — 'burg' suffix and 'Kayla' in east_asian; the eight JP surnames masquerading as east_asian given names; Gemini/Pleiades/Venus (and reconsider 'Zero') in mesoamerican femaleNames; 'Ixchel' from mesoamerican maleNames; the Spanish-colonial surnames Quijada/Valladolid/Yucatan. Land it inside the owner-gated round-21 Wave 5 go

**[content-immersion-5] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=True persistence=False owner_gated=False — Correct fix (lazy display read-model over already-persisted structured fields, newsVoice sidecar pattern) is golden-free + eager-free. No golden byte-pins generated summaries: worldpulseSpatialGolden projects to a mechanical summary with wizardNews entries:[] (test line 125/10-11); feedDistribution 
- Corrected: Accurate as stated; one calibration: "system-log register" is slightly strong — the summary is a human-readable sentence, but its prefix vocabulary ("Queued"/"Ready"/"Applied … via <channelType>") is verbatim engine-lifecycle wording and the reason pills are scoring-receipt vocabulary, so three registers genuinely coexist on every card beneath a diegetic headline and an in-world crier quote. Purel
- Fix: Route the card body through a lazy display read-model in the news chunk (mirror byte-inert newsVoice.js): re-compose from the entry's persisted structured fields — swap TRANSITION_LABELS engine words for in-world phrasing ("Word arrives of…", "Now felt in…", "Has run its course"), recast reason pills into the fiction register simulationProfile.js already uses for receipt headlines. Render in place

**[content-immersion-6] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — 
- Fix: Give plague arrivals the feed real significance grammar, local to materializationNews: significance becomes major when severity is at least 0.55 or the front is multi-settlement, else notable; delete the dead minor literal. Optionally promote scope to realm and lift the score so the marquee emergent event threads as the headline it is. Marker-gated and feed-layer only: no schema, no eager bytes, a

**[determinism-constitution-3] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — Lint config + test-only change; no product bytes, no bundle, no generator output. Worker is verified clean so the new error rule reds nothing existing. Pure prevention/hardening (structural-prevention doctrine), not owner-gated.
- Corrected: Finding is accurate as written. Refinement: this is an unrecorded coverage asymmetry, not a deliberate deferral — the temporal audit deliberately extended only the wall-clock class to workers (its charter), leaving the entropy/locale classes uncovered with no ledger note. Severity is medium at the prevention end: the worker is a thin shim delegating to covered domain code, so realistic attack surf
- Fix: Split the workers/kernel eslint block: add Math.random + localeCompare + toLocale*/Intl selectors to a workers-only block (workers have no legitimate ambient draw), and add the locale/Intl selectors (NOT Math.random) to the kernel block — kernel's rngContext.js:99 and prng.js:79 use Math.random by design but never locale. Extend localeCompareGuard/localeFormatGuard TREES with 'src/workers' and bum

**[sim-logic-counterparts-10] → PARTIAL (final severity low)**
- Blast: goldens_shift=True eager_bytes=True persistence=True owner_gated=True — Preset content edits are EAGER (first-paint budget). Lighting war/faith/seasons requires those features to graduate their soak/convergence gates first; changes simulationRulesPreset.stability.test golden and preset re-inference for keyless saves matching the old shape. disastersEnabled is owner-park
- Corrected: dramatic_campaign genuinely lights zero drama systems (existence CONFIRMED) — but this is a deliberate, twice-documented, owner-gated design/tuning gap correctly parked as round-21 Wave 4, not an actionable defect on this branch. The finding's medium severity and priority-raise overstate it: lighting the flags is NOT "just preset data" — it activates soak/convergence-gated subsystems (warLayerEnab
- Fix: Do not act now. Keep as the already-scheduled Wave-4 owner-gated review. If pursued, the bold-but-safe shape is a mid-rung dramatic_campaign (warLayerEnabled + settlementStrategyEnabled + seasonsEnabled at dramatic intensity) sitting between realistic_regional and full_simulation — but ONLY after those subsystems clear their soak/convergence gates, with owner sign-off, a regenerated stability gold

**[test-quality-3] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — Test-only hardening; no product code, no digest/byte-golden value change, fixtures untouched. Aligns to a pattern already established by 6 sibling goldens in the same tree — repair, not new capability. Not owner-gated.
- Fix: Align the four keystone-family goldens to the safe sibling pattern: guard the writeFileSync behind `if (UPDATE)` only, guard the readback (`existsSync(MANIFEST) ? JSON.parse(...) : {}`), and add the sibling's failing pin `it('manifest exists (run UPDATE_GOLDEN=1)', () => expect(existsSync(MANIFEST)).toBe(true))`. A dropped fixture then reds instead of silently re-minting. Bolder structural option:

**[test-quality-4] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — Test-only, additive. New pins in tests/store/catchUpCampaignWorld.test.js. No src change, no goldens/eager/schema impact. Fixture settlement already carries a compromise-severity condition (activeConditions), so forcing a mid-loop major to trigger the living pause is feasible in the existing headles
- Fix: Add two living-mode pins to the existing suite (no source change): (a) seed progression:'living', drive a multi-week catch-up where a mid-loop tick surfaces a major → assert catch-up early-breaks (weeksCaughtUp < elapsed, capped:false), the major is queued as a proposal not auto-resolved, and the cursor sits at the last completed week; (b) after resolveIntervalMajors clears the pause, assert the n

**[test-quality-5] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — New jsdom test file only — not in the shipped closure, no product code, no goldens. Mitigant worth noting (does NOT refute the gap): the store action re-guards at slice.js:429 `if (!campaign || !advancesOnOpen(rules)) return`, and the store cursor lastLivingAdvanceAt makes a same-week re-fire a no-o
- Fix: Add one jsdom component test reusing the existing worldPulsePanel.test.jsx harness (extend the vi.mock actions map with a spy `catchUpCampaignWorld`). Mount with worldState.simulationRules.worldProgression='autonomous' (and 'living'); assert the spy fires exactly once on open, does NOT re-fire on re-render (caughtUpIds ref), and never fires for a dm_advanced/legacy campaign (absent worldProgressio

**[test-quality-7] → CONFIRMED (final severity medium)**
- Blast: goldens_shift=False eager_bytes=False persistence=False owner_gated=False — Test-config only: a per-test timeout arg lives in test files (not the app bundle) and cannot alter generation output, so same-seed byte-identity, budget closure, and persistence are all untouched. Mirrors an already-shipped repo pattern.
- Corrected: Confirmed as written: the seed-sensitivity test (and the sibling deep-identity test) run under only the 20s root default while ~13 sibling suites carry an explicit per-test override; the documented "pre-existing env red" is an unsound deferral because the repo's own one-line precedent fix is already applied elsewhere and labeled "not drift."
- Fix: Mirror the established precedent: append a per-test wall-clock allowance (120_000, matching generatorGoldenMaster's 400-vs-190-generation weight, or 60_000 like distributionEnvelopes) as the third arg to the seed-sensitivity test — and to the heavier same-seed deep-identity test (numRuns 100) — with the standing "wall-clock allowance, not drift" comment. Bold-safe within the constitution; no produ

**[test-quality-8] → PARTIAL (final severity low)**
