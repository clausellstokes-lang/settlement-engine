...
## on the LEDGER branch (review-fixes-2026-07-08 @ 151afbf6), UNCHANGED IN SUBSTANCE except for the ONE
## addition named next.
## ⭐ **SECTION 8 IS SPLICED IN AT THIS FOLD** from
## `docs/architected-volumes-pending-fold/WC_SEAM_TABLE.md`, at that document's own
## instruction ("the fold splices §8 / §8.1 in"). It is a TRANSCRIPTION of SECTION
## 4 into the house four-column shape plus ONE row authored under the 2026-08-07
## chair ruling (row 23, SP-D). It invents no seam and mints no contract. The
## parent's §9 pointer resolves HERE, which is why the splice is required rather
## than optional. ⛔ **THE HEADER SPELLING `Neighbour` IS LOAD-BEARING — DO NOT
## "FIX" IT.** `HABIT_countsweep.py`'s `seams()` matches header cells by EXACT
## equality, so `Neighbor` returns `LookupError` and the count reads as ABSENT
## rather than zero.
## ⛔ **THE FILENAME THAT SAID "in-progress" WAS ALREADY FALSE AND DOES NOT SURVIVE
## THE FOLD.** This volume's own PROGRESS blockquote reads ARCHITECTED, NOT
## STARTED; it carries three amendment rounds (round 3 closing with ZERO parked
## findings) and terminates `END OF VOLUME.`
## ⚠ **THE CLOSING LINE'S "CR-WC-1..CR-WC-22 await rulings" IS STALE AND IS NOT
## REWRITTEN HERE** (reported erratum, struck text preserved): **all 22 were RULED
## 2026-08-06** (`b75e8c4d`, rulings in
## `docs/architected-volumes-pending-fold/WC_CHAIR_RULINGS.md` on the ledger
## branch) and **all 17 waves are DISPATCHABLE**. CR-WC-21 was ruled Position B
## and **UNBLOCKS** WC-15. ONE escalation stands — CR-WC-9, the field batch,
## owner-gated on persisted shape — and it blocks WAVES, not the fold.
## ⚠⚠ **THE MEASUREMENT BASE IS `cbd348a5` AND IS MANY COMMITS STALE. RE-MEASURE
## AT BUILD; DO NOT INHERIT.** §8.4 records five premises worth re-checking and
## the ONE premise the fold REFUTES: §1.9.1's and WC-5's "`errandMint.js` is
## ABSENT at HEAD" was true at `cbd348a5` and is FALSE now — SP-D has landed.
## ⚠ **TWO NEIGHBOUR-STATUS HEADERS IN SECTION 4 ARE STALE BY EVENT AND ARE
## REPORTED, NOT REWRITTEN** (struck text preserved in place): §4.1's
## ~~"HABIT (the habit-conditioning directive; NOT YET ARCHITECTED)"~~ — HB
## IS architected and landed as docs/DESIGN_FP_ARCH_HB.md IN THIS SAME FOLD,
## ten waves and four flags, so every §4.1 contract is now a contract with a
## LANDED sibling; and §4.3's ~~"ESPIONAGE (…; ES-0 landed, waves in
## flight)"~~ — ES has advanced materially since `cbd348a5` and §8.4 item 2
## records that rows 12 and 13 and the whole §8.2 ES trio are written against
## a MOVING neighbour, so §8.2's recommended inbound repair must be checked
## against what ES has ALREADY landed rather than against the volume's
## snapshot of it.
## ⭐ **THE ENCOUNTER TABLE IS THE ONE PLACE THIS VOLUME MOVES A SIBLING'S
## GUARD.** SECTION 8 row 7's E16/E17 claim makes
## docs/DESIGN_FP_ARCH_WY.md §4's "anti-vacuity at fifteen" and WY-6's
## `sixteenth-arm plant` mutant go stale the day WC-12 builds. The erratum is
## reported in WY's own §4 and in the parent's WY-6 block IN THE SAME FOLD
## COMMIT, with the struck text preserved: the rows land WITH their resolvers
## in WC-3's and WC-12's commits, and WY-6's anti-vacuity arm must DERIVE the
## count from the table rather than restate a literal.
...
# SECTION 8 -- SEAMS PINNED BOTH SIDES

> ✅ **SPLICED IN AT THE 2026-08-07 INTEGRATION FOLD** from
> `docs/architected-volumes-pending-fold/WC_SEAM_TABLE.md` (ledger branch,
> review-fixes-2026-07-08 @ 151afbf6), whole and unedited below this note, at that document's own
> instruction. Its prose still speaks of itself as "a separate file" and of the
> volume as "not touched" — TRUE WHEN WRITTEN, and preserved rather than
> rewritten so the transcription stays auditable against SECTION 4 line by line.
> **The parent's §9 compressed range row points HERE**, and
> `docs/DESIGN_FP_ARCHITECTURE.md` §9 is normative for the range, this section
> for the twenty-three rows themselves.
