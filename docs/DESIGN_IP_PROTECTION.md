...
- **Status:** ARCHITECTED 2026-08-14 (Fable chair). Not yet folded to the build
  branch — this volume lands on the ledger branch first because the build branch
  is sealed under the GR-4B-IIIB implementation session at authoring time.
- **Fold obligation:** at the first integration point after the seal lifts, this
  file folds to the build branch verbatim, passing the enforcement-claims
  `CLAIM_RE` scan in the same change (the per-claim law binds gated docs). Until
  then, the ledger copy is canonical.
