...
*File:* `src/App.jsx:533-560` (mobile header), `src/App.jsx:733-776` (mobile bottom nav) — re-measured 2026-09-06 against a 947-line `App.jsx`. The former `:358-424, 661-726` predated the June chrome edits and named neither block; every citation in the table below was re-derived at the same commit.
...
| Q | Verdict | Finding |
|---|---|---|
| Cohesion with the whole flow: clean handoff, no dead-ends? | **good** | Bottom nav priority is explicit (`MOBILE_NAV_PRIORITY`, App.jsx:88) precisely to avoid silently dropping items, and the 5-seat cap is the `.slice(0, 5)` at App.jsx:482, which evicts only the lowest-priority entry — About. About does **not** demote to the account menu: that dropdown carries Account / Messages / Manage subscription & credits and nothing else (AccountMenu.jsx:185-201). Its mobile door is the footer's LegalRibbonRow (LegalRibbonRow.jsx:107, mounted at App.jsx:728), exactly as the source comment at App.jsx:85-86 states; the landing route suppresses the global footer (App.jsx:722) but LandingBelowFold mounts the same row (LandingBelowFold.jsx:326), so the door survives there too. No dead-end. Sound. |
| Placement by importance & touch targets (~44px)? | **good** | Bottom tabs use `flex: 1` with an explicit `minHeight: 44` (App.jsx:757) above their vertical padding of `SP.sm + 2` (App.jsx:758), so the floor is asserted rather than inferred from padding. The compact chip enforces `minHeight: 44` on both of its forms — the anon Sign In button (AccountMenu.jsx:97) and the signed-in chip (AccountMenu.jsx:153). Touch targets are adequate. |
| Accessibility: two-channel state, contrast? | **adequate** | The active tab carries four visual channels plus a programmatic one: GOLD_BG fill (App.jsx:759), a 2px GOLD top border (App.jsx:761), GOLD label colour (App.jsx:763), `fontWeight: 700` (App.jsx:764), and `aria-current="page"` (App.jsx:749). Inactive labels are PARCH_100, not SECOND (App.jsx:763, same ternary). The green `#4A7A3A` this row used to flag is gone — zero occurrences in App.jsx — because the chip that carried it now takes SHAFT_SAGE / SHAFT_STEEL (AccountMenu.jsx:136-138), pinned at exactly 4.63:1 on the chip's own lightest ground (tests/design/contrast.test.js:495-496). Verdict left at **adequate** pending a chair re-grade: the evidence above would now support a stronger cell, but this pass was scoped to citations. |
| Distinctness / intuitiveness: does the auth slot read as account vs a destination? | **adequate** | There is no bottom-nav auth slot to grade. The bar maps `mobileNav` to five destination buttons and renders nothing else (App.jsx:742-774); the code this row used to cite is deleted. The account control on mobile is the top bar's compact AccountMenu chip (App.jsx:550-558) — User icon, name, chevron, `aria-haspopup="menu"` (AccountMenu.jsx:145-149) — which does read as identity rather than as a destination. Adequate rather than good only for placement: it sits at the top of a phone screen, away from the thumb-reachable bar that carries every other primary control. |
| Consistency: does the mobile auth entry match the desktop account chip? | **good** | Re-graded weak → good on 2026-09-06 by measurement. There is one `AccountMenu` import (App.jsx:37) and both surfaces render that same component: mobile at App.jsx:550-558, desktop at App.jsx:651-658, with byte-identical `onSignIn` / `onAccount` / `onMessages` / `onManageSubscription` handlers (App.jsx:556 vs :656). The only difference is the `compact` prop (App.jsx:551), which changes size, padding, `maxWidth` and the 44px floor — not the interaction model: both open the same dropdown. The old "weak" rested entirely on the single-chrome slot's straight-to-`setView('account')` jump, and that slot was deleted at `8bf493d05`. |

*Overhaul:*
- **[retired 2026-09-06 with the `mobileSingleChrome` flag]** Give the bottom-nav auth slot a subtle visual separation so it reads as identity, not a destination. — There is no such slot to separate: the bottom bar renders five destination buttons and nothing else (App.jsx:742-774, inside the bar at :733-776), and the flag's read sites were deleted at `8bf493d05` (fix(nav), 2026-06-22). Whether to rebuild the single-chrome bar is an owner row (rebuild-or-bury); this item cannot be actioned until that is decided. (was P11)
- **[done — superseded by measurement 2026-09-06]** Re-check the green account-chip color on the dark gradient for AA. — `#4A7A3A` no longer appears in App.jsx (zero occurrences). The chip's status tones are now SHAFT_SAGE / SHAFT_STEEL (AccountMenu.jsx:136-138), pinned at exactly 4.63:1 on the chip's own lightest ground and above the AA text floor, with the superseded GREEN / GREEN_DEEP and V3 dark steps kept as negative controls so restoring any of them reds with the reason attached (tests/design/contrast.test.js:485-486, 495-496, 533-537). (was P7)
