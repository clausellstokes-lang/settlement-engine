...
| THE DOCTRINE BLOCK — SUPPLY-WEB + INFORMATION STATECRAFT + THE CORRUPTION WEB (3 of 4 waves) + FP-G4 −12,730B + RATCHET #6 | 12554bd3 (D1), eb1537dc/642c28d8 (D2a/b), 02b66c4c/0ae0f4f9/6481e5ce (D3a/b/c), (this: FP-G4 + ratchet) | THE POLITICS-OF-POWER BLOCK, 3 of 4 waves LIVE-DORMANT: SUPPLY-WEB WARFARE (belief-gated web read, the wrong-village strike, 6 instruments, atrocity brakes, the economic_strangulation peace feed wired) · INFORMATION STATECRAFT COMPLETE (SEE/HIDE/LIE/SHARE + CREDIBILITY AS A STOCK — the Blainey-credibility pin: wars against liars converge slower; the symmetric-isolation pin; the self-policing intel market; the fracture-credibilityHit seam closed) · THE CORRUPTION WEB COMPLETE (resolveLeash chokepoint + the innocent-guild live-bug repair; foreign creation w/ scarcity-as-law + evil-generosity recruitment; the capture FORK — foreign grip ≠ thievesGuild; direction-tamper + paid-eyes; the blowback triple w/ cutout double-surfacing; the beneficiary composer picker; official-pay/HIDE/purge counterplay partial). 3c took first-paint to EXACTLY budget (0 margin) → FP-G4 census found economicData dragging the 21KB TRADE_DEPENDENCY_NEEDS table eager for a 9KB classifier → finishedGoodsCategory leaf, −12,730 B (42× the ask). RATCHET #6: 1,161,902 → 1,149,256 (measured 1,149,172; window-close per the sequencing law; owner floor 1,214,050 honored by ~65KB). BENIGN SHARED-TREE parallel-commit race on D3b (staged files committed by a parallel session under its own msg; verified linear+identical, adopted — pattern in memory). OWNER-GATED + CHIPPED (untouched): the probe-CONFIRMED covert-impairment-text public leak (needs the client+SQL-twin scrub). Gate 9,370 green throughout. NEXT: W-DOCTRINE-4 settlement politics → THE ENGINE IS COMPLETE. |

| **THE ENGINE IS CLOSED** — W-DOCTRINE-4 settlement politics lands; every simulation system that will ever exist is built | 300aeb74 | The final engine wave: the 3-bloc capped politics ledger; formation = interests × THE PEOPLE (the leader-rivalry-blocks-the-alliance pin); glue typology (concession/patronage/doctrine/threat/compromise — succession dissolves loyalty not contracts; exposure detonates compromise into scandal); ruling-bloc decision loading (±0.3 §H — corruption-tamper's overt twin); the consolidated-court-raises-the-patron's-price cross-read CLOSED; differential term burden → faction revanchism (the peace seam consumed); conspiracies in autarchies via covert→revealed. FULL ENGINE INVENTORY NOW COMPLETE + DORMANT: M1-M11 physical world · E0 tempo · E1 generosity (6 instruments + verbs) · W-PEACE 1-3 (reasons/prices/treaties) · W-DOCTRINE 1-4 (supply-web / info statecraft + credibility / the corruption web / politics). Six ratchets banked: 1,255,985 → 1,149,256 (−106,729 B, −8.4% first paint) across the stretch that BUILT ALL OF IT. Suite 726→~845 files, 8,348→9,395 tests. REMAINING = surfaces + interpreter + proof: SM-3, W-GUIDE-1/2, W-COMPOSER-2 (forecast/docket + realm verbs incl. the parked manifest lifts), Surveyor S1-S7, SM-4, the re-grade, THE SOAK (charter: century probes + settlement-cap study + gate-matrix testing + tuning), the master merge per plan, owner deploy batch. Owner-gated standing: covert-leak scrub (chipped, owner started), E0+preset lighting regen, the vetoable stack. |

| **THE REOPENING** — the owner recommissions the engine for three final waves (convergence / navy / upswing); the design corpus fully freezes | db4bd86a (W-INTERVENTION commission), c88e0129 (DESIGN_CONVERGENCE), ee3919ef (blockade-is-a-siege law + naval addenda), 1b3d5ecd (DESIGN_NAVY) | Post-close scope reopening, owner-driven across four messages: foreign armies join internal contests on either side (empire-retention / regime-change / investment / kinship / denial); counterforce resolves as battle with the loser retreating; THE N-SIDED PUSHBACK ("you are thinking in binary") → sides are AIM-GROUPS, rivals never merge, the vulture dynamic; navies convoy own/allied armies, sea battles carry land parity (resolveFieldBattle verbatim, loser retreats to home port); THE BLOCKADE LAW verbatim: "a blockade is the same as a siege" (a blockade mints a siege via routeIntercepted + the interdiction term; blockade alone strangles, combined arms starves per the M8 both-cut pin); piracy = bandits on water, damped by patrol. Naval recon headline: M8 sea lanes FULLY BUILT — armies already implicitly sail; navies live in spatialLedgers, NEVER the frozen digest slot; the M8 "no fleet combat" scope is consciously superseded (DESIGN_NAVY §0). Gap-audit verdict: with W-CONVERGENCE + W-NAVY + W-UPSWING (the designed-unbuilt reconstruction/boom counterweight) the simulation is CONCEPTUALLY COMPLETE — nothing else passes the four-boundary filter. IN FLIGHT at this row's writing: SM-3 map edits BUILT + checker-passed (12 files unstaged in the main tree; 393 B over budget → FP-G5 micro-reclaim dispatched per the standing reclaim-first/no-raise ruling; integrates at window close per the RATCHET LAW); W-GUIDE-1 complete on claude/w-guide-1@a648dc44 (merges after SM-3); W-CONVERGENCE dispatched (Opus, worktree, per DESIGN_CONVERGENCE). |

| **THE SM-3 WINDOW CLOSES + RATCHET #7** — the map becomes editable; the guidance layer lands; first paint drops again | e275349f (FP-G5 merge), dd9b01a2 (SM-3), W-GUIDE-1 merge, (this: title fix + ratchet) | One integration window per the window-close law: FP-G5's census hunt found resourceData.js co-hauling the lazy-only RESOURCE_CHAINS + INDUSTRY_WATER_NEEDS eager → resourceChains.js data-lazy leaf, −7,006 B (the ask was 600). SM-3 SETTLEMENT MAP EDITS: drag-to-nudge anchor-keyed pins / deterministic reroll / labels+legend, blob-persisted via the applyEvent triple (never ghosts on reload), mapEdits ∉ public projection (fail-closed pin), absent ⇒ byte-identical, +~460 B eager (the persist action only) — reclaim-first honored, NO raise. W-GUIDE-1: the guidance registry + walker + one-whisper budget + firsts unification; ghost stratum deleted (ActionRail, PostGenCoach out); five violator dispositions live. CROSS-WAVE CATCH: the full gate's ONE red was W-GUIDE-1's new shrink-only title= census (476) catching SM-3's four new native tooltips (parallel builds, neither saw the other) — fixed to aria-label (accessible name kept, tooltip gone, the walker's own migration rule), focused re-verify green. RATCHET #7: 1,149,256 → 1,142,580 (measured 1,142,495, 7 files, + 85 house margin). Full gate 9,453/9,455 pre-fix (the census red + 1 standing skip), walker + SM-3 UI batteries green post-fix. IN FLIGHT: W-CONVERGENCE + W-UPSWING (Opus builds); W-DISCOVERY census banked, design next. |

| **W-CONVERGENCE MERGES — many banners, one field** | 9a45c6d0/a0a92d22/f241a17f/8cf97033 (stages 1-4 on claude/w-convergence), 990f4232 (merge) | The intervention + multi-sided-war wave lands whole: foreign armies join coup contests on either side (5 typed motives w/ full risk integration — time-discount, invited/uninvited legitimacy asymmetry, counter-intervention); interventionAdj into resolveCoupVerdict on the warSentimentAdj precedent (0-when-dark ⇒ byte-identical, pre-wire dormancy golden ×3 seeds); SIDES ARE AIM-GROUPS (rivals never merge; THE VULTURE PIN passes — hold-EV > engage-EV while rivals grind, receipted); counterforce resolves pairwise via resolveFieldBattle verbatim, loser retreats through the recalled homecoming; aftermath states w/ re-engagement dwell; deployments become loud degradable FAKEABLE rumor facts (phantom-column pin: a false belief triggers a real priced reaction); foreign_clash lands WITH its spheres_understanding peace mirror + non_intervention in the NEW sovereignty term family; installed-regime-owes obligation mint; proxy-stays-proxy. + THE MERCENARY CLAUSE (owner, same day): bounded facet-driven affordability-scaled reinforcement at the strength seam, never actors (4 pins). New lazy convergence.js — ZERO eager delta; 57 pins; branch gate 9,452 green; MERGED-TREE gate 9,511/9,512 + build contracts 115/115 at ratchet #7. Deferred w/ records: verbs registrable-shape → the W-COMPOSER-2 lift; spatial column-INTERCEPT seam inert (isolated-ledger JUDGMENT); rebellion-verdict tilts typed seams. Implementer self-remediated another wrong-lineage worktree (the in-brief guard). NEXT: W-NAVY dispatches on this tip. |

| **W-UPSWING MERGES + RATCHET #8 — recovery becomes an event; first paint breaks 1.13 MB** | 45eb5387..7ed60013 (7 commits on claude/w-upswing), 9aa95107 (FP-G6), bf9cb406/b43866de (merges), (this: ratchet + row) | THE CALAMITY BUCKET lands as ruled (stage 0): mechanics were already type-blind — flavor demoted to a cosmetic hint under the SAME persisted key; bucket-neutral prose (fixes the live quake-draws-flood voice mismatch); NORMALIZED exposure loading (realm strike budget provably unchanged — the mean-1 pin); FORCE_CALAMITY through the shared resolveStrikeOnSettlement path (force ≡ organic); calamityHistory's first UI ledger. The one-time shift CONTAINED: no lit hash-goldens existed — two assertions updated in place, dark worlds byte-identical throughout. THE ARCS: B1 reconstruction (ally credit MATURES obligations; completion = history beat + legitimacy dividend + upgrade UP the reversed lattice + the low-conscience skim), B2 boom→bust (hysteresis; the condition records its own fragile edges; bust names the artery; W-DISCOVERY seam), B3 flourishing (peace-dwell counter built; never snowballs; the 7th drama class registered), stage 4 the extraction-upswing EV term scaled by the conqueror's OWN corruption grip. The 8-settlement war→peace→rebuild→boom acceptance soak PASSES. Merge conflict (pulseKernel mover seam, convergence×upswing) resolved keep-both-landed-order; both dormancy goldens + generosity golden + the soak green on the MERGED tree (26/26). FP-G6 in-window: the ~24KB NPC-secret STRESS_INSTITUTION_EFFECTS table rode first paint on ONE pass-through re-export → stressInstitutionEffects.js leaf, −21,664 B (18× the ask). RATCHET #8: 1,142,580 → 1,122,663 (measured 1,122,578; −10.6% total from the program's 1,255,985 start). Merged-tree full gate: 9,566/9,568 with the only reds = the documented load-flake set (all five files 51/51 in isolation, twice). Upswing's +948 B eager = the condition-archetype catalog only (the :714 structural-eager hazard). |

| **W-NAVY MERGES — the sea is a road where losing means drowning** | c7fb4433..cf0e20e3 (8 commits on claude/w-navy), 70334dfa (merge, ZERO conflicts) | One navy per port (capability DERIVED: digest isPort ∧ maritime facets, economy-scaled, AFFORD_MIN floor — a standing shipyard always fields a token squadron); convoys as navalTransit records (owner ≠ cargo supported; the armyRecordOf role-coercion trap FIXED; storm seasons priced into convoy EV; SEA_LANE_CAPACITY gains its first consumer); SEA BATTLES by resolveFieldBattle VERBATIM on the new shared-sea-edge predicate — loser retreats, SHARED FATE = the heaviest bounded band + debark, never annihilation (pinned); THE BLOCKADE LAW end-to-end: the navy cuts the sea supply axis through the shared supplyKernel seam (BOTH the M2 and commodity-flow paths), blockade-alone strangles / land+sea starves (the M8 both-cut pin proven); PIRACY PARITY ships its EMPTY behavior-change set (sea-arrival loss pin + shared-seam guard + display naming only). ALL naval state in spatialLedgers.navalTransit; the frozen M8 slot BYTE-UNCHANGED; the no-fleet-combat scope comment consciously superseded (the one sanctioned amendment, frozen-slot keys asserted intact). blockade_declared registered across all 6 authority sites (contract-tested). Zero eager delta at ratchet #8. Fenced dormancy golden (spatial ∧ virtual navalEnabled). MERGED-TREE GATE: **9,612 passed / 0 failed / 1 skipped — the cleanest full run of the program** (all three reopening waves' dormancy goldens green together, 54/54 focused battery). Implementer self-remediated the wrong-lineage worktree (again). SEAM-NOTES tracked to W-MOMENTUM's window: blockade→economic_strangulation peace-reason wire; naval movements into the rumor carrier (phantom fleets); allied-convoy auto-derivation; blockade-alone war_front entry. Verbs ORDER_CONVOY/DECLARE_BLOCKADE registrable-shape → the W-COMPOSER-2 lift. |

| **W-DISCOVERY MERGES — the ground gives, and the ground gives out** | 847f4f54/9803baa7/d68faa05 (claude/w-discovery), b9eddc3b (merge, zero conflicts) | Organic resource dynamics on the tierResourceDynamics candidate lane: DISCOVERY from the terrain-organic latent pool (JUDGMENT upgrade: generation's own rollable vocabulary via getCompatibleResources, not the display table — geography-inconsistent strikes impossible by construction; prospecting integrator floor+cooldown, §H-loaded by need/boom/extraction presence); REMOVAL only for nonrenewables after the 156-tick depleted dwell (integer arithmetic, catch-up-collapse-proof, pinned); ONE atomic writer (membership + regen-surviving resourceEdits dual-write with force≡organic PINNED against the real addResource verb + the surgical reconcile BOTH directions — discovery activates chains/exports next tick, removal prunes them + resourceHistory + typed conditions). The upswing seams CONSUMED: vein_exhausted → the B2 bust flip; resource_strike → boom sources.discovery. The mine founds itself (gap-detector integration, zero new wiring, pinned). Empty-roster tolerance pinned. Zero eager delta (JUDGMENT: conditions planted fully-specified at the lazy writer, NOT catalog templates — saves +330 eager B and the faction-1:1 pin). MERGED-TREE GATE: **9,654 / 0 / 1 skipped — matches navy's perfect run.** Implementer survived a mid-build API stall (resumed from checkpoint, zero loss). Pre-existing taxonomy bug surfaced honestly (grain_fields→magical via the 'bar-LEY' substring; coal→renewable) → CHIPPED w/ golden-shift protocol (task_ae3b00f3, owner running it in a parallel session). 4 of 6 engine waves home. NEXT: W-LIFECYCLE dispatches on this tip. |

| **W-LIFECYCLE MERGES — the ladder gains its two ends (5 of 6)** | fcbfb051/ceabc93d/11e2859a (claude/w-lifecycle), 1c18a116 (merge) | Birth and death land at the thorp rung, +21 B eager TOTAL (one condition-key string; the +948 ceiling respected 45×). SATELLITES: town+ parents seed steadings (§H integrator; tier caps deferral-receipted); grow on conserved parent transfers / starve into dwelled seeded death (residents return) / CONVERGE (sums, both named); CHARTER-PENDING at village (the owner's graduation ruling — visible, never silent); one capped steading_tributary lift. FIRST-CLASS: terminal death = thorp tier + 104-tick dwell (never sudden), proposal-gated + campaign-altering; DEATH IS A STATUS — the digest cell survives (geometry pin: reference-identical through a full die-cycle); THE SCARCITY PIN (relic_ruin iff peakTier ≥ city — the monotone stamp catches every promotion path incl. DM SHIFT_TIER); THE FATES PIN (dispersal stamps only, roster invariant — no fate resolved at the grave's edge); dispersal + RESETTLEMENT conserved to Σ=0 (the relic name half-returns, peakTier restarts). Verbs registrable-shape (force ≡ organic pinned; the manifest park holds by pin). Ancients OPT-IN (existing seeds byte-identical). Dormancy golden PRE-WIRE FENCED w/ a real negative control. INCIDENT LOG: (1) the implementer first REFUSED its brief as suspected injection (new hazard class → the VERIFY-FIRST preamble is now standing in all briefs); (2) its resumed session hit the worktree-vanish fallback and checked out IN THE MAIN TREE — caught mid-build, finished under explicit shared-tree rules, protocol held (no foreign files staged, stash untouched, no checkout at exit; coordinator restored review-fixes + merged). Merged-tree: budget green (1,122,599 ≤ 1,122,663), all 5 wave goldens + lifecycle battery 53/53, full suite green modulo the documented load-flake files (51/51 isolated). THE ENGINE AWAITS ONLY W-MOMENTUM. |

| **W-GUIDE-2 MERGES — the world teaches; the software whispers (the voice + the reference)** | 8b2e59c8/4189ddb9/61587515/a1c51814/72de7534 (claude/w-guide-2), 9d13e8bd (merge, zero conflicts) | THE VOICE lands: guidanceNotes.js (F3a sidecar — frozen NOTE_LINES cells holding COPY KEYS through the t() harness (JUDGMENT: diverges from newsVoice's inline prose to honor the copy-harness law; the walker binds both directions), totality floor, FNV-1a variants) + SurveyorNote (suggestion-card + ✦ eyebrow + serif + "— S.") + THE REGISTER GUARD (note register requires second person, bans UI verbs; persona never on commerce — library=note, gallery/realm=plain per the precedence guards). EMPTY-STATES-AS-INVITATIONS registered; legacy fragment ledger 8→6. THE GENERATED GLOSSARY: 52 terms DERIVED FROM CODE + generate script + byte-identity freshness test (reference cannot drift) + the honesty-gated what-am-I-reading affordance (first consumer: HealthPip) w/ Compendium deep-links. Title census RATCHETED 476→471; whatPhrase never-leak pin; dissociation checklist artifact. ZERO eager (closure 1,122,548 — BELOW base; 3 new lazy-leaf dist guards; the wave's own guards caught 2 real bugs mid-build: a tree-shaken sentinel + a frozen-baseline trip). Branch gate 9,704/0. MERGED-TREE: build contracts 119/119, batteries 60/60, full suite 9,770 green w/ only the documented load-flake files (35/35 isolated). DEFERRED w/ records → W-GUIDE-2b task: the Keeper's Handbook compression + the deep title tranche (text-bearing controls need per-title wiring). |

| **⬛ THE ENGINE IS COMPLETE — W-MOMENTUM merges; every simulation wave that will ever exist is in the tree** | 1c1da462..251c7dec (8 wiring stages on claude/w-momentum, atop the 5-commit pure core), 52cc0240 (merge, ZERO conflicts) | The psychology layer goes LIVE behind momentumEnabled∧beliefsActive: the commitments ledger folds legible public acts (loudness-measured, lawfulness-scaled); the entity threshold reads the seat's ACTUAL temperament (TRAIT_MOMENTUM over authored personality, §G-clamped) + conscience doors + legitimacy fragility; THE LIMIT CLAUSE mechanical (the belief discount scales only value re-anchoring, bounded below; contradiction-widening keeps full weight — reality always wins, pinned); consumption at the chooser/belief-seam/abandon-gate; THE CRACK priced once, mediation genuinely reduces it. RECEIPTED EMERGENCE: the same past-cliff stock holds under the proud king and CRACKS under the pragmatic heir — the NEW-RULER PEACE lives with zero code written for it. Zero eager; no new draws (M9d green); naval seam 0(a) landed (blockade→economic_strangulation), (b)(c)(d)+3 face-save signals+court-structure reader seam-noted to micro-waves. Shared-tree note: the wiring agent took the branch over from the stale core worktree via --ignore-other-worktrees (non-destructive; stale worktree pruned after merge). **THE ENGINE-COMPLETE GATE: 9,843 passed / 0 failed / 1 skipped — PERFECT, all six wave dormancy goldens + 119 build contracts green on one tree.** Program totals at engine close: suite 8,348→9,843; first paint 1,255,985→~1,122,5xx (−10.6%, 8 ratchets); every wave dormant, gated, receipted. NEXT: W-COMPOSER-2 (the manifest lift) + FP-G7 dispatch on this tip. |

| **W-COMPOSER-2 MERGES + RATCHET #9 — every capability forceable; the realm lift lands NET-NEGATIVE on first paint** | fa4813cf..135148bc (7 commits on claude/w-composer-2), 60f257d4 (merge), 00f791ab (window close: trim + ratchet) | THE MANIFEST LIFT: all **14 parked verbs** registered into the new lazy realmManifest leaf behind a fail-closed coverage walker (12 executable via ONE force-as-proposal lane — DECLARE_CASUS/SUE_FOR_PEACE/ORDER_SUPPLY_RAID/DECLARE_TRADE_EMBARGO/ORDER_INTERVENTION/ORDER_CONVOY/DECLARE_BLOCKADE/FORCE_RECONSIDERATION/FORCE_CALAMITY/FORCE_FOUND_STEADING/FORCE_ABANDON/FORCE_RESETTLE; REINFORCE+INTERCEPT registered grayed-with-reason, execution on their own convergence seam; SELL/LIE exempt w/ documented seam). Force ≡ organic: each verb resolves through its wave's kernel arm; the DM's word replaces the loaded dice per the M9d held-mint law; BOTH proposal re-mint deferrals (intervention_ordered, blockade_declared) CLOSED. THE FORECAST (§10): queue-inclusive clone-runs through the REAL drain + REAL interval orchestrator — preview ≡ apply PINNED byte-for-byte. THE MUTABLE DOCKET (owner law verbatim): every queued commit editable/cancelable until consumed; lapsed orders refuse VISIBLY. THE VOICE: RealmVerbComposer (grayed-with-reason as visible text), glossary 52→66 regenerated, mechanism whispers, title ratchet honored. Composer eager: only **+2,500 B** (the single lane ≈5× cheaper than the per-verb precedent). WINDOW CLOSE: FP-G7's receipt-backed census proved the safe reclaim well otherwise DRY (the data monoliths walled by customRegistry whole-row retention; light-surface refactor = owner-gated); the ONE reclaim = the engine-core over-inclusion trim (4 generator-only/lazy-only leaves excised from the derived ENGINE_SHARED_DOMAIN; formatNumber deliberately kept — 27-consumer churn risk) → **−3,260 B. RATCHET #9: 1,122,663 → 1,121,903 (measured 1,121,818 — BELOW the pre-composer base: the eager-threading wave SHRANK first paint).** No raise (owner away ⇒ not self-ruled; reclaim was). Window gate: **9,887 / 0 / 1 skipped.** Incident: the composer agent hit the resume-worktree-fallback (2nd recurrence) — contained under shared-tree rules, protocol held. |

| **THE MASTER MERGE OPENS — W0 complete, W1 (the merge commit) dispatched** | 8c430c5b (chip harvest + the W0 tip) | Owner-resequenced (verbatim: "move merge and deploy up") + push-by-PR ruling. **W0 SETTLED:** master UNMOVED @ d024286e (== the worktree-trap commit — mystery solved: provisioning defaults to master's tip) so every master-side plan disposition HOLDS; censuses re-derived (568 conflicts / C2 435 / C3 231 vs plan's 554/433/241 — same shape); satellites settled (persist-gap 151a8ee3 ALREADY ancestor — the §8 owner call is MOOT; the taxonomy chip HARVESTED @ 8c430c5b — barley≠magical, coal≠renewable, full-catalog classification pins, no goldens shifted; ab1c30ba/ccd0d670/c0600f8c/263e53e0 scheduled W4/W5; 92973282 moot; 312a5025 deferred past merge); baseline gate 9,887/0. PLAN-STALENESS corrections issued to the implementer (live budget 1_121_903 not 1,215,520; RF-wins governs where RF-side lists drifted). W1 RUNNING: claude/master-merge-r1 cut from 8c430c5b, `git merge master`, dispositions per §4/§5, Stage-1 golden neutrality, the arbiter = golden byte-identity. Then W2 fence → W3 ports + THE owner-signed regen → W4 F24 bytes → W5 guard-port/entity-link → W6 test estate → W7 the full constitutional gate → **W8 OWNER BATCH (push claude/master-merge-r1, PR INTO master, §8 decisions; merge button = owner's).** |
