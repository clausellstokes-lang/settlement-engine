...
|---|---|---|---|---|---|
| LANDING | TE-UCSTACK-LANDING (Fable) | laneUCSTACK-tree (fresh, from a5c6c4fa) | laneUCSTACK-receipt.md | UC-0 then UC-3 rebased onto f32c548e (one jointVocabulary copy); both packets at LANDED; tip in `ucstack-rebased-tip.txt`; terminal `ucstack-TERMINAL-<tip8>.log` + `ucstack-TERMINAL-exit.txt` | collect the terminal if it ran; else resume; CAS `f32c548e → <tip>` on green; prune holding-uc0/uc3 |
