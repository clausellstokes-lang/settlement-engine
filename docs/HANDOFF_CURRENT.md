...
3. For EVERY lane in the table below (grep BOTH scratchpads — 6298872d AND 695a70c5 — before declaring a lane artifact-less, §470): read its receipt's LAST `RESUME POINT` / `STARTED` block, `git -C <worktree> log -3 --oneline && git -C <worktree> status --porcelain`, then re-dispatch a fresh executor that RESUMES FROM THE RECEIPT (survey-first; verify every claimed proof). Lanes die with the session; their worktrees and receipts do not.
...
|---|---|---|---|---|---|
| LANDING | TE-HKA-LANDING (Fable) | laneTEHKA-tree | laneTEHKA-receipt-r2.md §"THE LANDING SLOT … slot 64da7d5d" | rebasing f2b2711b onto 64da7d5d (+ the §478.3 WEB-2 row deletion + D-HKA-1 comment fix in the landing act); tip in `hka-rebased-tip.txt`; terminal `hka-TERMINAL-<tip8>.log` + `hka-TERMINAL-exit.txt` | collect the terminal if it ran; else resume; CAS `64da7d5d → <tip>` on green; prune holding-hka |
...
| RESEARCH | TC-R-INST-4 RESUME (Fable, solo) | — (read-only) | laneTCRINST4-receipt.md §"RESUME STARTED" + draft-R-INST-4-HOSPITALITY-POVERTY-UTILITY.md §0.3 status map + RINST4-merge/ | merging six killed sub-lane transcripts, WebFetch-only | on completion: record §, then the second seat takes UC-4 build (Opus) |
...
COMPLETE AND PINNED (holding for landing slots): HK-A `f2b2711b` (IN the landing seat) · UC-1 `53d7e492` (holding-uc1; 8 over c1295938; carry the census DELTA +1/+0/+1/+8/+1, never its tuple — §480.2) · (the map stack LANDED at 421c7345).
...
⭐ §456 OWNER ORDER: the DWELLINGS program runs AFTER its research (not post-endgame) and lands BEFORE the terminal soak; the 300-year soak gains a DW leg (DW-S). CHARTER RULED-AS-AMENDED §473: website-2 = `695a70c5/scratchpad/draft-WEBSITE-PLAN-2.md` — W-C = WEB-8 → WEB-9a → WEB-9b → WEB-10 (money-path, STOP-class, builds last); W-D = WEB-11 (one car). RULED CHARTERS: undercity = `draft-UNDERCITY-PLAN.md` (§441/§445); producers = `draft-PRODUCERS-PLAN.md` (§433/§434); website = `draft-WEBSITE-PLAN.md` (§402). Research delivered, owner taste-gate owed: `draft-DWR1A-CONSTRUCTION-HISTORY.md`, `draft-R-INST-1-CIVIC-DEFENSE.md`, `draft-R-INST-2-TRADE-CRAFTS.md` (582 KB; §462), `draft-R-INST-CIRC-ADDENDUM.md` (281 KB; §466), `draft-R-INST-3-FAITH-LEARNING.md` (370 KB; §476) — all ACCEPTED as grammar inputs at §464.3.
STANDING LAWS FOR EVERY BRIEF: a MODIFY of any edge-shared bundle input (grep the `.meta.json` rosters at S0) owes `npm run build:edge-shared` in the same commit and `tests/edgeFunctions` in the sweep (§475) · an acceptance arm over a shared namespace pins INTENT (positive control + exclusion set), never `toEqual` a list the next train car must grow (§474) · a census-class ratchet "pre-existing" claim needs the RECEIVED-LIST DIFF, never title identity (§469) · builders NEVER run `npm run check:tail` (§460.1 — the widened mutexed sweep is their proof) · short per-lane TMPDIR (`/tmp/<lane>`) · the mutex exports INLINE ON EVERY COMMAND LINE — `export GATE_MUTEX_LOCK_DIR=/tmp/settlementforge-vitest-gate.lock; export GATE_MUTEX_MAX_POLLS=480; export TMPDIR=/tmp/<lane>; sh scripts/gate-mutex.sh --run -- …` (§440.2/§457: the harness gives each command a fresh shell; an earlier export is gone) · `npm run check:tail` BARE, never under the mutex · exits captured in-shell, never `${PIPESTATUS}` · receipt stub FIRST — BEFORE any reading (§468) — and a `RESUME POINT` block at every proof boundary and at least every 30 min (§448) · baseproof at b10ed1a1: `/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/695a70c5-80ee-4ebd-b806-a8c102244d16/scratchpad/chair-baseproof-b10ed1a1` (re-create with `git worktree add --detach … b10ed1a1` + `cp -Rc` node_modules if purged) · seat model §447 (builds Opus; research/compile/landings Fable).
- ⭐ §481 SEAT MODEL AMENDED (08-23): LANDINGS → OPUS under a Fable brief (UC-1 landing first); research/sweep sub-lanes carry an EXPLICIT `model` (Opus; Sonnet for fetch-and-extract); phase-split any lane at ~150 tool calls with a resume brief — transcript replay is the dominant cost (measured: Fable 1.14B context tokens vs Opus 0.47B this session).
OWNER DOCKET (by nature only, §464): the db-push train 195/196/197/198/199 (198 DESTRUCTIVE — read DEPLOY.md's preambles section first) · every push/deploy · the tuning SIGNATURE · legal (TinyMCE/GPL, FMG, founder transfer) · `/code-review ultra` at the §290 stop (the owner's keystroke). Everything else was RULED at §464 (R-5 lights in tuning; R-6 no; R-7 via the ONE REGEN; O-P1 de-advertise → WEB-8; O-P2 Surveyor tier $14.99 → WEB-9; O-P3 pin → WEB-8; taste-gates discharged as research inputs).
