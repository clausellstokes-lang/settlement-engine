...
⭐ **STATE AT §702 (2026-08-27):** ledger through **§702** · seals: GROW-FOLD `56ee43dad` · DRESS-1 `ad2b8d399` · DRESS-1b `6e33a0637` · SPINE-3 `96bcbbbf5` · DRESS-FABRIC `f8f6456a0` · instruments **r6** `02e63b9f4` · toolkit **r7** `9915e0786`. ⛔⛔ **THE VISUALS QUESTION IS SOLVED AND IT IS FRAMING, NOT PAINT (§702.1).** The reference **fits the settlement diameter to the page's short side from the settlement radius alone** — settlement at **44 % of a 16:9 plate, TIER-INVARIANT**; countryside generated to 3× the frame radius, **~89 % off-page**, remainder painted with **ZERO INK**. **We do NO framing at all** — a fixed 1000×1000 viewBox against a radius varying 20× in area, which is why our settlement share measured a *range* (1.7–6.9 %). Fields were added by the author a year AFTER the framing, *to reduce empty space* — **so L-REG-33 was never on the scale.** ✅ **TWO OWNER ROWS STRUCK** (page budget · rotation dial — we already implement the reference's angle gate exactly); aggregation downgraded to informational (the reference's mass count comes from **paint order**: stroke at double weight, fill over — free, keeps the plot bijection). **NEXT: WALL-CURTAIN in flight; then `DRESS-FRAME`** (§702.8 — fit the page · countryside toward bare paper · **the wall:road weight ratio, ours 0.30 vs the reference's 0.95** · the paint-order silhouette). Sequential — the gate is a mutex. ⛔ **POINT EVERY MAP LANE AT THE CURRENT SEAL, NEVER MAINLINE** — mainline has 5,565 changed paths and no harness drivers (§702.7).
