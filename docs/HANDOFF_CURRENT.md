...
2. Re-arm the §388 wakeup (CronCreate `13,33,53 * * * *`; the four checks; ⛔ LANE CAP = FOUR per §508 (a sub-lane still counts; the gate is SERIAL so at most ONE landing/terminal + three builds on DISJOINT file sets; `GATE_MUTEX_MAX_POLLS=960`; disk check FIRST) — the landing seat + one; NO fan-out, a sub-lane counts) — it dies with the session.
3. For EVERY lane in the table below (grep BOTH scratchpads — 6298872d AND 695a70c5 — before declaring a lane artifact-less, §470): read its receipt's LAST `RESUME POINT` / `STARTED` block, `git -C <worktree> log -3 --oneline && git -C <worktree> status --porcelain`, and resume it MID-LANE from that point with a resume brief (never restart from zero; §448).
4. Any DETACHED GATE survives: check `<lane>-TERMINAL-*.log` for `TRUE_EXIT=` and `[gate-tail] exit:`; a header-only log with no live `gate-tail` process was killed → re-fire via `chair-detach.py` (program scratchpad). On a two-part green + disk ≥300MB: CAS the build ref (`git update-ref refs/heads/claude/composite-r4 <tip> <old>`), delete the holding pin, prune the worktree, then the lane TMPDIR only after its final report.
5. Ledger every act by the PRIVATE-INDEX method (never `git add -A`; the main worktree matches no branch); rewrite THIS card (the whole block, not a string patch — a missed patch went unnoticed §452→§482).
...
NEXT DISPATCHES: CH compile (IN SEAT) → CH skeptic (§441 J7) → chair rules → CH-1 build → landing; **DW-0 charter compile at the next free seat (§482/§493.4)**; then CH-2 → CH-3 → UC-5 → the W-C cars (WEB-8, 9a, 9b, 10) → WEB-11 → HK-4/5/6. ALL CH cars land before DW's BUILD waves. → **DW-0 charter compile (Fable, §482 — starts the moment R-INST-6 delivers)** → UC-5 → WEB-8 (Opus) → WEB-9a → WEB-9b → WEB-10 (last, STOP-class) → WEB-11 → HK-4 → HK-5 (head-figure guard, §479.2) → HK-6 (strip "Eberron" from three catalog/variant strings, §488.3) → housekeeping sweep (requiredSymbols rows skipped at stacked landings; MF-T1X prose; S0 schema-10 mint) → content train CT-4/CT-5/WF-8 → DW build phases (Opus) → ONE trailing OSR mint → parity train → `/code-review ultra` (owner) → endgame.
⭐ §456/§482 OWNER ORDER: the DWELLINGS program runs AFTER its research and lands BEFORE `/code-review ultra`; the 300-year soak gains a DW leg (DW-S). CHARTERS: undercity = `6298872d/scratchpad/draft-UNDERCITY-PLAN.md` (ruled §441/§445); website-2 = `695a70c5/scratchpad/draft-WEBSITE-PLAN-2.md` (ruled-as-amended §473; W-C = WEB-8→9a→9b→10, W-D = WEB-11; set `STRIPE_PRICE_PREMIUM` before creating the Surveyor price).
⭐⭐ §499.2 THE MASTER PRE-REVIEW SEQUENCE (canonical; do not re-derive): **A** CH compile + DW-0 charter/architecture compile → their skeptics → chair rules → ⭐ THE OWNER'S BAND SITTING (gates every DW build) · **B** CH-1 → CH-2 → CH-3 → UC-5 → MP-1 (halo v1) · **C** WEB-8 → 9a → 9b → 10 (STOP-class, last) → 11 · **D** HK-4 → HK-5 → the sweep · **E** DW-1 → DW-2 → ⭐ THE ESTATE WAVE → DW-3 → DW-4 → DW-5 → DW-6 (MP-1 re-points here; the UI is built ONCE) → DW-7 → DW-S · **F** CT-4/CT-5/WF-8 (interleaves; finishes before G1) · **G** the ONE trailing OSR mint after the LAST DW car → the parity train · **H** `/code-review ultra` (owner). Then the endgame. RULES: CH lands before DW-1 builds · MP-1 early, re-pointed not rebuilt · the estate wave sits between geometry and fixtures · the mint waits or it becomes two · §482's ordering already satisfies the one-regen constraint · ~40–50 cars against one landing seat, and no lane quotes a date.
⭐ §500 OWNER LAW: acquisition has a REVERSE — swap, split/partition, dissolution, each a dated event; the remembered merge seam IS the split line; a split never un-builds (party wall / encroachment as typed shared conditions); dissolution feeds the RECLASS chain; and DW-S gains an OWNERSHIP-BOUNDEDNESS arm because unchecked acquisition converges on one owner in 300 years.
⭐ §498 OWNER LAW: an ESTATE is an OWNERSHIP SET (multiple buildings, not necessarily connected, under one institution or household — never a named individual), distinct from the PARCEL/property line (contiguous, one boundary, merge only within a ward). At generation: assemble the ground, then draft ONCE. At time advance: the ground merges and buildings ACCRETE — never redraw lived ranges; the visible seam is a FEATURE. Every amalgamation is a dated event with a cause and reaches the chronicle (news address law). The halo gains two tiers: the hovered property strong, the owner's other holdings faint.
⭐ §497 OWNER LAW (supersedes the chair's R2): an institution's MINIMUM PROGRAM is a truth from the dossiers and an INPUT TO ALLOCATION — the plot is allocated (or parcels merged) to fit the institution; the institution is never trimmed to fit the plot. Existence is decided upstream, so a shrunken draw makes the MAP CONTRADICT THE ROSTER. Minimums are programs (cells + adjacencies + open ground), never bare area; a tier's smaller true form (the hamlet inn as a room over a stable) is a DIFFERENT minimum, not a degradation; the only lawful refusal is a REPORTED CONTRADICTION with a receipt.
⭐⭐ §506 THE DWELLINGS CHARTER IS RATIFIED AS AMENDED (charter 237.5 KB + architecture 141.7 KB + a 5.4-page ratification brief, preserved at `refs/preserve/research-dossiers-2026-08-23` = **2f98a169**). **THE OWNER'S SIGNING SHEET IS READY — 20 rows, BAND ZERO first — and it is now the critical path.** ⛔ §506.2 CORRECTION TO §504.4: a scale EXISTS — `PLAN_UNIT_CM_BY_TIER` at `compileTownSceneManifest.js:99-108` (town 30 cm/unit), read at `:286` and already used at `cartographyBuildings.js:333`; two lanes and the chair missed it by grepping the CONCEPT WORD. **LAW: grep the engine's own spelling, not the concept.** Band Zero is now the MEASURABLE, and the chair ruled it the SLOT FACE (41.8 ft, 77% admitted) because the whole-plot edge is unknowable at allocation time. §506.3: the identical-footprint rate is a DEFECT → **CG-2**, not built over.
⛔ §504.3 BLOCKER: the cartography stage THROWS on 27 of 48 real pipeline settlements (56%, every village and town) — it is proved only against `makeTournFixture` FIXTURE output. Chartered as **CG-1, the cartography ground**, and it LANDS BEFORE DW-1 BUILDS. ⭐ §504.5 MEASURED: a metropolis = ~30 ms generate + ~140 ms map compile = **~170 ms of the 2,000 ms budget**; lazy per-building 1.3 ms on click; eager worst case ~309 ms — the two-second claim survives with ~10x headroom (C5 discharged, no lane ordered). ⛔ §504.4: **no plan-unit-to-physical scale exists in `src/`**, so every frontage band and the §497 R2′ eligibility gate are vacuous until the owner signs BAND ZERO — the scale.
⛔ §503.2 MEASUREMENT LAW: **`facetOf` is called ZERO times in the generation pipeline**, so a whole-record corpus digest is STRUCTURALLY BLIND to any `FACET_INFERENCE` change (proved: break the faith regex to `/./i` — an interior moves in all 420, the digest does not). Pin such changes at the DERIVED INTERIOR with a control that can FAIL; a digest that cannot fail proves nothing. And **any catalog-row key addition (`facets`, `minTier`, `priorityCategory`) reds `generatorGoldenMaster`** because `assembleInstitutions` spreads the row onto the record.
⭐ §519 OWNER-SIGNED SCHEMA (SUPERSEDES §518's faction reading): **ownership is POWER-scoped** — `Estate.ownerRef` = a POWER (the governing seat or a coup contender; `powerStrata.js:75-82`, already flagged `isPower` on every roster entry) or `ANONYMOUS_FABRIC`. Never a household, never a named individual; EST-4's refusal STANDS. **Criminal fronts are excluded from POWERS by `coupContenders` and ride the contract's `occupies` relation, not `owns`.** ⭐ Powers already change hands on DATED CAUSED events (`RULING_POWER_CAUSES` = coup/election/succession/conquest/appointment, with `authorityTransferEpochFor`) — the acquisition and dissolution driver is LANDED, not to be built. The halo highlights a POWER's holdings.
⛔ §527.2 **A COMPOUND THAT ENDS IN `grep`/`diff`/`cmp` INVERTS A GREEN RESULT** — they exit non-zero on the GOOD outcome. CG-1b's perfect gate (97/97) was published as "failed, exit 1" because its last statement was `grep -E "× "`. Chair-reproduced. **Never end a command whose exit you read with grep/diff/cmp; always quote the collected counts beside the exit.** With §520.1 (exit 1 over ZERO collected tests) this means **an exit code alone is never a verdict in either direction.**
⛔⛔ §536.6 **A LANE WORKTREE HAS NO HUSKY SHIM — `git commit` THERE BYPASSES PRE-COMMIT SILENTLY.** Chair-verified: `core.hooksPath` = `.husky/_`; the main checkout has it, worktrees do not. **Every worktree commit in this program ran with no pre-commit**, and the blob-sha law was confirming nothing changed *because the hook never ran*. The real safety net was the lanes' own explicit eslint runs. **A lane runs `npx lint-staged --diff` explicitly and says so; it may not claim hook coverage it does not have.** ⭐ §536.2 a C2PA manifest **hard-binds to its bytes**, so no resized derivative can carry a valid one however obtained — re-attaching yields a manifest that FAILS validation, worse than none (proof by surviving instance: `realm-journey.mp4` is valid *because it was never re-encoded*). ✅ §536.3 the stripping is stopped at source, guarded by 20 tests with four convicting mutations; payload +2,362 B and **not one media byte moved**.
⭐⭐ §535.1 **THE AI MEDIA IS CLEAR** — all 52 files (74.2 MB) from ONE PAID Higgsfield account on 2026-07-18 (first transaction 20 minutes before the first generation; Wayback-verified the clause was in force the day before), unrestricted commercial use, no tier condition, no attribution duty. ⛔ §535.3 BUT a NEW §5.5 forbidding provenance stripping **binds from 2026-08-27 — three days** — and mozjpeg/ffmpeg stripped C2PA and SynthID from 38 files; **TE-AIP-1 is fixing the PIPELINE first** (a restored credit is worthless if the next build strips it). ⭐ §535.5 the picture narrows to **ONE cohort: 23 files / 7.4 MB introduced six weeks before the AI account existed — the estate's ONLY unlicensed shipped imagery**, and the reverse-image search settles it.
⭐⭐ §534.2 **A MISSING CHARGE FAILS SILENTLY** — `fetchCharge` returns `undefined`, `join("")` drops it, a four-charge coat renders 2 of 4 and never prints an error. **55.7% of 40,000 draws would have requested a missing file** under the chair's first (directory-wide) ruling. Cured by pruning the fork's pick table, verified three ways (231 violations → 0; 40,000 draws → 0; `validate:map` with a broken-bundle control). ⚠ §534.4 **RUN A NEW GUARD WITH ITS NEIGHBOURS, NEVER ONLY ALONE** — the licence walker raced a sibling's planted-and-deleted probe and reported a licence failure that was not one, and it PASSED in isolation.
⛔⭐ §533.3 **A CARD IS A CACHE.** My slot-facts card's golden row went stale by one landing (MF-CH2A's DECLARED re-record moved it `cd8d125ef8` → `4bdd69f201`), so a lane obeying my own "if the golden moves, STOP" would have stopped on a LAWFUL landing. **Any row a landing can legitimately move — goldens, tuple, packet count, ratchet — is RE-READ AT THE SLOT; the card's value is the rows that do NOT move.** Goldens live in `tests/fixtures/`, not `tests/property/`. ⛔ §533.2 the slot MOVED MID-ACT and the lane's last-second re-read caught it — everything downstream redone, and the §410 flip became a FOUR-place flip. ✅ §533.4 the banked ELEVEN is closed: the ratchet baseline IS the roster (7 + 4 in `tests/copy`), confirmed by a whole-estate sweep with an empty title diff both directions.
⛔⛔ §532.4 **NOTHING HAS DEPLOYED FOR 27 DAYS** — the deploy gate's own decision function returns `skip` with all six CI checks green because **prod is at migration 121 and the repo head is 199**; `applied-head.json` is dated 2026-07-28. That is the whole cause of the notices-page 404 (the page is in `dist/`, byte-identical). **The migration and the deploy are owner-gated and now urgent.** ⭐ §532.2 the charge census overturned the chair's own ruling in the good direction: **104 of 338 are CC0** — delete the 234 (NC + copyleft + unlabelled), KEEP the 104 and name them in the notices. ⚠ §532.3 the fork's own bundle fetches `./charges/${t}.svg`, so the picker must offer only what ships — a feature that 404s is worse than one with fewer lawful options.
⛔ §531 TEXTURES.COM, PRIMARY-SOURCED: *(a) sell or distribute any Content **(modified or not)** by themselves or **in a texture pack**; (g) **bundle Content with software such as … 3D engines**; (k) release … **under Open Source Licences*** — identical wording in the Dec-2020 revision, so in force when the images were sourced. **No attribution fix exists: removal or nothing**, and if any Family-A file came from there, FMG's MIT redistribution was already a breach we inherit. **TE-IP-1's deletion already covers it — no new action.** ⚠ Family A remains UNKNOWN; the decisive step is a **reverse-image search on `spain-small.jpg` and `pergamena-small.jpg`** — a human with a browser, ten minutes. TC-AIMEDIA-TOS is auditing the 63 MB of AI video + 8 MB of stills, the last unowned IP question.
⛔⛔ §530.6 **179 OF 338 HERALDIC CHARGES ARE NON-COMMERCIAL BY THEIR OWN EMBEDDED METADATA** (+37 share-alike), served live at HTTP 200 with the NC term in the bytes, and **commercially distributed since April** — the fork concedes it at `emblems-editor.js:502`. **TE-IP-1 is deleting them** (nothing references either directory; chair-verified). Also: fonts re-cut in June while three notices say 'unmodified'; **`third-party-notices.html` is HTTP 404 in production**; heightmaps are DOCUMENTED but owe attribution; no remedy forces a second regeneration. ⚠ UNOWNED: the vendor ToS for 63 MB of AI video + 8 MB of AI stills. ⚠ §530.5 **MP-1 IS BUILT BUT DARK** — `townCartographyEnabled` has no default entry, so the plan view is byte-identical for every existing world; **lighting it is the owner's call.**
⛔ §529.1 **FMG's MIT LICENCE CONVEYS NO RIGHTS TO THE BUNDLED RASTERS** — it covers 'the Software' and its derivative clause covers OUTPUTS, not input assets; upstream credits only JS libraries. Of 23 textures: **4 PROVED CC BY** (mars/mercury pixel-identical to Solar System Scope, 13x negative-control separation) needing only a credit line · **3 UNKNOWN and highest risk** (1460x900 Earth photos; Google Earth would flatly prohibit texture use) · **16 UNKNOWN**, metadata destroyed upstream. ⭐⭐ §529.6 CHAIR'S RECOMMENDATION: the estate already owns 37 files of byte-deterministic baked texture machinery with ZERO importers — **wire it up and delete the third-party rasters**, retiring Families A and C at once. Legal call is the OWNER'S.
⭐⭐ §528.1 **THE SOLD PDF IS CLEAN** — no raster image exists in ANY generated PDF (all 42 react-pdf imports enumerated, none is `Image`; fonts are the only external asset), the town-map plate is a closed procedural loop, and **no heightmap data reaches the $2.99 dossier** (`SingleDossierSuccessPage.jsx:182` passes no campaign). ⛔ §528.4 the exposure is elsewhere: the realm-map PNG download emits the FMG terrain raster · map-share thumbnails upload it to a PUBLIC bucket · and **`Ctrl+S` downloads the full `.map`** (hotkeys live behind the CSS-hidden menu; iframe unsandboxed) whose index 7 is the height array and index 5 the terrain SVG, persisted to Supabase `saved_maps.map_data`. Gallery import already strips it to the seed.
⛔⛔ §526.3 **176 MB OF SHIPPED ART HAS NO PROVENANCE RECORD** — `THIRD-PARTY-NOTICES.md` covers code, fonts and npm ONLY (texture/heightmap/heraldry/png/jpg/svg/mp4/artwork all ZERO hits), while `public/` ships 11.6 MB of FMG textures, 3 MB of heraldic charges, 23 real-world heightmaps, 15.4 MB of paintings with no recorded painter, 62.8 MB of video. **§524 made it worse: we now SELL artifacts containing it**, and licensed asset packs are therefore UNBUYABLE (a downloadable PDF is not 'incorporation'). **TE-AD-1 is auditing; the decision is the OWNER'S (legal, §510.2).** ⭐ §526.1: there is NO long-lead art branch — THE ILLUSTRATED TOWN is already landed, live and FREE, and `townMap/arch/` is 37 files of baked-texture craft with ZERO importers. The art program is a WIRING job. ⚠ §526.6: **`git grep -E` does not honour `\b`** — the eighth silent liar.
⭐⭐ §525.1 **MF-CG1b BUILT — the lit throw rate goes 287 of 504 → 0 of 504** (a hamlet 13/84 → 84/84 drawing); caps derived from the corpus behind one headroom parameter (BAND 21), `bindings <= buildings` true by arithmetic, NO golden moved, the invariant arm green UNEDITED. ⛔ §525.5 **THE RATCHET'S ELEVEN IS RECONCILED: 7 swept + 4 in `tests/copy/voiceMechanics.test.js`, a tree NO sweep has ever covered.** The card's 'banked = SEVEN' means seven-within-the-swept-trees; STACK-2 sweeps ALL 2,517 files to confirm. ⚠ §525.4: a FOURTH control that could not fail (`toEqual` against a table that agrees today) — **every control must now state what makes it capable of failing.**
⭐⭐ §524.4 **THE PAYWALL TRICHOTOMY (supersedes the two-way line; rules every future case, vetoable as a PRINCIPLE):** **VIEW/INTERACT free** (reads + transient local state) · **AUTHOR paid** (writes state that outlives the session and others consume) · **TAKE AWAY paid** (produces a durable artifact leaving the product). Owner ruled export PAID and **included in the existing $2.99 `single_dossier`** — verified in code (`pricing.js:297-304`, `requiresAccount: false`, so an anonymous visitor can buy it) and the map plate is ALREADY in `draft_brief` and `canon_dossier` (`SettlementPDF.jsx:102-180`); `timeline_packet` and `campaign_state` deliberately omit it. **No new SKU may be minted.** ⚠ D-EXPORT-1: `hasDrawableMap` self-gates the plate, so when the cartography stage becomes its source a stage that throws would silently sell a mapless dossier — pinned in WEB-8b and DW-6.
⭐⭐ §523.2 **THE PAYWALL LINE (rules every future case, vetoable as a PRINCIPLE):** AUTHORING = writes state that OUTLIVES THE SESSION and that OTHERS CONSUME · VIEWING/INTERACTION = reads, including transient local state (hover, pan, zoom, selection, inspection, address-chain following). Ruled under it: **`change-view` FREE** (reading history is reading) · **`map-chains` FREE** (a view layer of the map is the map) · **`fog-table` STAYS PAID** (reveal writes persisted state the player view consumes). Two cars chartered: **WEB-8b** (frees the two — ⛔ cannot dispatch until WEB-8 LANDS, its paths are reserved) and **WEB-12** (gate `commitEdits` at the chokepoint).
⛔ §522.3 **THREE VIEWING ROWS STILL CLAIM A PAYWALL** against the owner's §514.1b ruling — `map-chains`, `change-view`, `fog-table` — carried in a pinned `VIEWING_PAYWALLS_PENDING_514` ledger and **put to the owner** (freeing them removes what paying customers get today = owner-gated). Zero authoring rows are free — no revenue leak. Only TWO rows are advertised-but-ungated: `interiors` (zero product importers) and `v2-redraw` (**inverted** — free already gets v2). ⚠ §522.4: `commitEdits` is the single writer of `mapEdits` with NO gate of its own — nothing leaks today, but the invariant lives at N render boundaries, not the chokepoint.
⛔⛔ §521.1 **`git grep` IS BLIND TO UNTRACKED FILES** — UC-5's grep arm returned `<none>` for all 35 tokens including its own exported function, because it ran before committing. Chair-reproduced. **Use `git grep --untracked` or a disk grep, and PROVE the arm sees the member's own new file before trusting it.** ⭐ The undercity train is BUILT END TO END (UC-5 = 8/8 with eight mutants one-to-one; `sealed_door` structurally dark, D-UC5-1). **TE-STACK-1 is the first stacked landing since #40**: MF-CH2A + MF-UC5, one gate, sum-of-deltas with a sibling control.
⛔⛔ §520.1 **THE THREAD CAP FLAG WAS WRONG** — `--poolOptions.threads.maxThreads` does NOT exist in vitest 4.1.8; it exits **1 with ZERO tests collected**. Use **`--pool=threads --maxWorkers=2`**. STANDING RULE: **an exit code with no collected-test count is not a verdict** — cross-check the printed file and test counts every run. ⭐ §520.5: MF-CH2A READY and independently landable (`holding-ch2a` = 90a12cd1); **MF-CH2B HOLDS AT DRAFT** — `ARCANE_INST_TAGS` contains `alchemy`, so a TRADE vocabulary is doing a MAGIC-DEPENDENCE job and an alchemist cannot exist in a magic-free world; chartered as **CH-5**, a CONTENT car.
⛔⛔ §517.7 **THE MAIN CHECKOUT IS A DIFFERENT TREE** — townCartography 0/13, undercity 0/6, interior 0/7, worldPulse 126/396 on disk versus the slot. A disk `grep -r src/` is a FALSE-ABSENT generator: use `git grep <ref> -- src` ALWAYS. And **`grep -c` reports an EMPTY count on a file it deems binary**, so a C0 scan can silently pass on a corrupted file — scan with **`grep -ac`** and decode as UTF-8. ⭐ §517: the B8 loop is BUILDABLE (its home `urbanFabricKernel.js` lights with the estate wave), but **`noble` is unreachable — 0 of 10 quarters, 'Resi(den)tial' → criminal — the substring defect's THIRD home → CH-4**; hysteresis does NOT bound the runaway, the geometric capacity cap does; and **§500.5's delta arm would certify the runaway as converged — use SATURATION YEAR + RESIDENTIAL FLOOR (5.5x separation)**.
⛔ §516 **STACK THE LANDINGS — the last stack was #40 and 41–48 were eight singletons in eight gates.** The standing template is `695a70c5/scratchpad/STACK-BRIEF-TEMPLATE.md`; a stack is a form to fill, not a decision to remember. **Next stack pre-planned: CH-2 · UC-5 · WEB-8 as ONE lane** (disjoint by construction) the moment all three hold. Also unenforced until now and now enforced: the §481 **150-turn split** (a 499-turn lane burned 216M context tokens on transcript replay alone).
⭐⭐⭐ §515 **THE BAND SITTING IS SIGNED AND CLOSED** — the instrument of record is `SIGNED-BANDS-2026-08-23.md`, preserved in git at `refs/preserve/research-dossiers-2026-08-23` = **029268fe**, carrying the owner's VERBATIM words for every row. DESIGN_DWELLINGS_PROGRAM §11's band half is DISCHARGED; ⚠ the TUNING SIGNATURE is NOT and no lane may read this as covering it. **Unblocked: CG-1b now (B21 set); DW-1 once CH-2/CH-3 and CG-2 land; the ESTATE wave awaits the B8 recon verdict; AD chartered not compiled.**
⭐⭐ §514 **THE BAND SITTING IS COMPLETE** — all 21 rows ruled or ratified, except B8 which turned out not to be a dial. **B16/B16b: the map is FREE to view and interact at every level including anonymous; AUTHORING is the paywall at every level** — converging with `viewerCanAuthor`, the gate §449 found the code already uses. **B1: there is NO tier at which credibility is optional** — every tier has a minimum program, some are two rooms and a kitchen; HOSTED and NO_BUILDING are programs, not exemptions. **B15: illustrated art moves BEFORE LAUNCH** — new scope, chartered as the AD program between DW-S and the content train. **B8 is a two-way demographic loop** (wealth concentrates → plots merge → people displace → building pushes outward; decline reverses it) — TC-B8-RECON is measuring whether the engine can express it before the estate wave is specified further.
⭐ §512 **SLOT-FACTS CARD** at `695a70c5/scratchpad/SLOT-FACTS.md` — the six per-slot facts (census tuple, ruin count, banked set, packet count, migration head, typecheck ceilings, OSR red, sweep cost) chair-measured by READING the blobs. **Lanes CITE, never re-derive**; the card is VOID when the slot moves and a lane's first act is to compare the sha. Plus: **a mutant drives the ARM it should convict, not the whole acceptance file** (nine whole-file batteries is nine times the work the proof needs; the full file runs once at the end).
⛔ §511 THROUGHPUT LAW (measured: 8 cores, load 21/34/42, nine vitest workers, seven mutex waiters): (1) every `npx vitest run` carries `--pool=threads --poolOptions.threads.maxThreads=2 --poolOptions.threads.minThreads=1`; (2) the banked failing set is **SEVEN** — CITE §507.3/§509.1, never re-derive it in a fresh baseproof worktree (four lanes did, identically, at ~15 contended minutes each); (3) a landing whose slot did not move re-uses the build's sweep and runs only the terminal; (4) ⭐ **STACK LANDINGS** wherever change paths are disjoint — the proven pattern (map stack 4 cars / one gate; producers 3) turns ~60 terminals into ~20.
STANDING LAWS FOR EVERY BRIEF: receipt stub FIRST, `RESUME POINT` every proof boundary / 30 min (§448) · builders NEVER run check:tail (§460.1) — the widened mutexed sweep is their proof · every vitest battery exports `GATE_MUTEX_LOCK_DIR=/tmp/settlementforge-vitest-gate.lock` INLINE on the same command line (§440.2/§457) · gates fire via `chair-detach.py` and the lane OUTLASTS them in-turn · a MODIFY of any edge-shared bundle input owes `build:edge-shared` in the same commit + `tests/edgeFunctions` in the sweep (§475) · an acceptance arm over a shared namespace pins INTENT, never `toEqual` a list the next car grows (§474) · census-class "pre-existing" needs the RECEIVED-LIST DIFF (§469) · a landed packet never pins the migration head — filename OR figure (§455.2/§479.2) · carry a census DELTA across a moved slot, never a tuple (§420/§480.2) · read `*.exit` before relaunching a gate (§463.4) · every Agent dispatch names its `model` (§481) · the widened sweep gains a GREP-DRIVEN ARM over ALL 37 test trees for every symbol/key/flag/enum the member ADDS (§489.3) · a flag mint is SIX surfaces incl. `tests/soak-harness/coveringArrayCoverage.test.js` (§489.2) · `git rev-parse --verify --quiet` for blob readings (§489.4) · `adversarial-verify` is NOT registered — briefs say "apply deep-work §5's verification core" (§486.2) · the §410 LANDED flip is THREE places: manifest status + packet header + INDEX STATUS column (§486.2) · never `while read … path` in zsh (§486.2).
⛔ §484 OWNER ORDER (08-23): EVERYTHING RUNS ON OPUS 5 until lifted — every `Agent` dispatch carries `model: "opus"`; the chair stays whatever model the session runs (the owner switches it in the app). MARK every Opus-run ledger row `[OPUS-RUN · FABLE-VALIDATION OWED]` and enter it here; Fable validates retroactively ONLY when the owner says so (§484.4 protocol).
OPUS REGISTER (owed Fable validation; strike when `[FABLE-VALIDATED §N]`): UC-1 LANDING §486 (slot 0f3897a5 → tip 1b1de759, the 44th; re-prove: carry table, tuple walk, §410 three-place flip, terminal lines) · R-INST-5 §488 (at refs/preserve f4f77672; re-verify the analogue figures, the 71:73 digest ratio's load-bearing rows, the verdict table, and G1–G7 against live code) · UC-2 BUILD (TE-UC2; re-prove at the holding tip) · R-INST-4 FINISH §485 (dossier at refs/preserve 3b164bde: §L/§M/APPENDIX + the law-4 paragraph + banner were Opus; re-verify every PARENT-VERIFIED figure and the NO_BUILDING/HOSTED verdict table) · UC-4 BUILD+LANDING §487/§490 (landed tip f1e4d515, the 45th; C-2 `powerHighWater` fold, the six-surface flag bill, the §489 cure — re-prove at the landed tip) · UC-2 BUILD §490.2 (holding 69496557; the vault-licence re-cut, the 0.45 floor, the T2Q computed-key cure — re-prove) · UC-2 LANDING (slot f1e4d515 → tip in `uc2-rebased-tip.txt`) · R-INST-6 (draft-R-INST-6-CRIMINAL.md). PRE-DIRECTIVE Opus builds (landed under Fable landing lanes + chair verification — owner includes or excludes): WEB-2 build · WEB-3 build · UC-3 build · T2Q resume · HK-A build · UC-1 build (53d7e492) · website-2 skeptic panel.
⭐ §495 CORRECTION + CHARTER: the cartography block's parcel rows ALREADY carry `polygon` (`cartographyParcels.js:68-71`) — the painter drops it; the property line is a PUBLISH, not a build. Map car **MP-1 (the property-line layer)** chartered: emit a parcel op, thread the polygon to the 2D pane, hover/pin draws the yellow property line, yard = parcel minus footprint. Limits written into its packet: the shape is a fan WEDGE until DW's frontage work; `PARCELS_PER_WARD` is a tier band so some buildings have no parcel row (degrade honestly); court-vs-yard and detached outbuildings stay DW-1/2. Membership needs no new relation — same property iff same `parcelId`. MP-1 sits after the CH cars unless the owner moves it up.
⭐ §494 OWNER DIRECTIVE (08-23): hover must halo the PROPERTY LINE (compound: yard/court/detached outbuilding as ONE property), not the building. Measured answer: today it does NOT — the pane draws an 8→11 px square, `cartographyPaint.js:19-21` emits NO parcel op by design, and no compound exists in the model. Three items: (1) the `compound` + `CompoundMember` contract in DW-1/DW-2 (written into DESIGN_DWELLINGS_PROGRAM.md), (2) the parcel/compound projection at DW-6, (3) the halo UI car after it. A parcel-only outline is available sooner if the owner asks.
⭐ §510 THE GRANT RE-AFFIRMED (third time): everything proceeds without asking. FOUR carve-outs survive BY NATURE: the physical `supabase db push` (198 is destructive against a live DB — an irreversible outward act takes a contemporaneous confirmation, never a standing one) · every push/deploy · legal sign-off · the tuning SIGNATURE and the `/code-review ultra` keystroke.
⛔ §509.3: the cartography stage cannot draw **town 84/84, village 83/84, hamlet 71/84** — 287 of 504 throw. CG-1b re-chartered: the caps become DERIVED from the corpus with one HEADROOM parameter (lands with current values = zero shift); **the headroom value is BAND 21 in the owner's sitting.**
OWNER DOCKET (by nature only, §464) — ⚠ the band sitting is DONE (§515) and is NOT the tuning signature: the db-push train 195/196/197/198/199 (198 DESTRUCTIVE — read DEPLOY.md's preambles section first) · every push/deploy · the tuning SIGNATURE · legal (TinyMCE/GPL, FMG, founder transfer) · `/code-review ultra` AFTER the DW program (§482; the owner's keystroke). Everything else was RULED at §464 (R-5 lights in tuning; R-6 no; R-7 via the ONE REGEN; O-P1 de-advertise → WEB-8; O-P2 Surveyor tier $14.99 → WEB-9; O-P3 pin → WEB-8; taste-gates discharged as research inputs).
