...
**AS OF 2026-08-23 13:54 CDT · ledger §492 · ⛔ EVERY DISPATCH `model: "opus"` (§484) · build `claude/composite-r4` = `f1e4d515` (45 landings; 0 READY across 167 packets; `undercityHighWaterEnabled` on the branch DARK; migrations 197/198/199 on the branch) · LANE CAP = TWO (§465)**
...
|---|---|---|---|---|---|
| RESEARCH | TC-R-INST-6 (**Opus**, solo) `[OPUS-RUN · FABLE-VALIDATION OWED]` | — (read-only) | laneTCRINST6-receipt.md · RINST6-merge/ · draft-R-INST-6-CRIMINAL.md | criminal/underground fronts — the LAST research tranche; UC seam table | on completion: ledger `[OPUS-RUN]`, register, preserve (refs/preserve/research-dossiers-2026-08-23); RESEARCH COMPLETE → DW-0 charter compile starts in this seat (§482) |
...
RESEARCH DELIVERED (taste-gated inputs, §464; ALL preserved at `refs/preserve/research-dossiers-2026-08-23` = f4f77672): DWR1A · R-INST-1 · R-INST-2 · R-INST-3 (16/16) · R-INST-4 (13/13, §485) · R-INST-5 (32 entries/11 families, §488 — five charter findings + seven engine defects G1–G7) · CIRC addendum; R-INST-6 (criminal) queued — the last; DW-0 starts when it delivers (§482).
LANDING ORDER: UC-2 (IN SEAT) → UC-5 (after R-INST-6 / DW-0 interleave) → each later car as built, singly.
NEXT DISPATCHES (§492 GO): at UC-2's CAS → **CH charter compile (Opus; brief ready at `$SP/CH-charter-compile-brief.md`)** → CH skeptic (§441 J7) → chair rules → CH-1 build → landing; R-INST-6 (IN SEAT) → DW-0 charter compile (§482); then CH-2 → CH-3 → UC-5 → W-C cars. ALL CH cars land before DW-0's BUILD waves (DW-0 is a compile; it may run beside them). → **DW-0 charter compile (Fable, §482 — starts the moment R-INST-6 delivers)** → UC-5 → WEB-8 (Opus) → WEB-9a → WEB-9b → WEB-10 (last, STOP-class) → WEB-11 → HK-4 → HK-5 (head-figure guard, §479.2) → HK-6 (strip "Eberron" from three catalog/variant strings, §488.3) → housekeeping sweep (requiredSymbols rows skipped at stacked landings; MF-T1X prose; S0 schema-10 mint) → content train CT-4/CT-5/WF-8 → DW build phases (Opus) → ONE trailing OSR mint → parity train → `/code-review ultra` (owner) → endgame.
