...
⭐ **STATE AT §689 (2026-08-26):** ledger `review-fixes-2026-07-08` through **§689** · **GROW-FOLD SEALED** = `map-sandbox-gfold-order` = **`56ee43dad`** (§686 — the raise precedes its growth; metropolis 2,633 → 2,748; band median **1.71**, the reference invariant; C1's magnitude re-based and §655.3 closed) · **DRESS-1 SEALED** = `map-sandbox-dress1-ink` = **`ad2b8d399`** (§688 — the partition wears ink; gate 19 files/425 tests; **8 of 10 exits met**; `wall:all` improved to 6.08–7.00 over the 3.00 floor) · ⛔ **DRESS-1 IS NOT COMPLETE: the gestalt round 1 DISAGREED at 61.1 % against the ≥80 % bar** though PA.7's hard clause held (zero wrong-class reads, 18/18 both fields) — **DRESS-1b IS IN FLIGHT** with the cure and round 2 (brief `$SP/briefs/DRESS1B-BRIEF.md`). ⚠ **A SECOND DISAGREEING ROUND ESCALATES TO THE OWNER** (PA.7). · **`docs/PROVENANCE_MAP.tsv`** is the standing provenance instrument (693 rows; **130 UNKNOWN in four runs — §18–§23, §287–§290, §311, §322–§439** — the genuinely owed surface, not the 608 the text-silence count suggests) · **toolkit r7 = `9915e0786`** (four gates) · **§689: Inkarnate leaves the COMPETITIVE set** (asset canvas, not generator) **and stays in the AESTHETIC one**; the code base remains watabou/FTG per §656.
