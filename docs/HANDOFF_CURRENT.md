...
⭐ **STATE AT §712 (2026-08-26):** ledger through **§712** · seals: DRESS-2 `c4d772590` · **STATE-BRIDGE `b1f0dfc71`** (the last build) · instruments **r8** `e25979624` · toolkit **r7** `9915e0786`. Gate **20 files / 493 tests**, re-executed in-turn. ⛔⛔ **REGISTERS/ZOOM IS STRUCK — MEASURED, NOT SUSPECTED (§712).** Zero WARD and zero BLOCK faces on all 18 leaves, and the absence is **STRUCTURAL** (161 write sites scanned; no path writes either class). Ward's one earned quarter is **EMPTY**; **block needs no face** — and ⭐ **the page ALREADY draws at block grain** (page→block fan-out median **1.04×** vs page→plot's 3.7–4.5×). **THE SUBSTRATE HAS TWO GRAINS, NOT FOUR: the ladder is `page → plot`.** Its one real rung is **not wired to the drawing** (`renderFolio` imports no `partitionView`) — **that wiring is the PORT's**, so this wave's content collapses into REG-P. A true four-rung ladder needs §708.2's C2 (legacy district cells joined to the spine — a substrate act, on the docket). ⛔ **TWO FINDINGS THAT LAND ON THE RECORD ITSELF:** (1) **the byte ceilings are PROSE — nothing enforces them** (§712.8), so every "N of 830,000" in this ledger is a hand comparison, not a gate result; an enforcing pin is owed before the port. (2) **`OP_CEILING=2200` is declared twice with two meanings** — dead in one file, **LIVE in the other as the countryside's whole budget** — and the error **inverts by tier** (thorp 2.2× over, metropolis 6.5× under). Owner docket: it rides the tuning signature. **NEXT: WORDS (§3)** — cartouche language · the enumerated denylist · label placement and the clutter ladder · L-REG-35 event addresses · **L-REG-29 intact (no generated street/bridge/gate names)** — then JUDGING → REG-D → REG-11/F0/F → REG-P.
