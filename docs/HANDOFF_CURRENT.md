...
⭐ **STATE AT §710 (2026-08-26):** ledger through **§710** · seals: DRESS-FRAME `a58e19abf` · SEATING `6bc1a5051` · **DRESS-2 `c4d772590`** · instruments **r8** `e25979624` · toolkit **r7** `9915e0786`. Gate **20 files / 485 tests**; byte **703,133/830,000 (84.71 %)**; wrap digest unmoved; folio dormancy 29/29. ⭐ **THE CORPUS WAS 14 DRAWINGS PRESENTED AS 18 — NOW 18** (§696.4's naive reader vindicated by measurement). The page carried **ZERO relief marks**; now 3–232/leaf. ⛔⛔ **THE FRAMING LAW IS IMPLICATED THREE TIMES (§710.4) → OWNER DOCKET, PRICED BOTH WAYS.** The fit put the hinterland off-plate (farmland inside frame **0.00 % on 8 leaves**), so **the field mosaic is NOT BUILDABLE as chartered** (A14.4 vs §707.3 in structural conflict, fit won), and `fjord` **reads dry** because its water is clipped to a corner — the single hard-clause failure in round 4. ✅ But the quietness win **HELD** (worst movement 0.83 %) and is far larger than recorded: field carries **0.69×** the fabric's mass where `tones()` still says 18×. ⛔ **ROUND 4 FAILED (1 wrong-class) — RULED NOT AN ESCALATION (§710.5)**: PA.7 fires on two CONSECUTIVE disagreeing rounds and round 3 PASSED; plus the lane declared a protocol divergence making it non-comparable. **The wrong-class read is not dismissed — it reaches the owner via framing.** ⭐⭐ **THE CLASS (§710.6): a census that asks whether a mark EXISTS has not asked whether it is ON THE THING IT MEANS** — 9 tents with 0 on the page, a barred gate 16.1 road widths from its wall, camp huts in the bay. Cause: `deriveStateMarks` anchors on LEGACY, the dress draws PARTITION. → **`STATE-BRIDGE` chartered.** **NEXT: `STATE-BRIDGE`** (the placement defect; its census already reds 4/9 and is deliberately OUT of the gate — a gate arm that reds at its own tip is a broken gate), **then REGISTERS/ZOOM → WORDS → JUDGING → REG-D → REG-11/F0/F → REG-P.** ⚠ Owed at the next seal: `lib/classify.mjs` knows 30 `dress-*` groups and **not `dress-register`**; this wave's 11 new families widen it.
