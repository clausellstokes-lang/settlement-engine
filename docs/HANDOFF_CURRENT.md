...
## ⭐⭐⭐⭐ PICKUP AT §799 (2026-08-30, TE-EFF-1 CAS + W-FAITH VOLUME) — START HERE; supersedes everything below.
...
**W-FAITH IS ARCHITECTED (§798, `docs/DESIGN_W_FAITH.md`)** — nine vetoable rulings; cars 0–8 seated (flaw vocabulary/Herald voice/owner-taste pack = FABLE cars); dispatches AFTER T7 with a skeptic panel first (§441 J7); the drift/flaw/channel word lists are CANDIDATE registers, owner signs at the tuning pass; three small owner questions in the volume's §5. Substrate law minted §797.5: NPC drift = instantiations of `bandedStock`/`dispositionLedger` (decay's home point is a PARAMETER), never a new lifecycle; the NPC authored core is immutable (nothing writes `personality.dominant`/`.flaw` — grep-verified).
