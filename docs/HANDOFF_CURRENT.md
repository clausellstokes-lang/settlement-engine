...
⭐ **STATE AT §711 (2026-08-26):** ledger through **§711** · seals: SEATING `6bc1a5051` · DRESS-2 `c4d772590` · **STATE-BRIDGE `b1f0dfc71`** · instruments **r8** `e25979624` · toolkit **r7** `9915e0786`. Gate **20 files / 493 tests**; folio dormancy **29/29 diff-clean at 11,411,375 B**; determinism 18/18; wrap digest unmoved; byte 703,131/830,000. ✅ **§710.6 IS CLOSED**: the state marks now land on the thing they mean (siege tents 9/9 off-page → 0; the barred gate 16.1 road widths → **1.63, inside the drawn gate face**; camp huts 12/12 in water → 0), **and the placement census is IN THE GATE.** ⭐ The cure was a **third layer** — a domain pass reading the projected page (`partitionState.js`), sibling to SEATING's; `fabric.stateMarks` is read-never-written so **folio dormancy holds by construction.** ⛔ **THREE UNCHARTERED DEFECTS FOUND (§711.5/§711.6):** `page.gates` is **not** the drawn circuit's gates (worst 12.0 road widths off) · and ⛔⛔ **one numeric field is read three different ways by three consumers** (`ang` as trig index / 1024-table / degrees) — **a field with no declared unit gets a different unit at every consumer and nothing ever reds.** ⛔ **THE FRAMING TRADE HAS A THIRD MEASURED COST (§711.7)** — a besieged camp cannot stand at either gate of its own wall (36/36 vertices off-frame at one) because the page fits `bound.radius` while the band sits at 0.98–0.99 R. **Reported, not widened; §710.4 is the owner's.** **NEXT: REGISTERS/ZOOM (§2)** → WORDS → JUDGING → REG-D → REG-11/F0/F → REG-P. ⚠ Carry into it: **the partition owes a MARKET FACE** (`page.voids` is 100 % `court`; `town-2`'s case is pinned `RULED_DARK` so a second one reds).
