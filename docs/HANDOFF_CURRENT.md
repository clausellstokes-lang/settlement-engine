...
⭐ **STATE AT §713 (2026-08-26):** ledger through **§713** · seals: STATE-BRIDGE `b1f0dfc71` · **WORDS `24e60c890`** · instruments **r8** `e25979624` · toolkit **r7** `9915e0786`. Gate **20 files / 512 tests**; byte **−15,965 B**; wrap digest unmoved; determinism 18/18 with 18/18 distinct sha256. ✅ **THE PAGE HAS WORDS**: ward glyphs inside their own quarter **84.8 % → 100 %**; folio legend 88 rows / 18 leaves, 0 fails; L-REG-35 UNADDRESSED **0**. ⭐ **One constant explained a two-part finding**: `ADVANCE = 0.52` is documented "conservative" and is the opposite (true max 0.7093 em) — it set glyph steps (47 % overlap = C3 clause 1) AND box sizes (C3 clause 2) AND fit decisions. **Third sighting of one-number-many-consumers.** ⛔⛔ **A DORMANCY INSTRUMENT PASSED BY COMPARING NOTHING (§713.2):** `out/` holds ~75 stale `.full-CROP4-*` crops and **ZERO corpus leaves**, so its `diff -rq` was a total FALSE PASS — self-caught. On the correct comparator the build was **not dormant**: 1 leaf of 29 moved on floating-point associativity, and **28 of 29 rounded identically, so a smaller corpus would have shipped it.** ⭐ **A DORMANCY CLAIM IS A BIT CLAIM.** ⚠ P1: sweep this arc's other dormancy proofs for the same shape. ⛔ **THREE LEAVES DECLARE A LANDFORM THEY ARE NOT** (§713.4 — declared riverside/coastal, all solve to fjord). ⭐⭐ **A legibility pass over a surface that is WRONG makes the wrongness more credible, not less** — so the cartouche says the status in words rather than deleting the marker. **Owner docket: substrate, a declared shift.** ⚠ **FOUR CHARTER CORRECTIONS (§713.5)**, one of them an **unsourced citation this document propagated** ("the §657 marginalia lesson" — §657 has no such content; the referent is C3 clause 2). **The enumerated denylist never existed.** **NEXT: JUDGING (§4)** — the pre-ruled rubric, rounds under §695's amended key, **the chair closes rounds and the owner closes the specimen** — then REG-D → REG-11/F0/F → REG-P.
