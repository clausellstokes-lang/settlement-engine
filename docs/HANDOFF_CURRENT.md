...
|---|---|---|---|---|---|
| LANDING | TE-UCSTACK-LANDING (Fable) | laneUCSTACK-tree @ 0ccd5794(+1 regen) | laneUCSTACK-receipt.md | §475 edge-shared regen → packet GENERATED rows → sweep-3 (+edgeFunctions) → terminal `ucstack-TERMINAL-<newtip8>.log` + `ucstack-TERMINAL-exit.txt` | collect the terminal if it ran; else resume; CAS `f32c548e → <new tip>` on green; prune holding-uc0/uc3; a THIRD UC-0 class drops UC-0 from the stack |
...
| (held) | second seat HELD for UC-1 (Opus) until the UC stack lands (§476) | — | — | dispatch UC-1 from the ruled charter the moment UC-0 is on the branch | if the stack is still landing at pickup, R-INST-4 (resume, solo) may take the seat instead |
...
COMPLETE AND PINNED (holding for landing slots): WEB-2 `1d93458a` (holding-web2; migration 198) · WEB-3 `da9d9c40` (holding-web3; after WEB-2) · HK-A `f2b2711b` (holding-hka; D-HKA-1 comment fix at its landing act) · UC-0 `a5c6c4fa` + UC-3 `64a4b259` (IN the landing seat as a stack) · (the map stack LANDED at 421c7345).
