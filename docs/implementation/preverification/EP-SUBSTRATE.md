...
| EP-S018 | `prng.js#fork` is `` fork: (label) => createPRNG(`${seed}::${label}`) `` — the `::` delimiter law (§2.3c) | **MEASURED-TRUE** | `src/kernel/prng.js :: fork` verbatim |
...
| EP-S023 | `advanceMultiTick` is a REAL default-ON flag that ALREADY forks the pulse stream identity: `simulateCampaignWorldInterval` builds every composed tick with the LITERAL `interval: 'one_week'` (§2.4, J-EP-7) | **MEASURED-TRUE** | `src/lib/flagRegistry.js:88` `advanceMultiTick: true`; `advanceInterval.js:467` `interval: 'one_week',` |
