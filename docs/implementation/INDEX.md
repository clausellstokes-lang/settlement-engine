...
- **Measured branch/SHA:** `claude/composite-r4` at
  `357e7778e6318895c2ecb06dd5c7caed4dfe8863`
- **Measured:** 2026-08-13 — **re-derived at the GTR-1 promotion.** H26's detached
  code/genesis proof reproduced its exact schema figures, then correctly stopped before
  branch movement when the clean-checkout full gate exposed a `dist/`-dependent source
  ratchet. GTR-1 is the sole dispatchable packet; H26 remains the sole census holder but
  is blocked until GTR-1 lands and H26 is resealed on that descendant. No product code,
  golden, feature flag, tuning value, dependency, or census figure moves in GTR-1.
...
| READY-GTR1 | [`GTR-1`](./packets/infrastructure/GTR-1.md) | **READY** | Makes the full gate invariant between clean and warmed checkouts by assigning source tests and post-build artifact tests to disjoint, exhaustive phases. The source ratchet excludes `tests/build/**`; strict dist verification discovers that subtree exactly and refuses every non-running row. Lands as a coordinator-owned 4+1 code/baseline pair. | any product source, dependency, new test title/file, raw-test filtering, duplicate CI build, skip-ceiling raise, product golden, tuning, flag, timeout, budget, census or non-test baseline |
| BLOCKED-H26 | [`H26`](./packets/infrastructure/H26.md) | **BLOCKED** on GTR-1's clean-checkout gate repair | Schema 7 BANK-BY-RULE remains accepted and its detached pair reproduced 44 reads / 31 rows / 31 files and 1,998 / 1,412 / 387. Its mandatory full gate stopped on the foreign gate invariant before branch movement. After GTR-1 lands, H26 is revalidated, resealed and regenerated against that exact parent; the old detached objects are not reused. H26 remains the sole census holder. | AO-0 and every scalar/corpus producer seam; SCW-1b and item (a); any `src/**` edit; a fifth declaration; scan/corpus/sentinel movement; hand-edited JSON; untagged raises; every golden, tuning value, budget, timeout, floor and unrelated baseline |
...
⭐ **CURRENT HOLDER: H26, and H26 alone.** The promotion re-derived 24 manifest rows with
zero non-terminal holders before adding H26. H26's eight-file code half adds executed guard
