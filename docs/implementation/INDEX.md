...
| HB-1-M2 | [`HB-1`](./packets/fp/HB-1.md) | **READY** at `6784bf62de455ca13b701c09b6b241476cf9d555` | Member M2 of the `hb-1` train and its terminal content member, promoted on a CORRECTED charter: `OWNER_DECISION_QUEUE.md` §36 struck three of its clauses on executed evidence. It mints the estate's single closed strategy-move vocabulary as a dependency-free leaf, which is the WC volume's highest-value cross-volume collision resolving in HABIT's favour on facts — WC-0 is measurably absent, so the volume that builds first mints and the second amends. The leaf carries TWO exports, the emitted totality and the union the fences key on, asserted equal today, because the auxiliary moves a later WC wave adds are dispatched rather than emitted and a naive widening would red this wave's own emitter pin. It also lands the habit fork registry with every row deferred and a written close owed, the two totality walkers that freeze both, and the at-most-one-exporter scan the absent WC wave was chartered to land first. The local martial-history predicate becomes a NAMED DERIVED SUBSET rather than the retirement the charter asked for: retiring an eight-member semantic predicate to an eleven-member totality is a live behaviour change on the war chooser. It buys NO size headroom — the swap is one-for-one and the file measures exactly its baseline — and this packet holds the estate lighting-census reservation | the size baseline and both typecheck ratchets; the observed-shape reader count; the argued roster ceiling and the unlayered baseline; `scoringObjective.js`, `convergence.js`, `mobilizationReactions.js`; every flag manifest row and coupling registry row; `pulseKernel.js` and `applyWorldPulse.js`; every persisted shape, golden and tuning value |
...
⛔ **ROW TAKEN, 2026-08-14 — HB-1 (the `hb-1` train's member M2).** It adds four credited test files carrying twenty-eight literal `it` titles and four literal `describe` titles, so it moves the tuple and must hold the reservation. Its reserved whole tuple is `2425/366/2059/20074/5650`, re-derived WHOLE at the train's last `tests/`-moving commit rather than at the terminal, which is what leaves that terminal docs-only and makes the regenerated capsule's stamped window literally true. Its sibling member HB-0 moves the tuple too but does NOT hold the row: inside a train the members are strictly sequential on one private ref, so the interior figure is a working number and the re-derivation happens ONCE. The live manifest has **42 rows and one dispatchable packet**. For this run no diagnostic, weekly, release or research soak worktree was created.

⚠ **DATED CORRECTION, 2026-08-14 at the `hb-1` promotion.** The GR-4b-iii-a paragraph below remains true history but no longer describes the current holder or packet count.
