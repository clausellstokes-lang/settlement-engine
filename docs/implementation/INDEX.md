...
- **Measured:** 2026-08-13 — **re-derived at the H26 post-GTR re-promotion.** The coordinator-owned
  ...
  H26's strict 6→7 bundle must bind that post-GTR tree while retaining exactly the ruled
  31-row inventory movement. No product, golden, flag, tuning, dependency, or census moved.
...
| READY-H26 | [`H26`](./packets/infrastructure/H26.md) | **READY**; revalidated at `b96fae42`; fresh seal and pair required | Schema 7 BANK-BY-RULE remains accepted: 44 reads / 31 rows / 31 files and target 1,998 / 1,412 / 387. GTR changed only governed scanner input `package.json`; the strict 6→7 review binds that post-GTR detector tree with zero extra inventory motion. Old seal and detached objects are evidence only. H26 remains the sole census holder. | AO-0 and every scalar/corpus producer seam; SCW-1b and item (a); any `src/**` edit; a fifth declaration; scan/corpus/sentinel movement; any other scanner drift; hand-edited JSON; untagged raises; every golden, tuning value, budget, timeout, floor and unrelated baseline |
