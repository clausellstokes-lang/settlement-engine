...
- **Measured branch/SHA:** `claude/composite-r4` at
  `cd4049481954539d8e4c9753f493cf18d0b33295`
- **Measured:** 2026-08-13 — **re-derived at the GTR-1 landing.** The coordinator-owned
  ...
  terminal. H26 is STALE pending descendant revalidation and remains the sole census
  holder. No product code, golden, flag, tuning value, dependency, or census figure moved.
...
| STALE-H26 | [`H26`](./packets/infrastructure/H26.md) | **STALE** pending post-GTR revalidation | Schema 7 BANK-BY-RULE remains accepted and its prior detached pair reproduced 44 reads / 31 rows / 31 files and 1,998 / 1,412 / 387. GTR-1 discharged the foreign gate invariant; H26 must now be revalidated, resealed, and regenerated against this descendant. The old detached objects are evidence only. H26 remains the sole census holder. | AO-0 and every scalar/corpus producer seam; SCW-1b and item (a); any `src/**` edit; a fifth declaration; scan/corpus/sentinel movement; hand-edited JSON; untagged raises; every golden, tuning value, budget, timeout, floor and unrelated baseline |
