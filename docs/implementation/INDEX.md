...
| READY-11 | [`TC-5a`](./packets/town-cartography/TC-5A.md) | **READY** | The cartography painter's headless half: a pure draw-list plus palette ROLES, no colour, no mount | colour binding + the sub-tab (TC-5b), PNG goldens (TC-5c), skins (TC-5d), AI controls, the lynch-rubric gate (its own synthesis slice) |
| READY-12 | [`ES-6a`](./packets/foreign-policy/ES-6A.md) | **READY** | The double agent's leak: a compromised operative's mission reaches the enemy court while succeeding silently | vetting quality (ES-6b, rides the dispatcher), the retroactive Herald clause (folds into ES-7), any new belief key, rumor-reach predicates |
