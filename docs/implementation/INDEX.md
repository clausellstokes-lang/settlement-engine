...
⭐ **ROW CLAIMED — `HB-1`, train `hb-1p`, 2026-08-14.** The re-chartered member takes the
estate lighting-census reservation. Its I1 commit is the last commit in the train that moves a
`tests/` byte, so the whole tuple is re-derived THERE and the terminal stays docs-only, which
is what lets the capsule regenerate in one terminal commit. Declared movement, BY FIGURE:
`2421/366/2055/20046/5646` → `2425/366/2059/20077/5650` (+4 files, +0 parked, +4 credited,
+31 titles, +4 suite titles); `runtimeTests` `28062` → `28093`. Parked holds at 366 because
every one of the thirty-one titles is a literal in a straight-line registration — no
`test.each()` and no `describe.runIf()` anywhere in the member. The live manifest has **42
rows and one dispatchable packet**.

⚠ **DATED CORRECTION, 2026-08-14 at the `hb-1p` promotion.** The paragraph below is true
history of the prefix landing but no longer describes the current holder or packet count.
