...
| A2 | Absent / disabled — FOUR fences | On one fixture: (a) espionage dark ⇒ `rulingBlocOf` AND `evaluateFactionRules` outputs are **byte-identical** to base; (b) roads-lit + espionage-dark fenced explicitly; (c) espionage-lit + `factionCompetitionEnabled` dark ⇒ **the faction arm is byte-identical, AND the reason the bench arm cannot move is asserted in-test** — `settlementBlocs` `.toEqual([])` and `rulingBlocOf` `.toBeNull()`, against a lit control that is `.not.toBeNull()`; (d) `errandSpineEnabled` false ⇒ `espionageActive` false ⇒ byte-identical. ⚠ **A2(c) REWORDED to its executable half per chair ruling H17, 2026-08-11.** As originally written this clause asked for the faction arm to be byte-identical *"while the bench arm still moves"*, and **that second half is unreachable**: `settlementPoliticsActive` is the CONJUNCTION `r.settlementPoliticsEnabled === true && r.factionCompetitionEnabled === true` (`src/domain/worldPulse/settlementPolitics.js:169` — note the real path carries `worldPulse/`), and both `settlementBlocs` (`:530`) and `rulingBlocOf` (`:716`) return early on that gate, so darkening faction competition **darkens the bench too and leaves no bench reading left to move.** The unexecutability is a two-line structural fact, not a judgment call. The lane took the honest path — pin the surviving executable claim and **assert the contradiction itself in the test so it cannot decay into prose** (FENCE 3, `espionageAbsenceDormancy.test.js:174-198`) — rather than shipping a weaker test under A2(c)'s name, dropping the clause, or re-gating anything to make the sentence true. **The in-test contradiction assertion is the permanent record.** |
