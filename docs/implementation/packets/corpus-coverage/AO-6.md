...
- **Baseline posture:** three existing mutation rationales become three standing planted
  ...
  target-count movement beyond the exact conversions, three named legacy-probe repairs, and
  one modal Perl syntax repair in this amended packet is licensed.
...
Each new-mutant source anchor in section 3 occurs exactly once. The headline and Chronicle
positive product targets are not in `MUTATED_FILES` yet; they are added by this packet. The
negative control uses the already-added headline target. The stopped proof also established
that the legacy voice plant must move to a third newly guarded product target,
`src/copy/en.js`, while the patch-key plant replaces its obsolete `src/store/aiSlice.js`
guard with `src/store/aiChronicleAppend.js`. Because `src/data/stressTypes.js` remains guarded
by the independent data-purity plant, that exact repair adds one unique guarded target; the
patch-key replacement is count-neutral.

### 1.1 Immutable stopped proof and narrow reauthorization

The first implementation candidate
`b02bc709c81d3d7c3b30d13aa9126a5cbac04b27` is an immutable direct child of the
promotion. Its disposable-tree whole-sweep log has SHA-256
`d582b8a06a909f5b05c7321e690a5702a7140f87829ab906ed80709d2eb99d9a` and true exit
`4`: `77 CAUGHT / 1 CLEAR / 4 MISSED-or-BROKEN`. It is permanently stopped and must not
reach the shared ref. The four gaps were all measured legacy drift, not permission to widen
the packet:

1. `enforcement/meta-pin naked claim` ran the intentionally banked-red whole
   `enforcement-claims` file instead of its newer anti-absorption case;
2. `voice/exclamation in scanned data prose` ran the intentionally banked-red broad
   voice-mechanics file and planted in a baseline-governed data file instead of a clean copy
   registry;
3. `patch-keys/call site writes an unadmitted key` still named the Chronicle writer's former
   `aiSlice.js` home after the decomposition wave moved it; and
4. `corpus-coverage/modal summary admitted under indicative home` contained one stray quote
   in its Perl program, so the source plant changed zero bytes.

Version 2 authorizes only the exact repairs in section 3.5. The implementation must remain the
same four committed paths, three new manifest labels, three new positive calls, one CLEAR call,
and zero title or census movement. Any other response to the stopped proof is outside authority.
...
- exact dirty-tree guard totality for the three newly touched source paths and the one
  count-neutral patch-key target replacement;
- exact one-to-one manifest/label closure; and
- a fail-closed attribution helper that rejects an unrelated red.

Explicitly out:

- any product-source edit in the committed implementation; all transient product targets are
...
- parsing Vitest JSON, changing a reporter globally, adding a dependency, weakening an existing
  mutation, deleting a rationale other than the three exact rows, changing any existing
  mutation-manifest claim except the one patch-key `what` path correction, or moving
  `uncoveredBaseline`;
...
apart from the exact three stale caller repairs in section 3.5:
...
For a caller with no fourth argument, behavior and output stay exactly as today except where
section 3.5 supplies an expected title and a narrower clean command. For a caller with an
expected title, `CAUGHT` requires all four facts:
...
### 3.5 Exact legacy-drift repairs licensed by the stopped proof

Keep all three existing labels stable. Change only the target, plant, command, expected title,
and one manifest `what` string expressly named below. All `-t` values are token-only single
shell words so the existing `$check` execution road preserves them under word splitting. Clean
preflight proved that each filter selects exactly one green test.

| Stable existing label | Sole repaired target and plant | Exact repaired command and required full title |
|---|---|---|
| `enforcement/meta-pin naked claim` | Keep `ARCHITECTURE.md` and the existing appended sentence formed from `This guarantee is `, then `machine-`, then `enforced.` unchanged. | Replace only the command with `npx vitest run tests/docs/enforcement-claims.test.js -t seventh --no-file-parallelism --reporter=verbose`; after left trim, exactly one output line must begin `FAIL ` and end ` > the banked naked-claim debt is frozen PER CLAIM — a seventh cannot hide inside it`. The unfiltered file is intentionally red on six banked naked claims and is not an admissible clean attribution command. |
| `voice/exclamation in scanned data prose` | Retarget from `src/data/stressTypes.js` to `src/copy/en.js`; replace the unique common-registry anchor `    save:        'Save',` with `    save:        'Save!',`. Keep `src/data/stressTypes.js` in `MUTATED_FILES` because the independent data-purity plant still targets it. | Replace only the command with `npx vitest run tests/copy/voiceMechanics.test.js -t en.registry --no-file-parallelism --reporter=verbose`; after left trim, exactly one output line must begin `FAIL ` and end ` > en registry is clean`. Keep the stable label unchanged even though the repaired target is the copy registry. |
| `patch-keys/call site writes an unadmitted key` | Retarget from `src/store/aiSlice.js` to `src/store/aiChronicleAppend.js`; at its unique live Chronicle writer, replace `get().updateSavedSettlement(saveId, { aiData: nextAiData });` with `get().updateSavedSettlement(saveId, { aiData: nextAiData, aiRevisionCount: 1 });`. Replace the obsolete `aiSlice.js` guard row rather than retaining it. | Replace only the command with `npx vitest run tests/store/savedSettlementPatchKeysWalker.test.js -t censused --no-file-parallelism --reporter=verbose`; after left trim, exactly one output line must begin `FAIL ` and end ` > (a) every censused call-site key is admitted by SAVED_SETTLEMENT_PATCH_KEYS`. In the existing mutation-manifest object's nonblank `what`, change only the writer path from `src/store/aiSlice.js` to `src/store/aiChronicleAppend.js`. |

The new modal-summary call keeps its target, label, command, and expected title from section 3.3,
but its malformed Perl program changes exactly from
`perl -0pi -e "s/'npc_bargain', //'"` to
`perl -0pi -e "s/'npc_bargain', //"`. This is a one-character syntax repair, not a fourth
legacy caller move.

No other pre-existing guard row, label, plant, command, result, expected title, or manifest
claim moves. In particular, the full broad legacy commands are not accepted as substitutes for
the three filtered commands above.

## 4. Exact mutation-manifest closure

Replace only the three existing AO rationale objects with mutation objects. Keep each row in
place; do not reserialize the file or edit the shared `rationales` dictionary. The required
labels are exactly the three stable labels in section 3.3. Each object also carries a nonblank
`what` that states the source deletion, the intended named red, and the green restored
attribution run. In addition, section 3.5 licenses one raw-splice correction inside the existing
`tests/store/savedSettlementPatchKeysWalker.test.js` mutation object: change only the obsolete
writer path in `what` from `src/store/aiSlice.js` to `src/store/aiChronicleAppend.js`. No kind,
label, key order, or other prose moves.
...
| In-place-or-missing mutation calls / unique target files | `70 / 56` |
| `MUTATED_FILES` rows / unique rows | `56 / 56` |
| Positive sweep result | `81 CAUGHT / 0 MISSED-or-BROKEN` |
| Negative sweep result | `1 CLEAR / 0 red` |
| `uncoveredBaseline` | `198` |

Add exactly `src/domain/worldPulse/worldPulseFeedCuration.js`, `src/lib/chronicle.js`, and
`src/copy/en.js` to `MUTATED_FILES`; replace exactly the obsolete
`src/store/aiSlice.js` row with `src/store/aiChronicleAppend.js`. Apart from the exact section
3.5 repairs, no existing guard row, label, plant, command, result, expected title, or manifest
claim moves.
...
| Existing manifest `what` corrections | exactly `1` — patch-key writer path only |
| New sweep labels / guarded-target row movement | exactly `3 / 3 additions + 1 replacement` |
| Hazard/pre-mortem/census/baseline edits | `0` |
| Acceptance cases | exactly `8` |

| Action | File | Symbol/region | Coding instruction |
|---|---|---|---|
| MODIFY | `scripts/mutation-sweep.sh` | counters, `MUTATED_FILES`, `check_caught`, new `check_clear`, exact section 3.5 legacy repairs, AO-6 control block, results footer | Preserve every existing caller except the exact three filtered retargets and one modal Perl syntax repair in section 3.5; add exact-title attribution, three positive plants, one negative control, three guard rows, one guard replacement, and exact post-count reporting. |
| MODIFY | `scripts/mutation-coverage-manifest.json` | the three exact AO invariant rows plus the patch-key mutation object's writer-path phrase | Convert the three rationales to mutations with exact labels and nonblank `what`; change only `src/store/aiSlice.js` to `src/store/aiChronicleAppend.js` in the existing patch-key `what`; raw splice only, no reserialization or baseline movement. |
...
1. Dispatch and seal at the clean promotion descendant; prove 31 packets / 1 READY, exact
   four-target cleanliness, this version-2 amendment descendant, and the complete pre-state in
   sections 1 and 5.
2. Prove each new and repaired source anchor occurs exactly once and that each token-only filter
   in sections 3.3 and 3.5 selects exactly one clean green title before editing.
3. Extend `check_caught` additively with exact detailed-line `FAIL ... > <full-title>`
   attribution and add count-verifying `check_clear`; make exactly the four section 3.5 repairs;
   add exactly three guard paths, replace exactly one guard path, and add the three positive plus
   one negative calls.
   Run `bash -n` before any probe.
4. Make the headline walker's exact one-line setup correction, convert exactly three AO manifest
   rows, correct the exact patch-key `what` path, and update exactly the two compatibility
   assertions.
...
|---|---|
| A1 | The clean pre-state is exactly 541 invariants split 68/275/198, ten meta mutations, 78 exact one-to-one labels, 67 in-place-or-missing calls, 53 unique guarded targets, and uncovered baseline 198; every section 3.3/3.5 source anchor is unique and every token-only selector chooses exactly one clean green title. The immutable stopped proof is recorded as `b02bc709` at true exit 4, 77 CAUGHT / 1 CLEAR / 4 gaps and log SHA-256 `d582b8a06a909f5b05c7321e690a5702a7140f87829ab906ed80709d2eb99d9a`, and is not landed. |
| A2 | `check_caught` remains backward-compatible for every old caller except the exact three filtered legacy repairs in section 3.5; its optional-title arm requires changed bytes, mutated nonzero, exactly one captured physical line that after left trim begins `FAIL ` and ends with literal ` > <expected-full-title>`, restored zero, and no whole-output shortcut/pipeline/eval. The meta-pin, voice and patch-key calls use exactly `seventh`, `en.registry` and `censused` with their recorded full titles; the modal Perl loses exactly its stray quote. A skipped, wrong, absent or ambiguous title is `BROKEN`, never `CAUGHT`. |
...
| A7 | Exactly three AO rationale rows become exact mutation labels and exactly one existing patch-key `what` changes only its writer path; the post-state is 541 at 71/272/198 plus ten meta mutations, 81 unique claims/labels, 70 in-place-or-missing calls, 56 unique guarded targets and 56 unique `MUTATED_FILES` rows, with baseline 198 while hazard, pre-mortem, corpus baselines, OSR and lighting census remain exact. |
...
sh scripts/gate-mutex.sh --run -- npx vitest run \
  tests/docs/enforcement-claims.test.js -t seventh \
  --no-file-parallelism --reporter=verbose
sh scripts/gate-mutex.sh --run -- npx vitest run \
  tests/copy/voiceMechanics.test.js -t en.registry \
  --no-file-parallelism --reporter=verbose
sh scripts/gate-mutex.sh --run -- npx vitest run \
  tests/store/savedSettlementPatchKeysWalker.test.js -t censused \
  --no-file-parallelism --reporter=verbose
sh scripts/gate-mutex.sh --run -- npx vitest run \
...
1. The original three-document READY promotion is
   `0ff79e6b6c75d77a2396703c07f982615d9f31f9`; the immutable first implementation child
   stopped on the measured four-gap contradiction in section 1.1 and remains unlanded.
2. The coordinator commits packet version 2, the index amendment and the manifest amendment as
   one three-document direct descendant of that promotion. A fresh dispatch seals the clean
   amendment descendant after the exact preflight in section 7.
...
4. The coordinator audits scope, then uses a private index to construct one direct green child
   of the sealed amendment descendant without moving the shared ref.
...
Authority amendment, implementation and terminal record are not squashed. No staging by the
coding agent, merge, shared-tree checkout/reset, stash, dirty override, partial promotion, or
source commit is authorized.
...
- any pre-existing sweep guard/label/plant/command/result/expected-title or manifest claim moves
  beyond the exact section 3.5 repairs, any negative label enters the manifest join,
  `MUTATION_SWEEP_ALLOW_DIRTY=1` is needed, or the sweep must run in the shared worktree;
- invariant post-counts differ from 71/272/198, total claims/labels differ from 81/81, mutation
  target/guard counts differ from 70/56, `MUTATED_FILES` differs from 56 unique rows,
  `uncoveredBaseline` moves, or a manifest row is reserialized outside the exact three AO
  objects and one patch-key `what` path phrase;
...
Report promotion, authority-amendment, sealed-parent and final SHAs; the stopped `b02bc709`
receipt and log SHA-256; exact four paths and raw deltas; pre/post manifest, label, target and
guard counts; exact three AO manifest objects and one patch-key `what` correction; each plant
anchor, token-only filter and expected title;
