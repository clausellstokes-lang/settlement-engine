...
- **Packet version:** 3 (compiled at `58fcfe614`, re-cut at `ad7ddf2c9`, pre-proofed at `141a1d775`)
  **What version 3 changed and why.** An Opus pre-proof lane re-measured every verified fact at
  ...
  consistency group. Five things changed, every one measured (E-35…E-48):
  (1) ⭐ **THE SEVENTH CONSISTENCY GROUP IS DECLARED** — `history-age`, rooted at `history` with the
  two-member list `['age', 'founding.age']`, as ODQ §934.47 addendum 38 ruling (1) orders. The
  premise is re-executed on this lane's own harness: `history.age ≡ history.founding.age` in
  **63/63**, the value a NUMBER, and the group's root and both members resolve 63/63 (E-41). The
  member list is load-bearing: `members: null` would swallow the three ORDER-BEARING collections
  `history` also carries.
  (2) ⛔ **A6's NON-NESTING TEST IS RE-CUT FROM ROOTS TO COVERED PATH SETS, because the packet's own
  register REDS under the literal text it carried** — `economicViability.metrics.foodBalance` (G2) is
  nested inside `economicViability` (G3) by root prefix (E-42). Design §22.3 item 2 created the
  member list for exactly that pair, so the test is over what a group COVERS. Under the re-cut every
  covered set is disjoint, the new group included. **The chair's stated REASON for refusing a
  defence-pair group does not survive the re-cut** (its outcome does) — §11 Q1.
  (3) ⭐ **Q4 IS DISCHARGED BY EXECUTION, not prediction** — the observed-shape register does NOT
  move: the leaf's only property access is `Object.freeze`, which resolves to no shape, and 7,301
  live occurrences of it under `src/` mint nothing today (E-44). Writer-reach likewise (E-45), where
  the packet's "seven `SURFACE_ROOTS`" is corrected to **six**.
  (4) ⛔ **THE CHANGE-PATH RESERVATION CHANGED OWNER.** EM-B1d is LANDED; the manifest holds 193
  packets with ONE non-terminal, **EM-P2 (READY)**, and it is EM-P2 that now names
  `scripts/mutation-coverage-manifest.json` (E-39). §4 is rewritten.
  (5) ⚠ **ONE GOVERNING ENFORCER WAS MISSING** — `tests/lint/contractTestAntiVacuity.walker.test.js`
  governs `tests/lint/*.test.js` by its own declared scope and is added to `checks` and §10, with the
  `tests/lint` WHOLE run the 2026-09-20 directory rule demands (E-48).
...
- **Verified base:** `fixes-2026-09-18-consist` at `2dc08a595134d7571012dcc515ec4bb0dd147b73`
  ⛔⛔ **THE PLACEHOLDER'S FORM IS LOAD-BEARING AND WAS MEASURED** (E-25). `2dc08a595` is one token
  ...
  `<40-hex>` or a bare 40-hex; a bare `` `2dc08a595` `` parses to `null` and `:889` then errors
...
- **Last revalidated:** 2026-09-20 at `2dc08a595134d7571012dcc515ec4bb0dd147b73` — stamped by the chair (session a9df403c) at placement, the window re-run in the same command as the stamp: over every path of this packet except the expected movers (scripts/mutation-coverage-manifest.json) `git diff --stat 141a1d775..HEAD` printed NOTHING; every `requiredSymbols` row resolves; the CREATE targets are absent and untracked; no non-terminal packet reserves a change path. THE CHAIR'S CUTS AT PLACEMENT: MODIFY row for `tests/store/decreeRegistryPersistence.test.js` (the chair's placement cut: COMMENT-ONLY — once `recordRegister.js` exists the sentence is ); MODIFY row for `tests/lib/editTravel.test.js` (the chair's placement cut: COMMENT-ONLY — the sentence stays true but its premise ('the ve); MODIFY row for `docs/content/wiring-census.json` (the chair's placement cut (addendum 78): `node scripts/wiring-census.mjs` in the last gate); the anti-vacuity walker in `checks`. The Status value stands alone on its line (the parser's law). Its gated batches PAUSE-AND-RESUME.
  ⭐ **THE REVALIDATION SENTENCE, with this pre-proof's measured window facts.** *"Compiled at
  `58fcfe614`, re-cut at `ad7ddf2c9` under design §22.3, pre-proofed at `141a1d775` by execution
  (E-1…E-48). Re-validated at `2dc08a595`: the J-T1 window over all six change-manifest and
  `requiredSymbols` paths is quoted — 33 commits moved under the packet and exactly ONE of its paths
  moved, `scripts/mutation-coverage-manifest.json`, by APPEND only (+19/−0, EM-B1d v5's and
  EM-B1k2's rows), so every `requiredSymbols` path is byte-identical to the compile's base; both
  CREATE targets ABSENT and Git-clean; all four `requiredSymbols` rows resolve verbatim at the same
  lines the compile recorded (77 · 76 · 60 · 36) and `retiredSymbols` is empty as the post-edit
  simulation's result; `scripts/mutation-coverage-manifest.json` is a REGISTER row and therefore
  SUBSTRATE, so the base is the tip (§4); the observed-shape and writer-reach registers were RUN and
  do not move (1,964 findings matching the frozen inventory; WRWALKER HOLD); the eager first-paint
  closure was measured BY IMPORT at 268 modules with no `src/domain/edit/` member; and the
  register's figures — 41 generated keys, 4 saved-only, 48 keyed, 15 atomic, 2 cross-entry totals,
  **7 groups**, the receipt's one HISTORY sub-path — were re-measured, the 41 and the 63-row stride
  on this lane's own harness."*
  ...
  (5) The lighting census tuple — no absolute is quoted anywhere here; this packet states only its
  own DELTA (§7), and the live tuple is FROZEN at the tip by the fourth refreeze, which is the
  chair's act and not this packet's to read as a constant.
  (6) ⛔ The `scripts/mutation-coverage-manifest.json` reservation — **RE-MEASURED AND IT MOVED**:
  EM-B1d is LANDED and its reservation is discharged; **EM-P2 (READY) now holds that path** (E-39).
  §4.
- **Depends on:** **NONE for its substrate.** Sequencing only: **EM-P2** must be TERMINAL before this
  packet's manifest entry is added (§4) — re-measured at `141a1d775`, where EM-P2 is the manifest's
  ONE non-terminal packet and EM-B1d, the version-2 holder, has LANDED (E-39). ⭐ **EM-R0d does NOT
  precede this packet** — the family's order is EM-R0a → EM-R0d → EM-R0b (the charter's amendment of
  2026-09-19 18:30), and the three band ladders EM-R0d exports are EM-R0b's input, never this
  register's: this packet names no band threshold and imports nothing. ⚠ **EM-R0b version 3 is
  ACCEPTED** (ODQ §934.47 addendum 38) and its `V-HISTORY-AGE` check is the DETECTION half of the
  same ruling whose PREVENTION half this packet's seventh group discharges; neither waits on the
  other, and this packet declares no `recordInvariants` surface (§2's explicit non-goal).
- **Collision group:** `scripts/mutation-coverage-manifest.json` with **EM-P2** (both add one row;
  the file is never re-serialised whole). EM-P2's four other change paths are CREATEs under
  `src/domain/generation/`, `tests/lint/`, `tests/generators/` and `tests/helpers/` — **none is a
  path of this packet**. No other packet of the manifest's **193** names any path of this packet
  (E-39). ⭐ EM-R0b, EM-R0c, EM-R6 and EM-R7 will all IMPORT
  `src/domain/edit/recordRegister.js` and must not re-declare any of it.
- **Commit authority:** edits only; the chair commits
- **Baseline posture:** **measured, by execution, at `ad7ddf2c9` and RE-MEASURED at `141a1d775`** — the whole version-1 census
...
5. Live code and executed evidence at `ad7ddf2c9` (E-1…E-34) and, re-measured, at `141a1d775`
   (E-35…E-48) — `EM-R0a.evidence.md`.
6. ⭐ **ODQ §934.47 addendum 38 ruling (1)** (2026-09-20 02:47 EDT), which orders the seventh
   consistency group added AT THIS PRE-PROOF with its measurement cited, and draws the
   detection/prevention division between EM-R0b's `V-HISTORY-AGE` and this register's group.
...
| New/changed effective production lines | **`≤ 210`** — the emitted draft measured **163** (E-33); version 3's seventh consistency group adds **one** declaration line → **164** |
| Effective lines per new leaf | **`≤ 210`** (measured 164; the cap is 250 — **no split needed**) |
...
Expected: both CREATE targets ABSENT (verified at `141a1d775`, E-38); the REGISTER target clean;
every `requiredSymbols` row resolving verbatim (E-37).

| check | source | verdict once the chair stamps the base to the tip |
|---|---|---|
| status is READY | `:274` | the chair's promotion supplies it |
| manifest validates | `:271` | ⛔ **PASSES ONLY AFTER EM-P2 IS TERMINAL** — below |
...
| CREATE targets absent and Git-clean | `:208-210` | PASSES — both ABSENT at the tip (E-38) |
| non-CREATE targets Git-clean | `:213` | PASSES on a clean lane worktree |

⚠ Two header line numbers version 2 cited were measured OFF BY ONE and are corrected here (E-40):
the `verifiedBase disagrees with packet Markdown` error is **`:890`** (was cited `:889`) and the
READY non-blank `verifiedBranch` arm is **`:892`** (was cited `:891`). `parsePacketHeader` `:294`,
its accept range `:302-310`, and every `implementation-session.mjs` arm above are EXACT as cited.

⛔⛔ **THE CHANGE-PATH RESERVATION, RE-MEASURED AT `141a1d775` — AND IT HAS CHANGED OWNER (E-39).**
`implementation-packets.mjs:676` sets `reservesChangePaths = !TERMINAL_PACKET_STATUSES.has(status)`
with `TERMINAL = {LANDED, SUPERSEDED}` (`:43`), and `:699` errors `duplicate change path across
packets` — **a DRAFT reserves.** The rule is unchanged; its SUBJECT moved. The manifest now holds
**193 packets, 190 LANDED, 2 SUPERSEDED and one non-terminal: EM-P2 (READY)** — and it is **EM-P2**
whose change manifest names `scripts/mutation-coverage-manifest.json` (a REGISTER row). **EM-B1d has
LANDED and its reservation is discharged** (§934.47 addendum 31).

→ **EM-R0a's entry may not enter `PACKET_MANIFEST.json` at any status until EM-P2 is terminal.** The
chair ruled SEQUENCE (version 2's Q4); the family builds after train EM-T7 and EM-P2 v4 is placed
READY for its build, so this still costs nothing — but it is an act the chair executes, not a
surprise the terminal discovers, and **the packet it waits on is no longer the one version 2 named.**
...
### 5.3 THE CONSISTENCY GROUPS — SEVEN (six ratified at §22.3 item 4, the seventh ordered at pre-proof)

Seeded from the MIXED census's **five** shapes outside the trace and the receipt (E-11, E-12;
§22.3 item 4 ratifies the correction of "nine"), plus **G8 `history-age`**, which ODQ §934.47
addendum 38 ruling (1) orders added *"at EM-R0a's pre-proof with this measurement cited"*.
...
| **G8 ⭐ `history-age`** | `history` | `age` `founding.age` | **ONE FACT AT TWO PATHS:** `history.age ≡ history.founding.age` | ⛔ ordered by ODQ §934.47 addendum 38 ruling (1). **Re-executed at this tip on this lane's own harness: equal in 63/63, the value a NUMBER (sample 157); root and BOTH members resolve 63/63** (E-41). EM-R0b's `V-HISTORY-AGE` DETECTS the split; this group PREVENTS it |

⛔⛔ **G8's MEMBER LIST IS LOAD-BEARING; `members: null` WOULD BE A DEFECT.** `history` carries eight
keys, three of them the ORDER-BEARING collections §5.2 deliberately keeps out of every group
(`eventsTimeline`, `historicalEvents`, `currentTensions`), measured at this tip. A group rooted at
`history` taking the object WHOLE would move all eight whenever the age moved — the exact
"adding a tavern moves the food card" failure §22.2 item 2 exists to forbid. The two-member list
covers precisely the two leaves that hold one fact and nothing else.

⛔⛔ **AND "GROUPS NEVER NEST" IS A TEST OVER COVERED PATH SETS, NEVER OVER ROOT STRINGS — because
the ROOT reading REDS ON THIS REGISTER'S OWN DATA.** Executed over the declared groups (E-42):
`economicViability.metrics.foodBalance` (G2) **is** nested inside `economicViability` (G3) by root
prefix, so A6 as version 2 spelled it would have reddened at the build before G8 was considered.
Design §22.3 item 2 created the member list for exactly that pair — *"`economicViability` whole would
swallow `metrics.foodBalance`, which is a group of its own"* — so what a group OWNS is its root
expanded by its member list (`members: null` covering the root itself), and non-nesting is disjointness
of those covered sets. Measured under that reading: **every covered set is disjoint, G8 included.**
...
⛔⛔ **THREE CONSTRUCTION RULES THE LIGHTING CENSUS AND THE ANTI-VACUITY WALKER IMPOSE — a `credited
+1` that lands as `parked +1` is a mispriced census, and the anti-vacuity walker is a RED.**

1. **NEVER bind `it`, `test` or `describe` a second time anywhere in the file** — not as a variable,
   not as an arrow parameter. The census resolves an opener only where the module binds that word
   EXACTLY ONCE; EM-B1e's file parked as `OPENER_UNRESOLVED:it` because a `settlements.map((it) => …)`
   parameter was copied in from another suite, and 26 of the estate's 384 parked files park for that
   reason alone. This walker's arms iterate records, collections and entries, so the risk is live:
   spell every callback parameter `record`, `entry`, `row` or `path`.
2. **The build lane RUNS the census walker's own `parkReasonsFor` over the emitted file and prints
   what it returns, BEFORE claiming `credited +1`.** The predicted delta in §7 is a prediction until
   that print is in the receipt.
3. ⭐ **THE ARMS ITERATE IMPORTED SETS AND DERIVED RECORDS, NEVER A LOCAL LITERAL.**
   `tests/lint/contractTestAntiVacuity.walker.test.js` governs `tests/lint/*.test.js` by its own
   declared scope, and its Rule 2 convicts an exhaustive-claim title that backs its claim by
   iterating a LOCAL PURE LITERAL in a file that derives nothing from source. Six of these seven
   titles are exhaustive claims. The file is lawful **only because** every declared set is imported
   from `src/domain/edit/recordRegister.js` and every record is produced by running the real
   generator. ⛔ Inlining a convenience copy of any table into the test reds that walker.
...
| **A6 GROUP LIVENESS AND NON-NESTING** | every root resolves (a `foo[]` root at every entry); every member path resolves under its root; ids unique; and **no group's COVERED SET intersects another's** — a group covers its root expanded by its member list, `members: null` covering the root itself. ⛔ Asserted over covered sets, NEVER over root strings: by root prefix `food-balance` sits inside `viability-counts` and the arm would red on its own register (E-42) |
...
| `MODIFY` | `tests/store/decreeRegistryPersistence.test.js` | the header comment's sentence '`src/domain/edit/**` (which does not exist at this commit …)' (near :17) | `n/a` | the chair's placement cut: COMMENT-ONLY — once `recordRegister.js` exists the sentence is false; re-cut it to say the directory exists since EM-R0a and this test still imports nothing from it; no assertion moves; the file's own tests unchanged (run them) |
| `MODIFY` | `tests/lib/editTravel.test.js` | the header comment's sentence 'nothing here imports from `src/domain/edit/**`' (near :15) | `n/a` | the chair's placement cut: COMMENT-ONLY — the sentence stays true but its premise ('the veil precedes the writer') gains the fact that EM-R0a created the directory; one clause; no assertion moves; the file's own tests unchanged (run them) |
| `MODIFY` | `docs/content/wiring-census.json` | `producerIndexFiles` (a file count) | `n/a` | the chair's placement cut (addendum 78): `node scripts/wiring-census.mjs` in the last gated batch, the regenerated file committed with the packet — the count moves by the packet's new `src/domain`/`src/generators` leaves; never edited by hand |

**Generated artifacts: `NONE`, and §P2 rows 10, 11 and 12 are answered rather than assumed
(E-46, E-47).** No `checks` command of this packet runs a generator (`npx vitest` and `npx eslint`
only); the new leaf is imported by nothing and is therefore an input to none of the five edge-shared
bundles; and this packet widens no EXISTING named set, so row 12's consequence sweep has no subject.
**EXECUTED, because "a declared command writes" is the rule and not "a generator writes":**
`grep -n writeFileSync scripts/implementation-packets.mjs` returns NO MATCH, so §10's
`validate` is read-only; and §10's two typecheck commands write their baselines only behind an
explicit update flag (`check-domain-strict.mjs:138`'s `if (UPDATE) {` guarding `:140`;
`check-full-typecheck.mjs:254`'s write reachable only via `npm run typecheck:ratchet:update`). No
generated path is owed. **Row 11's skipped-byte-arm sentence has no subject here**: the packet
declares no byte arm because its delta is 0 B in every budget (E-46).
...
|---|---|---|---|
| Lighting census (§P2.1) | **YES — one new TEST file** | ⛔ **THIS PACKET'S DELTA, AND NO ABSOLUTE:** `files +1` · `parked +0` · `credited +1` · `titles +7` (A1–A7; A8 runs two files it does not edit) · `suiteTitles +1`. The delta is counted from this packet's own CREATE rows — one new test file, one literal `describe`, seven literal `it`s — and from nothing else. Version 3 adds a GROUP ROW to the register's data and no arm, so the title count is unchanged from version 2. ⚠ `credited +1` holds only if the emitted file parks for nothing; §6's three construction rules and the build lane's `parkReasonsFor` print are what make it a measurement rather than a hope. The live tuple is the chair's to read at promotion; this packet quotes none | a named INTERIOR RED; re-derived whole at the terminal, BY THE CHAIR |
| Mutation-coverage (§P2.2) | **YES** | `+1` row — `tests/lint` is an `ENFORCER_DIR` | §7's REGISTER row. ⛔ It forces the base to the tip and collides with **EM-P2** (§4, E-39) |
| Observed-shape readers (§P2.3) | ⭐ **NO — CONFIRMED BY EXECUTION AT PRE-PROOF, no longer predicted** | the scanner RUNS green at the tip (`1964 finding(s), exactly matching the frozen inventory`, exit 0); a finding requires a property access whose receiver resolves to exactly ONE known shape (`legacy-reader-shape-scan.mjs:544-556`), and the draft leaf's ONLY property access is `Object.freeze` — 15 occurrences, one distinct, receiver the `Object` global, which resolves to no shape. Live precedent at estate scale: **7,301 `Object.freeze` under `src/` across 1,084 files** mint zero findings today (E-44) | ✅ **DISCHARGED.** No `EXPLAINED_WRITER_EXEMPTIONS` row, no migration-bundle door, no chair act |
| Writer-reach (§P2.4) | **NO — RUN at pre-proof** | `WRWALKER HOLD`, exit 0. `SURFACE_ROOTS` holds **SIX** surface classes, not seven (`scripts/lib/writer-reach-scan.mjs:93-102`; the version-2 figure is corrected), each a set of ENTRY files; a module with zero importers is reachable from none of them (E-45) | — |
| Bundle ceilings (§P2.11) | **NO — 0 B in all four** | a module with no importer is emitted in no chunk (§5.4). **Re-measured BY IMPORT at pre-proof exactly as row 11's amendment demands** (`never by reading a list`): `EAGER_FIRST_PAINT_MODULES` imported from `vite.config.js` is **268 modules** — the amendment's own figure, independently re-derived — with **zero `src/domain/edit/` members** (E-46) | EM-R0c pays the register's bytes as its first importer. No budget TEST row and no re-mint is owed |
| Prose-numerics · decision-fork · mechanism-coverage · size-baseline · edge-shared | **NO** | zero | — |
| ⚠ Line-addressed baselines (pre-proof step 13) | **NO — CONFIRMED** | `tests/lint/proseNumerics.test.js:9-10` freezes debt *"by exact path + line + snippet identity"* in `.prose-numerics-baseline.json` — the one FILE+LINE register found. This packet MODIFIES no existing file, so it shifts no line above any baselined row (E-47) | — |
| ⚠ Stamped producer files (pre-proof step 15) | **NO — CONFIRMED** | `docs/content/wiring-census.json`'s `stamp.files` names **7** producers, none under `src/domain/edit/`; the packet modifies no `src/` file, so `stale-bytes` cannot fire (E-47) | the census JSON is NOT a manifest row |
| ⚠ `path:line` citations (pre-proof step 16) | **NO — CONFIRMED** | the packet shifts no line in any existing file, so no citation in `src/`, `docs/content/` or `tests/` can be re-addressed (E-47) | — |
| ⛔ **The `tests/lint` directory-whole rule** (the chair, 2026-09-20) | **YES — a CREATE under `tests/lint` opts into every walker governing it** | `tests/lint` holds **172** `*.test.js` at the tip. One governing enforcer was MISSING from version 2: `contractTestAntiVacuity.walker.test.js` declares `tests/lint/*.test.js` in its own scope header, and EM-B1k2 and EM-B3c both name it in their `checks` (E-48) | added to `checks` and §10, with the `tests/lint` WHOLE run as the build lane's instrument |
...
| A6 | Group liveness and non-nesting | the same records | all **seven** roots resolve (per-entry roots at every entry); members resolve under their root — `history-age`'s `age` and `founding.age` both 63/63; ids unique; and **covered sets disjoint** (root expanded by member list), never root prefixes | same |
...
GATE_MUTEX_TIER=shared GATE_MUTEX_MAX_POLLS=100000 GATE_MUTEX_POLL_SECONDS=20 \
  ...
  tests/lint/contractTestAntiVacuity.walker.test.js --maxWorkers=2

# ⛔ THE DIRECTORY-WHOLE RUN (the chair, 2026-09-20, after run 19): a CREATE under tests/<dir>
# opts into EVERY walker governing that directory. 172 files. Runs ONCE, as an instrument.
# The sovereignty-lighting census is its ONE lawful interior red and is named in advance.
GATE_MUTEX_TIER=shared GATE_MUTEX_MAX_POLLS=100000 GATE_MUTEX_POLL_SECONDS=20 \
  sh scripts/gate-mutex.sh --run -- npx vitest run tests/lint --maxWorkers=2

# The census's own park reasons over the EMITTED file — §7's `credited +1` is a prediction
# until this prints, and an OPENER_UNRESOLVED here means the delta was mispriced.
# (Read parkReasonsFor in tests/lint/sovereigntyLightingContract.walker.test.js.)
...
**Q-A — ⛔ THE NON-NESTING TEST HAD TWO READINGS AND THE ONE VERSION 2 WROTE REDS ON THIS REGISTER'S
OWN DATA. Ruling recommended, and it costs the chair a stated reason.** Measured (E-42): by ROOT
PREFIX — A6's literal text in version 2 — `economicViability.metrics.foodBalance` (G2) is nested
inside `economicViability` (G3), so the arm would have reddened at the build before the new group was
considered. By COVERED PATH SETS (root expanded by its member list) every group is disjoint, G8
included. **This pre-proof adopts the COVERED-SET reading**, because design §22.3 item 2 created the
member list for precisely that pair and says so in words. ⚠ **The consequence the chair must see:**
ODQ §934.47 addendum 38's parenthesis refuses a defence-pair group *"because a root at
`defenseProfile` would nest `G7 defense-readiness`"* — the ROOT reading. Under covered sets, a group
`{ root: 'defenseProfile', members: ['magicDependency','scores.magicDependency','traditions','scores.traditions'] }`
would NOT clash with `defenseProfile.readiness`, so **that reason does not survive**.
**RECOMMENDATION: keep the covered-set reading and keep the ruling's OUTCOME** — the defence pairs
stay detection-only in EM-R0b, on the §22.2 item 3 division rather than on a nesting bar — and re-cut
the addendum's parenthesis to say so. One sentence in the ODQ; no packet changes either way.

**Q-B — should this walker ASSERT `history.age === history.founding.age`, or only that the group is
LIVE?** The ruling divides the work — *"the `V-HISTORY-AGE` check DETECTS in this packet [EM-R0b],
and a `history` consistency GROUP PREVENTS the split [here]"* — so A6 proves the group resolves and
asserts no identity. The cost is named as CANNOT-CATCH row 12: if the generator ever stopped
duplicating the leaf, this walker stays green while the group quietly becomes pointless.
**RECOMMENDATION: no new arm.** It honours the ruling's own division, it avoids a second spelling of
a check EM-R0b already ships with its control and mutant, and it keeps the title delta at `+7` —
which matters while the lighting census is frozen. A7 stays the deliberate exception because a
cross-entry TOTAL is the *reason* a collection is ATOMIC, and that reason lives in this register.

**Q-C — the sequencing target moved and the chair executes the entry.** EM-B1d is LANDED; **EM-P2
(READY) now holds `scripts/mutation-coverage-manifest.json`** (E-39), so EM-R0a's manifest entry may
not be added until EM-P2 is terminal. **RECOMMENDATION: confirm the chair adds the entry after
EM-P2's landing flip, not before** — the family builds after train EM-T7 either way, so the wait
still costs nothing, but version 2's §4 named the wrong packet and a terminal should not discover it.

**Q-D — the `tests/lint` whole run: instrument or sealed `checks`?** The 2026-09-20 directory rule
requires the directory be run WHOLE once; the standard keeps a sealed `checks` array to ONE directory
per invocation, and `tests/lint` is 172 files with the lighting census red inside it.
**RECOMMENDATION: as written — the four named files stay the sealed `checks` array (they are the
arms that must be green for the seal), and the WHOLE run is the build lane's §10 instrument with the
census named in advance as its one lawful interior red.** Putting 172 files behind the seal would
make the seal's chain unreadable and would bake a known red into `check:packet`.
...
**Q4 — the pre-proof's owed scanner run: ✅ DISCHARGED, not a question any more.** Both registers
were RUN read-only at `141a1d775` (E-44, E-45): observed-shape `1964 finding(s), exactly matching the
frozen inventory`, exit 0; writer-reach `WRWALKER HOLD`, exit 0; the tree clean before and after. The
prediction is now CONFIRMED by the detector's own code — a finding needs a receiver that resolves to
exactly ONE known shape, and the leaf's only property access is `Object.freeze` — and by 7,301 live
occurrences of that construct under `src/` minting nothing. **No migration-bundle door, no chair act,
no STOP.** Recorded here rather than deleted so the chair can see the owed measurement was paid.
...
- Base SHA: `2dc08a595` (stamped by the chair at promotion).
- Dispatch bundle and seal identity: **NOT DISPATCHED** — status DRAFT; compiled at `58fcfe614`,
  re-cut at `ad7ddf2c9`, pre-proofed at `141a1d775` (READY-able there; the chair stamps).
- Final commit or working-tree state: **no edit made in any tree.** The lane wrote only under
  `$SP/lane-em-compile-EM-R0a-scratch/` and, at pre-proof, `$SP/lane-preproof-EM-R0a-scratch/`.
- Exact changed files and effective-line deltas: `NONE` — the emitted draft measures **164** effective
  lines against a `≤ 210` bound (163 at version 2, plus the seventh group's one declaration line).
- Acceptance cases: `0 of 8 executed`. Focused commands: **no vitest, eslint, `npm run check`, build
  or writing script was run by either lane.** The pre-proof ran only plain `node` probes and the two
  READ-ONLY registers (`check-observed-shape-readers.mjs`, `check-writer-reach.mjs`), both exit 0 with
  the tree clean before and after. Every measurement is in `EM-R0a.evidence.md` (E-1…E-48).
...
| ⛔ **A6's non-nesting test, as version 2 spelled it, REDS on this packet's own register** (`economicViability.metrics.foodBalance` inside `economicViability`) — a walker that would have failed at the build on data the same packet declares | **CLOSED — cured in this packet** (§5.3, §6's A6, §9): the test is re-cut to COVERED PATH SETS and the rule is written into the module's own comment so EM-R0c cannot re-derive it wrong. The ODQ sentence it invalidates is §11 Q-A, the chair's |
| ⛔ **`tests/lint/contractTestAntiVacuity.walker.test.js` governs `tests/lint/*.test.js` and was absent from version 2's `checks`**, though EM-B1k2 and EM-B3c both name it for the same reason | **CLOSED — cured in this packet** (§7, §10). ⚠ Its Rule 2 is a LIVE risk for six of this walker's seven exhaustive-claim titles; §6 construction rule 3 pins the cure (iterate IMPORTED sets, never a local literal) |
| ⚠ **Two dispatch line numbers in version 2 were off by one** (`:889`→`:890`, `:891`→`:892`) — a citation-rot instance inside a packet header, exactly the class FIX-C2 is chartered for | **CLOSED — corrected here** (§4, E-40). ⭐ Worth one line to FIX-C2's owner: packet headers cite `scripts/` by line and are OUTSIDE the citation walker's declared roots, so this class of rot is unguarded in `docs/implementation/**` |
| ⭐ **`SURFACE_ROOTS` holds SIX surface classes; version 2 (and any packet that copied its §5.4 table) says seven** | **CLOSED — corrected here** (§5.4, §7, E-45). The figure is decorative in this packet — zero importers means zero roots reach the leaf however many there are — but a wrong denominator propagates, and three sibling EM-R packets inherit this table |
| ⚠ **`history` carries three ORDER-BEARING collections** (`eventsTimeline`, `historicalEvents`, `currentTensions`) INSIDE the new group's root, which is why G8 must carry a member list | **SLOT: EM-R0c's compile**, beside the existing order-bearing row: G8 is now the first group whose root also contains order-bearing collections, so R0c's order arm and its group arm meet at `history` and the interaction wants one measured trial |
| **The Markdown-vs-JSON change-table validator arm is still ABSENT, re-verified at `141a1d775`** (`implementation-packets.mjs` reads the Markdown only through `parsePacketHeader`) | **CLOSED — already slotted:** TOOL-1 is composed at train EM-T4. This packet's §7/JSON equality was printed by this lane's own re-implementation of the stated contract (E-20) and will be re-printed by the arm when it lands |
...
- Judgment calls (PRE-PROOF lane, 2026-09-20, each vetoable, each with its measurement):
  7. ⭐ **Declared G8 with a TWO-MEMBER LIST rather than `members: null`**, because `history` carries
     three order-bearing collections a whole-object group would swallow (E-41). The ruling said
     "a group rooted at `history`"; it did not say whole, and whole would have been a defect.
  8. ⛔ **Re-cut A6's non-nesting test from ROOT PREFIX to COVERED PATH SETS rather than reporting
     the packet BLOCKED**, because the contradiction is inside the packet's own prose and design
     §22.3 item 2 already rules the substance (the member list exists to stop G3 swallowing G2).
     The chair's REASON in ODQ addendum 38 is affected and is raised as Q-A rather than rewritten
     by this lane — a lane does not edit the ODQ.
  9. **Discharged Q4 by driving the detector rather than by overlaying a file into a tree** (E-44):
     parsing the draft leaf with the scanner's own TypeScript and reading the finding condition at
     `legacy-reader-shape-scan.mjs:544-556` is exact and touches nothing, where writing the file
     into a worktree to re-run the scanner would have edited a tree this lane may not edit.
  10. **Added the `tests/lint` WHOLE run as a §10 instrument and NOT as a sealed `checks` array**
     (Q-D), because the standard keeps one directory per array and the directory carries a known
     interior red the seal should not swallow.
  11. **Kept the title delta at `+7` and the acceptance matrix at 8 cases.** The seventh group is a
     DATA row; adding an arm for it would have cost a title against a frozen census and duplicated
     EM-R0b's `V-HISTORY-AGE`. Recorded as CANNOT-CATCH row 12 instead, and put to the chair as Q-B.
