...
1. `action = CASCADE_WRITERS[`${coords.cardType}:${coords.field}`]`; absent → `{ ok:false, reason:'not_a_draft_field' }`
   (already closed; its gloss already covers a declaration the first door does not carry).
...
4. ⛔ **`await get()[action](index, value)`** — never an un-awaited call. The writer is **ASYNC BY
...
7. consult `REDERIVE_SEAM`; take the no-op branch (§6.4). Return `{ ok:true, saveId, keys, layer }`.
...
resolved off `get()` by name, exactly as `settlementPendingEdits.js` resolves it — which is why
...
CAPSULE AND NOT §7: NONE · SET-EQUAL: YES`. ⭐ **Version 2 moved no path**, so the nine rows are
