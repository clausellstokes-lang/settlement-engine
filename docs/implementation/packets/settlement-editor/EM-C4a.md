...
- **Depends on:** ⭐⭐ **VERSION 4.1 RE-MEASURES EVERY ROW AT `80e855f26`. TWO PROVIDERS HAVE LANDED, AND TWO ORDERING MEMBERS ARE ADDED.**
  ...
  **EM-B2a1 — LANDED** (`f9b68387a`, train EM-T10): `src/domain/edit/dmLayer.js`, read WHOLE (239 lines). ⭐ **CONFIRMED AGAINST LANDED CODE:** `applyEdit(layer, op, declarations)` is ARITY THREE (`dmLayer.js :: applyEdit`); its op shape is exactly `{ kind: 'set-root', key, cardType, field, value }`, with `cardType` and `field` declared TRANSIENT COMMAND COORDINATES that persist nothing (`:154-157`); `APPLY_EDIT_REASONS` is `['invalid_op', 'undeclared_field', 'unknown_target']` (`:72`) and the REFUSAL ORDER the leaf's own header fixes is `invalid_op` → `unknown_target` → `undeclared_field` (`:68-69`), exactly as §6.1 step 6 states it; an unusable consult FAILS CLOSED to `unknown_target` (`:186-187`); `layerRead` is own-property only and never throws (`:126-130`); `EMPTY_DM_LAYER` is deeply frozen (`:56-61`); and on `ok:true` the returned layer is a NEW deeply-frozen object whose `keys` holds exactly the keys the call wrote (`:209-215`).
