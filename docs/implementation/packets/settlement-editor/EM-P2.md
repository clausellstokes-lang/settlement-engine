...
- **Status:** READY
- **Packet version:** 2 — the chair's re-pin at promotion (2026-09-19): EM-P0 landed as ONE registration with an INTERNAL root/derive seam (its version 2), not the two-step split this packet was compiled against; and the lighting refreeze moved to the train's terminal (preamble §P2.1 as amended). §4, §5 row 3, §7, §8, §9 A4, §10, §11 and §12 corrected; the change manifest, the budget and the acceptance ids are unchanged.
- **Verified base:** `fixes-2026-09-18-consist` at `d86aabae65d99cd07d90526f383cd25eca75b905`
- **Last revalidated:** 2026-09-19 at `d86aabae65d99cd07d90526f383cd25eca75b905` by the chair at promotion — re-pinned from d31af2cee. Measured across the window by `git diff --stat d31af2cee d86aabae6`: both CREATE targets absent; the TEST target `tests/lint/entropyRootCensus.walker.test.js` unmoved; `scripts/mutation-coverage-manifest.json` moved by +4 (EM-P0's row for `tests/generators/pipelinePinnedMode.test.js`, beside which this packet's surgical REGISTER row sits); of the required-symbol paths, `src/generators/pipeline.js` (+50) and `src/generators/steps/generatePopulation.js` (174 lines touched) moved with EM-P0's landing at 429ceed54 and every §5 symbol still resolves at the tip (checked by exact substring at promotion). The lighting baseline at the tip is EM-P0's (`2646 / 383 / 2263 / 25005 / 6671`, measured at baf8ccc1d).
...
5. Live git state at `d31af2cee` (compile) and `d86aabae6` (the chair's re-pin, with EM-P0 version 2 in the tree at 429ceed54).
