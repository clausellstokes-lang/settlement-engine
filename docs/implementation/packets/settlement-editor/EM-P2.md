...
- **Status:** STALE
- **Packet version:** 2 (STALE 2026-09-19 15:0x: the build lane's STOP at f31ca0eb8, zero edits, refuted §0's measurement (b) — the three scanned spellings are a proper subset of generation's draw vocabulary: `pickRandom2` at 50 sites, four keyed forks on receivers other than `rng`, the ambient `rngContext` channel in 22 files — so A1's both-directions totality would have run green over a denominator it did not have; also: one row cannot carry four fork strings, stream mints have no truthful `outputKey`, `powerStructure.seats[].holder` exists nowhere, and the copied resolver mis-attributes `registerStep`-hosted choosers. RE-CUT as version 3 per design §21 / ODQ §934.47 addendum 14: a two-tier register measured BY EXECUTION, compiled after an Opus recon prototypes the census) — the chair's re-pin at promotion (2026-09-19): EM-P0 landed as ONE registration with an INTERNAL root/derive seam (its version 2), not the two-step split this packet was compiled against; and the lighting refreeze moved to the train's terminal (preamble §P2.1 as amended). §4, §5 row 3, §7, §8, §9 A4, §10, §11 and §12 corrected; the change manifest, the budget and the acceptance ids are unchanged.
- **Verified base:** `fixes-2026-09-18-consist` at `00fab686d31dd7bfaeb8363caa49921fb70c63e8`
- **Last revalidated:** 2026-09-19 at `00fab686d31dd7bfaeb8363caa49921fb70c63e8` by the successor chair (session 7d3418f8) — RE-PINNED from d86aabae6 after train EM-T2's terminal went red and was cured on the branch. Measured across the window by `git diff --stat d86aabae6 00fab686d` over this packet's paths: both CREATE targets absent; the TEST target `tests/lint/entropyRootCensus.walker.test.js` unmoved; `scripts/mutation-coverage-manifest.json` unmoved; of the required-symbol paths exactly ONE moved — `src/generators/pipeline.js`, one line (023eda2ec: the partial-pin refusal's em dash became a period; `const stepRng = rng.fork(name);` untouched) — which is why the re-pin was owed: the sealed dispatch refuses a verified-base descendant that changed declared substrate. All nine §5 symbols resolve at the tip (checked by exact substring at the re-pin). The window's other commits touch none of this packet's paths: 91d5f155b (the two bundle ceilings in `tests/build/`, ODQ §934.19 addendum 2) and 00fab686d (design §19, the charter, the survey). The lighting baseline at the tip is unchanged from promotion (`2646 / 383 / 2263 / 25005 / 6671`; `tests/lint/.lighting-census-baseline.json` unmoved since d86aabae6), so §10's predicted tuple stands. Previously: revalidated at `d86aabae65d99cd07d90526f383cd25eca75b905` by the chair at promotion, re-pinned from d31af2cee (both CREATE targets absent; the TEST target unmoved; the mutation-coverage manifest +4 for EM-P0's row; `pipeline.js` +50 and `generatePopulation.js` 174 lines with EM-P0's landing at 429ceed54, every §5 symbol resolving).
- **Depends on:** `EM-P0` — LANDED at 429ceed54. It defines the pin key spelling this registry registers against: a pin is keyed by THE RECORD PATH the chooser writes (`chooseOrPin(pins, key, draw)`, `generatePopulation.js:41`), and the population step's four keys are its `provides` (`npcs`, `relationships`, `factions`, `conflicts`). ⚠ As landed there is ONE registration and `drawPopulation` is a module-local function (the root half, `:126`), not a registered step — a row naming it resolves against SOURCE exactly as `pickFirst` does
...
- **Preamble:** docs/implementation/preambles/EM-PREAMBLE.md (SHA-256: 95e9a5f4aee51bb5aaa434883a708c5f2a4c71d2a49aa4ec99f9076a98c2afaa)
...
5. Live git state at `d31af2cee` (compile), `d86aabae6` (the chair's re-pin, with EM-P0 version 2 in the tree at 429ceed54) and `00fab686d` (the successor chair's re-pin after train EM-T2's terminal cures).
...
⚠ **EM-P0 is LANDED (429ceed54) and its shape is the one this packet registers against:** ONE
registered step `generatePopulation` (`provides: ['npcs', 'relationships', 'factions', 'conflicts']`),
the root half `drawPopulation(ctx, rng, pins)` and the derive half `derivePopulation(...)` both
module-local, pins consulted per chooser by `chooseOrPin` keyed on the RECORD PATH. Preflight
must find `chooseOrPin` in `generatePopulation.js`; a second `registerStep(` in that file is a STOP.
...
| Root-writing step 3 | `src/generators/steps/generatePopulation.js` | `registerStep('generatePopulation', {` | at 429ceed54 (EM-P0 v2): ONE registration, `provides: ['npcs', 'relationships', 'factions', 'conflicts']`; the ROOT half is the module-local `drawPopulation(ctx, rng, pins)` (`:126`; `rollNamedMass` then `generateNPCs`, both under `chooseOrPin(pins, 'npcs', …)`), the DERIVE half `derivePopulation` (`:165`; relationships → factions → conflicts on THE SAME stream). The derive half holds one draw outside the three idioms — the power-weighted scatter `rng.random()` at `:101` in `linkFactions` | Rows for the ROOT keys only (`npcs…`); `relationships` and `conflicts` are derived without a chooser of their own and get NO row; whether the scatter earns a `checklist` row keyed `factions[].powerFactionName` (`cardShape: 'faction'`) is the lane's MEASURED call, recorded in §12 |
...
**Generated artifacts:** `NONE`. ⛔ **The lighting baseline is NOT regenerated by this packet.** The
refreeze door demands a commit, a commit drifts the sealed HEAD, and the sealed verbs then refuse —
measured at EM-P0's build (preamble §P2.1 as amended, 2026-09-19). The chair re-derives the census
ONCE at train EM-T3's terminal. The lane MEASURES the walker's red and records the tuple in §12:

```sh
# expected RED by design — the interior red; read the tuple it prints, do not refreeze
npx vitest run tests/lint/sovereigntyLightingContract.walker.test.js
```

**Predicted INTERIOR RED:** `2646 / 383 / 2263 / 25005 / 6671` (EM-P0's baseline at the tip) →
**`2647 / 383 / 2264 / … / …`** — `+1 file, +0 parked, +1 credited`, caused by the ONE new TEST file. ⚠ The new `src/domain/**` leaf
...
6. Add the mutation-coverage row; re-record the entropy census by execution; MEASURE the lighting
   walker's interior red (never refreeze — the terminal's act); re-run the two fork walkers plainly.
...
| A4 | ⛔ Boundary — EVERY `outputKey` RESOLVES ON A REAL RECORD | each row's `outputKey` walked against a generated settlement | every path resolves (`npcs[].role`, `institutions[].category`, `powerStructure.seats[].holder`, …) and every one sits under a ROOT half's output — none names a product derived WITHOUT a chooser of its own (`relationships`, `conflicts`, `economicState`, `history`); `factions[].powerFactionName` appears only if §5 row 3's scatter was measured as a chooser. A typo'd path reds here rather than silently costing EM-A1 a field | same |
...
GATE_MUTEX_TIER=shared GATE_MUTEX_MAX_POLLS=100000 GATE_MUTEX_POLL_SECONDS=20 \
  ...
  tests/lint/entropyRootCensus.walker.test.js tests/lint/mutationCoverageManifest.test.js
# the lighting walker is run SEPARATELY and is expected RED by design (§7): measure, never refreeze
...
- `chooseOrPin` is absent from `generatePopulation.js` (EM-P0 not in the tree), or that file holds a
  second `registerStep(` — the registry's rows assume ONE registration with an internal seam;
...
- Row count of the new registry, and the scanned-set size it equals; the scatter's disposition (row or not, and why):
- Lighting census tuple before, and the walker's MEASURED interior red after (not refrozen):
- Generated artifacts: `NONE` (the lighting refreeze is the terminal's act)
