...
- **Verified base:** `em-t10-b1a-2026-09-21` at `e348d59b609e2c62c957a4cd78653849792043b7`
...
|---|---:|---:|---:|
| op rows (ten declared fields each, tightly written) | 8–9 | **14** | **112–126** |
...
|---|---|---|---:|---|
| `CREATE` | `src/domain/edit/operations.js` | `OP_STAGES`, `OP_CONSEQUENCE_POLICIES`, `OP_TYPES`, `makeOp`, `validateOp` | **250 eff (cap); ≈213–227 estimated** | The vocabulary of §6 with exactly **THE FOURTEEN HOME ROWS of §6's one list** (R8), every row carrying all ten fields. Both closed vocabularies carry BOTH members. Import no PRNG, no `src/store/**`, no `src/components/**`. Author `consequenceFor` nowhere; call `rederive` nowhere. Leave the structure open for B1b to append. |
...
| **A8** | **The `ok`/`errors` invariant, and `set-world-fact` as a DECLARATION** | For every case in a table of valid and invalid ops, `result.ok === (result.errors.length === 0)`. `errors` is FROZEN and `compareCodepoint`-sorted (a three-error case asserted in exact sorted order). ⭐ `set-world-fact` validates `fact` membership against the **FIVE** world-fact keys EM-A1 version 5 declares (`terrain, culture, monsterThreat, resources, stressors`) and the PRESENCE of `value`, and **`goods`, `services` and `tradeAccess` are asserted ABSENT by name** so a later author cannot restore a struck key without a red (the chair's BLOCK-7 ruling, judgment 33); it is asserted **not to resolve any pool and not to re-derive anything** — proved by a source scan finding no `rederive` and no pool import, so the §14 engine stays EM-B2's. |
...
| `npcPresent` | **`NpcStatus === 'active'`** — `entities/npcs.js:30`; EM-B1d's union | ✅ **BOTH ALREADY HELD, and by the cleanest route.** SUBJECT: the people are **the record's own** — `record` IS the subject. STAGE: **`status === 'active'` IS the live stage**, the union's own word | ✅ **LIVE** |
