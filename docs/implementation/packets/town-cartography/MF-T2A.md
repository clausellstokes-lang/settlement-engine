...
- **Family preamble:** `docs/implementation/preambles/MF-PREAMBLE.md`, SHA-256
  `7a39a28ee7a54799489601a9b7ea9333b33ce83158c7e8c4aec878c89a204de8`. Its §P2 hazard dispositions,
...
- `mutation-coverage-manifest.json` diff statistics:
- Census before → after, and `parked`:
- Both typecheck configurations:
- Wave-end gate steps actually executed (`npm run check` is a 17-step `&&` chain; a red step blacks
  out every later step — say which steps ran):
