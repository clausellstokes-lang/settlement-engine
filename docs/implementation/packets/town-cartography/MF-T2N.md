...
| # | question | finding |
|---|---|---|
| (a) | a seed-stable identity input exists on the settlement? | **YES — `settlement.id`**, `s_<16hex>` from `idFromSeed(_seed)` (`normalizeSettlement.js:191`, `_seed` attached at `assembleSettlement.js :: settlement._seed = ctx._seed`), identical on same-seed regeneration (`true`); `name` also present. 210/210 real settlements (5 seeds × 6 tiers × 7 terrains), zero missing. No STOP |
