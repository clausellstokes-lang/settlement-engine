...
| 11 | the certification precedent | `src/domain/certification/subsystemRowsVirtual.js:54` `VIRTUAL_SUBSYSTEM_ROWS`; the `underwaysOrganicFoundingEnabled` row at `:988-1036` and — the closer precedent for THIS member — the GR-5A **MONOTONE MEMORY** row `treatyRenewalEnabled` at `:1041+` | CONFIRMED |
