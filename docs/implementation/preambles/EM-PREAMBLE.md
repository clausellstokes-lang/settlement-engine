...
| `docs/DESIGN_EDIT_MODE_AND_DECREES.md` | the product design; **§12 GOVERNS** every earlier section and **§13 states the phantom consequence rule** (ODQ §934.43): the real rewind (`undoLastPulse`, session-only, ten deep, later-staged decrees re-appended); canon is a save's state and the advance is a campaign act; names are join keys (`free-cascade`); neither persisted key travels; the layer is applied on write; phantoms are minimal hidden save records; pools read the generator's catalogue; guards fold and never refuse; the observed-shape door is the exemptions bundle; the gate is `TIER_GATE.premium.editMode`; an applied decree reopens read-only |
