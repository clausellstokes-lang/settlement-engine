...
| P123 | A-2 — Mobile chrome reconciliation | **retired 2026-09-06** | Shipped flag-off, then the flag-on path was deleted at `8bf493d05` (fix(nav), 2026-06-22) along with the dead mobile-nav branch: the bottom nav became a hard-coded `.slice(0, 5)`, the mobile top header renders unconditionally, and the 6th auth slot no longer exists. With no reader left, the `mobileSingleChrome` registry entry was retired at §904 as bookkeeping. Rebuilding the single-chrome mobile nav (top header suppressed, auth chip as a 6th slot, ~52px reclaimed) is an owner row, not a flag flip. |
