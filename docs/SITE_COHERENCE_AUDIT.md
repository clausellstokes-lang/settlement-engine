...
⛔ **SUPERSEDED at `2a7fb033` / `2fe94f77` — do not re-chase (re-derived at `b5442c07`, 2026-08-12).** The row is gone: `townLayoutV2.js`'s whole frozen entry is `{"biome on config": 2}`, and `"exports on economicState"` left the baseline at the schema-4 genesis. A revert no longer stays green either — `compare()` reads an absent row as `ceiling 0`, so a restored read arrives as growth and the walker asserts `{violations: 0, stale: 0}`. Everything below is a schema-2 measurement.
...
⛔ **SUPERSEDED at `2a7fb033` / `2fe94f77` — do not re-chase (chair decision CD-6, 2026-08-12).** The finding's core — *do not go hunting four stale rows* — is discharged along with Wave 1. Its wider claim (31 `id on factions` reads across 15 files are live pre-existing debt) was measured against the schema-2 inventory of 551 files / 2,168 identities; at schema 6 the inventory is 381 files / 1,381 identities and four post-filters narrow the detector's output. **That claim is neither confirmed nor refuted here, and must not be assumed to have survived** — it is an open question for whoever next touches the OSR programme, out of the site-coherence program's scope in either direction.
...
⛔ **SUPERSEDED at `2a7fb033` / `2fe94f77` — do not re-chase (re-derived at `b5442c07`, 2026-08-12).** `--write` is no longer an amnesty: growth and identity swaps throw (*"maintenance is shrink-only"*), a detector-digest change throws, a schema mismatch throws unless a reviewed migration bundle is passed, and the whole path requires a clean input snapshot over `src`, the baseline and all eleven governed scanner paths. It is now the **only lawful shrink path**, re-deriving `inventory`, `total` and `identities` from one scan together. The single residual truth is that no `--bank` helper exists — and that rider was declined (plan owner-queue #4).
...
⭐ **STILL LIVE, and SHARPER than recorded — re-derived at `b5442c07`, 2026-08-12.** The consequence is no longer a walker static-test red: a row-only edit now hits a **hard throw in `validateLeafBaseline`** (`scripts/lib/observed-shape-baseline.mjs`, *"observed-shape baseline totals are inconsistent"*), so every consumer of the baseline dies and the CLI cannot reach the scan to explain itself. The refusal itself is already pinned in `tests/lint/observedShapeBaseline.test.js`, so **no new pin is owed — the gap is purely the instruction text.** ✅ **The gate-facing half is CURED (2026-08-12)**: the SHRINK-ONLY assertion message in `tests/lint/observedShapeReaders.walker.test.js` now states the whole legal act — the governed `--write` re-freeze on a clean tree, never a hand-edit of individual rows. ⏸ The CLI half (`ratchetMessage`, the two STALE-ROW strings, the generated `_doc`) is **PARKED as `SCW-1b`**: all four sit in a governed scanner path, so they ride the next schema mint rather than minting one for prose.
...
⛔ **SUPERSEDED at `2a7fb033` / `2fe94f77` — dead, do not re-chase (re-derived at `b5442c07`, 2026-08-12).** `OSR_FREEZE_SHA` occurs **zero** times anywhere in `scripts/` or `tests/`. The freeze sha is derived from the subject sha of the authoritative snapshot, and the write path additionally refuses a dirty tree. Documenting this variable would document something that does not exist.
