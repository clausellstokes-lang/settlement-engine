...
- Canonical builder: `.../minifold/src/design/organic/logo.js` — `deviceMarkup`. `tests/design/organicLogo.test.js` pins the geometry and the golden SVG set.

**⚠ AMENDMENT 2026-09-19 (ODQ §934.17) — THE SHIPPED MARK IS NO LONGER THIS DRAWING.** The owner ordered the brand derivatives rebuilt from his own arrow-header painting, so every shipped icon and share card is now a CROP of `public/brand/arrow/arrow-strip.webp` — the brass plaque bearing the wordmark with a crimson wax seal struck in place of its `o` — cut by `scripts/derive-brand-marks.mjs`. The drawn device of 2026-07-18 survives only as the dossier's vector mark (`src/pdf/primitives/HouseDeviceSeal.jsx`) and as documentation goldens; `HouseDevice.jsx` now renders `public/brand/seal.png`.

**This changes the mark's provenance and is stated here for counsel, not resolved here.** That painting is recorded in `scripts/ai-media-provenance.json` as `openai:gpt-image@2026-09-16` — AI-generated output the owner supplied, whose signed C2PA manifest the cut removed and which now carries an estate-written XMP credit instead. So the artifacts a trademark application would depict descend from generative-AI output rather than from an estate-authored drawing. The register's `_indemnity_standing_risk` and `_restorable` blocks already state what the estate can and cannot assert about that origin; nothing in this repository makes, or may make, a claim about registrability, authorship or ownership of an AI-derived mark.
- Raster/vector artifacts (current): `.../minifold/public/brand/seal.png`, `brand/plaque.png`, `favicon.ico`, `favicon-32.png`, `favicon-192.png`, `favicon-512.png`, `apple-touch-icon.png`, `og-default.png`, `og-craft.png`. **Deleted 2026-09-19:** `favicon.svg`, `favicon-dark.png`, `og-default.svg`.

**The fixed rule (appears in three places):** `HouseDevice.jsx:8` — "The mark **NEVER** carries text — the name is always adjacent type." `logo.js:6` — "NO text, NO legend, NO motto, NO monogram, NO pellets inside the mark" (with `logo.js:99` on the motto as adjacent caption, "never inside the mark"). `marketing/copy/brand-kit.md:56` — "**no text inside the mark, ever**." The 2026-09-19 re-cut does not breach it: the painted SEAL carries no text, and the plaque is the LOCKUP register (§8 below), where the wordmark sits beside the seal rather than inside it.
