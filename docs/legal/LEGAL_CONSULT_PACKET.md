...
**The fixed rule (appears in three places):** `HouseDevice.jsx:8` — "The mark **NEVER** carries text — the name is always adjacent type." `logo.js:130` — "the mark never carries text — the name is adjacent type." `marketing/copy/brand-kit.md:56` — "**no text inside the mark, ever**." The 2026-09-19 re-cut does not breach it: the painted SEAL carries no text, and the plaque is the LOCKUP register (§8 below), where the wordmark sits beside the seal rather than inside it.
