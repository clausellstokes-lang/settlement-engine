...
- `surveyor_adoption` — **awaiting-metric**: the VISION-WAVE adoption signal (ONE id-free `{ surface, verdict }` event shared by the Interview / Oracle / Corpus Factory surfaces). It carries the accept/decline/revise verdicts the S4+ tuning-knobs decision reads; formalize into a per-surface adoption/acceptance metric when those surfaces launch. INTEREST + acceptance data only, no id.
