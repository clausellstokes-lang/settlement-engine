...
**How to apply (SettlementForge).** Everywhere a band appears, keep the colour + glyph + TEXT LABEL triple — but the label is a WORD, not a shout. This paragraph used to name `primitives/BandPill.jsx` as exemplary and mandate its uppercase label; that component had zero production importers and was deleted on 2026-09-18, and the uppercase pattern was itself the defect the label ladder removed. The rule above is unchanged and is in fact WHY the capitals could go: colour and weight are already carrying the state, so case was a fourth channel doing no work and flattening the page's hierarchy by making a datum shout as loudly as its section. The ladder (`src/components/new/labelLadder.js`, `src/domain/display/labelCase.js`) is the current pattern — a section eyebrow keeps its capitals, a field name and a status value are sentence case through `tokenCase` / `statusCase`, and an initialism like NPC keeps its letters. Pair 'at war' with a crossed-swords icon and text, 'famine risk' with an icon, negative meters with a down arrow plus colour. Tier badges (Wanderer/Cartographer/Founder) and credits (violet) carry labels/icons, not hue alone. Verify emphasized text meets 4.5:1 on parchment; keep muted-500 out of body copy (chrome-only, as the tokens already mandate).
...
5. Token & escape-hatch burn-down: Are the deferred bypasses — the ~140 swatch hex keys, inline rgba/gradient literals, and any raw px sizes — migrated onto semantic tokens so consistency (and a future dark theme) is real rather than aspirational?
...
18. Review meta: For each finding, is the violated heuristic/principle named, the exact location cited, a severity assigned, and a concrete fix proposed — and are existing strengths (token system, EngineSections self-gating, multi-channel band pills (P7), PipelineReveal pacing, anti-vacuity gating, lazy first-paint) documented and preserved, not just problems?
