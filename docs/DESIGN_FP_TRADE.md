...
| `corneringEnabled` | corner positions, the price-band corner term, moral pricing | TR-6 |
...
**THE FLAG-DEPENDENCY RULING:** independent dark switches, ORDERED lighting (the
WR precedent verbatim). Lit-preconditions: TR-2 ⇒ TR-1; TR-3 ⇒ SP-2 landed + TR-1;
TR-4 ⇒ `commodityFlowEnabled` + `demographicsEnabled` (grain reads harvest/
K_food/storageMonths — wave-P machinery); TR-5 ⇒ SP-3 landed + TR-1 (+ TR-4 for
...
degraded read); TR-6 ⇒ TR-2 + TR-3 + TR-4; TR-7 ⇒ TR-2; TR-8 ⇒ SP-1 landed +
TR-5. A flag lit out of order is an invalid config the TR-9 certification walker
reds.

**SEAM RULING ONE — THE PACT SEAM (J-WR-1's commercial sibling, binding):** the
pact program does not fork the peace engine — it extends its CATALOG and replaces
its TRANSPORT, never its MATH. `peaceTerms.js` remains the single writer for
terms/appraisal/compliance; the commercial term family (TR-5) joins TERM_CATALOG
exactly as sovereignty_transfer does in WR-10; formation runs SP-3's two-sided
...
**SEAM RULING TWO — THE GRANARY SEAM (TR-4's architecture):** when
`foodCaravansEnabled` is LIT, the standing FOOD_IMPORT_RATES import term is
REPLACED by the physical arrivals summary — grain units delivered by caravan,
converted to storage-months by ONE authored constant at the single granary
applicator, which remains the only writer of food-months (T3). Dark ⇒
byte-identical rate arithmetic. The T7 walker asserts exactly-one-import-term in
both states. Treaty grain streams (tribute, grain_provision) ride the same seam in
slice 2: dispatched as caravans when lit, conserved arithmetic when dark.

**SEAM RULING THREE — THE TWO SCARCITIES (T1/T8 made structural):** truth-side
...
acts, pact triggers, corner motives). No module reads both (import-pinned per
side); no code path averages them; truth reaches belief only through arrivals,
...
- **Force/counterforce:** every severance reason's mirror scores off the SAME
  evidence (amendment-2 law) — the dependency that frightens is the dependency
  that binds; the toll that gouges is the toll that, relieved, warms. No entry is
  a ratchet: magnitudes decay as the producing state decays (§1b-B suppression:
...
- **Pins (negative hardest):** a healthy partnership mints no severance entry
  ...
  without exemptions; the §1b-B suppression receipt (a famine_profiteering entry
...
- **Herald contract:** severance/partnership band-crossings are Herald-worthy with
  full address chains; registered with the pacing governor (significance scales
  with magnitude band).
...
- **Pins (negative hardest):** the no-hidden-governor import pin on houseLedger.js
  ...
  npcs[]); writer/reader spelling pin boots the real writer; dormancy golden;
...
- **Bands:** holdings/credit band ladders, act thresholds per verb, appetite learn
  rates (SP-4a instance), credibility moves (SP-5 instance), the house cap.
...
- **Couplings:** TRADE×INTERIOR (house vs seat — the house's credit to a strained
  ...
  TRADE×FAITH (tithe: a band of house fortune flows to the temple — FAITH owns
  the tithe read).
...
- **Pins (negative hardest):** dark ⇒ the omniscient dispatch path byte-identical
  ...
  scarcity derivation); the no-merge pin (no code path averages truth and belief
  bands — token scan for both imports in one module); staleness reachable (a
...
- **Slice 1 — grain rides the pipeline.** Grain becomes an M6a good end-to-end:
  producer surpluses derive from the SAME harvest/foodLedger truth the rate
  arithmetic reads today (one source, two denominations — the derivation is the
  pin); integer grain units on caravans (MAX_SHIP discipline), warehouse ceilings,
  taps, banditry, seizure-conserves-to-seizer — all existing physics, grain now in
  the balance. Delivery lands through THE GRANARY SEAM (§3 Seam Two): units →
  storage-months via ONE authored constant at the single granary applicator; the
  standing FOOD_IMPORT_RATES term is REPLACED when lit (T7 walker asserts
  exactly-one). Interdiction becomes physical: a cut road starves by absence of
  wagons, not by a blockadePct approximation (the approximation term suppresses
  when lit — same walker).
...
- **Pins (negative hardest):** dormancy golden (dark ⇒ rate arithmetic
  ...
  exactly-one-import-term walker green in BOTH flag states; conservation: the
  ...
  months regardless of any believed band — negative pin); the robbery-claim
  round-trip (slice 2): robbed tribute mints the claim, compliance does NOT mark
...
- **Lifecycle line:** NO new ledger — grain rides commodityStocks/supplyShipments'
  existing serialize/regen/undo pins; the suppressed-term flag state is derived,
  never stored.
...
- **Scope:** THE COMMERCIAL TERM FAMILY joins TERM_CATALOG (Seam One; peaceTerms
  stays the single writer): `market_access` (opens NO_TRADE/exclusion — the
  ...
  at last), and posture (SP-4). Proposal → two-sided drafting via
...
- **Couplings:** TRADE×GRAMMAR (this wave IS SP-3's trade consumer — formation
  triggers and the term family are declared there, elaborated here) ·
...
- **THE COUNTERFORCES (each named, each scoring off the corner's OWN evidence,
  ...
  3. **The commons:** famine_profiteering at magnitude feeds the popular ladder
     (FP-POPULATIONS' commons voice — coupling, consumed here): petition →
     gathering → the granary riot that SEIZES stock (units conserved
     house→granary via the existing seizure physics); the factor flees to the
  ...
  4. **The seat:** the seat's books bleed legitimacy while bread is dear
     ...
     SALE edict (stock moves to the granary at a band price; the house mints a
     grievance against the seat — tyranny costs too) or the EMERGENCY PURCHASE
...
- **Pins (negative hardest):** NO PHANTOM CORNER (held share below band cannot
  ...
  out of glut; reachability of the FALSE branch proven); §1b-B suppression
  ...
  the riot seizes it, the seat forces the sale, carry-cost ruins it) — a corner
...
- **Bands:** corner share band, corner term magnitude (one band, capped — never
  ...
  consequence bands, forced-sale price band, riot seizure share.
...
- **Couplings:** TRADE×POPULATIONS (the commons ladder + famine migration — the
  ...
  walked in COUPLINGS).
- **Endings wired:** `cornered` (held through the season — rare), `ruin` (the
...
- **Pins (negative hardest):** stake genuinely at risk (the failed venture's
  ...
  casus (both arms); outcome-learned appetite reversal executed; DM-KILL of the
...
- **Pins (negative hardest):** the speed-floor walker extends to commercial
  ...
  pin (snapshot band ≠ gate band on arrival, receipted — the wrong-market tragedy
...
- **The tradeConvergence contract, contract-first** (the warConvergenceContract.js
  ...
  dear-band evidence); collapse = a market/entrepôt estate dying (centrality +
  route-decay evidence); severance = the formal cut with casus; cornered = TR-6's
...
- **J-TR-4 (the granary seam semantics):** lit ⇒ REPLACEMENT of the standing
  import term with physical arrivals, calm-equivalence pinned; never both terms
  live. VETO runs rate + physics additively (and accepts the double-count
  consciously — recorded as rejected here).
...
- **J-TR-8 (grain in the balance):** grain extends assertGoodsConservation's
  ...
  convert only at the one applicator. VETO a parallel grain-conservation
  assertion.
...
ladders + act thresholds + house cap + SP-4a/SP-5 instance rates · TR-3 belief
...
surplus-export bands + calm-equivalence tolerance + lean-year floor · TR-5
crossing thresholds + term budgets/durations + exclusivity strain + wardenship
curve · TR-6 corner share band + corner term cap + carry strain + profiteering
