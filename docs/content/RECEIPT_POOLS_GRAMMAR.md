...
⛔⛔ **CORRECTED 2026-08-14 (CR-GR4B-12, after GR-4b-iii-a's terminal landing) —
THE DOSSIER POOL REPEATED THE SUCCESSOR / GROUPED-INSTRUMENT DEFECT AND ADDED FACTS
NO QUESTION OR PARCHMENT CARRIES.** Each durable row names one exact question: its
treaty key, acting court, other court, fallen-holder id, opened tick, and typed
terminal map. The selected current treaty names its two parties, its written live
terms, and the fallen holder on the acting court's persisted `sworn` stamp. Neither
surface names a successor, a grouped oath count, envoys, prepared letters, a
town-wide wait, a market position, or universal term compliance. Eligibility admits
any stamped live-term treaty, not only a peace. Therefore `{npc}` below binds only to
the **FALLEN HOLDER**, never a successor; `{band}` leaves the block; and all eight
variants are re-authored to say only what a validated pending row and its current
treaty entail. Two distinct pending questions may lawfully point
at one treaty, so each validated row renders one line and no family claims per-treaty
uniqueness. All eight families, their order, `public` audience and `n/a` dossier
significance remain. Ruled at **A-22** under R-28, A-11, A-15, A-18, A-20 and A-21.

### succession_question_open (GR-4) — dossier: WarFaithTab / TreatyPanel — significance: n/a (dossier line)
SLOTS: {settlement} {counterpart} {npc}
AUDIENCE: public
1. The new seat has not yet said whether the old oath holds. `[exemplar, R-28]`
2. The oath {npc} swore for {settlement} remains before the new seat, with no answer entered.
3. From {counterpart}, the treaty still reads as standing while {settlement}'s answer remains pending.
4. The parchment still joins {settlement} to {counterpart}; the pending question has not broken it.
5. One docket entry holds the whole choice: honor the old oath or disavow it.
6. Until {settlement} answers, the treaty with {counterpart} remains live under the terms already written.
7. Since {npc} left the seat, {settlement}'s treaty question has remained open and the treaty itself in force.
8. If the seat gives no answer, the oath stands.
...
| A-25 | GR-6 wires `brokered_back` (the war that did not happen) as authored, and three of its four families assert more than the mediation receipt carries: family 1 promises no war "this spring", family 3 has both courts standing their musters down "in the same week" and binds the two `{counterpart}` fills in order, and family 2 names a person where no legate or priest is cast (2026-09-24, GR-6) | **The authored block is wired as authored and made honest by declaration, never by rewording, under R-28.** No variant is reworded, renumbered or dropped and the `SLOTS:` line is untouched. Family 1 declares the context `spring` and is drawn only when the calendar's season at the receipt's tick is spring. Family 3 binds its first `{counterpart}` to the would-be besieger and its second to the court it meant to march on (the slot convention's in-order law, the second fill carried as `counterpartSecond`), and declares the context `musters_stood_down`, which the producer does not supply at this wave because it reads neither court's muster; the family stays authored and reachable, and draws the day a producer reads that fact. Family 2 needs `{npc}` and is ineligible while the broker is a court and no person is cast, the block's own institution-voice fallback. Family 4 is slotless and context-free, so every receipt has an honest sentence. The kind files the treaty cohort's trade desk and the letter's courts section, as the GR-2b pair does. The other seven `# GR-6` blocks stay authored and unwired: GR-6 mints one kind. Vetoable. |
| A-26 | Cure lane LIT1b-pre (unit U4) wires `signed`, the `# GR-2` formation ending "Herald (the signing beat)", as the pact stage's one news kind, and three of its five families assert more than a signature always carries: family 4 has BOTH courts take something away (false for a one-sided grant), family 5 has wagons move (false where no clause moves goods), and families 1 and 2 name the goods exchanged, which no drafted pact clause records (2026-09-24, LIT1b-pre) | **The authored block is wired as authored and made honest by declaration, never by rewording, under R-28 and A-25's precedent.** No variant is reworded, renumbered or dropped and the `SLOTS:` line is untouched. Family 4 declares the contexts `both_gain` and `both_gain_goods` and draws only when every court takes something away (a clause both hold, or clauses owed to each); family 5 declares `goods` and `both_gain_goods` and draws only when a signed clause is a catalogue stream; the producer computes the one token from the clauses the signature actually wrote. Families 1 and 2 bind their repeated `{good}` in order (the second carried as `goodSecond`, A-25's `counterpartSecond` idiom) and stay ineligible while no pact clause records its goods, under NO FABRICATED names; family 3 is context-free. `{settlement}` is the court that asked and `{counterpart}` the court that answered, bound by the producer from the proposal row, and the beat is minted only when both courts resolve to a real name. Minted and amended signatures both speak through this block, per the coverage ledger's LINEAGE ACTS note. `nap_signed` stays authored and unwired: the standalone non-aggression pact speaks through `signed`'s `both_gain` family until its own wiring lands. The kind files the treaty cohort's trade desk and the letter's courts section, as the GR-2b pair does. Vetoable. |
