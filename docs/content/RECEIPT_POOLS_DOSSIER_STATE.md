...
1. `[ledger]` {npc} sits at the top of {faction} at {settlement} with a clear gap below, and the gap has held long enough that the faction plans as though it will keep holding.
2. `[street]` Inside {faction} at {settlement} there is one person to see and everybody knows who; the rungs below are for people who are waiting rather than climbing.
3. `[visitor]` A stranger dealing with {faction} at {settlement} is passed upward exactly once and then is dealing with {npc}, which is as simple as these arrangements get.
...
2. `[street]` {faction} at {settlement} has a first among several rather than a head, and the town has learned to ask which of them it is dealing with today.
3. `[unfolding]` Nothing at the top of {faction} has changed at {settlement} and the standings sit so close together that nothing needs to change for it to.

**high instability (churn at the top)**
1. `[ledger]` The top of {faction} at {settlement} has changed hands more than once lately, and the faction is weaker for it. A house that keeps re-learning who it answers to gets less done than its weight suggests.
...
3. `[visitor]` A stranger who dealt with {faction} at {settlement} within the year finds the arrangement void and the person gone from the rung, and has to begin again.
4. `[unfolding]` Turnover at the head of {faction} at {settlement} is eating the faction's real reach; each new arrangement at the top costs it another season of whatever the last one was building.

**low instability, long-held order**
1. `[ledger]` The order inside {faction} at {settlement} is settled rather than contested; the standings sit far enough apart that the rungs read as places rather than as positions being fought for.
2. `[elder]` Nobody has moved on the top rung of {faction} at {settlement} in a long while, and the faction's weight in the hall reflects the steadiness.
...
1. `[ledger]` {faction} at {settlement} has a head and very little structure beneath. The ladder is short because the faction is, not because the climbing has finished.
2. `[street]` {faction} at {settlement} is {npc} and a handful of others, and the town treats the handful as {npc}'s rather than as anybody's rivals.
...
1. `[ledger]` Nobody at {settlement} is positioned to take the {seat}. What {faction} holds it holds without a rival on the rolls, and the succession question is not a live one here.
2. `[street]` The question of who might replace the {seat} at {settlement} does not come up, because there is nobody the town would name if it did.
3. `[counterforce]` There is discontent at {settlement} and no contender to carry it; the {seat} is safe for the plainest reason a seat can be safe.
...
2. `[street]` {settlement} has people who would take the hall if they could, and they cannot, and everybody involved understands the arrangement precisely.
...
2. `[street]` {settlement} is no longer certain the hall belongs to {faction}, and neither, from the way it governs, is {faction}.
3. `[unfolding]` The advantage at {settlement} has passed to the challenger and the {seat} has not yet passed with it; what happens next is a matter of nerve rather than arithmetic.
4. `[visitor]` A stranger at {settlement} is quietly advised to be civil to two houses rather than one, and is not told why in so many words.
...
1. `[street]` The town is behind whoever sits at {settlement}, and a ruler with the town behind them can do things a ruler without it cannot attempt.
...
3. `[counterforce]` There are people at {settlement} who would move on the {seat} and do not, because moving on a ruler the town is behind costs more than the seat is worth.

**`legitimacyHold`: public opinion neither helps nor hurts**
1. `[ledger]` Public opinion at {settlement} is doing nothing to the {seat} either way, which leaves the hold resting entirely on what {faction} can muster for itself.
2. `[street]` {settlement} neither backs the {seat} nor moves against it, and the {seat} governs on its own strength alone.
3. `[threshold]` The {seat} at {settlement} is carrying its own weight and no more; a shift in the town's regard in either direction would be felt immediately, because there is nothing else cushioning the grip.
...
2. `[unfolding]` Public rejection is breaking the {seat}'s grip at {settlement} faster than {faction} can shore it up, and every measure taken to shore it up costs a little more of what remains.
...
1. `[elder]` {settlement} has changed governments before, and the last change is still legible in how the hall is arranged. The present {seat} was built to not fall the way that one did.
...
3. `[street]` The town remembers the government before this one and remembers what ended it, and refers to both in the same breath whenever the present {seat} overreaches.
...
3. `[visitor]` A stranger asking at {settlement} what came before the present arrangement is told there was nothing worth the name, and is told it without embarrassment.
...
1. `[ledger]` {settlement}'s army is in the field against {counterpart}. Everything the town does at home is done without those people.
2. `[street]` The town is short of the people who would ordinarily be here. The harvest and the gate feel it first.
3. `[unfolding]` What {settlement} wins from this campaign it is going to win somewhere else. What it spends, it spends here, in the open and without a break.
4. `[visitor]` A stranger finds {settlement} quieter than a town this size should be. He finds the reason is arithmetic rather than mood.

**`statusLabel: Occupied`**
1. `[street]` {settlement} is held by {counterpart}. The hall still sits and the market still opens. Neither of them decides anything {counterpart} has an opinion about.
2. `[ledger]` Whatever is decided at {settlement} is decided subject to the occupier. The town's own bodies run, and they run inside a line somebody else drew.
3. `[visitor]` A stranger at {settlement} deals with the town's own people. He notices eventually whose soldiers stand where the town's would.

**`statusLabel: At war` (no front at the walls, no army abroad)**
1. `[ledger]` {settlement} is at war with {counterpart}, and nothing has come of it here yet. No army has marched and none has arrived. The town is arranged for both.
2. `[street]` The town is at war in the way a town can be at war without seeing any of it. What it sees is prices, absences, and a great deal of talk about what might happen.
3. `[counterforce]` {settlement} is at war with {counterpart} and has not been touched by it. The distance between the two towns does the work walls would otherwise have to do.

**`warExhaustion: rested`**
1. `[ledger]` {settlement} carries no scar from fighting. Whatever the town has done, it has recovered from. What it could put in the field it could put there tomorrow.
2. `[street]` The town has its people and its temper both. Neither has been spent on anything lately.
3. `[counterforce]` {settlement} could put its full strength in the field this season and is not being asked to. Nothing has been spent, and somebody chose that.
...
2. `[unfolding]` The town is close to being done with the war it fought. The last of it is going out of the place slowly.
3. `[threshold]` {settlement} is at the edge of being clear of its war. Another quiet season closes it. One hard season would put the town back where it was.
4. `[street]` The town talks about the fighting in the past tense now, with the occasional slip.

**`warExhaustion: war-weary`**
1. `[street]` {settlement} has had enough. The fighting has gone on past what the town believed it agreed to. The agreement is being withdrawn quietly, in a hundred small refusals.
2. `[ledger]` Sustained fighting has worn {settlement} down to the point where the hall has an appetite for the war and the town does not. The pressure to end it now comes from inside.
3. `[unfolding]` The town is being pushed toward asking for peace by its own weariness rather than by any defeat. Each season the pushing gets harder to resist.
4. `[threshold]` {settlement} has reached the point where the next demand is the one that gets refused. Nobody in the hall can say in advance which demand that is.

**`warExhaustion: exhausted`**
1. `[ledger]` {settlement} is spent. There is no further war in this town, and none is being withheld. Anything asked of it from here is not going to arrive.
2. `[street]` The town has nothing left to give the war and has stopped pretending it might find some.
3. `[unfolding]` Whatever {settlement} agrees to next, it is going to agree to because it is exhausted, and the terms are going to show it.

**mobilization: climbing the ramp, still distant**
1. `[ledger]` {settlement} is turning toward war and is not near it. The muster has begun. The economy has barely started to shift behind it.
2. `[street]` The town has begun to prepare the way towns do: quietly, and well ahead of anybody admitting what for.
3. `[visitor]` A stranger at {settlement} notices the town buying things it does not ordinarily buy. He is told nothing about why.
...
2. `[unfolding]` The town is very close to war-ready now. From inside, none of the preparation looks as if it can be undone.
3. `[street]` The town has stopped pretending at {settlement}. The preparations are visible, and being seen is now part of the point.
...
2. `[street]` The soldiers are ready at {settlement} and are being kept waiting. The waiting is its own kind of pressure.
3. `[unfolding]` {settlement} is holding a war it has already paid for. Every season it holds it, the paying goes on and nothing is gained.
...
2. `[street · dm-only]` Nothing at {settlement} looks like a town going to war. At this stage the appearance is intended.
3. `[counterforce · dm-only]` {settlement} would be visibly arming by now if it intended to be seen. The absence of any sign is the strongest sign there is.
...
2. `[unfolding]` {counterpart} is bleeding into {settlement} rather than out of it. Every season of that makes the holding harder to justify at home.
3. `[street]` The soldiers holding {settlement} are costing whoever sent them more than the town has ever produced. The town is aware of the arithmetic.

**`occupation.pays` true**
1. `[ledger]` {settlement} has begun to pay {counterpart} back for taking it. The town's yield now exceeds what the garrison costs. The arrangement has become worth keeping.
2. `[street]` What the town produces leaves it. The leaving has become regular enough that the town budgets around it.
3. `[unfolding]` What leaves {settlement} for {counterpart} has become regular enough to be planned around at both ends. That is what makes an occupation permanent.

**`occupierHoldings.stretchedThin`**
1. `[ledger]` {counterpart} holds more than it can garrison properly. {settlement} is one of the places where the thinness shows.
...
3. `[street]` The garrison at {settlement} is smaller than it was and has not been reinforced. The town has drawn the obvious conclusion about how {counterpart} is doing elsewhere.

**`occupierHoldings.strengthened`**
1. `[ledger]` What {counterpart} holds elsewhere is feeding what it holds here. The garrison at {settlement} is stronger for the occupier's other wins rather than in spite of them.
2. `[unfolding]` The occupier's grip on {settlement} is tightening because the occupier is winning elsewhere. Nothing local is causing it and nothing local can undo it.
...
1. `[ledger]` An attempt on {settlement} by {counterpart} would be a real contest, with the outcome genuinely open. That is the most dangerous position a town can be in.
2. `[street]` The town believes it could be taken. It believes it could hold. It lives with both beliefs at once.
3. `[unfolding]` Neither {settlement} nor {counterpart} can be confident of the outcome. That is precisely the condition under which somebody eventually tries.

**feasibility `auto_fail`**
1. `[counterforce]` {counterpart} cannot take {settlement}. The gap is not a close one, and everybody on both sides has done the arithmetic.
2. `[ledger]` An assault on {settlement} by {counterpart} would be hopeless. That is why no assault comes, and why the hostility goes on in other forms.
3. `[street]` The town does not lose sleep over {counterpart}, and it is right not to.

**feasibility `harassment`**
1. `[counterforce]` {counterpart} is too weak to take {settlement} and strong enough to make the approaches unpleasant. That is precisely what it does.
2. `[ledger]` What {settlement} faces from {counterpart} is raiding rather than war. The town itself is not at risk. The country around it is.
3. `[street]` The outlying country around {settlement} is dangerous because of {counterpart}. The town itself is not. The difference matters a great deal to the people living outside the walls.
...
1. `[counterforce · dm-only]` {settlement} cannot be taken from outside by {counterpart}. It would have to break apart first, and the breaking apart is what {counterpart} is actually working on.
2. `[ledger]` The danger to {settlement} from {counterpart} can only come from inside. The walls are not the way in, and both parties understand which way is.
3. `[street · dm-only]` {counterpart} has stopped looking at {settlement}'s walls and started looking at its quarrels. That change of attention is the whole threat.

**feasibility `require_magic`**
1. `[counterforce]` Nothing {counterpart} could bring by ordinary means would take {settlement}. Only decisive arcane force would open this town, and that is a different kind of undertaking entirely.
2. `[ledger]` {settlement}'s defense against {counterpart} holds against everything except the arcane. That narrows the threat a great deal and does not remove it.
3. `[street]` The town's confidence at {settlement} stops exactly where the arcane begins. It has no comfortable way of talking about that.
...
2. `[unfolding]` The town is arming past what it agreed to and is not hiding it. That means the next move belongs to {counterpart}.
...
3. `[unfolding]` The garrison at {settlement} keeps the walls without incident, and has kept them long enough. That is why the absence of incident is beginning to look like the arrangement rather than the truce.
...
**`political` · `strained`**
1. `[ledger]` The installed seat at {settlement} wavers. Its writ runs thinly beyond the hall itself. The parts of the town that ignore it are growing.
...
2. `[counterforce]` There has been a contest at {counterpart} that {settlement} could have reached into and did not. The holding back is entered in the record as restraint.
...
2. `[street]` Whatever was agreed is being done. The town has stopped watching to see whether it is going to be done.
...
1. `[unfolding]` This part of the peace at {settlement} has {timeband_span} left on it. It is not going to reach the end of that time in the shape it started.
...
3. `[visitor]` A stranger can predict what {settlement}'s hall is going to do about a grievance. That is the most useful thing a neighbour can say about a town.
...
**`Pacific`**
1. `[ledger]` {settlement} is willing to go a very long way to avoid a fight. Its neighbours price that in.
...
3. `[unfolding]` {settlement} is ready and idle. That is the most expensive condition a town can hold, and the hardest to hold for long.
...
2. `[street]` The town is short of its own people. It is going to stay short until the campaign ends, however it ends.
...
2. `[unfolding]` The town has reached the end of what it is willing to give and has not formally said so. The gap between those is where the next few seasons are going to be spent.
...
2. `[street]` People are coming back to the town. They are not returning to what they left, because what they left has been reorganized around their absence.
...
**resistance `restive`**
1. `[ledger]` {settlement} is restive. Sabotage and noncompliance wear at the garrison steadily and never amount to a rising.
...
2. `[unfolding]` What is happening at {settlement} is going to decide whether {counterpart} keeps it, and the deciding comes within the season.
...
3. `[unfolding]` {settlement} is open again and is not yet what it was. The counting has started, and nobody has said aloud how long the making-good is going to take.
...
3. `[unfolding]` What {settlement} took is being turned into something permanent by the people who hold it. The town's shape after that is not going to be the shape it had.
...
1. `[unfolding]` Devotion is climbing, and the reason is plain. The town lives by the creed it professes, and the professing has followed the living.
...
2. `[threshold]` The unaffiliated are the fastest-growing part of {settlement}'s religious life. That is a way of saying its religious life is shrinking.
...
3. `[street]` This faith has grown into a contender. Nothing about its position is settled, and that is the point.
...
2. `[ledger]` One of {settlement}'s ties carries more weight than the rest. The weight shows in what the town does and refuses to do.
...
4. `[unfolding]` {band} of {settlement}'s relationships rest on conditions rather than on choices. That makes them exactly as durable as the conditions.
...
**`THIN`**
1. `[ledger]` {creed} keeps a footing at {settlement} rather than a hold. The observance is present, and presence is the whole of it.
