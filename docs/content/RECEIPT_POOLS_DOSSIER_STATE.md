...
**`river`** *(includes the inland river-port resolution)* — 1. `[visitor]` {settlement} is laid along its water rather than around a square, and you meet it lengthwise. · 2. `[street]` Everything in {settlement} is upstream or downstream of something else. The town gives directions that way. · 3. `[ledger]` The barge traffic sets {settlement}'s calendar. What the river cannot carry in season, {settlement} does without. · 4. `[counterforce]` {settlement} sits on navigable water and ships almost nothing on it. The river here is a boundary the town lives beside rather than a road it uses.
...
**`ordinary` (route `road` and the default)** — 1. `[visitor]` {settlement} announces itself barely, the way a working town does. · 2. `[street]` {settlement} is on the way to somewhere and has made a modest living out of being on the way. · 3. `[ledger]` Goods come into {settlement} and goods go out of it. Neither the coming nor the going is remarkable. · 4. `[counterforce]` The road runs past {settlement} and {settlement} takes very little from it. The traffic is real, and the town has stayed outside it.
...
**`port`** — 1. `[visitor · no deficit]` {settlement} exists because of the water, and nothing about the town makes sense without it. · 2. `[ledger · no deficit]` {settlement}'s living comes off the quay. The fields behind it are a convenience the town could manage without. · 3. `[ledger · deficit]` What {settlement} eats arrives by the same water that gives the town its trade. That makes the harbour the whole of the argument for the place. · 4. `[street · deficit]` {settlement} looks seaward for everything, including dinner.
...
- `thorp` / `hamlet` — 1. `[street]` {settlement} is small enough that every household knows its purpose. Surplus, where there is any, is modest. · 2. `[visitor]` {settlement} is a handful of roofs with a reason, and the reason is usually the ground. · 3. `[ledger]` {settlement}'s whole economy fits on one page and always has room left.
- other tiers — 1. `[ledger]` {settlement} is the size that a place of this kind settles at, and it has settled there. · 2. `[street]` {settlement} is neither small enough to be simple nor large enough to be complicated. It manages both ways. · 3. `[visitor]` {settlement} is a town, in the ordinary sense, and the ordinariness is accurate.
