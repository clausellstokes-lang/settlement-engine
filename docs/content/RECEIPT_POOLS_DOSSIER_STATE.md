...
**RECEIPT:** the demographic trend band is derived by `trendBandFromHistory(history)` (`src/domain/worldPulse/beliefAxes.js`), which sums the recent net delta over `AXIS_TUNING.TREND_WINDOW` against the last reading's population and bands the ratio to −2..+2, returning 0 for an absent or empty ring · `config.tradeRouteAccess` (live enum `port` · `river` · `crossroads` · `road` · `isolated` · `mountain_pass` · `mountain_road` · `desert_road`) · ⚠ **A NAMING CORRECTION, RECORDED RATHER THAN MADE SILENTLY:** `DS-POP-2`'s title spells this deriver `populationTrendBand(settlement.populationHistory)`, but there is no function of that name in the tree. `populationTrendBand` is the KEY of the D-1 belief axis (`beliefMap.js`), which holds a BELIEVED band and exists only when `beliefAxesEnabled`. This block keys on the GROUND-TRUTH band, which is `trendBandFromHistory` over the settlement's own ring, and names it correctly. `DS-POP-2`'s title is left untouched: correcting it would edit an existing block, which this chapter does not do.
