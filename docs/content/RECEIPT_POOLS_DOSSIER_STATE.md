...
**`scores.military`**
- `STRONG` — 1. `[visitor]` {settlement} can put a real body of armed men on its walls. The men look like they have done it before. · 2. `[ledger]` The muster roll at {settlement} is long and current, and the arms it lists are accounted for. · 3. `[street]` Nobody in {settlement} expects the town to lose a straight fight. The expectation shapes how the town speaks to its neighbours.
- `ADEQUATE` — 1. `[visitor]` {settlement} keeps enough men under arms to be taken seriously and not enough to be feared. · 2. `[ledger]` The muster at {settlement} covers the gates and the walls and does not stretch past them. · 3. `[street]` {settlement} would hold against what it expects. It knows it is not proof against what it does not expect.
- `WEAK` — 1. `[visitor]` The watch at {settlement} is a watch and not a garrison, and the difference is visible at the gate. · 2. `[ledger]` {settlement}'s arms are counted in what the households own rather than in what the hall issues. · 3. `[street]` {settlement} does not think of itself as a place that fights. It is right not to.
- `CRITICAL` — 1. `[visitor]` There is nothing at {settlement} that would stop anybody who meant to come in. · 2. `[ledger]` {settlement} keeps no muster worth the name. The gate is shut at night by whoever is nearest to it. · 3. `[street]` What protects {settlement} is that nobody has wanted it. Everyone here understands the arrangement.

**`scores.monster`**
- `STRONG` — 1. `[visitor]` {settlement} is built for what comes out of the country around it: heavy shutters, close outbuildings, and nothing left standing alone. · 2. `[ledger]` {settlement} keeps a standing answer to the beasts of its own ground. The answer is men who know the work, and the tools kept for it. · 3. `[street]` Children in {settlement} are taught the rules about the dark early, and the rules work.
...
- `CRITICAL` — 1. `[visitor]` The country around {settlement} is not held at all. The town's edge is wherever the last house happens to stand. · 2. `[ledger]` {settlement} has no answer to what lives beyond its fields and nobody whose work it would be. · 3. `[street]` People in {settlement} do not go far out after dark. They do not need to be told why.

**`scores.internal`**
- `STRONG` — 1. `[visitor]` Order in {settlement} is kept quietly. That is the expensive kind. A stranger sees very little enforcement and very little disorder. · 2. `[ledger]` Complaints brought at {settlement} are heard and closed, and the closing is written down. · 3. `[street]` A quarrel in {settlement} goes to somebody who settles it, and everybody knows who that is.
...
- `WEAK` — 1. `[visitor]` There is more going on in {settlement} than the watch troubles itself with. It is not hidden particularly well. · 2. `[ledger]` {settlement}'s complaint book has more entries opened in it than closed. · 3. `[street]` Getting a thing settled in {settlement} depends on who you are. The town is arranged accordingly.
...
- `WEAK` — 1. `[ledger]` {settlement} runs close to what it takes in. One interruption would reach the households inside the season. · 2. `[visitor]` There is nothing spare in {settlement}. The absence of spare is what a careful eye notices first. · 3. `[street]` {settlement} is one bad turn from difficulty and lives with the arithmetic openly.
...
- `CRITICAL` — 1. `[visitor]` Nothing in {settlement} is held up by the Art. Nothing here would know what to do with it. · 2. `[ledger]` {settlement} keeps no practitioner, no arrangement, and no expectation of either. · 3. `[street]` The Art in {settlement} is a rumour about other places.
...
- `Struggling` / `Poor` — 1. `[visitor]` {settlement} is short of comfortable and knows precisely how short. · 2. `[ledger]` {settlement}'s year closes near enough to level that the closing is watched. · 3. `[street]` The households in {settlement} are careful with each other about money. That is its own kind of courtesy.
- `Moderate` / `Modest` — 1. `[visitor]` {settlement} is neither rich nor poor. Nothing about it is remarkable. That is the most accurate thing to say about it. · 2. `[ledger]` {settlement}'s books show a town that pays for itself and does not accumulate. · 3. `[street]` {settlement} eats well enough and does not talk about it.
...
- head word in {`Secure`, `Controlled`, `Quarantined`} — 1. `[visitor]` A stranger walks {settlement} at any hour and the town's only interest in them is commercial. · 2. `[street]` {settlement} is a safe place to be, and the safety has a cost that the town has agreed to pay. · 3. `[ledger]` The watch's returns at {settlement} are short. Short returns here mean quiet streets rather than an idle watch.
- head word in {`Tense`, `Strained`, `Restricted`, `Unsafe`} — 1. `[visitor]` {settlement} is workable and watchful: a stranger is served, and also noticed. · 2. `[street]` {settlement} is not dangerous so much as unrelaxed. The difference matters to the people living in it. · 3. `[ledger]` The watch at {settlement} is stretched across more than it can comfortably cover. The stretching shows in what gets deferred.
- head word in {`Dangerous`, `Desperate`} — 1. `[visitor]` {settlement} is a place to be careful in, and being careful is not enough. · 2. `[street]` People in {settlement} move in company after dark and do not consider that unusual. · 3. `[ledger]` What the watch at {settlement} records is a fraction of what happens. The fraction is not chosen by the watch.

**`economicViability.viable`**
- `true` — 1. `[ledger]` {settlement} is a town that works. What it needs, it can get. What it makes, it can move. · 2. `[visitor]` There is no obvious reason {settlement} should not be here. That is a plainer compliment than it sounds. · 3. `[street]` {settlement} expects to go on being {settlement}. · 4. `[threshold]` {settlement} works on the terms it currently has. The judgment is about those terms rather than about the town.
- `false` — 1. `[ledger]` The arithmetic of {settlement} does not close. What the town requires exceeds what its ground and its trade together supply. · 2. `[visitor]` {settlement} is held here by something other than its own economy. A visitor with an eye for it starts wondering what. · 3. `[street]` {settlement} goes on, and the going on is not explained by the books. · 4. `[threshold]` {settlement} does not close its own arithmetic and is standing anyway. That means something outside the books is paying. The page cannot say what.

**`defenseProfile.readiness.label`**
- `Fortress` — 1. `[visitor]` {settlement} is a hard place to take and looks it from a long way off. · 2. `[ledger]` Walls, garrison and stores at {settlement} are all of a piece. That is rarer than any one of them.
...
- `Secure` — 1. `[ledger]` {settlement} feeds itself out of its own ground. Its grain store is a convenience rather than a defence. · 2. `[street]` Nobody in {settlement} thinks about food. That is the most that can be said for a town's food. · 3. `[visitor]` The market at {settlement} is full and unhurried, and the fullness is local. · 4. `[threshold]` {settlement}'s security is its own harvest. That means it is proof against a closed road and not against a bad year.
...
- `Pressured` — 1. `[ledger]` {settlement} covers its need and covers it exactly. Nothing in its grain store is spare. · 2. `[street]` The price of bread in {settlement} is watched. It is not watched in a comfortable town. · 3. `[visitor]` {settlement} eats, and the eating takes more arranging than it should. · 4. `[threshold]` {settlement} is at the top of this condition rather than the bottom of a worse one. What separates the two is the margin its grain store is currently holding.
...
- `Deficit` — 1. `[ledger]` What {settlement} needs exceeds what it produces and what it can bring in together. The shortfall is carried by the households. · 2. `[street]` There is less in {settlement} than there are people to eat it. The arithmetic reaches the poorest quarters first. · 3. `[visitor]` {settlement}'s grain store is open and the queue at it is longer than the stock behind it. · 4. `[threshold]` {settlement} is short and not yet starving, and what stands between the two is the stores rather than the harvest.
- `Deficit × Active Famine` — 1. `[ledger]` {settlement} is short of food past the point that stores or purchase can answer. The rolls are being kept for reasons other than trade. · 2. `[street]` Hunger in {settlement} is general, and what people are arranging now is not meals but order. · 3. `[visitor]` The first thing a stranger is asked in {settlement} is whether they have brought anything. · 4. `[threshold]` There is no rung below this one on the ladder {settlement} is on. What changes from here is not the grade but the count.
