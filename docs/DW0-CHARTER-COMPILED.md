...
This compiled charter is held at **`refs/preserve/holding-dw0`**. **Both are named refs, not loose shas —
...
| **CH-1** | **LANDED** | `b2852ccc3` | ancestor | Never hand CH-1 a work order. `interiorKind` and the `\b`-anchored facet chokepoint are **available now**. |
...
**§519 RULED, and it NARROWS the draft.** *(Conflict C-2.)*

| | Draft §3.4 / EST-4 | Signed §519 |
|---|---|---|
| `Estate.ownerRef` | an **institution anchor** (`anchorForInstitution(inst)`) or `ANONYMOUS_FABRIC` | **POWER-scoped** — a member of THE POWERS, or `ANONYMOUS_FABRIC` |
...
>
> **THE CHARTER'S RECOMMENDATION: SPLIT THE ANCHOR FROM THE RELATION.** Keep §519's signed relation
> rule **verbatim** — `owns` is a POWER, `occupies` is everyone else — and rule that **estate
> membership is not carried by `owns`**. An Estate is a set under one **anchor**, and the anchor may
> be a POWER, a non-power institution, or `ANONYMOUS_FABRIC`. This preserves **both** owner sentences:
...
| | |
|---|---|
| **EXIT** | Five arms, **each with a deliberate mutation that reds it**: (1) the walker certifies a **ZERO-CELL plan and a ZERO-STORAGE plan as LAWFUL with a reason** — the `NO_BUILDING` verdict must survive validation, which is R-INST-5 §Σ.1's explicit requirement; (2) reachability accepts `approach: NONE` (the anchorhold, a cell with no door) and `HATCH_ONLY` (the prison pit); (3) the **ABSENCE** arm proves a `refused` joint is not there; (4) `furnaces ≤ flues + portableFurnaces` holds over the whole corpus; (5) **every concealed cell debits its host's area and the sum checks** — a hide is subtracted from a declared volume and never added, because the priest hunters measured ceilings. |
| **Observable** | five arms × two runs each (subject green, mutant red) = **ten results, all ten required.** An arm with no red mutant is struck from the count. |
...
|---|---|---|---|
| **G1** | Three unanchored-substring mis-inferences in `FACET_INFERENCE` — `Warden's Lodge`→vice (the `/den/` in "War**den**'s"), `Dragon resident`→vice, `Charlatan fortune tellers`→security (the `/fort/` in "**fort**une") | ⛔ **CH-1 IS LANDED (`b2852ccc3`) — do NOT hand it a work order.** But it leaves DW a **residual**. | The `\b`-anchored chokepoint and the `interiorKind` override are **available at this base**. ⛔ **J-CH-1 hands `Warden's Lodge` BACK to DW in writing — "DW-1 assigns the real kind" — and the first compile recorded neither the return nor an owning car.** The gap is the **interior KIND**, not the parti (`MOATED_PLATFORM` is already a DW-1 member): **DW-1's five chartered vocabularies contain no kind vocabulary**, and the draft makes `interiorKind` a *consumed input* to S2, so **nothing in the program mints it.** Owed a car (§I C-22). ⚠ G1's membership is far larger than three: R-INST-6 §Σ.3 adds **six** on the Criminal shelf from `guild\|market\|bazaar` plus **five** from `smuggl`; R-INST-4 E-1 adds every inn row → `generic`, `Gladiatorial school` → `learning`, `Hireling hall` → `civic`. |
...
**Nothing below was silently resolved.** Each row states both sides. **Twenty-three rows: C-1..C-15
from the first pass against the signature, C-16..C-23 from the second pass against the slot.**
...
| **C-17** | **Is `ring table` a fixture kind?** | §H's own missing list names it; R-INST-5 carries it 8 times in what the dossier calls its best-sourced passage (Uraniborg's circular working table). | The 60-member add list omits it — 0 hits in both frozen sources. | **Add it and restate `22 + 61 = 83`, declaring the one-time figure move as AR-7 did for 81 → 82** — or record a reasoned refusal. Check `LEDGE` and `WINDOW_SILL_BENCH` from the same source line. |
...
| **C-19** | **Two exit criteria cannot be executed as written.** | §G requires every criterion to name what it counts and what value it must take. | DW-2b's "within the declared tolerance" declares no tolerance anywhere; DW-1's "~35 storage / ~55 circulation sub-forms" are approximations, so §453's closets and pantries are the only vocabularies with no ratchet. | **Declare the tolerance as a number before DW-2b dispatches; recount both sub-form sets mechanically and pin them as integers with mutants.** |
| **C-20** ⭐ | **D-6 has no owning train.** | The ROOF and WATER-LANDING joint kinds were deferred "to the train that owns the closed enum". | **UC-5 was the undercity train's sixth and LAST car**, and it is landed — the train has no remaining dispatch vehicle. | **Re-home D-6.** Options: a DW-4c car, a new undercity car, or a recorded deferral naming who opens the enum. **The chair's call — this is a train-ownership question, not a DW one.** |
| **C-21** | **The AD charter is unreachable.** | ODQ §526 records `draft-AD-CHARTER.md` at 122,346 B / 1,377 lines, 5 waves / 17 cars, naming `cartographyPaint.js` as the DW-6 seam. | The file is **absent from every ref and from disk**, so AD's 17 cars could not be walked for a reverse dependency on DW. | **Get it into a preserve ref before DW-6a/6c build an unnamed art slot.** Ordering is sound either way (§514.4 puts AD strictly downstream of DW-S), but an unreachable charter cannot be a seam contract. |
| **C-22** | **Who mints `interiorKind`?** | J-CH-1 hands `Warden's Lodge` back to DW in writing: "DW-1 assigns the real kind". | DW-1's five chartered vocabularies contain **no kind vocabulary**, and the draft makes `interiorKind` a *consumed* input to S2. | **Nothing in the program mints it. Either DW-1 gains a sixth vocabulary car, or the return is refused back to CH.** Recommend the sixth car — CH-1 has landed and cannot be re-opened cheaply. |
