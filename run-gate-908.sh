#!/bin/sh
# run-gate-908.sh — the §908 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base 4243bdc61. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
...
BASE=4243bdc610fe5b380f1d0029973cf9088bae1631   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse 4243bdc610fe5b380f1d0029973cf9088bae1631)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base 4243bdc61 — land the previous consist first"; exit 7; }
