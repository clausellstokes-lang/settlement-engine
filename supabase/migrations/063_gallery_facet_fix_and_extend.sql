-- ────────────────────────────────────────────────────────────────────────────
-- 063_gallery_facet_fix_and_extend.sql — fix the broken gallery facets, then
-- add five new ones sourced from REAL persisted settlement attributes.
--
-- ROOT CAUSE (fixed here): the derived filter columns in
-- _gallery_public_tile_rows() (migration 019) read JSON paths the generator
-- NEVER writes, so terrain / government_type / stability came back NULL (or a
-- free-text label that matched no sidebar value) for every real row. Every
-- `col IN (...)` clause in list_gallery_dossiers therefore excluded all rows the
-- moment a facet was selected. The columns are COMPUTED at query time, not
-- stored, so there is nothing to backfill for the FIXED derivations — correcting
-- the JSON paths repairs every existing row at once.
--
-- What the generator actually persists (verified against the engine):
--   terrain   → settlement.config.terrainType  (coastal/riverside/plains/hills/
--               forest/mountain/desert), with config.terrainOverride as the
--               explicit-pick fallback. `config.terrain` is never written.
--   magic     → settlement.config.magicLevel    (none/low/medium/high only).
--   government→ powerStructure.government is a STRING (the governing faction's
--               NAME). There is no bounded government enum, so government is kept
--               as a DISPLAY column (read correctly) but is no longer offered as
--               an IN-list filter (no stable vocabulary to match — see the
--               sidebar change that drops that section).
--   stability → powerStructure.stability is a free-text composite label
--               ('Enforced Order (authoritarian)', 'Stable (theocratic
--               governance)', …). No bounded vocabulary, so likewise kept for
--               display only; the stability IN-list facet is retired client-side.
--
-- NEW facets (stored columns, backfilled, indexed, filterable):
--   gallery_facet_culture     ← config.culture            (11-culture catalog)
--   gallery_facet_prosperity  ← economicState.prosperity  (Struggling…Wealthy)
--   gallery_facet_deity       ← config.primaryDeitySnapshot.name (display + the
--                               boolean "has patron deity" facet; the name is
--                               unbounded so it is not an IN-list)
--   gallery_facet_at_war      ← publish-time snapshot of the owning campaign's
--                               live war status (boolean facet, like hasImage)
--   population range          ← the existing derived population integer column,
--                               filtered by populationMin / populationMax.
--
-- These five are STORED (not query-time-derived) because their sources are
-- either expensive to walk per-query (deity/prosperity nesting) or, for at_war,
-- a publish-time snapshot of LIVE campaign state that the gallery row cannot
-- recompute on its own. They are written by the publish-time extraction in
-- src/lib/gallery.js (galleryMetadataPatch) and BACKFILLED below for every row
-- already in the gallery.
--
-- Additive + idempotent + re-runnable. Postgres cannot `create or replace` a
-- function whose RETURNS TABLE row type changes, so the tile-rows helper and its
-- dependents are dropped and recreated faithfully from their latest definitions
-- (027 list_my_gallery_dossiers, 047 get_gallery_dossier, 019 list_gallery_*).
-- RLS, the public-safe sanitize, and every grant are preserved verbatim.
-- ────────────────────────────────────────────────────────────────────────────

-- ── 1. Stored facet columns ─────────────────────────────────────────────────
alter table public.settlements
  add column if not exists gallery_facet_culture text,
  add column if not exists gallery_facet_prosperity text,
  add column if not exists gallery_facet_deity text,
  add column if not exists gallery_facet_at_war boolean;

comment on column public.settlements.gallery_facet_culture is
  'Publish-time snapshot of config.culture for the gallery culture facet. Backfilled from stored data; refreshed on re-share.';
comment on column public.settlements.gallery_facet_prosperity is
  'Publish-time snapshot of economicState.prosperity (Struggling…Wealthy) for the gallery prosperity facet.';
comment on column public.settlements.gallery_facet_deity is
  'Publish-time snapshot of config.primaryDeitySnapshot.name (the patron deity, if any). NULL for deity-free settlements; drives the "has patron deity" boolean facet and tile display.';
comment on column public.settlements.gallery_facet_at_war is
  'Publish-time snapshot of the owning campaign''s live war status for this settlement (settlementWarStatus.atWar). NULL/false when not at war or no campaign.';

-- ── 2. Backfill existing gallery rows from their stored settlement data ──────
-- Only public rows matter for the gallery; the columns stay NULL elsewhere.
-- nullif('') keeps an absent attribute as NULL rather than an empty-string facet.
update public.settlements s
set
  gallery_facet_culture = nullif(s.data #>> '{config,culture}', ''),
  gallery_facet_prosperity = nullif(s.data #>> '{economicState,prosperity}', ''),
  gallery_facet_deity = nullif(s.data #>> '{config,primaryDeitySnapshot,name}', '')
where s.is_public = true
  and (
    s.gallery_facet_culture is null
    or s.gallery_facet_prosperity is null
    or s.gallery_facet_deity is null
  );

-- gallery_facet_at_war cannot be backfilled from s.data alone — "at war" is a
-- LIVE cross-settlement campaign state (worldState.deployments / regional
-- war_front), not a static field on the persisted settlement. Existing rows
-- therefore read NULL (treated as "not at war") until the owner re-shares, at
-- which point the publish-time extraction stamps the current snapshot. This
-- mirrors how realm-arc summary (§S4) is captured at publish time.

-- ── 3. Indexes for the new IN-list / boolean facets ─────────────────────────
create index if not exists idx_settlements_gallery_facet_culture
  on public.settlements (gallery_facet_culture)
  where is_public = true;
create index if not exists idx_settlements_gallery_facet_prosperity
  on public.settlements (gallery_facet_prosperity)
  where is_public = true;
create index if not exists idx_settlements_gallery_facet_at_war
  on public.settlements (gallery_facet_at_war)
  where is_public = true and gallery_facet_at_war = true;

-- ── 4. Drop dependents, then the tile-rows helper (RETURNS TABLE changes) ────
-- Drop order respects the dependency graph: the three list RPCs and the detail
-- RPC all read _gallery_public_tile_rows(), so they must go first.
...
-- ── 5. Recreate the tile-rows helper — FIXED derivations + new facets ───────
...
as $$
  select
    ...
    -- FIXED: the generator persists config.terrainType (coastal/riverside/…),
    -- with config.terrainOverride as the explicit-pick fallback. The old paths
    -- ({config,terrain} / {geography,terrain}) were never written ⇒ always NULL.
    coalesce(
    ...
    -- FIXED read for DISPLAY: powerStructure.government is a STRING (the
    -- governing faction's name). No bounded enum, so this is no longer an
    -- IN-list filter, but the tile shows the real value instead of ''.
    coalesce(
    ...
    -- FIXED read for DISPLAY: stability is the free-text powerStructure.stability
    -- composite label. No bounded vocabulary, so it is display-only (the
    -- stability IN-list facet is retired client-side).
    coalesce(
    ...
    -- NEW facets: prefer the stored snapshot (written at publish time +
    -- backfilled above), fall back to the live data path so a row published
    -- before this migration still surfaces its real value.
    coalesce(
...
-- ── 6. Recreate list_gallery_dossiers — FIXED + new facet WHERE clauses ──────
...
as $$
  ...
  select
    ...
    primary_deity, at_war, net_votes, comment_count,
    total_count
  from counted
  order by
...
-- ── 7. Recreate list_gallery_more_by_creator (019, + new facet columns) ─────
...
-- ── 8. Recreate list_my_gallery_dossiers (027, + new facet columns) ─────────
...
-- ── 9. Recreate get_gallery_dossier (047 verbatim body, new tile columns) ───
-- The detail RPC joins _gallery_public_tile_rows() and so had to be dropped with
-- it. Body is migration 047's verbatim — same data projection, same sanitize,
-- same grants. The new tile facet columns are not surfaced here (the detail page
-- reads stability/terrain for display only); the RETURNS TABLE is unchanged.
