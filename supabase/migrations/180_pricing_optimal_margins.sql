-- ────────────────────────────────────────────────────────────────────────────
-- 180_pricing_optimal_margins.sql — repriced AI schedule for optimal margins at
-- the Opus-default economics, plus the calibration-target knob bump.
--
-- MIGRATION-NUMBER NOTE (180-band reserved for the pricing lane): this uses number
--   180, NOT the first-contiguous 170, to avoid a cross-worktree collision — the
--   parallel admin/moderation lane (vision-i, claude/admin-twokey-founders-roll)
--   has already committed migrations 170, 171, 172, 173 (founders roll / content
--   moderation / comment tombstone / map reports). Two 170s cannot fold, so the
--   pricing lane sits in its reserved 180 band. CONSEQUENCE: on this isolated
--   branch the contiguity gate (scripts/check-migration-head.mjs) reports a gap
--   170-179 (vision-i's 170-173 live on another branch and are not visible here),
--   and the merged tree will still have a 174-179 gap. That gap is a KNOWN
--   FOLD-TIME item — the fold renumbers this file to the next contiguous slot
--   after the admin lane's head (174). Do NOT paper over it with dummy filler
--   migrations.
--
-- WHY THIS EXISTS
