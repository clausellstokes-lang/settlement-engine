-- ────────────────────────────────────────────────────────────────────────────
-- 151_surveyor_s4_s6_credit_costs.sql — Surveyor S4–S6 commercial: task-priced
-- managed credits for the four WRITE stages (DESIGN_AI_CONTROL_SURFACE §2 stages
-- 4–6 + the style-overhaul rung / §4 commercial mapping).
--
-- WHY THIS EXISTS
--   The S4–S6 wave adds four new task classes — 'customContent' (homebrew content
--   compile), 'styleOverhaul' (one bespoke map-style definition), 'constructSettlement'
--   (intent → generated settlement), and 'constructRealm' (intent → composed realm).
--   All are TASK-PRICED managed credits (users buy OUTCOMES, not tokens). This extends
--   the spend_credits CASE with those four features so the new edge functions'
--   spend_credits({feature:'customContent'|'styleOverhaul'|'constructSettlement'|
--   'constructRealm'}) resolve a cost instead of raising 'unknown feature'.
--
--   PRICING IS PROVISIONAL (vetoable — final Surveyor pricing is an OWNER-QUEUED
--   decision, design §6). Anchored to the S1/S3 precedent (interpret=5, parley=3,
--   analysis=3, brief=4): styleOverhaul=3 (a cosmetic bounded style def — the
--   ugly-never-unsafe first compile rung ≈ 'analysis'/'parley'), customContent=6
--   (a multi-entry content compile with coherence declarations — richer than a
--   session compile), constructSettlement=6 (intent→config→generate→compare→revise),
--   constructRealm=8 (realm scale — the composer places everything; the heaviest job).
--   Kept in lockstep with src/config/pricing.js SURVEYOR_AI_COSTS by the pricing
--   contract test (tests/config/pricing.test.js) — this block + that map change together.
--
-- WHAT
--   Re-defines spend_credits verbatim from 149 (the current definition) with FOUR
--   new CASE arms. All other behaviour — config-first per-profile cost, the elevated
...
--
-- Depends on: 149 (current spend_credits), 114 (ai_credit_costs config), 057
--   (account_is_active), 007 (credit_ledger, get_credit_balance). Re-runnable.
-- @rollback: re-apply 149's spend_credits definition (drop the four new CASE arms).
-- ────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  if cfg_cost is not null then
    ...
    -- S4–S6 (customContent, styleOverhaul, constructSettlement, constructRealm) features.
    cost := case feature
