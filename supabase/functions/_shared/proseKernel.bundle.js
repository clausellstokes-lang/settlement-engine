...
function refuteUnit(unit, card, extra = {}) {
  ...
  for (const f of walk2.entry.fails) emit2(out, finding2(id, f.klass, "FAIL", f.arm, str(f.clause).slice(0, 90), f.description));
  for (const f of walk2.entry.withheld) emit2(out, finding2(id, f.klass, "WITHHELD", f.arm, str(f.clause).slice(0, 90), f.description));
