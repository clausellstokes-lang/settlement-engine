...
function bodiesOfCard(card) {
  return (Array.isArray(card?.town?.bodies) ? card.town.bodies : []).map((body) => str(body?.name)).filter(Boolean).sort(compareCodepoint);
}
...
function fieldPathsOfPool(pool) {
  ...
  for (const row2 of Array.isArray(pool?.fields) ? pool.fields : []) {
    add(row2?.field);
    add(row2?.readFrom);
    for (const input of Array.isArray(row2?.inputs) ? row2.inputs : []) add(input?.field);
  }
  return out;
}
  ...
  for (const hit of text2.matchAll(PROVENANCE_CLAUSE)) {
    ...
    if (named.length) {
      emit2(out, finding2(
        ...
        "REFERENT-body",
        "REPORT",
        "a record kept by a named body rather than an institution",
        `${kind}: ${named[0]}`,
...
var namedUnder = (list, prefix) => (Array.isArray(list) ? list : []).filter((p) => String(p).startsWith(prefix)).map((p) => String(p).slice(prefix.length + 1, -1));
function summarise(moved, added, removed, pageLines) {
  return rec([
    ["schema", EPOCH_RECORD_SCHEMA],
    ["moved", moved],
    ["fields", rec(moved.map((row2) => [
      String(row2.path),
...
function mergeEpochRecords(older, newer) {
  ...
  for (const row2 of Array.isArray(a.moved) ? a.moved : []) {
    byPath.set(String(row2?.path), { before: row2?.before, after: row2?.after });
  }
  for (const row2 of Array.isArray(b.moved) ? b.moved : []) {
    const path = String(row2?.path);
    const held = byPath.get(path);
    byPath.set(path, { before: held ? held.before : row2?.before, after: row2?.after });
  }
  const moved = [...byPath.entries()].filter(([, v]) => v.before !== v.after).sort((x, y) => compareCodepoint(x[0], y[0])).map(([path, v]) => rec([["path", path], ["before", v.before], ["after", v.after]]));
  ...
  const added = [.../* @__PURE__ */ new Set([...aAdded, ...bAdded])].filter((p) => !(bRemoved.has(p) && aAdded.has(p)) && !(aRemoved.has(p) && bAdded.has(p))).sort(compareCodepoint);
  const removed = [.../* @__PURE__ */ new Set([...aRemoved, ...bRemoved])].filter((p) => !(aAdded.has(p) && bRemoved.has(p)) && !(aRemoved.has(p) && bAdded.has(p))).sort(compareCodepoint);
  const pageLines = rec([
  ...
  for (const row2 of [
    ...
    events.set(String(row2?.id ?? `${events.size}`), row2);
  }
  return rec([
    ["schema", EPOCH_RECORD_SCHEMA],
...
function fieldLine(field) {
  ...
  if (field?.unknown !== true) {
    ...
    return `    ${name} = ${JSON.stringify(field?.value ?? null)}${from ? ` (the engine's own field is \`${from}\`)` : ""}`;
  }
  if (String(field?.state) === "derived") {
    const inputs = (Array.isArray(field?.inputs) ? field.inputs : []).map((i) => `\`${String(i?.field ?? "")}\` = ${JSON.stringify(i?.value ?? null)}`);
    return `    ${name} = DERIVED: the engine computes this as \`${String(field?.expression ?? "")}\`` + (inputs.length ? `, from ${inputs.join(" and ")}. Write from those values and give this reading no value of its own.` : ". The card holds no input for it, so give this reading no value: what the engine decided about it is what the pool key and the page lines already say.");
  }
  if (String(field?.state) === "unreadable") {
    const note = String(field?.note ?? "");
    return `    ${name} = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)${note ? ` — ${note}` : ""}`;
  }
...
function buildScribeUserTurn(input) {
  ...
  if (page.length) {
    ...
    for (const row2 of page) parts.push(row2);
  }
  parts.push("");
...
function fillsFor(card, unit) {
  ...
  return (Array.isArray(pool?.slots?.fills) ? pool.slots.fills : []).map((f) => `    {${String(f?.slot ?? "")}} is ${JSON.stringify(String(f?.value ?? ""))} on this town, and the page prints it`);
}
...
function buildTier1Checklist(units, card) {
  ...
  for (const row2 of rows) {
    if (row2.unit !== at) {
      ...
      for (const row3 of corpusFor(card, unit)) lines.push(row3);
    }
