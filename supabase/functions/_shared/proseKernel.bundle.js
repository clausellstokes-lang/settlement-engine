...
function judgeUnits(units, card, refute) {
  ...
  for (const unit of Array.isArray(units) ? units : []) {
    ...
    for (const text2 of texts) {
      ...
      try {
        out = refute(
          { text: text2, blockId: unit.blockId, poolKey: unit.poolKey },
          card,
...
function tier1Lines(units) {
  ...
  for (let u = 0; u < all.length; u += 1) {
    ...
    const labelled = [
      ...
      ...faces.map((text2, i) => ({ row: `face ${i}`, text: String(text2 ?? "") })),
      ...notebook.map((text2, i) => ({ row: `notebook ${i}`, text: String(text2 ?? "") }))
    ];
...
function applyTier1(units, answers) {
  const all = Array.isArray(units) ? units : [];
