  ...
  if (fields.length) {
    ...
    for (const field of fields) {
      if (field?.unknown === true) {
        rows.push(String(field?.state) === "unreadable" ? `    ${String(field?.field ?? "")} = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)` : `    ${String(field?.field ?? "")} = UNKNOWN (the engine has not decided this; assert nothing that depends on it)`);
        continue;
      }
...
function fieldsFor(card, unit) {
  const pool = cardPool(card, String(unit?.blockId ?? ""), String(unit?.poolKey ?? ""));
  return (Array.isArray(pool?.fields) ? pool.fields : []).map((f) => {
    if (f?.unknown !== true) return `    ${String(f?.field ?? "")} = ${JSON.stringify(f?.value ?? null)}`;
    return String(f?.state) === "unreadable" ? `    ${String(f?.field ?? "")} = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)` : `    ${String(f?.field ?? "")} = UNKNOWN (the engine has not decided this; assert nothing that depends on it)`;
  });
}
