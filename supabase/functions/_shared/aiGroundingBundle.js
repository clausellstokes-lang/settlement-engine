...
function deriveFoodSecurity(s) {
  ...
  if (fs) {
    if (typeof fs.surplusMonths === "number" && fs.surplusMonths >= 3) {
      score += 5;
      push(contributors, "economicState.foodSecurity", "surplus", 5, `${fs.surplusMonths}-month grain surplus.`);
    }
    if (typeof fs.deficitMonths === "number" && fs.deficitMonths > 0) {
      const d = Math.min(15, fs.deficitMonths * 4);
      score -= d;
      push(contributors, "economicState.foodSecurity", "deficit", -d, `${fs.deficitMonths}-month food deficit.`);
    }
  }
