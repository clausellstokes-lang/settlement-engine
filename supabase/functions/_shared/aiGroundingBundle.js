...
function legitimacyFor(faction, settlement) {
  ...
  const isGoverning = typeof faction === "object" && faction?.isGoverning === true || !!(govName && factionName && govName.toLowerCase() === factionName.toLowerCase());
...
function deriveRecentDisruption(settlement) {
  ...
  const recentAnn = anns.filter(
    /** @param {any} a */
    (a) => (Number.isFinite(a.yearsAgo) ? a.yearsAgo : Infinity) <= 50
  ).sort(
    /** @param {any} a @param {any} b */
    (a, b) => (Number.isFinite(a.yearsAgo) ? a.yearsAgo : Infinity) - (Number.isFinite(b.yearsAgo) ? b.yearsAgo : Infinity)
  )[0];
  if (recentAnn) {
    return {
      key: "recentDisruption",
