...
// src/data/stressorSpinePhrases.js
var STRESSOR_SPINE_PHRASES = Object.freeze({
  under_siege: {
...
function governingFactionOf(settlement) {
  const ps = settlement?.powerStructure || {};
  const factions = Array.isArray(ps.factions) ? ps.factions : [];
  return factions.find((f) => f?.isGoverning) || factions.find((f) => nameOf(f) && nameOf(f) === String(ps.governingName || "")) || null;
}
...
function labelOf(value) {
  ...
  const row = (
    /** @type {{ label?: unknown, name?: unknown }} */
    value
  );
...
function lowerLabel(label) {
  ...
  const isMinorWord = (word) => /^(?:of|the|and|to|a|an|or|in|on)$/i.test(word);
  const isCapitalized = (word) => /^[^a-zA-Z]*[A-Z]/.test(word);
...
function joinPhrases(phrases, cap = 3) {
  ...
  if (shown.length === 1) joined = shown[0];
    ...
    joined = carriesConjunction ? `${shown[0]}, and ${shown[1]}` : `${shown[0]} and ${shown[1]}`;
...
function composeSentence(rung, answer) {
  return rung.mode === "statement" ? asSentence(answer) : asSentence(`${rung.frame} ${answer}`);
}
function stressorEntries(settlement) {
  ...
  return asList(raw).map((entry) => typeof entry === "string" ? { label: entry } : entry).filter(
    /** @type {(entry: unknown) => entry is Record<string, unknown>} */
    ((entry) => !!entry && typeof entry === "object")
  );
}
function stressorTypeKey(entry) {
  const type = firstNonEmpty(entry.type);
  return type && Object.prototype.hasOwnProperty.call(STRESSOR_SPINE_PHRASES, type) ? type : null;
}
...
function deriveSurvivesBy(s) {
  ...
  const exports_ = asList(eco.primaryExports).map((entry) => nounPhrase(entry)).filter(Boolean).map((entry) => lowerLabel(
    /** @type {string} */
    entry
  ));
...
function deriveRuledBy(s) {
  ...
  const governing = nameOf(governingFactionOf({ powerStructure: power })) || firstText(power.governingName);
...
function derivePeopleFear(s) {
  ...
  const fromStressors = joinPhrases(
    /** @type {string[]} */
    fears,
    2
  );
  if (fromStressors) return fromStressors;
  const threats = asList(s.defenseProfile?.threats).map((threat) => nounPhrase(typeof threat === "string" ? threat : labelOf(threat))).filter(Boolean).map((threat) => lowerLabel(
    /** @type {string} */
    threat
  ));
...
var STABILITY_ARC = Object.freeze([
  ["crisis", /critical|desperate|siege/],
  ["test", /unstable|volatile/],
  ["continuity", /stable/]
]);
var LIKELY_FUTURE_ARCS = Object.freeze(
  STABILITY_ARC.map(([key]) => key)
);
function likelyFutureFacts(settlement) {
  const s = (
    /** @type {Record<string, any>} */
    settlement && typeof settlement === "object" ? settlement : {}
  );
  const tensions = asList(s.history?.currentTensions).map(tensionName).filter(Boolean).map((tension) => lowerLabel(
    /** @type {string} */
    tension
  ));
...
var SPINE_ARC_COMPLEMENT = Object.freeze({
...
function deriveLikelyFuture(s) {
  ...
  if (facts.arm === "stability") {
    return SPINE_ARC_COMPLEMENT[
      /** @type {LikelyFutureArc} */
      facts.arc
    ];
  }
  return "whatever the table decides to make it";
}
var PLACEHOLDER_SPINE = Object.freeze({
...
function deriveLabor(s, ctx) {
  ...
  return {
    supply: clamp2(supply),
    demand: clamp2(demand),
    supplyContributors,
    demandContributors
  };
}
function deriveHealing(s, ctx) {
  ...
  if (healers >= 3) {
    ...
    supply += 4;
    push2(supplyContributors, "availableServices.healing", "services_only", 4, `${heal.careServices.length} healing service(s) offered without a dedicated institution.`);
  } else {
    supply -= 10;
...
function deriveCraft(s, ctx) {
  ...
  if (rawTier === "major" || rawTier === "standard") {
    ...
    supply += 2;
    push2(supplyContributors, "config.tradeRouteAccess", rawTier, 2, "Seasonal pass supplies raw materials while it is open.");
  }
  const pop = populationOf2(s);
  if (pop >= 5e3) {
    demand += 12;
...
function deriveMagicProfile(settlement) {
  ...
  if (ledger.present && ledger.magicLevel === "none" && institutionsByPattern(settlement, ARCANE_INSTITUTION_PATTERN).length === 0) {
    return {
      magicExists: true,
      availability: "absent",
      legality: "absent",
...
var LIKELY_FUTURE_BEAT_TEXT = Object.freeze({
...
// src/domain/hookThemes.js
var HOOK_THEMES = Object.freeze([
  "hidden_debt",
  "forbidden_love",
  "secret_identity",
  "divided_loyalty",
...
var UNTYPED = "untyped";
var THEME_SET = new Set(HOOK_THEMES);
function hookThemeKey(text) {
  return String(text || "").toLowerCase().replace(/\s+/g, " ").replace(/^["'\s]+|["'\s.!?]+$/g, "").trim();
}
var AUTHORED_HOOK_THEMES = Object.freeze(
...
var THEME_OF_REL_ARCHETYPE = Object.freeze({
...
var THEME_BY_KEY = new Map(AUTHORED_HOOK_THEMES.map(([template, theme]) => [hookThemeKey(template), theme]));
function themeOfText(text) {
  return THEME_BY_KEY.get(hookThemeKey(text)) || UNTYPED;
}

// src/domain/dossier/hookRetention.js
var DEFAULT_RETENTION_TUNING = Object.freeze({
...
function fallbackK(tuning) {
  return Math.max(...Object.values(tuning.kByBand));
}
function retentionK(scaleBand, tuning = DEFAULT_RETENTION_TUNING) {
  const named = typeof scaleBand === "string" ? tuning.kByBand[scaleBand] : void 0;
  return Number.isFinite(named) ? (
    /** @type {number} */
    named
  ) : fallbackK(tuning);
}
...
function interestOf(hook, tuning = DEFAULT_RETENTION_TUNING) {
...
var DM_HOOK_KINDS = /* @__PURE__ */ new Set(["hook", "plotHook"]);
function editedHookTextKeys(settlement) {
...
function retainHooks(hooks, options = (
  /** @type {RetainOptions} */
  {}
)) {
  ...
  const edited = options.editedTextKeys || /* @__PURE__ */ new Set();
  const clocked = options.clockAnchoredKeys || /* @__PURE__ */ new Set();
  const K = retentionK(options.scaleBand, tuning);
  const budget = /* @__PURE__ */ new Map();
  const contenders = /* @__PURE__ */ new Map();
  ...
  hooks.forEach((hook, index) => {
    ...
    contenders.get(theme).push(index);
  });
...
function deriveAllStructuredHooks(settlement, options = {}) {
  if (!settlement) return [];
  const structured = (
...
var QUARTER_CATEGORY = Object.freeze({
...
function canonicalArchetypesById(settlement) {
  ...
  for (const f of factions) {
    ...
    if (!name) continue;
    out.set(`faction.${snakeCase8(name)}`, factionArchetype(f));
  }
  return out;
}
function inferDominantFaction(category, profiles, canonicalById) {
  const preferences = CATEGORY_TO_ARCHETYPE2[category] || [];
