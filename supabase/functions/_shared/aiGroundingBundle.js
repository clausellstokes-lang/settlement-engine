...
function deriveRulingAuthority(s) {
  ...
  if (governingName && profiles.length) {
    ...
    const gov2 = profiles.find((p) => p.name && lower.includes(p.name.toLowerCase().split(/[\s/(]/)[0]));
    if (gov2 && typeof gov2.power === "number") {
      const c = Math.round((gov2.power - 30) * 0.5);
      if (c !== 0) {
        score += c;
        push(contributors, gov2.id, "governing_power", c, `${gov2.name} commands power ${gov2.power}.`);
      }
    }
  }
  for (const cond of deriveAllActiveConditions(s)) {
