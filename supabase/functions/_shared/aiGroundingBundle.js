...
function slugify(value, { sep = "-", max = 0, fallback = "", empty = "", raw = false } = {}) {
  const base = raw ? String(value) : String(value || empty);
  const escSep = sep.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  let out = base.toLowerCase().replace(/[^a-z0-9]+/g, sep).replace(new RegExp(`^${escSep}+|${escSep}+$`, "g"), "");
