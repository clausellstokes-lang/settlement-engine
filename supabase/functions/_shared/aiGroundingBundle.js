...
function deriveTradeConnectivity(s) {
  ...
  if (trade === "major") {
    score += 18;
    push(contributors, "config.tradeRouteAccess", "major", 18, "Settlement sits on a major trade route.");
  } else if (trade === "minor" || trade === "standard") {
    score += 8;
    push(contributors, "config.tradeRouteAccess", String(trade), 8, `Settlement has ${trade} trade access.`);
  } else if (trade === "none" || trade === "isolated") {
    score -= 12;
    push(contributors, "config.tradeRouteAccess", String(trade), -12, "Settlement is isolated from regional trade.");
  }
  const chains = deriveAllSupplyChainStates(s);
...
function deriveFoodProduction(s, ctx) {
  ...
  if (trade === "major") {
    supply += 6;
    push2(supplyContributors, "config.tradeRouteAccess", "major", 6, "Major trade route supplements food.");
  }
  const pop = populationOf2(s);
  if (pop >= 5e3) {
    demand += 15;
...
function deriveTransport(s, _ctx) {
  ...
  if (trade === "major") {
    supply += 22;
    push2(supplyContributors, "config.tradeRouteAccess", "major", 22, "Major trade route.");
  } else if (trade === "minor" || trade === "standard" || trade === "road") {
    supply += 12;
    push2(supplyContributors, "config.tradeRouteAccess", String(trade), 12, `${trade} trade access.`);
  } else if (trade === "none" || trade === "isolated") {
    supply -= 15;
    push2(supplyContributors, "config.tradeRouteAccess", String(trade), -15, "Isolated from regional transport.");
  }
...
function deriveCraft(s, ctx) {
  ...
  if (trade === "major" || trade === "minor" || trade === "standard" || trade === "road") {
    supply += 4;
    push2(supplyContributors, "config.tradeRouteAccess", String(trade), 4, "Trade route supplies raw materials.");
  }
  const pop = populationOf2(s);
  if (pop >= 5e3) {
    demand += 12;
