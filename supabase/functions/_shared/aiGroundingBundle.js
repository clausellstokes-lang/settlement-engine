...
function deriveSurvivesBy(s) {
  ...
  const exports = Array.isArray(eco.primaryExports) ? eco.primaryExports : Array.isArray(eco.exports) ? eco.exports : [];
...
function canonicalSupplyChainStatus(legacyStatus) {
  ...
  return (
    /** @type {any} */
    LEGACY_TO_CANONICAL[legacyStatus] || "stable"
  );
}
...
function inferRegionalPressures(chain, settlement, conditions) {
  ...
  return (conditions || deriveAllActiveConditions(settlement)).filter((condition) => REGIONAL_CHAIN_ARCHETYPES.has(condition.archetype)).filter((condition) => conditionMatchesChain(condition, chain, haystack)).map((condition) => ({
    id: condition.id,
...
function deriveSupplyChainState(chain, settlement, conditions) {
...
function deriveAllSupplyChainStates(settlement) {
  ...
  return chains.map((c) => deriveSupplyChainState(c, settlement, conditions)).filter(Boolean);
}
...
function factionArchetype(faction) {
  ...
  if (cat && /** @type {Record<string, string>} */
  CATEGORY_MAP[cat]) return (
...
function deriveFactionArchetype(faction) {
  ...
  return (
    /** @type {Record<string, string>} */
    CANONICAL_TO_PROFILE[factionArchetype(faction)] || "other"
  );
}
...
function templateForArchetype(archetype) {
  const t = (
    ...
    ARCHETYPE_TEMPLATES[archetype] || ARCHETYPE_TEMPLATES.other
  );
  return {
    wants: [...t.wants],
    fears: [...t.fears],
...
function deriveAllFactionProfiles(settlement) {
  ...
  return factions.map(
    /** @param {any} f */
    (f) => deriveFactionProfile(f, settlement)
  );
}
...
// src/data/npcData.js
var TRAIT_PRESENCE_DISTRIBUTION = Object.freeze({
  both: 0.4,
  flawOnly: 0.3,
  temperamentOnly: 0.2,
  neither: 0.1
...
var _regexIdSets = /* @__PURE__ */ new Map();
...
function institutionMatchesRegex(inst, re) {
...
function archetypeFromCategory(category) {
  ...
  return CATEGORY_TO_ARCHETYPE[
    /** @type {keyof typeof CATEGORY_TO_ARCHETYPE} */
...
function templateForArchetype2(archetype) {
  const t = NPC_TEMPLATES[
    /** @type {keyof typeof NPC_TEMPLATES} */
    archetype
  ] || NPC_TEMPLATES.other;
  return {
    leverage: [...t.leverage],
...
function consequencesForRemoval(archetype, rank) {
  const archetypeMap = REMOVAL_CONSEQUENCES[
    /** @type {keyof typeof REMOVAL_CONSEQUENCES} */
    archetype
  ] || REMOVAL_CONSEQUENCES.other;
  const normalizedRank = String(rank ?? "minor").toLowerCase();
  const consequences = (
    /** @type {any} */
    archetypeMap[normalizedRank] || archetypeMap.minor || []
  );
  return [...consequences];
}
...
var CATEGORY_HINT_ALIASES = Object.freeze({
  crafts: "craft",
  magic: "arcane",
  noble: "government"
});
function inferInstitutionName(npc, settlement) {
  ...
  const category = (
    /** @type {keyof typeof CATEGORY_INSTITUTION_HINTS} */
    CATEGORY_HINT_ALIASES[
      /** @type {keyof typeof CATEGORY_HINT_ALIASES} */
      rawCategory
    ] || rawCategory
  );
...
var CATEGORY_INSTITUTION_TAGS = Object.freeze({
...
function inferInstitutionLink(npc, settlement) {
  ...
  return name ? `institution.${snakeCase4(name)}` : null;
}
function inferPrimaryRelationship(npc, settlement) {
  ...
  const candidates = rels.filter(
    /** @param {any} r */
    (r) => r.npc1Id === npc.id || r.npc2Id === npc.id
  );
  if (candidates.length === 0) return null;
  const withTension = candidates.find(
    /** @param {any} r */
    (r) => typeof r.tension === "string" && r.tension
  );
...
function deriveAllNpcProfiles(settlement) {
  ...
  return npcs.map(
    ...
    (n) => deriveNpcProfile(n, settlement)
  ).filter(Boolean);
}
...
function tradeRouteTier(value) {
  ...
  return (
    /** @type {Record<string, any>} */
    ROUTE_TIER[String(value).toLowerCase()] || "unknown"
  );
}
...
function defenseLedger(settlement) {
  ...
  return {
    ...
    readinessScore: num2(dp.readiness?.score, 50),
...
var LIFT_ARCHETYPES = new Set(WAR_RECOVERY_CONDITIONS);
...
function deriveEconomicCapacity(s) {
  ...
  for (const cond of cachedActiveConditions(s)) {
    if (cond.archetype === "war_spoils") {
      const magnitude2 = Math.round(cond.severity * 12);
      score += magnitude2;
      push(contributors, cond.id, "spoils", +magnitude2, `${cond.label} sustains the war economy (capped).`);
      continue;
    }
...
function deriveLawOrder(s) {
  ...
  const lawCount = institutions.filter((i) => institutionIsLawOrder(i)).length;
  if (lawCount >= 2) {
    score += 10;
  ...
  if (lawDir !== 0) {
    ...
    push(
      ...
      lift,
      `${deity.name || "The patron deity"} (${deity.lawAxis}) ${lawDir > 0 ? "strengthens law & order" : "erodes order and tolerates corruption"}.`
    );
  }
  return { score, contributors };
}
...
function deriveReligiousAuthority(s) {
  ...
  const deity = s.config?.primaryDeitySnapshot;
  if (deity && /** @type {Record<string, number>} */
  DEITY_RANK_AUTHORITY[deity.rankAxis] != null) {
    const lift = (
      /** @type {Record<string, number>} */
      DEITY_RANK_AUTHORITY[deity.rankAxis]
    );
    score += lift;
    push(
      contributors,
...
function deriveCausalState(settlement) {
  ...
  for (const name of SYSTEM_VARIABLES) {
    if (!settlement) {
      ...
      const { score, contributors } = (
        /** @type {any} */
        DERIVERS[name](settlement)
      );
...
function deriveAvailability(settlement, contributors) {
  ...
  const tmpl = (
    /** @type {any} */
    MAGIC_LEVEL_VALUES[magic] || MAGIC_LEVEL_VALUES.low
  );
...
function deriveRisk(settlement, causal, contributors) {
  ...
  const base = (
    /** @type {any} */
    MAGIC_LEVEL_VALUES[magic]?.baseRisk || "low"
  );
...
function deriveThreatProfile(source, _settlement) {
  ...
  const tmpl = (
    /** @type {Record<string, any>} */
    THREAT_TYPE_TEMPLATES[type] || THREAT_TYPE_TEMPLATES.other
  );
...
function institutionNamesMatching(settlement, pattern) {
  const inst = Array.isArray(settlement?.institutions) ? settlement.institutions : [];
  return inst.filter((i) => institutionMatchesRegex(i, pattern)).map((i) => i?.name || "");
}
...
function deriveAllCapacities(settlement) {
  ...
  const summary = (
    /** @type {Record<string, any[]>} */
    { surplus: [], adequate: [], strained: [], critical: [], collapsed: [], absent: [] }
  );
  for (const name of CAPACITY_NAMES) {
    const { supply, demand, supplyContributors, demandContributors } = (
      /** @type {any} */
      DERIVERS2[name](settlement, ctx)
    );
...
function deriveRecentDisruption(settlement) {
  ...
  const recent = events.filter(
    /** @param {any} e */
    (e) => severityScore(e.severity) >= SEVERITY_RANK.major
  ).filter(
    /** @param {any} e */
    (e) => (Number.isFinite(e.yearsAgo) ? e.yearsAgo : Infinity) <= 30
  ).sort(
    /** @param {any} a @param {any} b */
    (a, b) => (Number.isFinite(a.yearsAgo) ? a.yearsAgo : 0) - (Number.isFinite(b.yearsAgo) ? b.yearsAgo : 0)
  );
  if (recent.length > 0) {
    const top = recent[0];
    return {
  ...
  const recentAnn = anns.filter(
    /** @param {any} a */
    (a) => (a.yearsAgo || Infinity) <= 50
  ).sort(
    /** @param {any} a @param {any} b */
    (a, b) => (a.yearsAgo || 0) - (b.yearsAgo || 0)
  )[0];
  if (recentAnn) {
    return {
      key: "recentDisruption",
...
function deriveStructuredHook(rawWrapper, settlement) {
  ...
  const consequences = (
    /** @type {Record<string, any>} */
    ORIGIN_CONSEQUENCES[origin] || ORIGIN_CONSEQUENCES.other
  );
...
function deriveOutsiderImpressions(s, ctx) {
  ...
  const strainedCaps = VISIBLE_CAPACITY_LENSES.filter((n) => ["strained", "critical", "collapsed"].includes(ctx.capacities.bands[n]));
  const parts = [];
  if (dominantFaction) {
...
var __test__ = Object.freeze({ deriveOutsiderImpressions });
...
function inferHook(category, quarter, conditions, threats) {
  ...
  return (
    /** @type {any} */
    defaults[category] || defaults.other
  );
}
...
function canonicalRelationshipLabel(label) {
...
function inferRelationshipType(rawNeighbour) {
  ...
  if (legacy && /** @type {Record<string, string>} */
  LEGACY_RELATIONSHIP_MAP[legacy]) return (
    /** @type {Record<string, string>} */
    LEGACY_RELATIONSHIP_MAP[legacy]
  );
...
function deriveRegionalGraph(settlement) {
  ...
  const nodes = [
    ...
    ...links.map(
      /** @param {any} l */
      (l) => ({ id: l.to, name: l.toName, role: "neighbour" })
    )
  ];
...
function topHooksBySeverity(settlement, n) {
  ...
  const sorted = [...all].sort((a, b) => {
    const aw = (
      /** @type {Record<string, number>} */
      HOOK_SEVERITY_ORDER[a.severity] || 0
    );
    const bw = (
      /** @type {Record<string, number>} */
      HOOK_SEVERITY_ORDER[b.severity] || 0
    );
    return bw - aw;
  });
