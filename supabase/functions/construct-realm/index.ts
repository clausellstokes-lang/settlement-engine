/**
 * Supabase Edge Function: construct-realm — S6 REALM CONSTRUCTION
 * (DESIGN_AI_CONTROL_SURFACE §2 stage 6). intent → campaign/regional CONFIG (the instant-world
 * composer's realmSize/tone/mapKind knobs) + declared CONSTRAINTS ("the config vocabulary IS the
 * op"). It writes NO state — it returns validated realm knobs + constraints; the CLIENT composes
 * deterministically (composeInstantWorld — PLACES EVERYTHING, CANONIZES NOTHING until commit),
 * the deterministic comparator (intentComparator.js, generalized to realm scale) judges, and the
 * DM revises (bounded, delta-only) then commits (instantWorld + runSpatialCanonize).
 *
 * Same spine as construct-settlement at the BALANCED routing class: bot guard → auth →
 * account_is_active → entitlement → kill-switch('constructRealm') → governor → credit
 * (feature 'constructRealm') → BYOK-or-server key → adapter → THE SCHEMA WALL
...
// TOKEN EFFICIENCY (aiTaskConfig constructRealm): routing 'balanced' → sonnet (a bounded
...
const CONSTRUCT_FEATURE = 'constructRealm';
const CONSTRUCT_STAGE = 'constructRealm';
const CONSTRUCT_KIND: 'realm' = 'realm';
const CONSTRUCT_TIMEOUT_MS = 75_000;
const CONSTRUCT_SPEND_ESTIMATE_USD = 0.08;
const MAX_BODY_BYTES = 192 * 1024;
const MAX_OUTPUT_TOKENS = 1800;                      // aiTaskConfig constructRealm.maxTokens
const SLICE_BUDGET = { maxSlices: 6, maxChars: 2500 }; // aiTaskConfig constructRealm.sliceBudget
const TOKEN_BUDGET = 12000;                          // aiTaskConfig constructRealm.tokenBudget
...
export async function handleConstructRealm(
  ...
  const guard = botGuard(req, 'construct-realm');
  ...
  try {
    ...
    if (entErr) logError('construct-realm', user.id, `has_surveyor_entitlement errored: ${entErr.message}`, { stage: 'entitlement' });
    if (entitled !== true) return json({ error: 'AI construction is part of the Surveyor plan. You can still compose realms by hand (Instant World).' }, 403, cors);

    const { data: stageData, error: stageErr } = await supabaseAdmin.rpc('surveyor_stage_enabled', { p_stage: CONSTRUCT_STAGE });
    if (stageErr) logError('construct-realm', user.id, `surveyor_stage_enabled errored: ${stageErr.message}`, { stage: 'kill-switch' });
    ...
    const providerKey = await resolveProviderKey(
      ...
      (note) => logError('construct-realm', user.id, note, { stage: 'byok' }),
      supabaseUser,
    );
    ...
    const applyProviderError = async (cls: ProviderErrorClass) => {
      ...
      if (providerKey.byok && health) {
        ...
        } catch (e) { logError('construct-realm', user.id, e, { stage: 'health' }); }
      }
    };
    ...
    try {
      ...
      if (preErr) logError('construct-realm', user.id, `usage_precheck errored: ${preErr.message}`, { stage: 'governor' });
      ...
      if (pr && pr.allowed === false) {
        ...
        } catch (e) { logError('construct-realm', user.id, e, { stage: 'governor-audit' }); }
    ...
    const outcome = await runCreditedCall({
      async reserve() {
        ...
        if (error) logError('construct-realm', user.id, `reserve_ai_spend errored: ${error.message}`, { stage: 'spend-cap' });
      ...
      async rateLimit() {
        ...
        if (error) { logError('construct-realm', user.id, `rate_limit errored: ${error.message}`, { stage: 'rate-limit' }); return { allowed: true }; }
      ...
      async spend() {
        ...
        if (error) { logError('construct-realm', user.id, `spend_credits errored: ${error.message}`, { stage: 'spend' }); throw new Error('spend_failed'); }
      ...
      async refund(spendId, reason, elevated) {
        ...
        try { const { error } = await supabaseAdmin.rpc('refund_credits', { spend_ledger_row: spendId, refund_reason: reason }); if (error) logError('construct-realm', user.id, error.message, { stage: 'refund', spend_id: spendId }); }
        catch (e) { logError('construct-realm', user.id, e, { stage: 'refund', spend_id: spendId }); }
      },
      ...
      async meter(ok) {
        ...
        } catch (e) { logError('construct-realm', user.id, e, { stage: 'metering' }); }
      },
    }, 'construct realm compile failed');
    ...
    try {
      ...
      if (error) logError('construct-realm', user.id, `write_ai_operation_log failed: ${error.message}`, { stage: 'audit' });
    } catch (e) { logError('construct-realm', user.id, e, { stage: 'audit' }); }
    ...
    if (outcome.outcome === 'ok') {
      try {
        ...
        if (error) logError('construct-realm', user.id, `construct answer event failed: ${error.message}`, { stage: 'eval' });
      } catch (e) { logError('construct-realm', user.id, e, { stage: 'eval' }); }
    }
    try {
      ...
      if (rider) {
        ...
        if (error) logError('construct-realm', user.id, `construct rider event failed: ${error.message}`, { stage: 'rider' });
      }
    } catch (e) { logError('construct-realm', user.id, e, { stage: 'rider' }); }
    ...
    switch (outcome.outcome) {
      ...
      case 'model_failed':
        return json({
          error: capturedRefused
            ? 'The constructor declined this request.'
            : (capturedRefusalMessage || 'Construction failed. Your credits were refunded.'),
          refused: capturedRefused,
      ...
      case 'ok':
        ...
        return json({
          config: capturedResult.config,          // validated realm knobs (realmSize/tone/mapKind, schema-walled)
    ...
    logError('construct-realm', null, `outer handler error: ${(e as Error).message}`, { stage: 'outer' });
    return json({ error: 'The construct-realm request failed. Please try again.' }, 500, cors);
  }
}

serve((req) => handleConstructRealm(req));
