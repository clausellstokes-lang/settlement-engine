...
Deno.test("summarizeSettlement grounds terrain in terrainType, never the 'auto' UI sentinel", () => {
  // resolveConfig persists terrainOverride VERBATIM when the route is not
  // random_trade, so the default wizard config carries BOTH terrainType (the
  // route-derived terrain) AND terrainOverride:'auto'. The model must be
  // grounded in the former; 'auto' is not a terrain.
  const summary = summarizeSettlement({
    name: 'Port Hale', tier: 'town', population: 900,
    config: { terrainType: 'coastal', terrainOverride: 'auto', tradeRouteAccess: 'port' },
  }) as Record<string, unknown>;
  assertEquals(summary.terrain, 'coastal');

  const legacy = summarizeSettlement({
    name: 'Old Hollow', tier: 'village', population: 300,
    config: { terrainOverride: 'auto' },
  }) as Record<string, unknown>;
  assertEquals(legacy.terrain, null);

  const override = summarizeSettlement({
    name: 'Cragfast', tier: 'village', population: 250,
    config: { terrainOverride: 'mountain' },
  }) as Record<string, unknown>;
  assertEquals(override.terrain, 'mountain');

  // Imported dossiers can carry the sentinel in ANY leg (the engine only
  // writes it to terrainOverride) — no leg may ground the model in 'auto'.
  const tampered = summarizeSettlement({
    name: 'Wanderers Rest', tier: 'village', population: 150,
