...
// as a forced tool, so an unregistered bucket or an out-of-enum value stops being something
// the model emits and the wall rejects. The free-text parse stays as the fallback.
import {
...
import {
  ...
  contentRepairViolations, mergeContentCompiled,
...
const CONTENT_TOOL = buildOutputTool(
  'customContent',
  'Submit the proposed custom-content entries, each filed into a registered bucket, plus anything the manifest cannot express.',
);
const CONTENT_TOOL_CHOICE = forceOutputTool(CONTENT_TOOL);
...
async function callAnthropic(apiKey: string, model: string, prompt: string, providerFetch: typeof fetch, signal: AbortSignal, thinkingBudget = 0): Promise<Response> {
  return providerFetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model, max_tokens: MAX_OUTPUT_TOKENS, tools: [CONTENT_TOOL], tool_choice: CONTENT_TOOL_CHOICE, ...thinkingClause(thinkingBudget), messages: [{ role: 'user', content: splitForAnthropic(prompt) }] }),
  });
}
...
export async function handleCustomContent(
  ...
  try {
    ...
    const resolvedModel = resolveCapturedModel({
      byok: providerKey.byok, modelPref: capturedModelPref, surfaceDefault: CONTENT_MODEL,
    });
    ...
    const outcome = await runCreditedCall({
      ...
      async callModel() {
        capturedPrompt = buildContentPrompt(intent, clientDescriptor, bundle, anchorLabel, canary, SLICE_BUDGET, renderCoachingFor({
          profile: providerKey.probeProfile, probeModel: providerKey.probeModel,
          capturedModel, surface: 'customContent',
        }));
        ...
        const loop = await runWithRepair({
          ...
          roundTimeoutMs: CONTENT_TIMEOUT_MS,
          deadline: Date.now() + CONTENT_TIMEOUT_MS * (repairRounds + 1),
          usage: repairUsage,
          callModel: ({ prompt, signal }) => draftOnce(prompt, signal),
          parse: (text) => compileCustomContent(text, clientDescriptor),
          validate: (compiled) => contentRepairViolations(compiled.draft),
          merge: ({ accepted, repaired }) => mergeContentCompiled(accepted, repaired),
        });
      ...
      async meter(ok) {
        try {
          // TOKENS ARE SUMMED ACROSS REPAIR ROUNDS (wave L-6): the provider-reported
          // numbers accumulate in repairUsage, and the chars/4 fallback accumulates
          // beside them. The Math.max keeps the pre-wave single-round values exactly:
          // with one round the sum IS the round, and on a throw before any answer the
          // fallback is still the base prompt / empty answer estimate it always was.
