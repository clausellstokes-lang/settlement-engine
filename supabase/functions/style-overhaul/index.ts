...
export async function handleStyleOverhaul(
  ...
  try {
    ...
    capturedModel = (providerKey.byok && capturedModelPref && ANTHROPIC_SUPPORTED_MODELS.includes(capturedModelPref)) ? capturedModelPref : STYLE_MODEL;
    ...
    const outcome = await runCreditedCall({
      ...
      async callModel() {
        capturedPrompt = buildStylePrompt(prompt, vocab, bundle, anchorLabel, canary, SLICE_BUDGET);
        ...
        const compiled = compileStyleOverhaul(capturedAnswerText, vocab);
