#!/bin/sh
# run-ratchet-914.sh — census totals at the §914 consist tip (stamped). Quiet-window law; exits captured.
# ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT.
D=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad/laneINSTR2
cd $D || exit 9
STREAK=0; WAITED=0
...
echo "PREDICTED (E4, §914): totalFiles 2491 -> 2497 (+6: the six INSTR-912 tests/lint walkers — institutionTable, proseEntryContradiction, proseMeasures, proseMoveGrammar, proseRegisterLoaders, proseWiringCensus), entries 3 unchanged (no owed-ledger car; no banked row moves), totalTests REFUSED in advance — this run derives it (the lighting census credits +72 titles since §913, a hint not a prediction); NO ceiling(s) HELD line may print — a HELD line is a STOP"
