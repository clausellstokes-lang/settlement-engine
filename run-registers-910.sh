...
echo "PREDICTIONS (E4, §910, re-derived at the dock tip in predict-910.log BEFORE this runs): lighting — titles 23662 -> 23665 (+3: the SOAK-HONEST-909 cars new it() titles in tests/soak-harness — two by cars 2 and 3, one by car 5, the ordered twin guard — read by the farmed probe in predict-910.log at 04:52), suiteTitles 6335 UNCHANGED, files 2543 / parked 373 / credited 2170 UNCHANGED (no new file); OSR plain read 1972 exact, 0 drift, NO write; mounts unchanged; prose-numerics 225 exact; writer-reach dry HOLD with no refusal — provenance-only refresh expected (as at §906 and §908); tuning plain green (the lifted constants keep their values)"
...
echo "=== 1. LIGHTING (refuses a dirty tree — first):"; $M env LIGHTING_CENSUS_REFREEZE='Fable 5.1 chair — §910 SOAK-HONEST landing' LIGHTING_CENSUS_NOTE='the SOAK-HONEST-909 consist (five capacity-honesty cars over the §908 CAS at the tip; re-derived by the farmed probe printed in run-registers-910.sh); three it() titles added, no test file added' npx vitest run tests/lint/sovereigntyLightingContract.walker.test.js > $SC/reg910-lighting.log 2>&1; echo "LIGHTING_DOOR_EXIT=$? (non-zero EXPECTED)"; $M npx vitest run tests/lint/sovereigntyLightingContract.walker.test.js > $SC/reg910-lighting-plain.log 2>&1; echo "LIGHTING_PLAIN_EXIT=$? (must be 0)"; grep -E 'Tests ' $SC/reg910-lighting-plain.log | cut -c1-60
git add tests/lint/.lighting-census-baseline.json && git commit -q -m "§910 SOAK-HONEST landing (register car 1): the lighting census refreezes at the tip — titles 23662 -> 23665 (three it() titles the SOAK-HONEST-909 cars added; files, parked, credited and suiteTitles unchanged)

Derived by the farmed read-only probe before the door (predict-910.log at 04:52 and the probe/register pair in this script's log): the ONLY movement is titles 23662 -> 23665 (+3, the SOAK-HONEST-909 cars in tests/soak-harness: tripwireRegistry +2, soakInvariants +1) — suiteTitles 6335, files 2543, parked 373, credited 2170 unchanged; the door exited non-zero by design and the plain re-run is the receipt.
