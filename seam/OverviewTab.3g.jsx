...
export function OverviewTab({ settlement:r, narrativeNote, onNavigateTab, publicDossier = false, playerView = false, worldState = null}) {
  ...
  // THE STRESS LIST AND THE GENERAL DESK, BOTH MEMOISED AND BOTH ABOVE THE EARLY RETURN
  // (rules-of-hooks, the note on `mobile` above). ⭐ WHY THE DESK IS MEMOISED AT ALL (ARCH
  // §4.1, X-F9, SEAM car 3g): this call composes eleven of the desk's blocks, and from car 3
  // it composes them THROUGH `composeStateProse` rather than through a single kernel read.
  // The browser-side cost of that is unmeasured and the composed-prose manifest cannot
  // measure it, so the memo is insurance taken before the cost exists rather than after a
  // page feels slow. The result is a pure function of the settlement and the two view flags,
  // and `stresses` is memoised beside it so the dependency is stable rather than a fresh
  // array on every render.
  const stresses = useMemo(
    () => (Array.isArray(r?.stress) ? r.stress : r?.stress ? [r.stress] : []).filter(Boolean),
    [r],
  );
  ...
  }).overview, [r, publicDossier, playerView, stresses]);
  if (!r) return null;
