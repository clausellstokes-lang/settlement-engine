...
[ "$(git merge-base HEAD f73bdbf16d3f7a57c18d7fd57b0478b953043a73)" = "f73bdbf16d3f7a57c18d7fd57b0478b953043a73" ] || { echo "REFUSED: HEAD does not descend from f73bdbf16"; exit 8; }
...
echo "PROOF_HEAD=$(git rev-parse HEAD) start $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')" > $SC/typecheck-920.log
npm run typecheck:ratchet >> $SC/typecheck-920.log 2>&1; echo "TYPECHECK_EXIT=$? $(date '+%H:%M:%S')" >> $SC/typecheck-920.log
echo "PROOF_HEAD=$(git rev-parse HEAD) start $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')" > $SC/whole-920.log
sh scripts/gate-mutex.sh --run -- npx vitest run >> $SC/whole-920.log 2>&1; E=$?
echo "PROOF_EXIT=$E $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')" >> $SC/whole-920.log
