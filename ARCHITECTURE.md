...
> **Settlement-map transition (2026-08-20).** The current app still ships the legacy
> `src/domain/townMap`/TownScene/PDF paths. The unlanded future geometry lives in the map sandbox,
> not this source tree. The controlling build architecture is
> `map-corpus/docs/GENERATION-SPEC.md` §§6–10 and ODQ §287: repaired sandbox `fabric/**` becomes
> the single headless geometry authority; one addressable semantic scene feeds screen, PDF, SVG,
> thumbnail and hit-testing; dimensional/authorship code is design-only until its prerequisite
> waves land. Do not extend `townCartography` or `arch/**` into parallel settlement-map truths.
...
- **migrations/** (136) — prod applied head tracked in `supabase/applied-head.json`,
