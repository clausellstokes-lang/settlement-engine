...
grep -q '^GATE_CARS=25$' "$LOG" || { echo "⛔ gate did not run over 25 cars: $(grep '^GATE_CARS=' "$LOG")"; exit 1; }
...
python3 $SC/chair-verify.py "$D" "$BASE" "$LOG" 25 > $SC/chair-verify-901.out 2>&1 || { tail -6 $SC/chair-verify-901.out; echo "⛔ chair-verify RED — stop"; exit 1; }
