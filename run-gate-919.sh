#!/bin/sh
# run-gate-919.sh — the §919 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base 29ec62425. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
# a background task reports its LAST command's exit, and a trailing printf once reported 0 over a red gate.
D=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/4e3d2f70-f45f-4e14-b571-514c339cfa17/scratchpad/kit/laneREWRITE
BASE=29ec62425faa8e4c4e61b34e38ff871cc8e599a9   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse 29ec62425faa8e4c4e61b34e38ff871cc8e599a9)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base 29ec62425 — land the previous consist first"; exit 7; }
...
[ "$CARS" = "27" ] || { echo "REFUSED: expected 27 cars over $BASE, found $CARS"; exit 7; }
