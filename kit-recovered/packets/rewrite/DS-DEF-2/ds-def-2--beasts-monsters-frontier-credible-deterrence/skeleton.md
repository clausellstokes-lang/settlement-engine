...
Three variants, in the annex's order (`RECEIPT_POOLS_DOSSIER_STATE.md:2594-2597`). Tags: `SAFE` (nothing in the record denies it — the common case, and silence is permission) · `CONTRADICTED` (with the field, file and line, and which of the two is the record) · `FLOOR-2` (a magnitude outside the read's band word, an elapsed course, or a dependence on an unobserved field).
...
| the town has **a LINE** | **CONTRADICTED** — `forces.walls.present` is satisfied by `Citadel`, `Inner citadel` (inner keeps) and `Gates (if walled)` (a point), `defenseInstitutionBuckets.js:84-89`, `:169-182`; **F1-07**. **The roster row is the record.** |
