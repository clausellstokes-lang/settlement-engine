#!/bin/sh
# run-gate-913.sh — the §913 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base 3b1c0eaa5. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
...
BASE=3b1c0eaa51f77561a036ae7ec54682c39856192c   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse 3b1c0eaa51f77561a036ae7ec54682c39856192c)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base 3b1c0eaa5 — land the previous consist first"; exit 7; }
