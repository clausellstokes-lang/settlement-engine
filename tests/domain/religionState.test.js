...
describe('religionState — DM assign-deity reconciliation (SET_PRIMARY_DEITY)', () => {
  ...
  // a settlement carrying an existing pantheon (chief A, rival B) from prior pulses
  const priorState = () => ({
    deities: {
  ...
  it('a fresh assign (no prior state) seeds the patron as sole chief', () => {
    const s = ensureReligionState(null, { tier: 'town', config: { primaryDeitySnapshot: A } }, 'town');
    expect(s.chiefRef).toBe(ref('Aurum'));
    expect(s.deities[ref('Aurum')].share).toBe(100);
  });

  it('a RE-assign to a NEW deity installs it as the dominant chief (not overwritten by the stale chief)', () => {
    const s = ensureReligionState(priorState(), { tier: 'town', config: { primaryDeitySnapshot: C } }, 'town');
    expect(s.chiefRef).toBe(ref('Vael'));                       // DM is authoritative
  ...
  it('a RE-assign to an EXISTING non-chief deity promotes it to chief', () => {
    const s = ensureReligionState(priorState(), { tier: 'town', config: { primaryDeitySnapshot: B } }, 'town');
    expect(s.chiefRef).toBe(ref('Korl'));
    expect(s.deities[ref('Korl')].standing).toBe('ascendant');
  });

  it('in-sync (assigned === current chief) is a no-op on the chief', () => {
