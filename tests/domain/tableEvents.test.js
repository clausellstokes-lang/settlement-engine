...
// FOLD SEAM (pass 3): tableEvents MIRRORS V-F's tableLedger vocabulary. The import-
// unification the seam originally called for was REVERTED at fold — tableEvents is
// eager (campaignSlice imports it), so importing the otherwise-lazy tableLedger drags
// its whole module into the first-paint closure (+1,863 B measured, against a razor
// budget). The mirror stays; THIS pin is the value-identity guard the seam actually
// needs — it re-imports tableLedger directly and proves the shared vocabulary matches,
// so any future drift on either module fails here (not in prod). Zero eager cost (a
// dev-only test). Recorded follow-on: a shared-constants leaf both modules import
// would single-source without the eager pull.
describe('tableLedger mirror parity (fold seam pass 3)', async () => {
  const ledger = await import('../../src/domain/tableLedger.js');
  it('the closed kinds are the SAME frozen list as tableLedger', () => {
    expect([...TABLE_EVENT_KINDS]).toEqual([...ledger.TABLE_EVENT_KINDS]);
  });
  it('the provenance stamp is the SAME literal as tableLedger', () => {
    expect(TABLE_EVENT_SOURCE).toBe(ledger.TABLE_EVENT_SOURCE);
  });
  it('the band names are exactly the keys of tableLedger MAGNITUDE_BANDS', () => {
    expect([...MAGNITUDE_BANDS]).toEqual(Object.keys(ledger.MAGNITUDE_BANDS));
  });
  it('every band maps to the SAME bounded number tableLedger records', () => {
    for (const band of MAGNITUDE_BANDS) {
      expect(bandMagnitude(band)).toBe(ledger.MAGNITUDE_BANDS[band]);
    }
  });
});
