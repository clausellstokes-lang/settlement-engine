 ...
 * relative-path/import got broken in the split, the dynamic import or the
 * render below throws and this test fails.
 *
 * We mock the store (the wizard reads ~20 selectors) plus the analytics,
 * saves, and flags helpers so the mount path stays quiet and doesn't pull
 * network/Supabase wiring into the test. HomeHero is stubbed to keep the
 * test focused on the extracted pieces; with no wizardMode and no
 * settlement (signed-in), the empty-state branch renders the extracted
 * ModeSelector, whose stable copy we assert on.
 */
...
// Stub HomeHero — it drags in WelcomeBackCard / AnonTierTeaser and a
// pile of its own store selectors. The extraction under test is the four
// generate/* sub-components, so keep the empty-state surface focused.
...
describe('GenerateWizard — decomposition smoke', () => {
  test('mounts without throwing and renders the mode selector', async () => {
    ...
    // The empty-state (signed-in) branch renders the extracted ModeSelector.
    // Pinning its stable card labels means a broken extraction (e.g. a wrong
    // relative import in generate/ModeSelector.jsx) surfaces here.
