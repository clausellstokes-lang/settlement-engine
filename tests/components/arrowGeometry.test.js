...
import {
  ...
  STRIP_W, UNDERLINE_ROW, WOOD_RUNS, WORD_FLOOR, WORD_ROWS, SEAM_BAND_TO, SEAM_HANG_FROM,
...
describe('(b) the measured tables agree with each other', () => {
  ...
  it('the slip sits inside the plate field, clear of both rivets, and the underline row is under every descender', () => {
    ...
    expect(UNDERLINE_ROW + 2).toBeLessThan(BAND_H);
  });
...
describe('(e) the hit rectangles', () => {
  it('ordered, disjoint, in bounds, each widened only by its own slots, at least 24 px at the floor', () => {
    ...
    for (let cw = 1009; cw <= 3840; cw += 7) {
      ...
      for (const id of ids) {
        ...
        const u = hits.underline[id];
        if (u.x < hits.nav[id].x || u.x + u.w > hits.nav[id].x + hits.nav[id].w) failures.push(`${cw}: ${id} underline outside its region`);
        if (Math.abs(u.y - UNDERLINE_ROW * s) > EPS) failures.push(`${cw}: ${id} underline row`);
      }
  ...
  it('the compact arrow has home, plate and slip and no painted-word regions', () => {
    for (const cw of [320, 390, 640, 1023]) {
      ...
      expect(Object.keys(hits.underline)).toEqual([]);
