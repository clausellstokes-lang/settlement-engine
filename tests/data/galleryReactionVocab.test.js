...
const MIG_146 = resolve(process.cwd(), 'supabase', 'migrations', '146_gallery_reactions.sql');
...
describe('gallery reaction vocabulary (GALLERY-2 phase 2)', () => {
  ...
  it('migration 146_gallery_reactions.sql exists (renumber must fail loudly)', () => {
    expect(existsSync(MIG_146), `migration missing: ${MIG_146}`).toBe(true);
  });
