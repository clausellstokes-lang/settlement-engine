 ...
 *
 * NO LONGER a blind spot either (2026-09-01, the COUPLED consist's landing):
 * both detectors scanned RAW source, so a config key written in PROSE counted
 * as a READ. A picked car's docblock in src/generators/generateSettlementPipeline.js
 * cites `vite.config.js's computeEngineSharedDomain()`; the alias dot-scan
 * censused a config key named `js`, the predicate refuses it, and this walker
 * red with no defect behind it. Both source texts now pass through the estate's
 * ONE shared strip, tests/helpers/codeOnlySource.js (`codeOnly`), INSIDE the
 * detector functions, so comments and string CONTENTS are blanked before every
 * regex and the census measures code. The class, stated once: a detector making
 * a USE claim must read CODE, because a key cannot be read from inside a comment.
 * MEASURED at the cure: the reader census falls 75 -> 72 at the landing tip and
 ...
 *
 * RECORDED, NOT PRUNED (a review-phase row, never a landing act): five
 * CONFIG_PATCH_EXTRA_KEYS entries in src/store/configSlice.js become
 * non-derivable once the census reads code only -- `eventConditions`, `faith`,
 * `latentPantheon`, `primaryDeitySnapshot`, `seed`. They are INERT for this
 * walker, which asserts census SUBSET-OF allowlist and never the reverse, and
 * `seed` is pinned admitted by tests/store/updateConfigPatchValidation.test.js.
 * Pruning them would change saved-config LOAD behaviour, which is a product
 * decision and not a landing one.
 */
...
function pipelineReadKeys() {
  ...
  for (const file of walkFiles(resolve(ROOT, 'src/generators'), ['.js'])) {
    // `codeOnly` FIRST: a key named in a comment or inside a string is prose,
    // not a read, and this census is a USE claim.
...
function componentWrittenKeys() {
  ...
  for (const file of walkFiles(resolve(ROOT, 'src/components'), ['.js', '.jsx'])) {
    // Stripped first, and the short-circuit deliberately reads the STRIPPED
    // text: an `updateConfig(` inside a comment is not a write.
    const src = codeOnly(readFileSync(file, 'utf-8'));
