 ...
 *
 * Phase 46 shipped `src/domain/aiGrounding.js` — the central composer for
 * the AI prompt envelope. Per its own header comment:
 *
 *   "in this commit we ship the envelope shape + helpers; the edge
 *    function wiring is a follow-up that swaps its bespoke extraction
 *    for buildAiGroundingPayload."
 *
 * Until that swap lands (a substantial Deno-side refactor — the edge
 * function depends on every `deriveAll*` module in src/domain/), the
 * edge function MUST stay aligned with the envelope contract by a
 * drift-detector. That's this file.
 *
