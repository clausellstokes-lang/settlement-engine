...
const ARGUED_FIELD_SPELLERS = Object.freeze({
  ...
  'src/domain/worldPulse/envoyErrandVocabulary.js':
    ...
    + ' — as a closed five-member set, and three of the five ARE these field names, because'
    ...
    + ' legRef outside the five, and it refuses `pullBand` and `exports` specifically'
    + ' because no espionage product can fill either — SP-B\'s own populations road feeds'
    + ' the first, and the second has no belief slot at all (admitted at ES-1, declared dead'
    + ' at ES-3, cut at EP-r). Renaming a field here'
