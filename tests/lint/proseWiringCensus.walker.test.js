...
describe('the wiring census — TOTALITY over the corpus', () => {
  test('every (block, pool) the loaders enumerate has exactly ONE census row', () => {
    ...
    // ⛔ 708 -> 715 AT TASTE car M-2, AND THE SPLIT IS ASSERTED RATHER THAN THE SUM. The dock
    // births the estate's first SEVEN modifier pools; the corpus is 715 cells and 708 of them
    // are spines, which is the population every other integer in this file is pinned on.
    expect(cells.size, 'the R1 loaders enumerate this many pool cells').toBe(715);
    expect(census.rows.length, 'and the census carries exactly one row for each').toBe(715);
    expect(spines.length, 'of which SPINES, the population every corpus figure is pinned on').toBe(708);
    expect(modifiers.length, 'and MODIFIERS, the taste\'s seven').toBe(7);
    const ids = new Set(census.rows.map((r) => `${r.block} :: ${r.pool}`));
    expect(ids.size, 'no (block, pool) is counted twice').toBe(715);
...
describe('car 0 — `reads`, and the NARROWS line that may narrow it', () => {
  test('reads = the SELECTING BRANCH\'s tests where one was recovered, function-wide where none was', () => {
    ...
    expect(committed.rows.length, 'the census still covers every pool').toBe(715);
...
describe('car 0 — `covert`, `objectClass` and the numeric key', () => {
  ...
  test('the object class is read off the KEY, from the closed list the census emits', () => {
    ...
    // ⭐ AND THE TASTE'S OWN (TASTE car M-2): `stores: import-fed` names a STORE and a MARKET,
    // which is why the T-F12 refusal must read the SET on the modifier side too.
    expect(modifierRows(committed.rows).filter((r) => (r.objectClasses || []).length > 1)
      .map((r) => r.pool)).toEqual(['stores: import-fed']);
    for (const row of committed.rows) {
