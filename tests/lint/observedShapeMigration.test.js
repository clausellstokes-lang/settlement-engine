...
import {
  ...
  GENESIS_TIES_TARGET_SCHEMA,
  TREASURY_ADMISSION_TARGET_SCHEMA,
  ...
  LINEAGE_REANCHOR_TARGET_SCHEMA,
  EXEMPTION_RETIREMENT_TARGET_SCHEMA,
...
describe('observed-shape schema-2 -> schema-4 heuristic migration', () => {
  ...
  test('the CLI derives its target from the inputs and refuses a contradictory pair', () => {
    ...
    try {
      ...
      ])).toThrow(/predecessor baseline must be a schema-20 object/);
      ...
      // ⭐ THE INVERSE PROBE MOVES UP A RUNG TOO, for the reason written just above: 16
      ...
      // the probe moved to 21 — a probe left at 20 would be asserting that the LIVE target
      ...
      // day, so the probe moves to 22 and the message names 21 as the live leaf.
      expect(() => runMigration([
        `--predecessor=${predecessorPath}`, `--legacy=${legacyPath}`, '--target-schema=22',
      ])).toThrow(/--target-schema must be 21/);
  ...
  test('every leaf target pairs by TABLE, and a schema-2 predecessor cannot reach the live one', () => {
    ...
    expect(TREASURY_ADMISSION_TARGET_SCHEMA).toBe(11);
    expect(GENESIS_TIES_TARGET_SCHEMA).toBe(12);
    expect(DEAD_DEPENDENCY_TARGET_SCHEMA).toBe(13);
    ...
    // ⚠ 17 IS NOW A RETIRED TARGET AND KEEPS ITS NUMBER, by the same law as 8 above.
    ...
    // ⚠ 20 IS NOW A RETIRED TARGET AND KEEPS ITS NUMBER, by the same law as 8 above.
    ...
    // ⭐ 21 IS THE FIRST RUNG WHOSE DECLARATION IS A MOVE. It admits one DECLARED-identity
    // address (DS-REL-1's list assembler reading `neighbourNetwork`), which `rowTagsOf` tags
    // like all 25 of its siblings, so the bank goes 60/39 -> 61/40 and the fence has a
    // disagreement it can actually catch.
    ...
    expect(LEAF_MIGRATION_PREDECESSOR).toEqual({
      4: 2, 5: 4, 6: 5, 7: 6, 8: 7, 9: 8, 10: 9, 11: 10, 12: 11, 13: 12, 14: 13, 15: 14,
      // ⭐ 16 -> 15 AT §893.4, the companion-gate rung. The chain stays SINGLE-STEP: 16
      ...
      17: 16,
      // ⭐ 18 -> 17, the lineage-reanchor rung. The chain stays SINGLE-STEP: 18 pairs
      // only with 17, so a schema-16 predecessor cannot reach the live number by skipping
      // the rung that re-anchored the receipt inside the lineage carrying the register.
      18: 17,
      // ⭐ 19 -> 18, the exemption-retirement rung. The chain stays SINGLE-STEP: 19 pairs
      ...
      20: 19,
      // ⭐ 21 -> 20, the relationships-mount rung. The chain stays SINGLE-STEP: 21 pairs
      // only with 20, so a schema-19 predecessor cannot reach the live number by skipping
      // the very rung whose fence the bank growth 60/39 -> 61/40 had to pass.
      21: 20,
    });
    ...
    expect(() => heuristicMigrationReport(fixture.predecessor, fixture.legacy, text, 22))
      .toThrow(/leaf migration target must be 4 or 5 or 6 or 7 or 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19 or 20 or 21;/);
