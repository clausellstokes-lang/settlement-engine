...
const CENSUS = Object.freeze([
  ...
  },
  {
    ...
    why: 'THE VEIL HOLDS AT THE DOOR, NOT BY A STRIP: the account import MINTS a fresh campaign world and reads the incoming one NOWHERE — `accountImportBody.js` hands `createImportedCampaign` exactly `{settlementIds, contentBinding, contentBindingHistory}`, `campaignImportedCreation.js:57` answers `worldState: createNewCampaignWorldState(...)`, and `importReconciliationAdmission.js:396` DECLARES the refusal as `world_state_not_imported`. So although a paused advance can park this snapshot durably on a campaign record, no foreign snapshot can ever arrive on one. PINNED in A6, because that premise is the whole sentence. (The way OUT — campaigns ride the export payload by reference — is real and is EM-B3f\'s, export-only, a train later.)',
  },
  {
