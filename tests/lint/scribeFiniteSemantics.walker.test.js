...
const DECLARED_IMPORTERS = Object.freeze({
  ...
  'src/lib/scribeEpochStamp.js': 'THE EPOCH RECORD AT ADVANCE TIME (ruling 21). Reads whether a settlement carries a rendered survey and parks the typed record of the advance on it through the writer; builds the card pair from the two SETTLEMENTS, never from stored prose, so nothing here reads prose as an input to anything.',
  ...
  'src/components/dossier/ScribeSurveyNotes.jsx': 'THE DM\'S SURVEY NOTES (ruling 6). Reads the epoch\'s receipt rows so the game master can see which pools drew the written corpus and on which arms; writes nothing, changes no line, and never renders on a player or public surface.',
  'src/components/new/tabs/DailyLifeTab.jsx': 'THE DAILY LIFE TAB (design §5c rule 4). Reads the five DS-DAILY beats the render landed and draws them in place of its Generate button; writes nothing. It is a DISPLAY reader, which is the one class the fence exists to permit — `src/domain/display/` is exempt for the same reason.',
