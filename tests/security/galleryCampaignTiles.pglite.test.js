...
const MIG_147 = resolve(dir, '147_gallery_title_aliveness_columns.sql');
const MIG_149 = resolve(dir, '149_gallery_maps_campaign_tiles.sql');
const allExist = existsSync(MIG_147) && existsSync(MIG_149);
...
describe('gallery campaign-tiles migration fixtures exist (guards against silent vacuous skip)', () => {
  it('147 + 149 are present (a renamed/renumbered file must fail loudly)', () => {
    ...
    expect(existsSync(MIG_149), `missing: ${MIG_149}`).toBe(true);
  });
});
...
describe.runIf(allExist)('publish_map + list_gallery_maps — execution against the real 149 SQL (pglite)', () => {
  beforeAll(async () => {
    ...
    await db.exec(readFileSync(MIG_149, 'utf-8'));
  }, 30000);
