...
const MIG_146 = resolve(dir, '146_gallery_reactions.sql');
const MIG_125 = resolve(dir, '125_action_velocity_guards.sql');
const allExist = existsSync(MIG_146) && existsSync(MIG_125);
...
describe('gallery-reactions migration fixture exists (guards against silent vacuous skip)', () => {
  it('146_gallery_reactions.sql is present (a renamed/renumbered file must fail loudly)', () => {
    expect(existsSync(MIG_146), `migration 146 missing: ${MIG_146}`).toBe(true);
...
describe.runIf(allExist)('gallery reactions — execution against the real SQL (pglite)', () => {
  beforeAll(async () => {
    const src146 = readFileSync(MIG_146, 'utf-8');
...
describe.runIf(allExist)('gallery reactions — RLS + grant posture pins (migration text)', () => {
  const sql = readFileSync(MIG_146, 'utf-8');
