 ...
 * gallery-comment perimeter/moderation layer (migration 168, V-27f).
 *
 ...
 * mirrors, and the REAL migration-168 SQL (the hidden-columns ALTER, the report
...
const MIG_168 = resolve(dir, '168_gallery_comment_moderation.sql');
const MIG_125 = resolve(dir, '125_action_velocity_guards.sql');
const allExist = existsSync(MIG_168) && existsSync(MIG_125);
...
describe('gallery-comment moderation migration fixture exists (guards a silent vacuous skip)', () => {
  it('168_gallery_comment_moderation.sql is present (a renumbered file must fail loudly)', () => {
    expect(existsSync(MIG_168), `migration 168 missing: ${MIG_168}`).toBe(true);
...
describe.runIf(allExist)('gallery comment moderation — execution against the real SQL (pglite)', () => {
  beforeAll(async () => {
    const src168 = readFileSync(MIG_168, 'utf-8');
    ...
    await db.exec(`
      ...
      -- Minimal gallery_comments mirror (019 shape, pre-168); the real 168 ALTER
    ...
    `);
    // The REAL migration-168 statements + the REAL 125 velocity counter.
    await db.exec(extractHiddenAlter(src168));
    await db.exec(extractTable(src168, 'gallery_comment_reports'));
    await db.exec(extractFn(src125, '_consume_action_rate_limit'));
    await db.exec(extractFn(src168, 'report_gallery_comment'));
    await db.exec(extractFn(src168, 'set_gallery_comment_hidden'));
    await db.exec(extractFn(src168, 'list_gallery_comments'));
  }, 30000);
...
describe.runIf(allExist)('gallery comment moderation — RLS + grant posture pins (migration text)', () => {
  const sql = readFileSync(MIG_168, 'utf-8');
