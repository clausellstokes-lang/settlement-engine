...
describe('FENCE 2 — the purity pin: no clock, no rng, no global state', () => {
  ...
  test('no leaf in the set reads a wall-clock or an rng', () => {
    // 4 since W-FAITH F6c: the tuning signature surface joined the set, so the purity
    // pin covers it too — a "tuned" value smuggling a clock or rng draw in reds here.
