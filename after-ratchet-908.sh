#!/bin/sh
# after-ratchet-908.sh <expected-totalFiles> <expected-entries> — the §908 version (from after-ratchet-906.sh by substitution 2026-09-07 01:25; the CAPSULE step is ACTIVE again — the OSR CLI is open at rung 18): the totals car (guarded on TRUE_EXIT,
...
# re-stamp of the kit at the FINAL car count, then the train seal. No owed-ledger car at §908 (entries predicted 3). The gate is
# launched SEPARATELY (run-gate-908.sh). Every exit captured; refuses on anything unexpected. POSIX sh.
SC=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad
D=$SC/laneB6; LOG=$SC/ratchet-908.run.log; BASE=4243bdc610fe5b380f1d0029973cf9088bae1631; SEAL=captrip-2026-09-07
...
echo "--- capsule car (ACTIVE at rung 18 — the OSR CLI is open):"; sh scripts/gate-mutex.sh --run -- node scripts/base-state-capsule.mjs --runtime-tests=$TT > $SC/capsule-908.log 2>&1; CE=$?; echo "CAPSULE_EXIT=$CE"; tail -3 $SC/capsule-908.log | cut -c1-160
[ "$CE" = "0" ] || { echo "REFUSED: the capsule did not regenerate — read capsule-908.log"; exit 7; }
CP=$(git status --porcelain -uall); echo "capsule porcelain=[$(echo "$CP" | tr '\n' ' ')]"
if [ -n "$CP" ]; then git add docs/implementation/BASE_STATE.json && git commit -q -m "Register (capsule car): the base-state capsule regenerates at the §908 tip (rung 18 still; no OSR movement at this landing)
...
echo "--- re-stamp the kit at $N cars:"; python3 $SC/mk-landing-kit.py 908 laneB6 $BASE landing-captrip-2026-09-07 $N 2>&1 | tail -2
grep -q "^GATE_CARS=\|cars=$N\|EXPECT.*$N" $SC/run-gate-908.sh || grep -n "$N" $SC/run-gate-908.sh | head -2
git update-ref refs/preserve/$SEAL "$(git rev-parse HEAD)" && echo "SEALED $SEAL = $(git rev-parse --short refs/preserve/$SEAL)"
echo "NEXT: launch run-gate-908.sh in the background (quiet law; ~20 min; NO lane vitest meanwhile), then after-cas-908.sh, then fill texts-908 (__CARS__=$N __ENTRIES__ __TESTS__ __TOTALS__) and collect."
exit 0
