...
export const GROWTH_SUBSYSTEM_ROWS = Object.freeze([
  ...
  Object.freeze({
    ...
    module: 'src/domain/worldPulse/demographicsKernel.js,src/domain/worldPulse/demographicsRates.js,src/domain/worldPulse/demographicsPushPull.js,src/domain/worldPulse/demographicsMigration.js,src/domain/worldPulse/demographicsPlans.js,src/domain/worldPulse/demographicsResponses.js,src/domain/worldPulse/demographicsLand.js,src/domain/worldPulse/demographicsWorks.js,src/domain/worldPulse/demographicsRisk.js,src/domain/worldPulse/demographicsWar.js,src/domain/worldPulse/demographicsLadder.js,src/domain/worldPulse/demographicsHerald.js,src/domain/worldPulse/demographicsObservation.js',
    aliveness: Object.freeze({
    ...
    invariants: Object.freeze([
      Object.freeze({
        ...
        description: 'Lit, the demographic kernel is the ONLY source of population change that is not a TRANSFER. populationDynamics returns null for any positive delta under the same flag, so the two lanes can never both mint the same people, and since WAVE P4 it returns null for a bare decline too, so they can never both bury the same people either. This is design law 1 expressed as a countable absence in BOTH directions, and it is the one claim about this lane a receipt can settle today. WAVE P4 WIDENED IT, and the widening is the reconciliation: P1 left the decline term riding the legacy lane beside this engine death term, in writing, and two independent rolling soaks measured what that cost — a lane reading no bound of any kind, running as a one-way ratchet with no fixed point, taking 95.3% of a realm that ended at 4% of its start. A shrink that is not a death breaks law 1 exactly as a growth that is not a birth does.',
        check: 'In any receipt whose subsystems.rules records demographicsEnabled true, the summed eventTypeCounts for population_growth AND for population_decline are each exactly zero across every observed year, while population_emigration is unconstrained (a departure is a conserved TRANSFER with credited destinations, which is precisely the distinction the P4 gate draws). In any receipt recording demographicsEnabled false or omitting it, both are unconstrained. Expressible from v5 subsystems.rules plus the v4 eventTypeCounts.',
      }),
      Object.freeze({
      ...
      Object.freeze({
        name: 'the_realm_reading_is_gated_and_bounded',
        description: 'WAVE P4, design section 6. The realm carrying-capacity divergence is emitted per pulse as the additive yearly field realmDemography, on the terms J2 established for realmSelfSufficiency: it is CONDITIONAL on the wave being lit, so a dormant realm drops the key entirely rather than recording a zero that would read as infinite slack. It is bounded by construction on the same clamp the per-settlement pressure read uses, so the realm reading and the settlement reading share one scale and one ceiling, and an empty food denominator reads zero rather than dividing.',
        check: 'In any receipt whose subsystems.rules records demographicsEnabled false or omits it, no yearly record carries realmDemography. In any receipt where it is recorded true, every yearly realmDemography.realmPressure01 and realmDemography.loadRatio01 lies within 0 and 2 inclusive, and realmDemography.ladder carries all five viability grades as keys. Expressible from the v5 subsystems section plus the yearly records, with no behavioral evidence at all.',
      }),
      Object.freeze({
        name: 'risk_is_causal_and_never_a_population_target',
        description: 'WAVE P4, design law 5 and acceptance claim 10. Disease and raid incidence correlate with ENABLING CONDITIONS and never with a hidden population target. The coupling module is a function of one settlement own state and cannot reach a realm total, a settlement count or a target at all: demographicsObservation.js, which owns the realm reading, is not imported there and its import list is a closed set of two per-settlement readers. The realm reading exists and is used, but only as a MOTIVE weight in the war read, which is what law 6 says it is.',
        check: 'PARTIALLY expressible: in a receipt of at least one hundred observed years recording demographicsEnabled true, the disease and raid event series must not track the realm population series, but no envelope carries a per-settlement incidence series so the correlation is not computable from it. Pinned at source instead, by an anchored SOURCE SCAN asserting that the incidence module imports nothing but the kernel math primitives and the per-settlement readers, and by a guard-the-guard arm proving the same scan DOES find the realm token in the war module that is allowed it. The executed negative control makes the realm total reachable from the incidence deriver and the scan reds.',
      }),
      Object.freeze({
        name: 'the_curb_raises_conditions_and_never_the_disaster',
        description: 'WAVE P4, design law 5. The demographic coupling adds a bounded lift to two EXISTING pressure scores at the one seam incidence has, and emits no event, mints no candidate, writes no key and takes no draw. The disaster still fires on the stressor lane own birth-threshold gate and its own severity draw. The lift is continuous in the head count, which is what distinguishes it from every existing population contributor, all of which are steps at five thousand.',
        check: 'NOT expressible from any receipt schema today: no envelope carries the pressure scores that feed the stressor gate, only the events that came out of it. Pinned at source by driving the REAL pressure derivation over one settlement twice, dark and lit, asserting that exactly two of the nine pressure kinds move and that the seven others are byte-identical, and by measuring the disease lift at three populations all above five thousand and requiring three DISTINCT values. The executed negative control replaces the continuous crowding read with the same pop >= 5000 step the existing contributors use, and the three readings collapse to one.',
      }),
      Object.freeze({
        name: 'the_ladder_gates_a_mover_nothing_else_refused',
        description: 'WAVE P4, design section 7b. The viability ladder is a pure READ that names the rung wave P1a head-count gate already opened; it demotes no tier, closes no settlement and moves no person. Its permit table is closed on both axes, so an unknown grade and an unknown mover kind both read FALSE and a typo can only make the engine quieter. It bites where nothing bit before: a settlement whose bound has fallen through the population floor of the tier it wears is refused the founding lane by the name nonviable, having cleared every refusal the lane previously knew.',
        check: 'NOT expressible from any receipt schema today: no envelope carries the response menu or the grade. Pinned at source by driving the REAL response scorer over two readings that differ in exactly ONE way, whether the bound clears the tier floor, with the lane lit and the cap open and the ground open and the homeostat absorbing nobody in both arms; the viable arm is available and the nonviable arm is refused by name at weight zero. The executed negative control removes the gate from the availability expression and the nonviable arm reports available.',
      }),
      Object.freeze({
        name: 'the_herald_speaks_the_address_chain_and_no_number',
        description: 'WAVE P4, the NEWS ADDRESS LAW and the LEGIBILITY LAW. Every demographic line carries an id in the house shape (an id-less entry is refused by normalizeEntry AND skipped by the audit receipt sink, so it would reach no reader at all), the full address chain as settlementIds plus settlementNames, a typed cause, and a recorded reason. Counts are spoken through a closed quantity vocabulary, so no digit, percentage, ratio, band token or id reaches the prose. The lane mints NO new impactKind: the lines ride the existing hungry_gap and migration_flight kinds and are distinguished on the record by tags and sourceEventId.',
        check: 'PARTIALLY expressible: the entries reach the wizard-news feed and therefore the observation sink, so a receipt records their mover counts, but the mover classifier is contaminated by the wizard-news id token and no row may cite it. Pinned at source instead by building the lines from real receipt shapes and asserting the address chain field by field, that the prose matches no digit and no percentage, and that seven raw tokens including both settlement ids never appear. The executed negative control replaces the quantity vocabulary with the raw count and the no-digit assertion reds.',
      }),
    ]),
    // WAVE P4: the dispositive channel is instrumented ONLY by the v5 census, and every
    // completed release case is a v4 envelope, so no dispositive reading exists on disk
    // yet. That is the upswing row's situation exactly, and it takes the same honest
    // grade: `indirect` rather than `unobserved`. The distinction is load-bearing.
    // `unobserved` is the row telling the evaluator to downgrade its own zero readings,
    // which is the escape hatch the corpus guard ceilings; `indirect` says the soak
    // simply has not run against the instrument yet, and lets the SCHEMA decide, which
    // it does correctly: a v4 receipt has no census, so the channel is not derivable and
    // the verdict is UNOBSERVED anyway, while the first v5 receipt makes this row
    // answerable for real.
    soakEvidence: 'indirect',
  }),
]);
