...
| **C10** | (the identity worry this family has been bitten by) leash `settlementId` vs `beliefMaps` observer id | ✅ **CONFIRMED SAME ID SPACE — no bridge needed** | Both trace to `snapshot.settlements` / the same `regionalGraph.edges \|\| relationships` list: the leash endpoint via `hostileNeighborsOf` (`stressorDynamics.js:606-613`) and `mintLeashOnto`'s `patronIds` (`corruptionWeb.js:597-598`, which says *"the same id space"* inline); the belief observers via `relationshipNeighbourhood` (`beliefMap.js:707-731`, `:1247-1249`). ⚠ **Identity is not membership** — `beliefMaps` entries materialize only on arrival (`beliefMap.js:32-35`), so a leak may create FIRST-EVER entries: **the patron's whole observer slice, and its `(patron, subject)` row.** That is **O10**. ⛔ **NOT `(patron, home)` — that pair is UNREACHABLE from a cold patron.** Arm 2 is a CONFIRM, and `productGroundTruth` refuses a confirm with no prior (`espionageLeak.js:283-292` closes on `existence.refusal`) — the built-in no-fabrication refusal **O2** endorses. A patron who has never heard of home gains no row about home, by design; `(patron, home)` materializes only where a prior already exists. *(**Reworded per chair ruling H36, 2026-08-11.** This cell formerly asserted the unreachable `(patron, home)` claim. Case **A7(iii)** at §9 asserted it too and was reworded in the same change — the packet stated it in TWO places, and a rewording reaching only one would have left the contradiction alive in the more visible of them.)* |
...
| **A7** | ⭐ SILENT SUCCESS — the real writer-to-reader integration, and the hardest pin | Through `advanceEspionageProducts` on a real world, run the **same seed twice**: once with the operative clean, once with him leashed. Assert (i) the HOME observer's belief slice, the errand rows, `wizardNews` and every returned receipt except `leaks[]` are **byte-identical between the two runs**; (ii) the patron's slice differs; (iii) **the first-ever entries that are REACHABLE materialize — the patron's whole observer slice and its `(patron, subject)` row** — and that is the intended arrival semantics (**O10**); `(patron, home)` is pinned BOTH WAYS instead, **refused when the patron is cold and confirmed where a prior exists.** ⛔ Any home-visible difference is STOP. *(**Reworded per chair ruling H36, 2026-08-11.** This clause formerly required a first-ever `(patron, home)` entry, which **the arm that writes it cannot create**: arm 2 is a CONFIRM and `productGroundTruth` refuses a confirm with no prior — the no-fabrication refusal **O2** endorses. The no-fabrication law outranks an acceptance case's literal wording; O10's mechanism is intact and only this wording overreached, so this is a mis-worded case, not a broken ruling. **C10** at §0 carried the same claim and moved with this one.)* |
...
| **O9** | **THE GLOBAL CREDIBILITY BLEED.** The leaked report's `sourceId` draws on `spatialLedgers.npcCredibility`, which is keyed **by source id only, with no observer dimension** (`npcCredibility.js:137-138`) — so the patron's view of the agent and the home's view are the same number. | **ACCEPT IT AND RECORD IT.** A per-observer credibility surface is a new persisted record family and belongs to the information program, not to a double-agent wave. The shared stock is also arguably right: a man's reputation is one reputation. ⚠ **The consequence the chair should see:** if a future wave ever charges credibility for a detected leak, that charge will **bleed into the agent's home-side reputation too** — which may be exactly the drama wanted, or may be a bug, and it should be decided when it is built rather than discovered. ⚠ **PREMISE CORRECTED 2026-08-11 (chair ruling H35).** This question's opening sentence — *"the leaked report's `sourceId` draws on `spatialLedgers.npcCredibility`"* — **over-assumes a coupling the packet's own §6.1 signature never created.** As shipped, `deliverMissionLeaks`' signature (`espionageLeak.js:207-209`) is `{worldState, tick, landings, operatives, byId}` with **no `credibilityOf` closure**, matching `ES-6A.md:370` exactly, so the leaked `sourceId` does **not** draw on `npcCredibility` today. A second, live consequence stands in its place: the leak leaf holds the ROSTER ENTRY rather than the errand, so it spells `<home>#<npc.id>` (`espionageLeak.js:251`) while the home product spells `<home>#<errand.npcId>` = `durableId \|\| rosterId` (`espionageProductStage.js:727`, `espionageMissions.js:158`) — **for an operative already graduated into the durable ledger those two strings DIFFER, so one man carries TWO credibility addresses.** It is inert only because there is exactly one consumption site (`beliefMap.js:567`) and no closure is passed. ⛔ **"One man, one reputation" REMAINS THE LAW, and the reconciliation is a BINDING PRECONDITION on the first wave that ever passes a credibility closure into the leak** — not a nice-to-have. Until then the two-address state is a recorded deferral, declared in-source at `espionageLeak.js:66-73`. |
...
- **O9 ACCEPTED** (credibility is global, not per-observer — do not fork it) and
  **O10 RULED**: a first-ever belief entry is CREATED by the leak like any other, with no
  ...
  seeding rule.
  ⚠ **O9's PREMISE CORRECTED 2026-08-11 (chair ruling H35), and the correction BINDS a
  future wave.** The premise sentence at O9 above assumed the leaked `sourceId` draws on
  `spatialLedgers.npcCredibility`; **the shipped §6.1 signature never delivers a
  credibility closure, so it does not.** What is live instead is a TWO-ADDRESS divergence —
  the leak spells the roster id where the home product spells `durableId \|\| rosterId`, so a
  graduated operative carries two credibility addresses. **"A man's reputation is one
  reputation" REMAINS THE LAW.** The two-address state is a RECORDED DEFERRAL (declared
  in-source, inert today at one consumption site), and **the reconciliation is AFFIRMED AS
  BINDING on the first wave that passes a credibility closure into the leak** — that wave
  reconciles the addresses before it reads them, or it stops.
  ⚠ **O10 stands as ruled, but note what A7(iii) and C10 now say:** the reachable first-ever
  entries are the patron's observer slice and its `(patron, subject)` row. `(patron, home)`
  is NOT reachable cold — the CONFIRM arm's no-fabrication refusal (**O2**) forbids it —
  and both sites were reworded per H36 rather than a seeding rule being built. **O10's
  mechanism was never at issue; only the two cases' wording was.**
