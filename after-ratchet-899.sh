...
cd "$D"; N=$(git rev-list --count 5e28d5c83..HEAD); echo "CARS=$N (the stamped gate expects 28) · HEAD=$(git rev-parse --short HEAD) · porcelain=[$(git status --porcelain -uall | tr '\n' ' ')]"
[ "$N" = "28" ] || { echo "⛔ car count $N != 28 — re-stamp the kit (mk-landing-kit.py … $N) before the gate"; exit 7; }
