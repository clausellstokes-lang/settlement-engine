# ⛔ WITHDRAWN 2026-09-07 17:46 EDT — FAILED ADVERSARIAL VERIFICATION (the chair's ruling; see `refute-taste-912.md`, 39 CONFIRMED · 15 REFUTED · 11 PARTLY · 1 WITHHELD).
Four of five pairs carry a REFUTED item the chair upholds: ts-1 drops the self-naming claim on the canonical-at-zero line; ts-3's V4 label has no licence (the composer fills only {settlement, seat} on DS-POW-5 — `powerStateProse.js:895`); ts-4 raises the pool's settlement-token openers 1 → 2 and flattens its close kinds; ts-5 hardens a hedged comparative into a flat negation (a modality spent). The GAP demonstration (§3) rests on a PROBE ARTEFACT: `gen-probe.mjs` passed `{}` as readings, so `generalStateProse.js:1782` defaulted the history and `foundedPoolKey` returned GROWN-UNRECORDED for every seed; with the real history both towns are FOUNDED-OLD and the pool is unreached (48 of 48 settlements carry a founding). The three "grammars" of §2 reduce to two move orders, one of them unchanged from its BEFORE. What SURVIVES: the mechanical receipts, the two-seed draw of DS-POW-5 (indices 0 and 1, re-derived by the refuter — to be re-confirmed under real readings), the R4 lesson, and the finding that on today's composed fill sets an R1 STATE block licenses at most two move orders. A second cut (v2) follows under the refuter's cures 1b/3/6(ii)/7/8/9/10 and a corrected probe; nothing below is evidence for the owner.
