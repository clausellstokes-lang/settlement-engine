...
// politics projection needs a worldState and is left undefined, so only DS-POW-7 goes silent).
...
const { coupContenders } = await imp('src/domain/rulingPowerCoup.js');
...
const deskReadings = { ...(contenders ? { contenders } : {}), structuralLens: structuralLensOf(s) };
