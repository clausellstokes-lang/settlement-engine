...
for f in OWNER_DECISION_QUEUE.md FABLE_RETROVALIDATION_QUEUE.md HANDOFF_CURRENT.md REWRITE_RECUT_PROGRAM_PLAN.md; do
  [ -s "$L/$f" ] || { echo "ABORT: $L/$f missing"; exit 1; }
  git show HEAD:docs/$f > "$L/$f.head"
done
echo "--- C0 scan (must be 0 each) ---"; for f in OWNER_DECISION_QUEUE.md FABLE_RETROVALIDATION_QUEUE.md HANDOFF_CURRENT.md REWRITE_RECUT_PROGRAM_PLAN.md; do printf '  %s: ' $f; LC_ALL=C grep -c $'[\x01-\x08\x0b\x0c\x0e-\x1f]' "$L/$f" || true; done
for f in OWNER_DECISION_QUEUE.md FABLE_RETROVALIDATION_QUEUE.md; do
...
grep -qE '__[A-Z0-9_]+__' "$L/OWNER_DECISION_QUEUE.md" "$L/HANDOFF_CURRENT.md" "$L/FABLE_RETROVALIDATION_QUEUE.md" "$L/REWRITE_RECUT_PROGRAM_PLAN.md" && { echo "ABORT: unfilled placeholder"; exit 1; }
...
for f in HANDOFF_CURRENT.md FABLE_RETROVALIDATION_QUEUE.md REWRITE_RECUT_PROGRAM_PLAN.md; do cp "$L/$f" docs/$f; done
