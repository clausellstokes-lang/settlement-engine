...
function overviewBurgs(settings = {stateId: null, cultureId: null}) {
  ...
  function addBurgOnClick() {
    ...
    Settlements.add(point); // add new burg
  ...
  function downloadBurgsData() {
    let data = `Id,Settlement,Province,Province Full Name,State,State Full Name,Culture,Religion,Group,Population,X,Y,Latitude,Longitude,Elevation (${heightUnit.value}),Temperature,Temperature likeness,Capital,Port,Citadel,Walls,Plaza,Temple,Shanty Town,Emblem,Preview link\n`; // headers
    ...
    valid.forEach(b => {
      ...
      data += Settlements.getPreview(b).link;
  ...
  function triggerAllBurgsRemove() {
    ...
    confirmationDialog({
      ...
      confirm: "Remove",
      onConfirm: () => {
        pack.burgs.filter(b => b.i && !(b.capital || b.lock)).forEach(b => Settlements.remove(b.i));
        burgsOverviewAddLines();
      }
    });
  }
