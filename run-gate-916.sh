#!/bin/sh
# run-gate-916.sh — the §916 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base 3b22b5c56. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
# a background task reports its LAST command's exit, and a trailing printf once reported 0 over a red gate.
D=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/4e3d2f70-f45f-4e14-b571-514c339cfa17/scratchpad/kit/laneSEAM
BASE=3b22b5c569e5c93f709f2a6057e3a0c280ff18ac   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse 3b22b5c569e5c93f709f2a6057e3a0c280ff18ac)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base 3b22b5c56 — land the previous consist first"; exit 7; }
...
[ "$CARS" = "38" ] || { echo "REFUSED: expected 38 cars over $BASE, found $CARS"; exit 7; }
