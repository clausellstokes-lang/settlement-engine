...
const READ_SCHEMA = {
  ...
  properties: {
    ...
    tier1Yes: { type: 'object', properties: {
      certainty: { type: 'integer' }, quantifier: { type: 'integer' }, scope: { type: 'integer' }, actor: { type: 'integer' },
      forecast: { type: 'integer' }, mechanism: { type: 'integer' }, samePage: { type: 'integer' },
    }, required: ['certainty', 'quantifier', 'scope', 'actor', 'forecast', 'mechanism', 'samePage'] },
    mechanismLines: { type: 'array', items: { type: 'string' }, description: 'the lines you answered yes on question 6 for, verbatim' },
    samePageLines: { type: 'array', items: { type: 'string' } },
    judgeLine: { type: 'string', description: 'the judge\'s printed "verdicts {...} · kept N · dropped N (tier 1 took N) …" line verbatim' },
    notes: { type: 'array', items: { type: 'string' } },
  },
  required: ['poolsOnCard', 'unitsReturned', 'tier0Kept', 'tier0Dropped', 'tier1Dropped', 'finalKept', 'pass', 'withheld', 'fail', 'arms', 'tier1Yes', 'mechanismLines', 'samePageLines', 'judgeLine', 'notes'],
}
...
Step 2 — answer it AS THE MODEL WOULD. The product sends the second reader the SAME two cached system blocks the writer had, then the checklist as the user turn: so read brief.md and town.md whole (system blocks 1 and 2, in ${H}/out/sim/${it.seed}/${it.tab}/), then tier1-${it.seat}.md whole (the user turn) and tier1-schema.json whole. The checklist gives the facts and numbered lines and asks seven yes/no questions per line. Answer every line honestly and strictly from the facts printed in that file — nothing else — and write the JSON answers object (only the object) with the Write tool to:
...
const by = (key) => {
  ...
  for (const r of rows) {
    const k = r[key]
    if (!acc[k]) acc[k] = { cells: 0, pools: 0, returned: 0, tier0Kept: 0, tier0Dropped: 0, tier1Dropped: 0, finalKept: 0, pass: 0, withheld: 0, fail: 0, mechanism: 0, samePage: 0, arms: {} }
    const a = acc[k]
    ...
    a.mechanism += r.tier1Yes.mechanism; a.samePage += r.tier1Yes.samePage
