...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (process.argv.includes('--controls')) {
    ...
    process.stdout.write(`\nI5_CONTROLS ${controlsLive ? 'LIVE' : 'BROKEN'}\n`);
    ...
    // ⛔ C4 (§654 review): I5_FAIL used to exit 0. ⚠⚠ DECLARED BEHAVIOUR SHIFT, NOT A NEW RED:
    // the city plate's `street:ground` and `water:ground` floors are the RECORDED pre-existing
    // REG-0 charter-row state (both improving — 1.727→1.9118 and 1.1246→1.2762), so `i5` on the
    // city now exits 1 where it exited 0 yesterday. That is the fix working, not a regression:
    // C4's whole complaint is that a caller could `&&` past a recorded red and never see it.
