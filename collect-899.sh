...
[ -s "$SC/cas-899.sha" ] || { echo "ABORT: cas-899.sha missing/empty — CAS first"; exit 1; }
CAS=$(cat "$SC/cas-899.sha")
GOT=$(git rev-parse --verify refs/preserve/landing-composed-2026-09-05 2>/dev/null || echo MISSING)
...
[ -f "$SC/payload-899.json" ] || { echo "ABORT: payload-899.json missing"; exit 1; }
git show HEAD:docs/FABLE_RETROVALIDATION_QUEUE.md > "$SC/frq.head.899"
python3 "$SC/apply-892.py" "$SC/payload-899.json"
echo "--- C0 scan ---"; LC_ALL=C grep -c $'[\x01-\x08\x0b\x0c\x0e-\x1f]' docs/OWNER_DECISION_QUEUE.md docs/HANDOFF_CURRENT.md "$SC/queue-899.md" || true
git show HEAD:docs/OWNER_DECISION_QUEUE.md > "$SC/odq.head.899"
[ "$(git diff --no-index --numstat $SC/odq.head.899 docs/OWNER_DECISION_QUEUE.md | awk '{print $2}')" = "0" ] || { echo "ABORT: ledger deletions != 0"; exit 1; }
[ "$(git diff --no-index --numstat $SC/frq.head.899 $SC/queue-899.md | awk '{print $2}')" = "0" ] || { echo "ABORT: queue deletions != 0"; exit 1; }
echo "  ledger and queue are PURE APPENDS"
SP="$SC" sh "$SC/chair-tools/chair-commit.sh" "$TIP" "$SC/msg-899.txt" docs/HANDOFF_CURRENT.md:docs/HANDOFF_CURRENT.md "$SC/queue-899.md:docs/FABLE_RETROVALIDATION_QUEUE.md"
NEW=$(git rev-parse --short HEAD)
echo "--- READ-BACK AT $NEW ---"; git show --stat --format='%h %s' HEAD | head -6
echo "  ODQ §899 rows: $(git show HEAD:docs/OWNER_DECISION_QUEUE.md | grep -c '^§899 ')"
...
cp "$SC/queue-899.md" docs/FABLE_RETROVALIDATION_QUEUE.md
echo "COLLECT_899_OK tip=$NEW"
