...
- `{timeband_age}` is DS-CND-1's slot (not DS-STR-2's); no fill — DESK-TIMEBAND's refusal is pinned. Finding 5: the chair AUTHORED four duration-free variants (`timeband-words.md`, sealed) — a lane wires them at §900 (append variant 5 per pool; walker pins +1 each; both §0d tables byte-identical); the `anchored === undefined` darkness stays closed display-side and is an OWNER row producer-side; the adverbial sixth band is REFUSED (not display-local).
