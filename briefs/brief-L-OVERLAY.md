...
## TWO FACTS THE CHAIR READ AFTER WRITING THE ABOVE (test them, do not trust them)
- `lprobe/certify.sh` (lines 11–32) still documents "There is no `--preset`" and drives the soak through the `--rules-json` overlay from `preset-overlay.mjs` — the battery's certification ran on the OLD seam even though the tree at 38474a59e has DOCKET item 7's `--preset`.
- With `--preset <id>`, `whole-world-soak.mjs:326-327` composes `composeSoakRules({ preset: SOAK_PRESET.rules, … })` — `SOAK_PRESET.rules` is the SPARSE override table (`SIMULATION_RULE_PRESETS[id].rules`), which `preset-overlay.mjs:28-30` warns "would leak by construction on almost every key" IF spread over full_simulation; whether `composeSoakRules` then NORMALIZES it the way a real birth does (`prepareRulesUpdate` → `normalizeSimulationRules`) is exactly measurement 1(i) vs 1(ii). If the soak's `fullRules` for P equal the real-birth fixture's `resolvedRules` key-for-key, the `--preset` seam is honest and the old overlay's "leak" was the instrument's definition against a seam that no longer exists.
