#!/bin/sh
# predict-915.sh <dock> — the READ-ONLY prediction pass at the §915 dock tip BEFORE any register door (E4: every register figure is derived,
...
D=${1:-$SC/laneMEASURE}; cd "$D" || exit 9
...
echo "PREDICT_HEAD=$(git rev-parse HEAD) cars_over_8522a17b2=$(git rev-list --count 8522a17b2..HEAD) start $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')"
