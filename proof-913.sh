#!/bin/sh
# proof-913.sh — the §913 whole-suite PROOF at the L-MAT tip (EXPECT_CARS cars over 3b1c0eaa5: the lane cars) IN laneLMAT
...
# Every exit captured in-shell. Read `python3 chair-tools/reds-by-block.py whole-913.log` after. Drafted 2026-09-07 03:08 from proof-908.sh by substitution (chair, session b43943b4); a QUIET-WINDOW probe precedes the suite (S12 runs no vitest; the skeptic pass must be COMPLETE first).
...
echo "PROOF_HEAD=$(git rev-parse HEAD) start $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')" > $SC/typecheck-913.log
npm run typecheck:ratchet >> $SC/typecheck-913.log 2>&1; echo "TYPECHECK_EXIT=$? $(date '+%H:%M:%S')" >> $SC/typecheck-913.log
echo "PROOF_HEAD=$(git rev-parse HEAD) start $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')" > $SC/whole-913.log
sh scripts/gate-mutex.sh --run -- npx vitest run >> $SC/whole-913.log 2>&1; E=$?
echo "PROOF_EXIT=$E $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')" >> $SC/whole-913.log
