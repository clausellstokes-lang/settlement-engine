...
| 8 | **NO-DRIFT TRACE** `i8-nodrift-trace.mjs` | roster re-derived from renderFolio's own numbered section headers, checked against the committed trace table | **38** op classes at the REG-4 seal (31 at REG-1's; the table grew with the roster, REG-I1); an untraced class is a countable miss | **0 untraced misses, 0 stale rows**; every DRIFT row must carry a written justification | a planted section raises the miss count by exactly one; a removed section is reported as a STALE table row |
