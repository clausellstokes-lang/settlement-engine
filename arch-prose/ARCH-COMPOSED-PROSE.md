...
`$SC` = `/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad`. Product read at `$SC/laneB6` = **3b1c0eaa5** (`git log --oneline -1`, executed); instruments at `$SC/skepINSTR` = **74a1aa0e8** (executed). Paths are repo-relative to those docks unless written whole. The six reader maps are cited as `read-kernel`, `read-data`, `read-explanations`, `read-facts`, `read-specs`, `read-instruments` with their section numbers; the designs as A/B/C with theirs.
