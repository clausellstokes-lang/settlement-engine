...
answer for the numbers that appear and for no others.
  POOL "COMBINATION C1: a high rung on a working approach" in block DS-ECO-1, which reads:
    ...
    {access} is "road" on this town, and the page prints it
    {complexity} is "spread of trades" on this town, and the page prints it
...
1. (spine) A traveller says the road brings steady traffic and the spread of trades keeps more hands busy than the town needs, and that a man passing through is never long without work here. None of the prosperity looks improvised.
  POOL "INCOME MIX: two or three sources between them" in block DS-ECO-12, which reads:
    economicState.incomeSources = "[10 rows]"
    economicState.incomeSources.length = 10
    economicState.incomeSources.reduce = "{}"
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
    granted to them.
      spine: Ask what this town lives on and you will be given a short list rather than an answer.
2. (spine) Most of what the town lives on comes from two or three trades between them. The drinkers of the district each name a different trade, and each of them is paid by the one he names.
  POOL "TRADE PROFILE: exports and imports both present" in block DS-ECO-12, which reads:
...
3. (spine) The stalls at Rundgate carry the town's own goods and other towns' goods side by side. A drover says he cannot tell the one from the other without asking, and that nobody behind a stall tells him unasked.
  POOL "POSTURE: established" in block DS-ECO-10, which reads:
...
4. (spine) The yards are full of goods bound out and no two loads in them are the same. Loading them is the ordinary work of the day here, and nobody in the yards asks where any of it goes.
  POOL "SECURE" in block DS-ECO-9, which reads:
...
5. (spine) Rundgate feeds itself, and the margin left at the end of a season is small enough that nobody troubles to count it.
  POOL "STALLED" in block DS-GEN-18, which reads:
    ...
    {settlement} is "Rundgate" on this town, and the page prints it
    {institution} is "Garrison" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
    granted to them.
...
6. (spine) The garrison is kept on a supply that has stopped coming. The buildings stand, the men in them keep their skill, and the work waits on what does not arrive.
  POOL "TIER: minor shadow activity (≥3)" in block DS-ECO-6, which reads:
...
7. (spine) A little of the town's trade never reaches the under-clerks of the hall… too little to change what the place is, and enough that somebody is keeping the difference. Where that difference goes is in no account, and it would be worth knowing whose hands it passes through.