...
answer for the numbers that appear and for no others.
  POOL "COMBINATION C3: the middle rungs" in block DS-ECO-1, which reads:
...
1. (spine) Nothing about Spitzplatz's condition stands out in either direction. The road is kept open, the market keeps its stalls, and the town is neither building nor selling off.
...
2. (spine) Ask what the town lives on, and the answer is a short list, never one trade alone.
...
3. (spine) The market at Spitzplatz sells the town's own goods and other towns' goods from the same stalls, and gives a stranger nothing to tell them apart.
...
4. (spine) Something is always going out of the town. When one trade slows, another picks it up.
  POOL "PRESSURED" in block DS-ECO-9, which reads:
...
5. (spine) Spitzplatz is not hungry, and it has no cushion left over.
  POOL "STALLED" in block DS-GEN-18, which reads:
    ...
    {settlement} is "Spitzplatz" on this town, and the page prints it
    {institution} is "Caravaneer's post" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
    granted to them.
...
6. (spine) Spitzplatz keeps Caravaneer's post on a supply that failed upstream. The building stands, the skill remains, and the books wait.
...
7. (spine) What passes for a shadow economy here has not become one... there is unlicensed trade, and nothing organised stands behind it.