...
1. (spine) A stranger reads Rundgate's standing off the road before anyone tells him. The spread of trades keeps more hands busy than the town strictly needs, and the traffic looks steady and unforced.
...
2. (spine) This town lives off a short list of trades rather than one thing alone, and everybody names the same two or three when asked.
...
3. (spine) The stalls at Rundgate mix the town's own goods with goods brought in from elsewhere, and a stranger at the market cannot easily tell which is which.
...
4. (spine) Goods bound out fill the yards here in every direction, no two loads alike. This is a town that sells.
  POOL "SECURE" in block DS-ECO-9, which reads:
    (this pool declares no typed field)
5. (spine) A small margin carries Rundgate through every season, and the counts come out ahead without much fuss.
...
6. (spine) Garrison stands on a supply that has failed somewhere upstream. The building holds, the skill remains, and the books wait.
  POOL "TIER: minor shadow activity (≥3)" in block DS-ECO-6, which reads:
    economicState.safetyProfile.blackMarketCapture = 8
7. (spine) A little of Rundgate's trade goes unrecorded… small enough to trouble only the clerks, and not enough to change what the town is.