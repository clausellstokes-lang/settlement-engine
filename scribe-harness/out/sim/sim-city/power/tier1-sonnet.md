...
THE PAGE, as the reader meets it. Question 7 is about these lines and no others:
  ...
  [machine] recentConflict: City Council has been debating market levies for three months. The merchants have stopped attending the sessions. Both sides are now acting as if the other has already lost.
...
answer for the numbers that appear and for no others.
  POOL "Approved" in block DS-POW-1, which reads:
    (this pool declares no typed field)
    {seat} is "City Council" on this town, and the page prints it
...
1. (spine) The City Council at Rundgate holds a modest edge in its own favour, and few of its requests are refused.
  POOL "breakdown dominated by PROSPERITY, favourable" in block DS-POW-1, which reads:
    (this pool declares no typed field)
    {settlement} is "Rundgate" on this town, and the page prints it
    {seat} is "City Council" on this town, and the page prints it
    {seat} is "City Council" on this town, and the page prints it
...
2. (spine) Rundgate credits the City Council with its good years whether or not the council earned them, and prosperity is the whole of the case made in its favour.
  POOL "stable matched" in block DS-POW-2, which reads:
    (this pool declares no typed field)
    {settlement} is "Rundgate" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
...
3. (spine) What is settled at Rundgate is the hall and only the hall, and the town's steadiness is a statement about who answers and not about what the answers turn out to be.
  POOL "operation role criminal revenue stream (unclassified)" in block DS-POW-6, which reads:
    (this pool declares no typed field)
    {settlement} is "Rundgate" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
...
4. (spine) A criminal operation with real local weight sits inside Rundgate, whether counted in ground held or in coin taken, and no sharper account of it exists.
  POOL "council" in block DS-POW-5, which reads:
    (this pool declares no typed field)
    {seat} is "City Council" on this town, and the page prints it
    {settlement} is "Rundgate" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
...
5. (spine) The City Council watches Rundgate's own temper more closely than it watches any rival, since it is unrest and not a rival that unseats a council.
  POOL "governing body name: a SLOT, never a baked noun" in block DS-POW-5, which reads:
    (this pool declares no typed field)
    {seat} is "City Council" on this town, and the page prints it
    {settlement} is "Rundgate" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
...
6. (spine) A stranger who calls the City Council at Rundgate by any other name is corrected immediately, and the correction is polite.
  POOL "riskLabel: Holding" in block DS-POW-4, which reads:
    (this pool declares no typed field)
    {faction} is "City Council" on this town, and the page prints it
    {seat} is "City Council" on this town, and the page prints it
    {settlement} is "Rundgate" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
...
7. (spine) The City Council outweighs every rival at Rundgate who wants its place, and the hold is real without being comfortable.
  POOL "legitimacyHold: public backing hardens the hold" in block DS-POW-4, which reads:
...
8. (spine) Public backing at Rundgate is hardening the City Council's grip, so what would be a real contest elsewhere is no more than an inconvenience here. That public backing counts for more in the council's favour than its standing alone would earn.