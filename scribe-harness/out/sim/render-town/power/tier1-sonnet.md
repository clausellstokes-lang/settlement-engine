...
1. (spine) The Guild Council's standing at Spitzplatz is good, and the margin lets it spend goodwill on decisions the town would otherwise refuse it.
  POOL "breakdown dominated by PROSPERITY, favourable" in block DS-POW-1, which reads:
    (this pool declares no typed field)
2. (spine) The Guild Council's standing at Spitzplatz rests on the town's prosperity alone, and holds no further than the prosperity holds.
  POOL "stable matched" in block DS-POW-2, which reads:
    (this pool declares no typed field)
3. (spine) A stranger doing business at Spitzplatz needs only one name and gets one answer, and can plan a season ahead on the strength of it.
  POOL "operation role money laundering" in block DS-POW-6, which reads:
    (this pool declares no typed field)
4. (spine) Several houses at Spitzplatz show nothing amiss to anyone who checks, and showing nothing amiss is the service being paid for.
  POOL "mixed" in block DS-POW-5, which reads:
    (this pool declares no typed field)
5. (spine) The town cannot be described in one word as to how it is governed, and the hall does not try either.
  POOL "governing body name: a SLOT, never a baked noun" in block DS-POW-5, which reads:
    (this pool declares no typed field)
6. (spine) The town at Spitzplatz calls the hall by its full title in public business and something shorter everywhere else, and each name is exact in its place.
  POOL "riskLabel: Holding" in block DS-POW-4, which reads:
    (this pool declares no typed field)
7. (spine) The Guild Council holds its seat at Spitzplatz because no rival faction outweighs it, and the weighing is done again each season.
...
8. (spine) There are people at Spitzplatz who could move against the Guild Council and choose not to, since unseating a power the town stands behind costs more than the seat is worth.