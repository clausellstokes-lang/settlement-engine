...
1. (spine) The Guild Council is well thought of at Spitzplatz, and the good opinion is spent on decisions the town would otherwise refuse it.
  POOL "breakdown dominated by PROSPERITY, favourable" in block DS-POW-1, which reads:
    (this pool declares no typed field)
2. (spine) The Guild Council's standing rests on the town's prosperity, and nothing else in the town would hold it up.
  POOL "stable matched" in block DS-POW-2, which reads:
    (this pool declares no typed field)
3. (spine) A buyer off the road at Spitzplatz deals with one name, gets one answer, and finds the same answer standing the next season.
  POOL "operation role money laundering" in block DS-POW-6, which reads:
    (this pool declares no typed field)
4. (spine) The books of several houses here are in perfect order, and that order is the thing being bought.
  POOL "mixed" in block DS-POW-5, which reads:
    (this pool declares no typed field)
5. (spine) The town has no one word for how it is governed, and the hall does not have one either.
  POOL "governing body name: a SLOT, never a baked noun" in block DS-POW-5, which reads:
    (this pool declares no typed field)
6. (spine) The hall is given its full title in the market square and a shorter name everywhere else, and both names are exact.
  POOL "riskLabel: Holding" in block DS-POW-4, which reads:
    (this pool declares no typed field)
7. (spine) The Guild Council holds its seat because nothing else in the town outweighs it, and the weighing is done again every season.
...
8. (spine) Those who would move on the Guild Council do not, because moving on a seat the town stands behind costs more than the seat is worth.