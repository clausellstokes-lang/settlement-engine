...
1. (spine) The road is open, the market keeps its day, and nothing here is being built or sold off.
...
2. (spine) Most of what the town earns comes off one trade. The other sources of income do not come near it.
...
3. (spine) The stalls carry goods made here beside goods brought in, and nothing marks which is which for a stranger.
  POOL "SECURE" in block DS-ECO-9, which reads:
    (this pool declares no typed field)
4. (spine) The town eats, and the matter comes up only when a stranger asks.
...
5. (spine) The Cartographer's guild in Hochhausen keeps its building and its craft, and the supply it works from has failed upstream.