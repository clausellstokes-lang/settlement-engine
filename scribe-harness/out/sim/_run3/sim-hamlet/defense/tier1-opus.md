...
1. (spine) The town could not survive an attack in force, and it knows as much and picks no quarrels.
...
2. (spine) The crisis has changed the town's footing and not what the place is worth, and nobody here lets the two be confused.
  POOL "Invasion & War: neither walls nor force" in block DS-DEF-2, which reads:
    institutions = "[14 rows]"
3. (spine) The houses here stand with nothing around them, and nobody here is under arms.
...
12. (spine) A matter here is settled among the households themselves, since there is no court to carry it to and no cell to put a man in.
13. (face 0) A carter says a short payment here is argued out at the door where it was made, and a stranger who argues it loses.
14. (face 1) One of the elders says what the households settle among themselves stays settled, and what reaches the elders is settled the elders' way.
16. (face 3) The households that keep the custom say a boundary here is walked and talked over until both sides agree, and the walking is done in time neither side can spare.
  POOL "Economic Survival: WEAK" in block DS-DEF-2, which reads:
    defenseProfile.scores.economic = 21
20. (spine) Nobody here has ever settled what the keeping of the place would cost, or whose the cost would be.
23. (face 2) A woman old enough to remember the custom says a household that falls short here falls short alone, and that nobody counts it harsh.
...
29. (spine) The hamlet keeps no grain store to go to and no house the sick are carried to, and nobody here counts either as a lack.
30. (face 0) The older heads here say a house with sickness in it is sat up with by its neighbours, and whoever refuses the sitting is remembered for it.
32. (face 2) A rider who stopped a night says a sick house here is known from the lane by the food left on its step, and that nobody tells a stranger which house it is.
...
35. (spine) The ordinary safety of the place has stopped applying, and the danger on it now is what governs the lanes.
  POOL "First-Survey qualification (the reading is a first look)" in block DS-DEF-3, which reads:
    economicState.safetyProfile.safetyLabel = "Dangerous — Monster Threat"
36. (spine) What is known of the danger here is a first look only, honest about what has been seen and silent about what has not.
...
37. (spine) Nothing organised stands behind the wrongdoing here, and what there is of it goes household by household.
  POOL "walls ABSENT" in block DS-DEF-5, which reads:
    institutions = "[14 rows]"
38. (spine) A stranger comes into Warmholz from whichever side suits him and passes nobody on the way in.
...
39. (spine) Nobody here has the defense of the place as their business, and a stranger who asks is sent from door to door.
  POOL "charter hall ABSENT where the country warrants one" in block DS-DEF-5, which reads:
    institutions = "[14 rows]"
    standingDefenseForces(settlement) = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
40. (spine) No body here holds a charter to answer for the place, and what wants doing falls to whoever is nearest to it.
...
41. (spine) Nothing unseen has come at the place, and nothing here would know if it had.
  POOL "UNWALLED-SMALL" in block DS-DEF-11, which reads:
    institutions = "[14 rows]"
42. (spine) Warmholz is too small to wall and knows it, and what keeps the place is its neighbours, its distance and its unimportance.
  POOL "Logistics & Supply: No reserves, landlocked" in block DS-DEF-6, which reads:
    config.tradeRouteAccess = "road"
    economicState.compound.inst.hasGranary = false
    economicState.compound.inst.hasPort = false
43. (spine) Nothing is held back here against a lean year, and the road is the only way anything comes in.