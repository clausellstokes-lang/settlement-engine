...
1. (spine) The market squares are open to whoever brings custom in off the roads.
...
2. (spine) A garrison and a professional watch are kept here, both of them paid out of the town's purse.
...
4. (spine) This town pays its own way well enough that the trades take apprentices on for the full term.
  POOL "defenseProfile.readiness.label: Well-Defended" in block DS-GEN-3, which reads:
    name = "Rundgate"
5. (spine) The citadel and the gates are kept sound, and nothing on either is for show.
...
6. (spine) The garrison can man the wall in strength, and the men it sends up have been up before.
  POOL "scores.monster: STRONG" in block DS-GEN-3, which reads:
    axis = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
    readings.scores = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
7. (spine) Children here learn the rules about the dark early, and the rules hold.
  POOL "scores.internal: ADEQUATE" in block DS-GEN-3, which reads:
    axis = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
    readings.scores = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
8. (spine) The watch answers what is brought in at the door but goes looking for nothing in the streets.
  POOL "scores.economic: STRONG" in block DS-GEN-3, which reads:
    axis = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
    readings.scores = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
9. (spine) Prices in the markets hold from one day to the next, and no trade here lives on its last sale.
  POOL "scores.magical: ADEQUATE" in block DS-GEN-3, which reads:
    axis = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
    readings.scores = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
10. (spine) The mages' guild can do what is usually wanted here, and it names what is beyond its people.
...
12. (spine) A living is made here out of standing on the way to somewhere else.
  POOL "road" in block DS-GEN-6, which reads:
    readings.tradeRouteAccess = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
13. (spine) The town does not feed itself from its own ground, and what makes up the difference comes in on the road.
  POOL "tier overlay: city" in block DS-GEN-6, which reads:
    name = "Rundgate"
14. (spine) Rundgate has quarters that do not know one another, and a man out of the furrier's district is a stranger in the sage's quarter.
...
15. (spine) Nothing that has come at this town has changed the way it deals with its neighbours.