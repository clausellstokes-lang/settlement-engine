...
1. (spine) Market day is when Rundgate is most itself, and the roads bring in the custom that gives the stalls their reason for standing.
...
2. (spine) Armed order is part of the town's furniture, and professionals are kept for its safety rather than trusting to luck or its neighbours.
  POOL "prosperity: Comfortable / Prosperous" in block DS-GEN-3, which reads:
    name = "Rundgate"
3. (spine) People in Rundgate can afford to argue about things other than money.
...
4. (spine) The town expects to go on being itself.
  POOL "defenseProfile.readiness.label: Well-Defended" in block DS-GEN-3, which reads:
    name = "Rundgate"
5. (spine) Rundgate is defended properly rather than impressively, and the works are sound without being for show.
...
6. (spine) Children here are taught the rules about the dark early, and the rules work.
  POOL "scores.internal: ADEQUATE" in block DS-GEN-3, which reads:
    axis = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
    readings.scores = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
7. (spine) Reports that reach the watch get answered, and nothing further is sought out.
  POOL "scores.economic: STRONG" in block DS-GEN-3, which reads:
    axis = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
    readings.scores = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
8. (spine) Prices are steady in a way that suggests the town is not living on the last thing it sold.
  POOL "scores.magical: ADEQUATE" in block DS-GEN-3, which reads:
    axis = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
    readings.scores = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
9. (spine) Somebody here can do what is usually needed and knows what is beyond them.
  POOL "foodSecurity.label: Secure" in block DS-GEN-3, which reads:
    name = "Rundgate"
10. (spine) Nobody in Rundgate spends a thought on food.
  POOL "ordinary (route road and the default)" in block DS-GEN-5, which reads:
    (this pool declares no typed field)
11. (spine) Sitting on the way to somewhere has given the town a modest living.
  POOL "road" in block DS-GEN-6, which reads:
    readings.tradeRouteAccess = UNREADABLE BY THIS CARD (the engine decided it; the pool key states it; assert no value for this field beyond what the key says)
12. (spine) The road brings in what the town cannot grow, and the town's life depends on the road staying open.
  POOL "tier overlay: city" in block DS-GEN-6, which reads:
    name = "Rundgate"
13. (spine) Grown large, the town now has quarters that do not all know each other.
...
14. (spine) The town has faced pressure enough to bend its ties to its neighbours, and they have not bent.