...
1. (spine) The road works, the market works, and the town is neither building anything nor selling anything off.
...
2. (spine) The town lives on a short list of trades, and none of them carries it alone.
...
3. (spine) The market carries the town's own goods and goods brought in from elsewhere on the same stalls, and nothing on the stalls marks which is which.
...
4. (spine) Something is always going out, and when one trade is quiet another is not.
...
5. (spine) Caravaneer's post is kept on a supply that fails before it reaches the town, and the building stands, the skill stands, and the work waits.
  POOL "TIER: minor shadow activity (≥3)" in block DS-ECO-6, which reads:
    economicState.safetyProfile.blackMarketCapture = 8
6. (spine) What passes for a shadow economy here has never grown into one… the front businesses stand in plain sight and the gang behind them takes a small share of what moves. The thing worth watching is not how much is taken but whose hand takes it.