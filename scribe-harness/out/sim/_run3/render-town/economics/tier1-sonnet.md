...
1. (spine) Nothing about Spitzplatz marks it out in either direction. The road holds up, the market keeps its stalls, and the town is neither building nor selling off what it has.
...
2. (spine) Ask what the town lives on, and the answer comes back as a list, not a single trade.
...
3. (spine) Goods off the road and goods made in Spitzplatz share the same market stalls, and nothing there marks one from the other.
...
4. (spine) Something is always going out of the town. Let one trade go quiet, and another is still moving.
  POOL "PRESSURED" in block DS-ECO-9, which reads:
    (this pool declares no typed field)
5. (spine) Spitzplatz is not hungry, and it has nothing set aside against a bad turn.
...
6. (spine) The Caravaneer's post in Spitzplatz stands on a supply that failed somewhere upstream. The building holds, the skill remains, and the books wait.
  POOL "TIER: minor shadow activity (≥3)" in block DS-ECO-6, which reads:
    economicState.safetyProfile.blackMarketCapture = 8
7. (spine) Whatever black market exists here has never grown past its own small name… there is unlicensed trade, and nothing behind it worth calling an organisation.