...
POOL "economicViability.viable: true" in block DS-GEN-3
  ...
  THE CORPUS LINE, as the claim and the fallback:
    ...
    notebook: none. Return an empty list.
POOL "defenseProfile.readiness.label: Lightly Defended" in block DS-GEN-3
  vid: 2
  stance: ledger
  THE ORDER: the corpus spine realises the move order `V1` — `PRESENT` (the state key alone); keep that order in your spine.
  slots you may use: settlement
  ...
  faces to write: 0
  THE CORPUS LINE, as the claim and the fallback:
    spine: What {settlement} could hold, it could hold briefly.
    notebook: none. Return an empty list.
POOL "scores.military: ADEQUATE" in block DS-GEN-3
  vid: 3
  stance: street
  THE ORDER: the corpus spine realises the move order `V1` — `PRESENT` (the state key alone); keep that order in your spine.
  slots you may use: settlement
  ...
  THE CORPUS LINE, as the claim and the fallback:
    ...
    notebook: none. Return an empty list.
POOL "scores.monster: WEAK" in block DS-GEN-3
  vid: 2
  stance: ledger
  THE ORDER: the corpus spine realises the move order `V1` — `PRESENT` (the state key alone); keep that order in your spine.
  slots you may use: settlement
  ...
  faces to write: 0
  THE CORPUS LINE, as the claim and the fallback:
    spine: Nothing at {settlement} is organised against what lives outside the fields; each household answers for its own fences.
    notebook: none. Return an empty list.
...
POOL "foodSecurity.label: Secure" in block DS-GEN-3
  ...
  THE CORPUS LINE, as the claim and the fallback:
    ...
    notebook: none. Return an empty list.
POOL "intensity: low" in block DS-GEN-2
  vid: 3
  stance: visitor
  THE ORDER: the corpus spine realises none of the eight closed level-1 orders, so this pool sets no order for you to keep.
  slots you may use: faction, faction2, settlement
    {settlement} is "Hochhausen" on this town
    {faction} is "The Commercial Circle" on this town
    {faction2} is "The Administrative Circle" on this town
    WRITE THE FILL, NOT THE TOKEN: the words above are what the page prints.
  THE FIELDS this pool reads, with their values here:
...
POOL "road" in block DS-GEN-6
  vid: 1
  stance: ledger · marks no deficit
  THE ORDER: the corpus spine realises the move order `V1` — `PRESENT` (the state key alone); keep that order in your spine.
  slots you may use: settlement
    {settlement} is "Hochhausen" on this town
    WRITE THE FILL, NOT THE TOKEN: the words above are what the page prints.
  ...
  THE CORPUS LINE, as the claim and the fallback:
    ...
    notebook: none. Return an empty list.
POOL "tier overlay: other tiers" in block DS-GEN-6
  vid: 1
  stance: ledger
  THE ORDER: the corpus spine realises the move order `V1` — `PRESENT` (the state key alone); keep that order in your spine.
  slots you may use: settlement
  ...
  THE CORPUS LINE, as the claim and the fallback:
    ...
    notebook: none. Return an empty list.
POOL "prominentRelationship present" in block DS-REL-2
  vid: 2
  stance: ledger
  THE ORDER: the corpus spine realises the move order `V1` — `PRESENT` (the state key alone); keep that order in your spine.
  slots you may use: settlement
