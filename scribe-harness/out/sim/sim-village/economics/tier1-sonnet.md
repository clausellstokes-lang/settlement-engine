...
1. (spine) A traveller says Hochhausen shows nothing remarkable in either direction: the road holds, the market runs, and the town is neither putting up new work nor selling off what it has.
  POOL "INCOME MIX: one source carries the town" in block DS-ECO-12, which reads:
    economicState.incomeSources = "[4 rows]"
    economicState.incomeSources.length = 4
    economicState.incomeSources.reduce = "[4 rows]" (the engine's own field is `economicState.incomeSources`)
    {settlement} is "Hochhausen" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
    granted to them.
...
2. (spine) Hochhausen earns most of its keep from one trade alone, and nothing else in the accounts comes near it.
  POOL "TRADE PROFILE: exports and imports both present" in block DS-ECO-12, which reads:
    ...
    economicState.localProduction = "[2 rows]"
    economicState.primaryExports = "[2 rows]"
    economicState.primaryImports = "[4 rows]"
    {settlement} is "Hochhausen" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
    granted to them.
...
3. (spine) A pedlar says the market at Hochhausen keeps the town's own goods and other towns' goods on the same stalls, with nothing to mark which is which. What comes in is more varied than what goes out.
...
4. (spine) A drover says the yards here hold goods bound out, no two loads alike, and takes it for a town that sells rather than one that hoards.
...
5. (spine) The whole village eats, and it is not a thing anybody here troubles to raise unprompted.
  POOL "STALLED" in block DS-GEN-18, which reads:
    ...
    readings.activeChains = "[5 rows]" (the engine's own field is `economicState.activeChains`)
    ...
    readings.primaryImports = "[4 rows]" (the engine's own field is `economicState.primaryImports`)
    {settlement} is "Hochhausen" on this town, and the page prints it
    {institution} is "Cartographer's guild" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
    granted to them.
...
6. (spine) The Cartographer's guild in Hochhausen stands on a supply that failed somewhere upstream. The building is kept and the skill is kept, and only the chain that once fed them runs empty.