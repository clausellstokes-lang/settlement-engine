...
answer for the numbers that appear and for no others.
  POOL "COMBINATION C4: a low rung on a working approach" in block DS-ECO-1, which reads:
...
1. (spine) Prosperity here sits on a low rung, though the town's trade still keeps its ordinary pace.
...
2. (spine) One thing the town does for its money outweighs the rest. Most of what anyone earns here traces back to that trade.
...
3. (spine) The town both exports and imports, and the two are not simply the same trade in each direction. Most of what leaves here was raised or made in the town first.
...
4. (spine) Warmholz sells more than one thing beyond its own borders, and no single one of them carries the trade alone.
...
5. (spine) What is grown here feeds the town, with a little to spare, season after season.
  POOL "BOUGHT-IN" in block DS-GEN-18, which reads:
    ...
    {settlement} is "Warmholz" on this town, and the page prints it
    {good} is "salt" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
    granted to them.
...
6. (spine) Salt is worked here that the town cannot raise for itself, and buying it in is as settled a part of the craft as the working.