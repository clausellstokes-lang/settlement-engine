...
answer for the numbers that appear and for no others.
  POOL "Legitimacy Crisis" in block DS-POW-1, which reads:
    (this pool declares no typed field)
    {seat} is "Free Elder Council" on this town, and the page prints it
...
1. (spine) The Free Elder Council governs here by agreement and by nothing else, and almost nothing it asks of the place arrives.
  POOL "governanceFractured true" in block DS-POW-1, which reads:
...
2. (spine) A traveller says the Free Elder Council heard him out and settled nothing, while a household that sits on no council at all had the matter done before dark.
  POOL "no token matched: unclassified (the plain-description floor)" in block DS-POW-2, which reads:
    (this pool declares no typed field)
    {faction} is "Free Elder Council" on this town, and the page prints it
    {settlement} is "Warmholz" on this town, and the page prints it
    THE HAND-WRITTEN LINE THIS POOL SHIPS. It is not a line to judge: it is the claim, it ships
    on this page if the lines below are refused, and a record, an office or a body IT names is
    granted to them.
      spine: {faction} holds the hall at {settlement} and the town's affairs run through it.
3. (spine) The affairs of the place run through the Free Elder Council, and it keeps no building, no seal and no paid hand.
  POOL "capture pressure ADVANCING (weak security, poor prosperity)" in block DS-POW-6, which reads:
...
4. (spine) Pressure to buy a hold here is advancing, and little stands in its way: no watch, no purse worth the name, and a council that is only the older heads agreeing. No criminal hold has been taken yet, and that is the part of it most worth checking again.
...
5. (spine) A decision here is talked over until everyone can live with it, and once it is made it stays made.
...
6. (spine) A stranger who gives the Free Elder Council any other name is corrected at once, politely and by whoever is nearest.
  POOL "legitimacyHold: public rejection is breaking the hold" in block DS-POW-4, which reads:
    typeof power.publicLegitimacy === 'object' && power.publicLegitimacy !== null
    ? power.publicLegitimacy
...
7. (spine) The common grazing is shared out by the households and not by the Free Elder Council, and what the Council has lost here it has lost to the place and to no rival.