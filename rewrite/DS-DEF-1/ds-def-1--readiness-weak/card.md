THE CARD (mechanical sections) — block DS-DEF-1 · pool `readiness WEAK` · dir ds-def-1-readiness-weak

(1) THE KEY AND ITS PREIMAGE
  key function: READINESS_ROW_POOL (rung table) · site defense.postureHeader
  census predicate: scoreBand(readinessScore) (via READINESS_ROW_POOL in defenseStateProse.js) === WEAK
  read readinessScore -> defenseProfile.readiness.score           [SNAPSHOT] writers 0   (resolved through posturePoolKey's call site in the entry point)
  the key FIXES: readinessScore:range=[20,39] · leaves OPEN: readinessScore   (posturePoolKey, 20 combination(s))
  preimage on the 768-town rate grid: 237 towns (3086 bp) — thorp 64/128 · hamlet 76/128 · village 86/128 · town 10/128 · city 1/128
...
(2c) COVERT FIELDS AND THEIR SYMPTOMS ON THIS BLOCK (ADDENDUM 18 ruling 26) — the INVERTED test the refuter judges a `compromised` face under
    (none: no covert field of the engine marks a symptom pool on this block, so no face of this block may carry the `compromised` tag)
...
# THE READER'S SECTIONS (the marker, seat opus) — block DS-DEF-1 · pool `readiness WEAK`

**ARCHIVER'S TEMPERAMENT: `exact`** (the default; the card prints no other). It colours the joins and the
notebook's shades only — never a fact and never which source is heard.

**THE ONE THING TO CARRY INTO EVERY FACE.** This key fixes a NUMBER and nothing else:
`defenseProfile.readiness.score` in [20, 39] (`defenseScoreBands.js:38`, `posturePoolKey`,
`defenseStateProse.js:825-828`). That number is a MEAN of five arms plus a tier bonus minus a threat
penalty (`defenseGenerator.js:487-510`). **It fixes no body, no building, no force, no purse and no
roster row.** Every closed roster on this tab is OPEN (section 5): watch, garrison, militia, mercenary,
charter, walls, gates, granary, hospital, market, hall, court, prison, church. The preimage is
**237 towns and 226 of them are below town** (thorp 64 · hamlet 76 · village 86 · town 10 · city 1;
metropolis silent). A face is drawn by ALL of them.

---

## (7) THE SPEAKER ROSTER

### THE UNIVERSAL SPEAKERS — present on every one of the 237 towns

| speaker | what seats it on every preimage tier | its interest, in one line |
|---|---|---|
| **a stranger / a traveller** | nothing seats an arrival; it is universal by construction (the `[visitor]` stance's person) | what is visible on arrival and what did not happen on it — nobody stopped, challenged or counted them, and the town did not think that worth remarking (`safetyProfile.js:302-322`, `guardEffectivenessDesc` `:376-382`) |
| **the households** | `Dwellings (4-16)` thorp · `(17-80)` hamlet · `(80-180)` village · `Housing (180-1000)` town · `(1000-5000)` city — all `required: true` | they ARE the unpaid defence. `communityMilBase` is exempt from the upkeep gate entirely (`defenseGenerator.js:186-192`): the part of the arrangement that costs the purse nothing is the part they carry, and nobody has to ask them because nobody pays them |
| **whoever works the ground** | `Subsistence farming` thorp+hamlet · `Common grazing land` hamlet · `Farmland` village; at town+ the `Weekly market` / `Daily markets` stand in | their stake is OUTSIDE whatever line the town holds: the fields, the grazing and the walk to the mill are where the town is not covered, and they are the ones out in it |
| **whoever keeps the burial ground** | `Burial ground` thorp+hamlet · `Graveyard` village · `Parish burial grounds` town · `Burial grounds and charnel house` city — `Burial` at p 1 on every one | the one body at every tier whose work is the consequence of what the town did not stand against; ⚠ the RECORD is not theirs below town — `Register of the dead` is a service only of `Parish burial grounds` at TOWN (`holderTable.js` `parish` kind), so they may speak and may not be CITED |
| **where the town draws water** | `Water source` thorp+hamlet · `Multiple water sources` village+town · `Aqueduct or water system` city | the one place everyone is at some point; a scene ground rather than a power, and the universal seat for a `[public]` candidate (car 18n/28 pending — keep under NOTES) |
| **the older households (PLURAL ONLY)** | the `elders` holder kind resolves `Household elder · Village elder · Village headman · Town council` (`holderTable.js:332-336`) | they keep the `Record of custom`: their stake is that the arrangement is customary rather than funded, and that it has never yet been asked for more. ⛔ **PLURAL ONLY — see the named-office bar below** |
| **the archiver** | the compiling office itself | the bare statement of a recorded fact, the joins, the notebook, and at most one `[archiver · observed]` face per variant (car 18n NOT LANDED — keep under NOTES) |

### THE CONDITIONAL SPEAKERS — the field that seats each, and how much of the pool it reaches

| speaker | seated by | reach |
|---|---|---|
| **the hall / a clerk in the hall** | `Town hall` (town) · `City hall` (city), `required: true` | **11 of 237.** Below town the governing institution is `Head-of-household consensus` / `Informal elder consensus` — **a consensus, not a room** (`institutionServices.js:1448-1453`; `rulingStructure.js:162-163`; F1-20). Its interest where it exists: the ONE military purse, `milUpkeepMult = min(1, 0.6 + econOutput/50 × 0.4)` over wall keeping and wages together (`defenseGenerator.js:177-192`), which `fieldSynonyms.js:51` calls "the town's pay for its watch" — the hall defends what it chose not to fund |
| **the tavern** | `Taverns (5-20)` (town) · `Inns and taverns (district)` (city) | **11 of 237.** ⛔ The corpus's commonest source is NOT universal here. Its interest: who turns out and who does not, read off the safety label |
| **the guilds** | `Craft guilds (5-15)` (town) — **not a required row at city** | **10 of 237.** Their interest: `Quality certification` and `Apprenticeship programs` are theirs to lose; they pay a fee toward an arrangement the households use and do not pay for |
| **the market** | `Market square` + `Weekly market` (town) · `Multiple market squares` + `Daily markets` (city) | 11 of 237 |
| **the register / a local priest** | `Access to parish church` (hamlet) · `Parish church` + `Priest (resident)` (village) · `Parish churches (2-5)` (town) · `(10-30)` (city) | **173 of 237 — absent at thorp.** ⚠ at hamlet the row is "walk 2-5km to village church" (`institutionalCatalog.js:50-54`): a house of the faith STANDING in the town is F1-11 there |
| **the watch** | `Town watch` (town) · `Professional city watch` (city) | 11 of 237; OPEN below town, so never named AND never denied |
| **the garrison** | `Garrison` + `City walls and gates` (city) | **1 of 237** |
| **the muster, the gate-holder, the mercenary, the charter** | nothing in the preimage seats any of them | **0 guaranteed.** The `muster` holder kind's only holder in the shipped roster is `Citizen militia` (`holderTable.js`, the kind's own note) — which no required row seats at any preimage tier |
| **the crown's assessor** | — | **not a speaker anywhere.** The engine has no typed crown collector |

### ⛔ THE NAMED OFFICES A SPEAKER MAY NEVER BE (`npcGenerator.js:1511-1517`, F3-06)

    thorp    Elder, + deriveThorpSecondRole
...
⛔⛔ **THE TRAP THIS POOL WALKS INTO THAT NO OTHER DEFENSE POOL DOES: `Elder` IS A NAMED NPC AT
THORP AND HAMLET — 140 of the pool's 237 towns.** The safest universal speaker on the roster is
also, on the majority of the pool, a person sitting on the next tab with a personality and a
secret. So: **"the older households", "those who have been here longest", "the households that
remember it being arranged"** — never "an elder says", never "one of the elders", never a singular
person of that office acting. The same bar takes the singular gate-keeper or guard person
(`Guard Captain`, village and up), the singular priest (`Parish Priest` at hamlet, `High Priest` at
town+) and the singular head of the town (`Mayor`, village and up).

### ATTRIBUTION SYNTAX (ruling 25 as amended; car 18l NOT LANDED — keep the class word)

Vary the SHAPE, not only the role. No first three words repeat across a variant, and no attribution
verb three times running. Available here: *"the households say" · "asked at the water, they say" ·
"whoever keeps the burial ground has it that" · "a traveller's account is" · "those who work the
grazing say" · "at the hall they say" (town+) · "the guilds say openly" (town)*.

---

## (8) WHAT WOULD BE FALSE

**The row that governs this pool above all others is F4-06 — *the readiness band explained by the
works*.** `readiness = avgScore + tierBonus − threatPenalty` (`defenseGenerator.js:510`), and the tier
bonus is **+12 thorp · +8 hamlet · +4 village · +2 town · 0 city** (`:487-493`) against a threat penalty
of 15 plagued / 7 frontier (`:497-500`). On 226 of this pool's 237 towns the band reads WEAK in part
because the place is small and cheap to defend, and the band moves while every wall, gate and roster
row is unchanged: **any face that explains WEAK by the walls, the men, the money or the neglect states a
cause the engine does not hold.** Nor may a face state the band's own opposite at the top of its range:
the header beside the prose prints the readiness LABEL, and over [20, 39] that label is `Vulnerable`
(20–23), `Lightly Defended` (24–37) **or `Defensible` (38–39)** (`:516-521`) — so "there is nothing here
that would slow anybody" contradicts the same-page header on the upper third of the key's own domain.
Next, and measured on the shipped rows this rewrite replaces: **F1-07** (the shipped `[visitor]` face says
"how much of the *perimeter* has nobody on it" — `walls` is OPEN at thorp, hamlet, village and town, and
a perimeter is a line around the town that 236 of 237 towns are not guaranteed to have), and **F2-05 /
F2-08** (the shipped `[unfolding]` face says "the margin has been *narrowing rather than widening*" — a
comparative against a past, which F2-05 lists in its own words, and a trend). The whole of §1.1 is live
here because the key fixes no roster at all: **F1-01** a watch as a standing body · **F1-02** a garrison ·
**F1-03** a militia or a muster ROLL · **F1-04** "the guard" as a body · **F1-05** a mercenary company ·
**F1-06** a charter hall · **F1-08** a gate, checkpoint or toll-bar · **F1-09** a granary building ·
**F1-10** a standing market · **F1-11** a church STANDING in the settlement (thorp has no church row at
all; hamlet's row is a walk of 2-5km) · **F1-12 / F1-13** a trial, a gaoling, a cell · **F1-14** a
hospital · **F1-16** a craft or merchant guild · **F1-20** a hall, chamber or council room as a PLACE
below town. And **F1-25 — the negation direction — is the sharper half on this pool, because the sitting
named it as where the re-cut prose actually fails**: a face may not deny a body either, and `Town watch`
is `required: true` at town, `Professional city watch` and `Garrison` at city, so "nobody is set up to
respond", "no watch here", "no muster worth the name", "nobody stops anyone" are FALSE on the 11 towns
of the preimage that carry those rows — and **a body may not be inferred into the key's silence where a
required row or a same-tab machine sentence denies it** (`threatAssessment.js:115-125`;
`safetyProfile.js:281-322`; `guardEffectivenessDesc` `:340-382`, whose branch at `:376-382` prints the
town's own "no formal enforcement body" where that is true). **V-27** takes an ORGANISED or STANDING
manning — a rotation, a force, soldiers — on a town where no force row resolves; **V-09** takes any wage,
pay or arrears predicated of a citizen militia; **V-02** takes a possessive that SEATS a denied body
("the gate's watch"). On floor 2: **F2-01** any magnitude in a digit or a word (a handful, a dozen, most
of them, half the approaches) — the read hands you the band word and nothing else; **F2-02** any date,
season or duration; **F2-03** a raising or a founding narrated; **F2-04** an event the record did not run
(the men drifted off, the wall was left, the watch was stood down); **F2-06** any rate (most nights,
every spring, more often than not). ⭐ **But the perfect and the durative ARE licensed here**, and
generously: `defenseProfile.readiness.score` is the key's OWN read and has **zero** pulse writers, and so
have `defenseProfile.institutions.*`, `defenseProfile.scores.*`, `economicState.compound.inst.*`,
`economicState.safetyProfile.*`, `config.monsterThreat`, `stress` and `structureKey` (section 6) — so
"nobody has been asked to stand on the works", "the arrangement has never been tested", "it has never
been anybody's particular job" are lawful. What stays refused over the frozen fields is the
COMPARATIVE AGAINST A PAST and the TREND; and the durative is refused entirely over the LIVE roster
(`institutions`, and its walls/garrison/militia/watch/mercenary/charter buckets: **38 writers**). On
floor 3: **F3-05** cultural furniture — no thatch, no churchyard, no market green, no hearth-smoke, no
snow on the road (eleven culture profiles ship and this key reads none of them); **F3-06** as the office
table above, with the Elder trap; **F1-126** no minted proper name of a person, a lane, a family or a
house. On floor 4: **F4-02 / F4-03** one military purse, never split and never split in direction —
the wall's keeping and the wages move together on one multiplier; **F4-04** never a total collapse of
pay (every gate has a floor of 0.6 and the community baseline is exempt: short, late and thin are the
licensed extremes); **F4-01** no decay clock and no permanence; **F4-05** `plagued` is MONSTERS;
**F4-19** the pay gate DOES reach the watch, so "the watch is not paid out of that purse" is false.
**F1-34** bars a totality either way — no "nothing threatens this place" and no "nothing here would
survive anything", since `threatAssessment` builds an Invasion & War row for every town. **F1-31** bars
the tier word (the identity strip prints it beside the name) and F1-31's rider bars spelling the band.
⛔ **AND THE CITATION CEILING IS ZERO ON THIS POOL**: the shipped DS-DEF-1 rows carry **no named record
of any kind**, so under F1-24 and the provenance rule no face here may cite one — no muster roll (the
only holder in the shipped roster is a `Citizen militia` no required row seats), no toll book (gates
are OPEN), no parish register (the `Register of the dead` service sits on `Parish burial grounds` at
TOWN only), no accounts, no "the books", and no *"from the road"*, which matches the PROVENANCE shape
in `moveGrammar.js` and has already refused a whole packet once. An account here names a ROLE and never
a RECORD. Finally: section 2c prints **no covert field marking a symptom pool on this block**, so **no
face of this pool may carry the `compromised` tag at all**; and the `[archiver · observed]` and
`[public]` candidates stay under NOTES until cars 18n and 18m land, with 2b′ binding them hardest —
neither may claim not to have SEEN a service a required row turns on.

---

## (9) WHERE THE FLAVOUR IS

**What is in use.** Not a force — the required rows themselves, and on 226 of these towns they are
domestic. The water everyone draws from, the burial ground at the edge of it, the dwellings, the
grazing, the fields, and at thorp and hamlet the **mill you have to leave the settlement to reach**
(`Access to external mill` = "must travel to the manor mill"): the defence actually in use is a door
that bars, a field you can see across, a walk nobody makes alone after dark, and the fact that
everyone knows every face. At town the thing in use is concrete and different — `Town watch`'s
**night patrol** (p 1) and **gate duty** (p 0.8), both part-time by the row's own words.

**What is in dispute.** Who is supposed to do it, and out of whose hands. The engine models the split
exactly: one purse pays the keeping and the wages together at a rate the economy sets, and the
**community baseline is unpaid and exempt from the gate entirely** — so the argument on the ground is
between what is owed and what is merely volunteered. At town the guilds pay a fee toward an
arrangement the households use and do not pay for, and the hall answers for what it did not fund;
below town the older households hold that the arrangement is customary and has always sufficed, and
the ones who have arrived since have never seen it asked for anything.

**What the absence looks like on the ground.** WEAK is not emptiness — it is **coverage without
depth**: something answers once and nothing answers twice. The particular, visible form of that is a
thing that exists and is not continuously manned, and a person who gets further in than anyone meant
them to: the stranger who reaches the middle of the place before anybody looks up and is not asked
what they are doing there; the thing everyone can name that nobody has been given the job of; the
task that gets done because one particular person happens to do it and would simply not get done
otherwise. The hook is always the second demand — the town is fine until two things happen at once,
and nobody has ever had to decide which one gets answered.
