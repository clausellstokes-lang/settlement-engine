THE CARD (mechanical sections) — block DS-DEF-1 · pool `readiness CRITICAL` · dir ds-def-1-readiness-critical

(1) THE KEY AND ITS PREIMAGE
  key function: READINESS_ROW_POOL (rung table) · site defense.postureHeader
  census predicate: scoreBand(readinessScore) (via READINESS_ROW_POOL in defenseStateProse.js) === CRITICAL
  ...
  the key FIXES: readinessScore:range=[0,19] · leaves OPEN: readinessScore   (posturePoolKey, 20 combination(s))
  preimage on the 768-town rate grid: 140 towns (1823 bp) — thorp 64/128 · hamlet 50/128 · village 26/128
