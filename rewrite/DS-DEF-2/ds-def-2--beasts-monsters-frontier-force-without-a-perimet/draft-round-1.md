# DRAFT ROUND 1 — DS-DEF-2 · `Beasts & Monsters: frontier, force without a perimeter`
...
**The pool's whole discipline, obeyed in every one of the twelve.** `garrison || militia` is one boolean over a part-time citizen body and over multiple standing garrisons, so no face predicates the body's kind. The six namings used are *armed people* · *a muster / the muster* (F1-03: "the muster" as the class word is free everywhere) · *the town's own people* · *the people under arms* · *whoever the town has under arms* · *anybody armed* · *the people who would do the meeting*. None of them is `soldiers`, `professionals`, `full-time`, `the garrison`, `the militia`, `armed citizens`, `townspeople` or `the able-bodied` (CR-1, CR-2); none is `the watch` (CR-3); no muster ROLL is cited as a record (CR-9 / F1-24, the `muster` holder resolving only on the militia half).

**Unread buckets left alone.** Watch, mercenary, charter and magicDef are unread by this key, so no face asserts or denies any of them (CR-5, CR-6, CR-7). Every absence written is narrowed to the walls bucket or to `hasGates` — `no wall at all`, `no work standing between the country and the town`, `nothing on it says where the town begins`, `the town does not close at dark` — and never to defence in general, which is why no face says "and nothing else".

### Variant 1 · `[ledger]` · slot `{settlement}`

| face | claim | what licenses it |
|---|---|---|
| 1 (line) | frontier country around the town | `measuredMonsterFamily(config.monsterThreat) === 'frontier'`; the card's predicate. The tier word is the engine's own, one of exactly three (`monsterThreat.js:28`) |
| | answered by armed people | `garrison.present \|\| militia.present === true` (`defenseStateProse.js:655`). Class-blind, so true on both halves of the preimage |
| | and by no wall at all | `standingDefenseForces(settlement).walls.present === false`; the walls bucket (`defenseInstitutionBuckets.js:84-87`). Narrowed to the walls class, so it denies no unread bucket |
| | whatever comes chooses the place | the branch's own reading (`threatAssessment.js:88-90`), and a capability clause under the block's PROVENANCE fence. No creature named, no incident (CR-13); `whatever comes` kept from the shipped line as the marker asked (§V1.8) |
...
| 1a | nobody is stopped on the way in, nobody is turned away | `hasGates` is necessarily FALSE on this key (`priorityHelpers.js:53`: every member of its keyword set also matches the walls bucket), and `safetyProfile.js:463-464` prints the engine's own denial on the same dossier — *no gates to bribe and no checkpoints to avoid*. Skeleton §4 and §9(b) |
...
| face | claim | what licenses it |
|---|---|---|
| 2 (line) | a stranger reaches the town without being stopped, asked or turned aside | `hasGates` false + `safetyProfile.js:463-464`. W27's stance rules are struck, so a stranger may act, be turned away or not be turned away |
...
| 2c | the arrangement seen whole: frontier country, armed people, and no work standing between the country and the town | all three reads in one line, in the order the record draws them. `no work standing between` is scoped to a perimeter and so denies no unread bucket and no watchtower |
...
2. **Twelve constructions, twelve openers.** *The frontier country · Nobody · Nothing · Wherever · A stranger · The last house · By whatever way · Anyone arriving · The town can · Word comes · The town does not · The complaint.* No face is a permutation of a sibling, none swaps the halves of one `A, and B` join, and the pool spends no single noun more than three times across its twelve renderings.

### The twelve faces, measured

| face | words | sentences | slot | opener |
|---|---|---|---|---|
| 1 (line) `[ledger]` | 26 | 2 | `{settlement}` | The frontier country |
| 1a | 30 | 2 | `{settlement}` | Nobody |
| 1b | 21 | 2 | `{settlement}` | Nothing |
| 1c | 21 | 1 | `{settlement}` | Wherever |
| 2 (line) `[visitor]` | 25 | 2 | `{settlement}` | A stranger |
| 2a | 24 | 2 | `{settlement}` | The last house |
| 2b | 35 | 2 | `{settlement}` | By whatever way |
| 2c | 27 | 1 | `{settlement}` | Anyone arriving |
| 3 (line) `[street]` | 26 | 1 | none | The town can |
| 3a | 29 | 2 | none | Word comes |
| 3b | 28 | 2 | none | The town does not |
| 3c | 33 | 2 | none | The complaint |

Range 21 to 35 words; no two consecutive faces share a length and a grammar (R-DA-05). Mechanically checked: zero em dashes, zero exclamation marks, zero digits, zero `, which` tails, zero `[plain]` markers, and no face or variant line opening on a `proper`-typed slot.
