# CURE — curer seat (Opus 5, a different author from the writers) · block DS-DEF-2 · pool `Internal Security: full legal chain (court AND prison)`
...
- **v1 f4 `[guild]` — FAIL 1 CURED.** The finding was the trade denying `Town hall` → `Dispute arbitration` (on, p 0.8, "Bring commercial and civil disputes before a magistrate"; card.md:43), on the one tier the guild is seated at and the one tier where that is the hall's whole printed legal procedure. The face now AFFIRMS it — "the hall gets their quarrels" — and the greed moves onto the half of the key this pool actually fixes: the cells get nobody and the trade pays for their keeping anyway. The one purse is affirmed, not split (F4-02 / F4-03 clear), and the shape changed with the content: the hinge is `though`, not `and`, and the opener is the trades' own talk with its place named (ruling 39), which also breaks the run of three attributed subjects the draft carried at v1 f3 to f5.
- **v2 f10 `[gate]` — FAIL 2 CURED.** The finding was "before the watch or the hearing ever sees him" negating `Town watch` → `Gate duty` (on, p 0.8, "Check travelers entering and leaving"; card.md:44) on the town tier, where `hasGates` is open and the gate-sourced face therefore draws. "The watch" is gone; the precedence claim is now only against the HEARING, which no row denies at any tier. The quarrel with the hall survives in a second sentence rather than in an `and` clause, so the face also carries the two-sentence stop the pool had nowhere.

THE DULL VERDICT — WHAT WAS DONE, MEASURED
...
FACES — variant 1
- hall | the one purse, stated by the body that keeps it, with the hall's own preference for a single reckoning instead of a shortfall. Prepositional opener; two sentences; no split of direction.
- tavern | fifteen words, place-first, the comparative in a source's mouth the refuter passed. The shortest face in the variant, set against the two-sentence faces either side of it.
- watch | THE BILL for this variant (one per variant, ruling 32), landed on the watch's own round. The second sentence is the archiver reporting a silence, which is licensed where a bare assertion is not.
- guild | THE CURED FACE. See the target sheet above.
- court | the court's stake is its cost and the figure is withheld; the second sentence stops on the withholding instead of hinging on it.
- market | object-fronted and attribution-last: the coin is the subject, the traders are the source, and the second sentence refuses to follow the coin any further. Affirms the one purse.
- gate | the gate's pride in nine words, and a name ASKED, never written, so no gate record is cited.
- garrison | the overlap named as duty, one sentence, the `and` joining one predicate. Draws at city and metropolis, where `Garrison` and `Professional city watch` are two rows.
- stranger | the contrast on a full stop: other towns in the first sentence, this one in four words. The shortest close in the pool.
...
weigh | variant 2 | pair 1 | It may be that the difference is in what each counts as brought. <!-- shortened from the draft's "The two do not agree, and it may be that…": the first clause restated the pair and the hinge was the mould's. Conjecture, opens, favours neither, names no office and no record · car 18i -->
weigh | variant 3 | pair 3 | Which is right is a matter of debate in the town. <!-- the refuter's own cure applied: "and it is not settled here" read two ways and one of them was the dossier referring to itself (ruling 40), and the shorter candidate that stops sooner wins · car 18i -->

OBSERVED (car 18n; one per pool)
- `[archiver · observed]` | variant 2 | The bench where people wait is empty in the middle of the day. <!-- moved off the lock as the refuter's cure asks (the lock carried seven appearances). Bare passive, no observer named (ruling 40), no elapsed course, no number. The bench is card §9's second named object and nothing in §2b′ is inferred from its emptiness: no required row seats a person on it. -->

PUBLIC (car 18n; one per variant; the `and takes it that` frame retired from two of the three, as the refuter's cure asks)
- `[public]` | variant 1 | Everyone in the town has seen the door with the lock on the outside of it, and takes it that the lock is enough. <!-- the one candidate that keeps the frame. The seeing is true; the taking-it is a perception and may be mistaken. -->
- `[public]` | variant 2 | The townsfolk have all seen somebody walked in by the watch. What they make of it is that the matter is finished when the door shuts. <!-- frame retired; two sentences; the perception is explicitly the town's reading and not the thing seen. -->
- `[public]` | variant 3 | Anyone here can tell a stranger where to bring a complaint. Whether a stranger gets the same hearing as a local is not a thing the town doubts. <!-- frame retired, AND the opener no longer collides with spine 2's "It is common knowledge here that", which the refuter flagged as a craft fault. No future indicative ("can tell", never "will tell"). -->

COMPROMISED (car 18m; §2c marks this pool for the watch and the court; six shapes, still all different)
- `[watch · compromised]` | variant 1 | One of the watch says the patrol is kept as it has always been kept. Who walks it is the watch's own affair. <!-- MINIMISE · the durative runs over `defenseProfile.institutions.watch`, writers 0, FROZEN (card §6) · the mould hinge replaced by the stop -->
- `[watch · compromised]` | variant 2 | Asked about the walk after dark, one of the watch says the gate is the thing worth asking about. Nothing further is said about the walk. <!-- CHANGE THE SUBJECT · the subject changed to the gate rather than the wage, so it no longer overlaps v3 f5's wage -->
- `[watch · compromised]` | variant 3 | One of the watch says the talk at the tavern about who walks the circuit at night comes from people who are not out at night. <!-- BLAME THE TALK -->
- `[court · compromised]` | variant 1 | A magistrate holds that whatever the watch brings is heard as it should be heard, and asks that it be left there. <!-- REASSURE -->
- `[court · compromised]` | variant 2 | A magistrate says there is nothing in what is heard here worth a stranger's time. <!-- DISMISS -->
- `[court · compromised]` | variant 3 | A magistrate says that a matter brought is a matter heard, and says no more than that. <!-- SAY NOTHING BEYOND THE FORM · the refuter's own cure applied: the draft's "says what a magistrate is required to say" editorialised in the archiver's hand, which is the wink ruling 26 forbids and the warmer-or-colder verb ruling 19 bars -->
- No reassurance stands untagged in this pool: the key does not fix the internal upkeep gate, so an untagged reassurance on the purse would draw on the underfunded towns too.

NOTEBOOK (dm-only; ruling 17; shades and feelings spread)
- variant 1 | conjecture · curiosity | Who is on the walk after dark is the one thing the watch does not answer readily, and it answers readily on the rest… it would be worth knowing who fills the circuit, and who pays them to. <!-- points toward the watch's covert bloc (hard state on this pool) -->
- variant 2 | conjecture · eagerness | What the walk brings in and what is heard afterwards are not the same thing, and nobody here is placed to see both… whoever is placed to see both is worth finding. <!-- the hall is dropped from the note, as the refuter's caution asks: naming it risked a reader taking the note for the hall's capture, which §2c does NOT mark on this pool. The seam still points toward the court's field. -->
- variant 3 | accusatory | Whoever sits that day decides what is a matter and what is only a quarrel, and it may be that somebody has told them which is which. <!-- the refuter's own cure applied: the draft's settled term ("somebody HAS told them") was a verdict and shaded toward F2-04; "it may be that" makes it the suspicion ruling 17 wants. Still an accusation in shade, still names no singular office, still points toward the fact. -->

MECHANICAL SELF-CHECK (run over the rows and every line under NOTES): em dash 0 · digit 0 · exclamation 0 · semicolon 0 · contraction 0 · future indicative (will / shall) 0 · `{settlement}` inside a face 0, in one unit (spine 1) · self-citation ("the survey", "this office", "the record", "entered as", "set down here") 0 · named office (Mayor, Guard Captain, High Priest, Chief Magistrate, City Watch Chief, Governor) 0 · "the priest" 0 · "the mayor" 0 · source words all from the closed list (hall · watch · court · tavern · register · market · stranger · guild · garrison · gate) · pair kinds all from the closed list (disagree · aside) · notebook devices (ellipsis, colon, aside in commas, unfinished clause) on dm-only lines only · faces per variant 9 / 10 / 9, unchanged from speakers.md, every face keeping its seat and its index so the re-refuter's v1f4 and v2f10 still name the cured faces.

FLOOR RE-CHECK ON THE LINES THAT MOVED MOST
- v1 f4 "the hall gets their quarrels": affirms `Town hall` → `Dispute arbitration` and `City hall` → `Civic licensing` / `Appeals court`; asserts the guilds' own `Dispute resolution` nowhere, so nothing turns on an off-by-default row.
- v1 f6 "taken at the stall": `Weekly market` → `Tax collection` (p 0.9, ON) at town; nothing at city or metropolis denies a levy at a stall. The coin is one coin, and the face refuses to follow it, so no direction is split.
- v2 f8 "until one of them comes to the wall": `City walls and gates` is required at city and metropolis, which is where the garrison is seated; the face asserts no wall at town, where it does not draw.
- v3 f4 "let out for what he can pay": `Small prison/stocks` → `Fine payment` (p 0.9) and `Holding cells` "short-term detention pending trial or payment of fines" at town; at city `Bail hearings` is off by default, and an off service is not a denial (ADDENDUM 14). No term, no duration, no count.
- v3 f5 "the wage is the same whether the night is quiet or a man is walked in": `Night patrol` (p 1.0) affirmed, no shortfall asserted, no split of the one purse, no rate ("whether" is a condition and not a frequency).
- The observed candidate's bench: no required row seats anyone on it, so its emptiness infers into no silence §2b′ protects.
- Nowhere in these rows: a courthouse building, a criminal trial, a sentence, a term, a gaoling, a gallows, the stocks, a pillory, a warrant, a filed report, a court book, a gaol roll, a professional town watch, a militia, a mercenary company, a charter hall, a hospital building, an elder, a crown's assessor, an assize, a shire, a borough, a churchyard, a market green, or a proclamation read in the square.

REFUSED TARGETS: none. Both targets cured, and the DULL verdict answered at the grain the refuter named it at.
