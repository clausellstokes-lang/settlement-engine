THE CARD (mechanical sections) — block DS-DEF-2 · pool `Beasts & Monsters: plagued, perimeter but NO force to hold it` · dir ds-def-2-beasts-monsters-plagued-perimeter-but-no-force-to-hold-it

(1) THE KEY AND ITS PREIMAGE
  key function: BEASTS_ROW_POOL (rung table) · site defense.threatAssessment
  census predicate: beastsRowSituation(family, perimeter, force) (via BEASTS_ROW_POOL in defenseStateProse.js) === plagued country, perimeter without force
  ...
  the key FIXES: monsterThreat="plagued" · perimeter=true · force=false   (beastsRowPoolKey, 1 combination(s))
  preimage on the 768-town rate grid: 45 towns (586 bp) — thorp 34/128 · town 11/128
  silent tiers: hamlet, village, city, metropolis
...
(2c) COVERT FIELDS AND THEIR SYMPTOMS ON THIS BLOCK (ADDENDUM 18 ruling 26) — the INVERTED test the refuter judges a `compromised` face under
  ...
  PLACEMENTS A FACE MAY NOT ASSERT — where a body STANDS, stated by the data at some preimage tiers and not others, or stated with a hedge
    Market square: "in the square" — stated only at town — silent at thorp   [the `Civic announcements` service]
        the data says: "Official proclamations, wanted notices, and public notices read in the square."
    Parish burial grounds: "beyond the gate" — stated only at town — silent at thorp   [the catalog row]
...
(2d) THE KEY'S SIBLING RUNGS — what this pool's key would have been had ONE of the fields it fixes been otherwise
    ...
    perimeter = true HERE
        perimeter = false -> `Beasts & Monsters: plagued, NO perimeter and NO force`
    force = false HERE
        force = true -> `Beasts & Monsters: plagued, perimeter AND organized force`
...
(5) THE CLOSED ROSTERS THIS KEY DOES NOT FIX
  watch      OPEN — required only at town; open at thorp   [town: Town watch]
  ...
  granary    OPEN — required only at town; open at thorp   [town: Town granary]
  hospital   OPEN — the key does not fix it and no required row seats it
  market     OPEN — required only at town; open at thorp   [town: Market square, Weekly market]
  hall       OPEN — required only at town; open at thorp   [town: Town hall]
  court      OPEN — required only at town; open at thorp   [town: Town hall]
  prison     OPEN — the key does not fix it and no required row seats it
  church     OPEN — required only at town; open at thorp   [town: Parish churches (2-5)]
