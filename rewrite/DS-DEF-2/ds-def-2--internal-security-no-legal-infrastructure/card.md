THE CARD (mechanical sections) — block DS-DEF-2 · pool `Internal Security: no legal infrastructure` · dir ds-def-2-internal-security-no-legal-infrastructure
...
(1) THE KEY AND ITS PREIMAGE
  ...
  the key FIXES: court=false · prison=false   (internalRowPoolKey, 1 combination(s))
  preimage on the 768-town rate grid: 384 towns (5000 bp) — thorp 128/128 · hamlet 128/128 · village 128/128
...
(2d) THE KEY'S SIBLING RUNGS — what this pool's key would have been had ONE of the fields it fixes been otherwise
    court = false HERE
        court = true -> `Internal Security: court without detention`
    prison = false HERE
        prison = true -> `Internal Security: detention without process`
...
(4) THE SIBLING SENTENCES — on-page machine strings that can fire beside this key (excluded only where the predicate is definitely false under the key)
  ...
  • [buildThreatAssessment -> intA] src/domain/display/threatAssessment.js:151  FIRES ON EVERY PREIMAGE TOWN
