THE CARD (mechanical sections) — block DS-DEF-2 · pool `Invasion & War: walls with NO force` · dir ds-def-2-invasion-war-walls-with-no-force

(1) THE KEY AND ITS PREIMAGE
  key function: INVASION_ROW_POOL (rung table) · site defense.threatAssessment
  census predicate: invasionRowSituation(walls, garrison, militia) (via INVASION_ROW_POOL in defenseStateProse.js) === walls, no force
  ...
  the key FIXES: walls=true · garrison=false · militia=false   (invasionRowPoolKey, 1 combination(s))
  preimage on the 768-town rate grid: 125 towns (1628 bp) — thorp 60/128 · hamlet 6/128 · village 8/128 · town 51/128
...
(4) THE SIBLING SENTENCES — on-page machine strings that can fire beside this key (excluded only where the predicate is definitely false under the key)
  ...
  • [buildThreatAssessment -> mil] src/domain/display/threatAssessment.js:119  FIRES ON EVERY PREIMAGE TOWN
  ...
  • [generateSafetyProfile -> guardEffectivenessDesc] src/generators/safetyProfile.js:382  CAN CO-FIRE (predicate open)
      ...
      when: !(inst.hasMilitaryInst)  AND  !(inst.hasCourtSystem)  AND  !(['thorp', 'hamlet', 'village'].includes(tier))
  (excluded as contradicting the key: 16 strings)
