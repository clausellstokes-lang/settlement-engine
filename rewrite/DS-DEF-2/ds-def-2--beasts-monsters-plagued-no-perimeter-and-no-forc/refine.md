...
1. `[ledger]` An embattled country of monsters lies around {settlement}, and the place stands inside it without works or muster.
   ...
   - `[face]` The country about {settlement} is plagued with monsters, and the place is entered as neither walled nor mustered.
...
2. `[street]` In a country plagued with monsters, {settlement} stands open and unmustered.
   ...
   - `[face]` Creatures are thick in the country around {settlement}, and the place itself stands with no works and no force.
...
(and keeps `walled` once, on the engine's own predicate word).** Ground, three-fold and all of it
...
`walled` survives at face 1b alone, because `defenseProfileHasWalls` is the receipt's own
predicate name and "entered as neither walled nor mustered" is the office entering the negative
CONDITION (a LACK), never the record failing to hold it (a GAP — R-DA-08, which would be a claim
change). No claim moves anywhere: C2 before, C2 after.
...
| 1b | `Around {settlement} the country is…` → `The country about {settlement} is…`; `wall` untouched as `walled` | R-DA-05 / the tic count — the draft opened four of twelve faces on a fronted place; this one goes subject-first and the pool's fronted-place openers fall to two of twelve. `walled` is kept deliberately: it is the receipt's own predicate word (`defenseProfileHasWalls`) and the office's `entered as` formula (W7 / R-vi) needs an adjectival pair |
...
| 2c | `At {settlement} the country is thick with monsters, and the place itself stands with neither a wall nor a force.` → `Creatures are thick in the country around {settlement}, and the place itself stands with no works and no force.` | the draft's third fronted-place opener in one variant (the tic); the new opener is subject-first and finite. `thick` is kept (the band word the drafter defended), `force` is kept (the engine's own token at `defenseStateProse.js:655`) so the pool still lands once on the producer's own word |
...
sickness, war, raider or enemy word. W15 cured pool-wide at N0; `works`, `muster`, `force`,
`walled` and the family words (`monsters`, `creatures`, `beasts`) are the always-safe or
producer-own spellings. W16 no pay gate is read; no upkeep, wages, pay or purse. W17 no siege,
...
appears once, at 1b. W8 the ledger enters and cites nothing; the street states the standing plainly
...
no habitual triad, no citation; **no participial opener and no participle anywhere** (the gate's
one failing round-1 measure stays at 0.000 on 12 of 12).

| # | angle | words | first two words | close kind |
|---|---|---|---|---|
| 1 | `[ledger]` | 18 | An embattled | absence (muster) |
| 1a | `[face]` | 13 | Creatures range | absence (works) |
| 1b | `[face]` | 18 | The country | condition (mustered) |
...
| 2c | `[face]` | 19 | Creatures are | absence (force) |
...
Close kinds: seven absence, two condition, three object (the draft: seven / three / two) — the two
LACKs are this key's whole second half, so absence stays the natural majority while all three
drawable kinds are populated. Pronoun closers: zero. Abstract-noun closers: zero.
...
kept lawful (§21.2): every change above is either a law cured (N0's W15 / D-6 exposure, 3b's
missing band marker) or a named target beyond the gate (the band's interior, sibling distance, the
sharper fact, the read-aloud ear) — none of them trades density for plainness (§21.4).
