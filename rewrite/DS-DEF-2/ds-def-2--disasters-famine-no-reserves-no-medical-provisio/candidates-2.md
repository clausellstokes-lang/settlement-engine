# candidates-2 — DS-DEF-2 · `Disasters & Famine: NO reserves, NO medical provision`

writer 2 of 2 · seat Fable 5.1 · 2026-09-13
lens: THE SOCIAL POSITION (who pays, who is owed, who is resented; the source's stake shows in what it calls a fact) and THE WITHHELD REASON (a fact set down plainly and the reason not given)
reading: `rewrite/recut/EXEMPLAR-PACK.md` · the pool's shipped and current rows (identical, three lines) · this pool's `card.md`. Nothing else.

## THE MAPPING OF THE CARD'S SPEAKERS ONTO THE CLOSED SOURCE VOCABULARY (the selector's to veto)

The card's section (7) says the preimage is wholly below town: no hall, no guilds, no watch, muster, garrison, walls or gates, no tavern at thorp, and no record of any kind. The brief's closed vocabulary (stranger · elders · hall · tavern · guild · register · muster · watch · garrison · gate · market · court) names five of the card's speakers and leaves four without a word. This packet maps them so:

- the stranger → `[stranger]` (universal).
- the elders → `[elders]` (universal by the brief's word; the marker's own call, recorded for veto). The role is written "the older households", the marker's phrase, so the attribution survives ruling 16's strict reading and asserts no council.
- those who bury the dead → `[register]`. The card's mechanical section (7) seats "the register (parish)" on `Burial ground` at thorp, `Access to parish church` + `Burial ground` at hamlet, `Parish church` + `Priest (resident)` + `Graveyard` at village. A `register` face therefore draws on every thorp that has nothing but a burial ground, so EVERY register face below is written from the buriers' stake and asserts no church building, no priest and no book.
- the households who farm (card 7.1: `Dwellings` + `Subsistence farming` / `Farmland`, required at every tier) → NO closed word names them. Left UNTAGGED, which draws everywhere and is their true seat. The selector should count them as their own source and not as the stranger's; each such row carries a comment naming the card's row.
- whoever grinds the grain (card 7.3: `Access to external mill` / `Mill`, required at every tier) → likewise UNTAGGED. The stone is placed nowhere: below village it is outside the settlement and at village it is inside, so no face says where it stands.
- those who draw the water → ground, not a speaker. "The water" is where talk is heard and where a bad year is or is not admitted.
- the tavern → `[tavern]` (conditional: `Alehouse` 0.55 hamlet · `Ale house` 0.80 village · nothing at thorp). If the projector's `tavern` keyword does not match `Alehouse`, these faces resolve nowhere on this preimage and the selector holds them.
- the market → `[market]` (conditional: `Periodic market` 0.12 hamlet · `Weekly market` 0.60 village).
- the reeve or steward, and those who treat the sick without a hospital → conditional, and no closed word; offered under NOTES with the seat named, held until a seat exists.
- `compromised` is REFUSED on this pool (card 2c marks no covert field here), so no such candidate is offered and no reassurance closes any face.
- `[public]`, `[archiver · observed]`, the weighings and the notebook notes are under NOTES per variant, as the brief directs, in the row syntax where the syntax exists.

## WHAT EVERY CANDIDATE AVOIDS (card section 8)

A hall, clerk, guild, watch, muster, garrison, wall, gate or toll. Any cited record: no "the books", no register, no accounts, no count kept by anyone. A church building in the settlement (the hamlet's is two to five kilometres off). A mill in the settlement (the grain goes to "the stone", placed nowhere). "Nobody treats the sick" or any absence of PEOPLE: the absence this pool describes is of a granary and of a sick-house, and a village priest heals, a famine town mints a healer. "Nothing is stored" (the thorp's shared root cellar): the safe noun is the granary that does not stand. Any closing of the town off (the port). Any count, date, season, month, rate or past event; a durative only over the frozen reads and the archiver's own presence. The singular offices: elder, priest, mayor, guard captain, miller, healer. Thatch, churchyard, market green. The future indicative. `{settlement}` inside any face. The document naming itself.

Shapes are varied on purpose across the packet: subject first, place first, object fronted, attribution mid and attribution last, the bare observation, the entry form of the spine. The attribution verbs rotate (says · holds · in the account of · by the account of · at the tavern they say · in the view of) so no verb runs three times.
...
1. `[ledger]` No granary stands here and no house is kept for the sick. The grain is ground on somebody else's stone.
   ...
   - `[face]` `[register]` Those who bury the dead say their work does not slacken in a bad year, whatever else does.
   - `[face]` `[register]` Those who bury the dead hold that they are sent for when the bread stops coming to a door, and not before. <!-- who is owed, and when -->
   - `[face]` `[tavern]` At the tavern they say whoever grinds the grain eats in a bad year and everyone else counts. <!-- who is resented -->
   - `[face]` `[market]` Grain is bought here to eat and not to keep, in the account of those who sell at the market.
   - `[face]` `[elders · pair 1 · disagree]` The older households say the harvest is each house's own and nobody else's. <!-- joinable: though -->
...
3. `[visitor]` A stranger who asks here for the granary is shown a field, and one who asks for the sick-house is shown a door.
   ...
   - `[face]` `[market · pair 2 · aside]` Those who sell at the market say a buyer can have grain here on a market day and no place to keep it.
   NOTES:
   - `[face]` `[archiver · pair 1 · weigh]` The households' account is the likelier here, a pedlar counting a place without a store as a place with nothing to buy. <!-- a lean, with its reason drawn from the other source's stake; the only lean in the packet -->
   - `[public]` Anyone here can point a stranger to the field where a granary would stand if there were one, and the town counts the pointing as an answer.
   - `[archiver · observed]` A stranger asking here for the sick-house is sent to a door, and the door is a house like the others.
   - notebook (dm-only · shade: unsure · feeling: surprise): For a place with no granary and no sick-house, nobody here asks a stranger for either… it is possible the want is met somewhere that does not show, a cellar under somebody's floor, a woman who knows roots. Or it is possible the place has not yet been asked for more than it has. <!-- points at the conditional rows the card names (root cellar, midwife, apothecary) without seating them -->
   - notebook (dm-only · shade: clarity · feeling: eagerness): The sacks are counted going to the stone and not coming back, and there is only the stone between, and whoever keeps it. If the difference is ever wanted, that is where to ask for it, and it would be worth the asking.
