...
**Pool.** DS-DEF-2 · `Invasion & War: walls with citizen militia` · draft round 1 · Opus WRITER (Seat: Opus 5, Fable-unvalidated), for the Fable chair.
Card printed first: `node scripts/prose-licence-card.mjs DS-DEF-2 'Invasion & War: walls with citizen militia'` in `laneRW-DEF2`. The block's annex section read whole (`laneRW-DEF2/docs/content/RECEIPT_POOLS_DOSSIER_STATE.md:2568-2726`): this pool's three rows, its five sibling `Invasion & War` pools, and the twenty-one spines of the block's other four rows (arms A1 and A11 — nothing below restates or contradicts a sibling). The selecting branch read at source: `src/domain/display/stateProse/defenseStateProse.js:458-465` (`INVASION_ROW_POOL`), `:476-483` (`invasionRowSituation`), `:490-492` (`invasionRowPoolKey`), and the caller at `:656`.
...
| 2 · `[street]` | **10** | **8** | **5** | **6** |
| 3 · `[unfolding]` | **13** | **10** | **9** | **11** |

mean **10.25** · min **5** · max **19** · sd **3.39**. Every wording is ONE sentence; no semicolon and no colon anywhere, so a spine composed on any of these contributes no second segment and no arm-Q summarising beat to its unit.

---

## 1. WHAT THE CARD LICENSES — the components, read at the branch

The card prints one claim: that `invasionRowSituation(walls, garrison, militia)` selects the row `walls, citizen militia` of `INVASION_ROW_POOL`, **as a STANDING fact of the record**. Read against the branch (`:476-483`), that one row-selection is reached on `walls && !garrison && militia` and resolves to three components of ONE typed claim (one table row, one key string), never three separate facts:
...
| G | the town has no professional garrison | `garrison` false — the branch reads `garrison` **before** `militia` on both sides of the wall, so the row's identity includes the negative (`:472-474` docblock: *a garrison outranks a militia*) | an explicit contrast unless the old sentence made one; see §2 |
...
| W (the wall) | **NOT ADDED** | the old sentence never names the wall or any perimeter object; adding it would breach *never ADD a claim*. The variant is honestly thinner than its siblings, and that is the drop working. **Contestable — flagged in §5.** |
...
Every face is claim-equal to its three siblings (arm A6 reads ACROSS the family, not back to the old sentence). No wording opens on the `{settlement}` slot (T-F8); no wording opens on the settlement token at all, so R-DA-17's wall 10 (at most one such opener per pool) is satisfied with room.
