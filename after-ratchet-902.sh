#!/bin/sh
# after-ratchet-902.sh <expected-totalFiles> <expected-entries> — the §902 version (derived from 901; the CAPSULE step is ACTIVE again — the OSR CLI is open at rung 18): the totals car (guarded on TRUE_EXIT,
...
# launched SEPARATELY (run-gate-902.sh). Every exit captured; refuses on anything unexpected. POSIX sh.
SC=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad
D=$SC/laneOSR18; LOG=$SC/ratchet-902.run.log; BASE=b0cbc67a15baf0997028ab501c7ee09565f93980; SEAL=osr18-2026-09-05
...
echo "--- capsule car (ACTIVE at rung 18 — the OSR CLI is open):"; sh scripts/gate-mutex.sh --run -- node scripts/base-state-capsule.mjs --runtime-tests=$TT > $SC/capsule-902.log 2>&1; CE=$?; echo "CAPSULE_EXIT=$CE"; tail -3 $SC/capsule-902.log | cut -c1-160
[ "$CE" = "0" ] || { echo "REFUSED: the capsule did not regenerate — read capsule-902.log"; exit 7; }
CP=$(git status --porcelain -uall); echo "capsule porcelain=[$(echo "$CP" | tr '\n' ' ')]"
if [ -n "$CP" ]; then git add docs/implementation/BASE_STATE.json && git commit -q -m "Register (capsule car): the base-state capsule regenerates at rung 18 — the first regeneration since 2026-08-16
...
echo "--- re-stamp the kit at $N cars:"; python3 $SC/mk-landing-kit.py 902 laneOSR18 $BASE landing-osr18-2026-09-05 $N 2>&1 | tail -2
grep -q "^GATE_CARS=\|cars=$N\|EXPECT.*$N" $SC/run-gate-902.sh || grep -n "$N" $SC/run-gate-902.sh | head -2
git update-ref refs/preserve/$SEAL "$(git rev-parse HEAD)" && echo "SEALED $SEAL = $(git rev-parse --short refs/preserve/$SEAL)"
echo "NEXT: launch run-gate-902.sh in the background (quiet law; ~20 min; NO lane vitest meanwhile), then after-cas-902.sh, then fill texts-902 (__CARS__=$N __ENTRIES__ __TESTS__ __TOTALS__) and collect."
exit 0
