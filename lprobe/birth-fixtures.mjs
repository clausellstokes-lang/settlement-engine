 ...
 *
 * That fence is about the OUTPUT, not the INPUT. `SIMULATION_RULE_PRESETS[id].rules` is a
 * SPARSE OVERRIDE TABLE; reading it and calling the result "the preset's rules" is the
 * error — it omits every DEFAULT_SIMULATION_RULES key the preset does not override, and
 * it never runs the normalizer, so the mirror-key lockstep (faithSpread <-> religionDynamics),
 * the fail-closed boolean coercion and `presetId` inference all go unmeasured. The product's
 * OWN preset-application path feeds exactly that table in as a PATCH
...
for (const id of ids) {
  ...
  fixtures.push({
    ...
    // The sparse override table this preset carries, recorded so the DIFFERENCE between
    // the table and the resolved rules is legible rather than assumed.
