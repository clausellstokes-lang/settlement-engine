...
| 14 | **FABRIC DENSITY — THE INK-TO-WHITE AREA CENSUS** `i14-fabric-density.mjs` | five rows: **R1 toft density** `built/(built∪yard)` · **R2 quarter density** `built/(built∪yard∪void∪street)` — ⭐ **the row the reference figure is comparable to**, since MFCG's districts are quarters WITH their internal streets and courts · **R3 page budget** `settled/page` and `field/page` · **R4 shapes** per-mass vertices and area quartiles, re-measured every run so §698.1's exoneration is never inherited on trust · **R5 rotation** dominant-edge histogram and modal-20°-window share by count AND area, with the min-area-rect angle as an independent second reading | ⭐ **RASTERISED PAINTED AREA off the emitted SVG** — the union a viewer's eye integrates — **never summed vector polygon areas**, which double-count every overlap and cannot express a hole; ONE winding rule across all subpaths of a `<path>`, exactly as a renderer paints it (SPINE-3's banked class: a census must ask at the surface the reader SEES) | R2 target **0.431, band 0.38–0.52**. ⛔ **THE TARGET IS A DECLARED RECONSTRUCTION, NOT A MEASUREMENT** — `dissect/mfcg-everrise-census.json` holds per-building area QUARTILES and no mean (n 616 · p25 171.9 · p50 225.5 · p75 295.8 · districts 349,715), so the mean is reconstructed and **the reconstruction is declared rather than hidden** (J-FABRIC-2: a band around a reconstruction, never a point). Clean-room §668/§673: the reference's published NUMBERS are facts; its code and maps are not ours | **CONTROL_BENCH 6/6.** ⭐ DECLARED READINGS AT `map-sandbox-dressfab-grammar` = `f8f6456a0` (§701.1): toft **0.786–0.840** on all 18 leaves — where §697.2's "white gap as wide as the building" implies ~0.25 — and R2 **city 0.4485 · metropolis 0.4386 · town 0.4479**, 11 of 18 leaves IN BAND with every leaf outside it SPARSER. ⚠ **A BASELINE FILE INSIDE THE REF IS STILL OWED** (§707.8): these readings are declared here but not yet frozen under `out/`; the next instrument-consuming wave freezes them under the two-baseline law |
