...
grep -q '^GATE_CARS=13$' "$LOG" || { echo "⛔ gate did not run over 13 cars: $(grep '^GATE_CARS=' "$LOG")"; exit 1; }
...
python3 $SC/chair-verify.py "$D" "$BASE" "$LOG" 13 > $SC/chair-verify-917.out 2>&1 || { tail -6 $SC/chair-verify-917.out; echo "⛔ chair-verify RED — stop"; exit 1; }
