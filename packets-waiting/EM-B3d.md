...
- **Preamble:** `docs/implementation/preambles/EM-PREAMBLE.md` (SHA-256: TO BE STAMPED BY THE CHAIR — the compile lane measured `c9f33c8d2940372bde8a6d931e3d389b79516a90ed45405cbbbf3e698cc46675` at `ee204c827` with `shasum -a 256`, which is the fourth amendment the chair named in this lane's launch)
...
|---|---|---|
| First-paint eager closure | `EAGER_FIRST_PAINT_MODULES` imported live from `vite.config.js` — **269 modules** at the read tip — against `CLOSURE_BUDGET_BYTES` `1_048_000` | **NO** — all four paths `NOT-EAGER` |
...
# The instruments — tests/lint WHOLE, always, before the commit (ODQ §934.47 addendum 104)
GATE_MUTEX_TIER=shared GATE_MUTEX_MAX_POLLS=100000 GATE_MUTEX_POLL_SECONDS=20 \
...
**Expected:** every command exits `0`. ⛔ **A gate line with no printed test count DID NOT RUN** — the mutex's default gives up after 40 polls and exits 0, so never drop the two exports. Report actual counts; copy no historical count. No generator is among these commands, so no step invalidates a later one.
...
- `tests/lint` WHOLE: exit, count, and wall-clock:
...
- The lighting census delta, re-derived at the terminal against the DELTA predicted in §10 (`files +0 · parked +0 · credited +0 · titles +5 · suiteTitles +1`) applied to the baseline the chair stamped at promotion — and the pre-proof's `parkReasonsFor` output for both files:
