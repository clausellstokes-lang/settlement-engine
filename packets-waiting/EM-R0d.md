...
**Preamble:** docs/implementation/preambles/EM-PREAMBLE.md (SHA-256: TO BE STAMPED BY THE CHAIR — ⭐ **MEASURED AT THE v4 READ TIP `578272a99`:** `ce516004a5d8e680f4160b03ce660c66d78261988b78a82239664c596600af4b` (G-1). ⛔ **THE HASH TO STAMP IS THE ONE AT THE TIP THE CHAIR PLACES AT.** The preamble's FOURTH AMENDMENT lands ONE COMMIT LATER, at `2dc08a595`, where the hash is `c9f33c8d2940372bde8a6d931e3d389b79516a90ed45405cbbbf3e698cc46675` — both measured by `git show <rev>:docs/implementation/preambles/EM-PREAMBLE.md | shasum -a 256` (G-1). Version 3's `16dfb96f…` (the SECOND amendment) is STALE. The FOURTH amendment's law — a worker byte has NO placement cure — is priced at G-9.)
...
- **Verified base:** `__BASE__`
  ⭐ **The chair's revalidation sentence, rewritten by the v4 PRE-PROOF with the measured facts:**
  *"Compiled at `ad7ddf2c9`, re-compiled to version 2 at `e5bdfd031`, pre-proofed at `141a1d775`, and
  RE-PRE-PROOFED at `578272a99` — the composed, refrozen tip — by an Opus lane (evidence D-1…D-16, E-0…E-9,
  F-1…F-18, G-1…G-14). The J-T1 window `141a1d775..578272a99` over every change-manifest and `requiredSymbols`
  path is EXACTLY THREE FILES: `tests/components/dossierLabelCase.test.jsx` (+93/−2, FIX-P5's badge-capitals
  law), and two COMMENT-ONLY movers, `src/domain/prose/holderTable.js` (+1/−1, a `columnCensus` citation
  re-addressed `institutionTable.js:582 → :596`) and `tests/build/domainGeneratorsBoundary.test.js` (+3/−1, a
  header LOC figure re-measured) — no other declared path moved a byte (G-2, G-3). ⚠ THE WINDOW WAS MEASURED
  PATH BY PATH BECAUSE A BATCHED PATHSPEC READ FALSE-EMPTY: under zsh an unquoted variable does NOT word-split,
  so a path LIST in a variable is ONE pathspec, matches nothing, prints nothing and exits 0 (G-2b). All ELEVEN
  `requiredSymbols` rows resolve `1x` verbatim at this tip and every one survives this packet's own edits;
  `retiredSymbols` is EMPTY. The CREATE targets `src/data/bandLadders.js` and `tests/domain/bandLadders.test.js`
  are ABSENT and `src/domain/bandLadders.js` does not exist, while the homonym
  `src/domain/compendium/bandLadders.js` IS present (G-8). Across the manifest's 194 entries — **192 LANDED, 2
  SUPERSEDED, ZERO READY** — every change path is FREE except `tests/build/generationWorkerLazy.test.js`, which
  EM-P3 reserved and RELEASED at its landing; EM-P2 is now LANDED, so no non-LANDED entry holds anything of this
  packet's and the mutation-coverage manifest is FREE (G-11). ⛔ THE BASE MUST BE STAMPED TO THE THEN-TIP: the
  ceiling TEST row puts `generationWorkerLazy.test.js` in the dispatch substrate. THE BUDGETS, measured by
  IMPORTING the set as the amended §P2 row 11 demands: `EAGER_FIRST_PAINT_MODULES` is **269** at this tip (not
  268; sorted-list sha `a9f1f4e8ec8ce31c`), and the delta is **269 → 269** because NONE of the three producers
  is in the eager set — the only eager `/src/generators/` member is the specially-pinned `lookups.js` — so a
  leaf imported only by them cannot join the eager closure (G-10). The generation worker's bundle grows an
  estimated **+1,801 to +2,268 B** across three independent transcriptions against a ZERO-SLACK ceiling of
  1,401,208, inside the chair's named bound of 3,314 B with 1,046 B of worst-case room (G-9). The lighting
  census moves by a MEASURED delta, re-executed with the walker's own `classify` at this tip, where
  `dossierLabelCase.test.jsx` is now 14 titles (G-4); the wiring census `--check` is GREEN at the tip (708 pools
  / 2266 variants / 165 relation rows / 7 stamped files, exit 0) and stamps none of the three producers, but its
  CITE arm DOES bite (G-7); the tuning register SHRINKS from a re-measured 48 · 46 · 63 (G-12); and a NEW
  gate-wired citation walker exists whose margin is named in §3 and STOP-9 (G-5)."*
...
| ⭐ First-paint delta | **`0`. ⛔ THE ABSOLUTE MOVED AGAIN: the live `EAGER_FIRST_PAINT_MODULES` is **269** at `578272a99` (sorted-list sha `a9f1f4e8ec8ce31c`), not version 3's 268/`f21c167bcc41b3c3`, which are withdrawn. The DELTA holds **269 → 269**, and G-10 proves it structurally rather than by replica: NONE of the three producers is in the eager set (the only eager `/src/generators/` member is the specially-pinned `lookups.js`), so a leaf imported only by them cannot enter the eager closure. ⚠ `ENGINE_SHARED_DOMAIN` is NOT exported from `vite.config.js` and was not re-measured here; F-2's `51 → 51` stands as the prior lane's figure |
...
| manifest validates | PASSES — **12** change rows, every action in `PACKET_ACTIONS`; 8 acceptance cases as `{id, case}` objects (E-6); ⭐ re-measured at v4: **no duplicate change path across the manifest's 194 entries** (G-11) |
...
| Mutation-coverage | **NO — RE-EXECUTED AT v4** | `ENFORCER_DIRS` measured at the tip = `tests/lint · design · docs · data · copy · security · edgeFunctions · generators`; `tests/domain`, `tests/ui` and `src/data` are NOT among them, and `NAME_PATTERN.test()` is **false** for `bandLadders.js`, `bandLadders.test.js` and `compendiumBandLadders.test.jsx` (G-14). ⭐ `scripts/mutation-coverage-manifest.json` is named by 31 packets, **all LANDED**, so it is FREE — version 3's "held by EM-P2 (READY)" is WITHDRAWN (G-11). ⚠ Two traps recorded: `tests/generators/` IS an enforcer dir, and `…contract/walker/golden/pin.test.js` all match |
...
| ⭐ Wiring census, the STAMP arm | **NO — MEASURED** | `docs/content/wiring-census.json`'s `stamp.files` holds SEVEN entries, all `src/domain/display/stateProse/**`; none of the three producers is stamped, and neither `candidateLeaves` nor `producerIndexFiles` names one (E-5). **No `stale-bytes` re-take is owed** — re-measured at `141a1d775` (F-10). ⛔ **BUT THE SAME WALKER'S *CITE* ARM DOES BITE** — see the next row |
...
| A8 | THE ONE ADDRESS, AND THE LAYERING BOUNDARY | the live import graph and edge scan | ⛔ the **EXACT** path `src/domain/bandLadders.js` does not exist — **never a glob**, which would match the pre-existing `src/domain/compendium/bandLadders.js` and false-red (§5.1c); the same arm asserts that file IS present and is imported by no `src/generators/**` file; every importer of the ladder imports `src/data/bandLadders.js`; no file under `src/generators/` imports any `src/domain` band address (with the measured 283 → 285 consequence in the failure message); and the live `src/domain → src/generators` edge set equals `BASELINE_EDGES` (4 files / 5 edges) | same, plus `tests/build/domainGeneratorsBoundary.test.js` (run, not edited) |
...
- ⛔⭐ **STOP-9 — A LIVE-CODE CITATION PAST EOF (NEW AT v4; the instrument did not exist at version 3).**
  ...
  `578272a99` (G-5): 16 live-code citations address this packet's four files; the binding one is
  `defenseGenerator.js:639`, cited by `src/domain/display/stateProse/defenseStateProse.js:1560` and
  `tests/domain/defenseStateProseDesk.test.js:1611` against a **653-line** file — **14 lines of headroom.**
