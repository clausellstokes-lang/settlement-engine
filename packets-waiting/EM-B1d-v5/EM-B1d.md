...
- **Last revalidated:** `__BASE__` — **for the chair to stamp in the same command as the
  re-measurement.** The revalidation sentence this version needs, written out so the chair executes
  rather than composes it: *re-measure, over every change-manifest and `requiredSymbols` path, that
  (a) `git merge-base --is-ancestor __BASE__ HEAD` exits 0; (b) the five production files, the two
  declared TEST files and `scripts/mutation-coverage-manifest.json` are blob-identical to the
  window's start, or name what moved; (c) all seventeen `requiredSymbols` rows still resolve
  `grep -cF ≥ 1`; (d) the CREATE target `tests/lint/statusUnionTotality.walker.test.js` is absent
  and untracked; (e) the mutation-coverage insertion anchors `"tests/lint/dossierMountRegistry.walker.test.js"`
  and `"tests/lint/stepPresentationEngineFence.walker.test.js"` are still file-adjacent in that
  order; and (f) the live lighting tuple, since this packet quotes a DELTA and never an absolute.*
  ⓘ **The version-4 base `32f1ba048…` and every figure stamped against it are superseded**: the
  build lane executed this packet at `58fcfe614` and its measurements, quoted throughout below, are
  that tree's. A re-stamp to any descendant needs only the six checks above.
...
- **Preamble:** docs/implementation/preambles/EM-PREAMBLE.md (SHA-256: 1cf5442719f2236320068afb6b4bab2b4ea49f3b08457c04ccf5eaae6a11faf6 — re-verified by execution at `58fcfe614`)
...
| 3 | `verified-base descendant changed declared substrate` (`:195`) | passes when `__BASE__` is the tip; otherwise the chair's re-measurement (b) answers it |
...
|---|---|---|---|
| P2.1 | lighting census | **OWED — INTERIOR RED** | This packet quotes **no absolute**, only the delta it causes: **`+1 files / +0 parked / +1 credited / +5 titles / +1 suiteTitles`** — one new test file (one `describe`, four `it`) **plus one new `it` in `densityLaw.test.js`**. ⭐ Measured whole at `58fcfe614` with these edits: **`2649 · 383 · 2266 · 25019 · 6676`** for the four-title shape; version 5's extra `it` makes it **`2649 · 383 · 2266 · 25020 · 6676`**. Re-derived at the terminal, **by the chair, never inside this packet**. |
