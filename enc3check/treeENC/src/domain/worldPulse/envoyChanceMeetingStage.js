...
import { characterDriftOf, positionValue } from '../npc/characterDrift.js';
...
export function advanceChanceMeetings({
  ...
  const driftMap = characterDriftOf(state);
  ...
  for (const raw of meetings) {
    ...
    collectLessons({ receipt, seats: [left, right], driftMap, taughtThisPass, lessons, tick });
  }
 ...
 *    driftMap: Record<string, unknown>, taughtThisPass: Set<string>,
 *    lessons: Record<string, unknown>[], tick: unknown}} args */
function collectLessons({ receipt, seats, driftMap, taughtThisPass, lessons, tick }) {
  ...
  const entries = chanceMeetingLessonEntries({
    ...
    subjectOf: (nid) => {
      ...
      if (taughtRecently(driftMap, nid, now, cadence)) return null;
