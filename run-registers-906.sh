...
echo "PREDICTIONS (E4, §906, to be re-derived at the dock tip in predict-906.log BEFORE this runs — the anchor car adds one it() title in tests/lib/flags.test.js, the visibility car adds N in tests/components/{warFaithSurfacing,handbookVoice}.test.jsx, the docs car adds nothing): lighting — titles 23653 -> $EXPECT_TITLES, suiteTitles 6333 -> 6335 (two new describes, measured by the farmed probe in predict-906.log), files 2543 / parked 373 / credited 2170 UNCHANGED (no new file); OSR plain read 1972 exact, 0 drift, NO write; mounts unchanged; prose-numerics 225 exact; writer-reach: no writer string changed — dry run decides; tuning plain green"
...
git add tests/lint/.lighting-census-baseline.json && git commit -q -m "§906 ANCHOR+VIS landing (register car 1): the lighting census refreezes at the tip — titles 23653 -> "$EXPECT_TITLES" (the anchor car's one it() + the visibility car's six, inside three existing files) and suiteTitles 6333 -> 6335 (two new describes); files 2543 / parked 373 / credited 2170 unchanged

Derived by the farmed read-only probe before the door (predict-906.log and the probe/register pair in this script's log): the ONLY movement is titles 23653 -> 23660 (+1 anchor, +6 visibility) and suiteTitles 6333 -> 6335 (two new describes); files / parked / credited unchanged — the farmed probe read exactly these figures at 2f1f9381f (predict-906.log); the door exited non-zero by design and the plain re-run is the receipt.
