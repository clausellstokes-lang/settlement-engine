#!/bin/sh
# collect-898.sh — the landing collection (§898). set -e: the guard and the consequence live together.
# Reads the CAS sha from $SC/cas-898.sha, written by the CAS step; refuses if absent or unsealed.
set -e
...
[ -s "$SC/cas-898.sha" ] || { echo "ABORT: cas-898.sha missing/empty — CAS first"; exit 1; }
CAS=$(cat "$SC/cas-898.sha")
GOT=$(git rev-parse --verify refs/preserve/landing-prose-2026-09-05 2>/dev/null || echo MISSING)
...
git show HEAD:docs/OWNER_DECISION_QUEUE.md > "$SC/odq.head.898"
