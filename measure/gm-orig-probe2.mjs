...
import { generateSettlementPipeline } from '../laneMEASURE/src/generators/generateSettlementPipeline.js';
import { CULTURE_PROFILE_KEYS } from '../laneMEASURE/src/domain/cultureProfiles.js';
...
const rows = corpus();
const manifest = JSON.parse(readFileSync('../laneMEASURE/tests/fixtures/generator-golden-master.json','utf8'));
const drift = [];
for (const c of rows) { const k = keyOf(c); if (manifest[k] !== hashFor(c)) drift.push(k); }
import { writeFileSync } from 'node:fs';
writeFileSync('../measure/gm-drift-orig.json', JSON.stringify(drift.sort()));
console.log('ORIGINAL drift', drift.length);
