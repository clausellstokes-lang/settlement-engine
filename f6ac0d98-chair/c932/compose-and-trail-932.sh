...
C=$SC/kit/laneCONSIST-932; DOCK=$1; BASE=$2
...
echo "CONSIST_TIP=$(git rev-parse --short HEAD) cars=$(git rev-list --count a5876c0ea..HEAD) porcelain=$(git status --porcelain | wc -l | tr -d ' ')"
