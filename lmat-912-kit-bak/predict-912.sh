#!/bin/sh
# predict-912.sh <dock> — the READ-ONLY prediction pass at the §912 dock tip BEFORE any register door (E4: every register figure is derived,
...
echo "PREDICT_HEAD=$(git rev-parse HEAD) cars_over_3b1c0eaa5=$(git rev-list --count 3b1c0eaa5..HEAD) start $(date '+%H:%M:%S') load=$(uptime | sed 's/.*averages: //')"
...
echo "--- negative-assertion frozen rows for the touched files:"; grep -n -E "'tests/(security/livingContentRosterPublicDrop|domain/livingContentLawWiring|domain/densityCreateBoundary\.walker|lib/accountSettlementContentPortability|store/accountImportSlice)\.test\.jsx?'" tests/lint/negativeAssertionAnchor.walker.test.js | cut -c1-120 || echo "  (none — at 0 / never frozen)"
