#!/bin/sh
# run-gate-906.sh — the §906 consist's bare gate (npm run check), stamped by mk-landing-kit.py;
# base 6582958ce. Quiet-window law. ⚠ THE LAST LINE IS `exit $TRUE_EXIT` AND NOTHING MAY FOLLOW IT:
# a background task reports its LAST command's exit, and a trailing printf once reported 0 over a red gate.
D=/private/tmp/claude-502/-Users-cstokes-Desktop-settlement-engine/26b2203a-14d7-409e-a7aa-8d0f3b0517db/scratchpad/laneANCHOR905
BASE=6582958ce7bdc10bcbb8c69d9789b4d957fc5890   # the declared base; the guards below refuse to run unless the product tip IS this sha
cd $D || exit 9
[ "$(git rev-parse claude/composite-r4)" = "$(git rev-parse 6582958ce7bdc10bcbb8c69d9789b4d957fc5890)" ] || { echo "REFUSED: product tip $(git rev-parse --short claude/composite-r4) is not the declared base 6582958ce — land the previous consist first"; exit 7; }
...
[ "$CARS" = "7" ] || { echo "REFUSED: expected 7 cars over $BASE, found $CARS"; exit 7; }
