...
Written 2026-09-11 by the Fable chair (session 4ed79a2b) at the owner's word: "comprehensively write the procedures and the architecture and sequence, etc. so that opus can do all of this work for you with the current plans." Owner rulings the same afternoon: the marker and judge seats may be Opus; **refutation stays Fable**; one lane at a time; pause is the owner's word and so is resume.
...
| **Refuter · Re-refuter** | **Fable — fixed in the script** | tries to break every face against the card, the tables and the bars; names the law and the cure | **no (owner 09-11: "Refutation can stay fable")** |
...
4. **Refute** — one FABLE refuter per pool over `kept.md`: every face PASS · WITHHELD · FAIL with the law, a ≤12-word quote and the CURE it names; writes `refute-round-1.md`. Barrier: all pools.
