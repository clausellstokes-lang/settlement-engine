...
// were synchronous become async; that's safe because the ones that use this
// bag set Zustand state via the `set` callback rather than returning a value
// (generateSettlement DOES return the settlement, and every caller awaits it).
// React components that fire-and-forget still work — they just see the
// resulting state update one macrotask later.
//
// ⭐ THE GENERATION LANE NO LONGER LIVES HERE. `generateSettlement` delegates
// to `settlementGenerateAction.js` through a dynamic import (the action KEY
// stays on the literal below), and that lane reaches the pipeline through the
// generation core — in a Web Worker when the door is open, in-thread on the
// same core when it is not. So this bag has shrunk to the two regeneration
// entries `regenSection` still needs, and the seed mint moved with the lane.
//
...
/**
 * The regeneration-time module bag.
 * @type {?{
...
/** @type {?Promise<typeof import('./settlementGenerateAction.js')>} */
let _generateLanePromise = null;
/** The generation lane's own lazy chunk, memoized like the engine bag above. */
function loadGenerateLane() {
  return (_generateLanePromise ||= import('./settlementGenerateAction.js'));
}
...
// The anonymous daily generation cap moved with the generation lane
// (settlementGenerateAction.js): that action is the single point of
// enforcement for EVERY generation path, so the counter has exactly one
// importer left in the eager graph — none — and leaves first paint with it.
...
export const createSettlementSlice = (set, get) => ({
  ...
  generationId:  null,
  // RETIRED-DARK, pending owner row WK-2. This once held the full pipeline
  // context from the last run. That context is function-bearing, so it cannot
  // cross a worker boundary, and capturing it on one transport while nulling it
  // on the other would make store state differ by HOW the world was generated —
  // a fork by the law's own definition. The generation lane therefore writes
  // null on BOTH paths. It has no readers in src/ or tests/; removing the key
  // outright is the owner's call, not the lane's.
  lastCtx:       null,
  ...
  // ── Generation ─────────────────────────────────────────────────────────────
  // ⭐ THE LANE IS NOT HERE. The action KEY stays on this literal (the
  // operation-registry walker's denominator counts an action that hands `set`
  // to a lazily-loaded body, the established `recordSnapshot` idiom), and the
  // ~290-line generation lane lives in settlementGenerateAction.js, reached
  // only through the import() below. That keeps the lane, the anonymous-cap
  // counter and the content-runtime resolver out of the first-paint closure,
  // and it is the seam that lets the work run in a Web Worker behind one code
  // path rather than a forked copy.
  //
  // The signature is UNCHANGED: generateSettlement(seedOverride?) resolves to
  ...
  /*
   * ⚰ applyMapEdit RETIRED HERE (ODQ §763.2, Q-STYLE arm 2, under the fresh owner grant
   * §763.1). It was the SM-3 persist triple for the blob-resident `settlement.mapEdits`,
   * and its only caller in src/ was StyleOverhaulPanel.jsx, deleted in car 1. The full
   * disposition — including what is NOT true of the persisted key — is recorded on the
   * registry tombstone in src/store/operationRegistry.js; it lives there because that
   * registry is the authority on which verbs exist.
   */
