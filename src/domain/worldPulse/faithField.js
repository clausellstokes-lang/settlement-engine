...
export const FAITH_FIELD_TUNING = Object.freeze({
  ...
  DAMP_MAX: 0.6,
  // W-FAITH F4c — THE CAUSAL SWING. The number and its whole calibration argument moved
  // to `faithChannelBindings.js` with the pricing function that reads it (SUBSTRATE wave
  // 6, REC-1); it is carried here under the SAME KEY and the same value, so the tuning
  // surface the owner signs at F6c is still ONE object and every
  // `FAITH_FIELD_TUNING.CAUSAL_SWING` reader is untouched.
  CAUSAL_SWING,
});
