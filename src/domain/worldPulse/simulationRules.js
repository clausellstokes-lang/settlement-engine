...
export const ENGINE_GATED_VIRTUAL_RULE_KEYS = Object.freeze([
  ...
  // ⚠ IT IS A SINGLE CONJUNCT AT THIS WAVE, deliberately, and that is worth saying because
  // every neighbour above is part of a conjunction: the HABIT ladder charters four doors and
  // this wave lands DOOR ONE ONLY (may this world STORE a habit at all). The conjunctions
  // arrive with HB-4, HB-5 and HB-6, whose doors compute, decide and spread; declaring them
  // now would be three dead arms.
  // ⛔ The read is spelled by name rather than through a frozen-list `.every()` for the reason
  // this census exists: a computed member access attributes to NO key and would be fully wired,
  // genuinely gated, and invisible here.
  ...
  // Joined 2026-08-15 by FP wave GR-5A under CR-WR10-C item 4 (the compiled charter's §3
  // flag law), in the SAME commit as its first real gate read —
  // `pactAmendment.treatyRenewalActive`, the ONE `=== true` by-name read of this key in the
  // tree — and its AUTHORED certification row. Never a pending entry: manifesting is itself
  // the act that makes a virtual key censusable and therefore the act that comes due.
  // ⚠ IT IS A SINGLE CONJUNCT AT THIS WAVE, deliberately. GR-5 as chartered is six
  // behaviours and five bands; this member is slice (a) alone — the monotone
  // `worstObservedEver` memory a treaty record cannot hold today — and every successor that
  // would add a door (the renewal window, the mid-term demand, the conversion arm) needs a
  // chair-signed band under the OQ §42/§43 value law before it can be written at all.
  // Declaring those doors now would be three dead arms.
  // ⛔ The read is spelled by name rather than through a frozen-list `.every()` for the reason
  ...
  // ⚠ THE THIRD SIBLING OF A1.2.12's three-key topology IS NOW MINTED — see
  // `irregularForceEnabled` at the end of this list. It waited for car SEAT-7a precisely
  // because a flag mint is a train boundary and a key minted ahead of its reader is a
  // manifested key with no measured gate; it arrives WITH its first gate read and its
  // authored certification row, in one commit, as this file's own law requires.
  // ⛔ THE NAME-SPELLING HAZARD THAT KEPT IT OUT OF THIS COMMENT STILL BINDS FOR ANY
  // FUTURE KEY. The mechanism lit-coverage walker builds its flag DENOMINATOR by scanning
  // this file's RAW SOURCE for /\b[a-zA-Z][a-zA-Z0-9]*Enabled\b/ — comments are NOT
  // blanked — so writing an UNBUILT key's literal name anywhere here mints it into the
  // denominator years before it has a gate or a lit proof, and the ratchet reds naming a
  // flag that does not exist. Measured, not guessed: spelling both siblings here once put
  // BOTH into `uncoveredFlags` on the first run. A name may be written here only in the
  // same commit that gives it a gate and a lit drive.
  'foreignSeatEnabled',
  // Joined 2026-08-31 by W-SEAT car SEAT-2b (T4 · SEAT-B) under CR-WR10-C item 4, in the
  // SAME COMMIT as its first real gate reads — `stressorGates.upheavalLit`, the ONE by-name
  // strict `=== true` read of this key, reached from the coup, rebellion and insurgency
  // spawn gates — and its AUTHORED certification row in subsystemRowsSeat.js.
  // ⛔ THIS KEY IS DOMESTIC AND THAT IS THE WHOLE REASON IT IS A SECOND KEY (A1.2.12). §736's
  // legitimacy-scaled upheaval involves NO foreign seat at all: a town's own archetype
  // deciding how brittle its seat is to crisis versus to slow decline is a fact about that
  // town, and folding it into the seat key would have made a purely domestic same-seed shift
  // ride a flag whose certification row promises a foreign counterpart. The two keys are
  // independent by construction — neither reads the other — and the seat's own aftermath
  // pull (the reseed scaled by SEAT WEIGHT) stays on the seat key where it belongs.
  'legitimacyUpheavalEnabled',
  // Joined 2026-09-03 by ENCOUNTERS car ENC-3 under CR-WR10-C item 4, in the SAME COMMIT
  // as its first and ONLY by-name strict `=== true` gate read — `chanceEncountersActive`
  // in `worldPulse/envoyChanceMeetingStage.js`, the one door the chance-meeting stage
  // opens — and its authored certification row in `subsystemRowsEncounters.js`.
  // ⛔ THE KEY'S LITERAL NAME MAY APPEAR IN THIS FILE ONLY HERE. The mechanism
  // lit-coverage walker builds its flag denominator by scanning this file's RAW SOURCE
  // for /\b[a-zA-Z][a-zA-Z0-9]*Enabled\b/ and does NOT blank comments, so a second
  ...
  // Joined 2026-09-04 by W-SEAT car SEAT-7a under CR-WR10-C item 4 (the compiled charter's
  // §3 flag law), in the SAME commit as its first real gate read and its AUTHORED
  // certification row. It is the THIRD and last key of A1.2.12's three-key topology, and
  // the topology is now fully spent.
  // ⛔ ONE GATE READ, and it is spelled POSITIVELY on the optional-chain receiver:
  // `irregularForce.irregularShareFactor` opens with
  // `worldState?.simulationRules?.irregularForceEnabled === true`. The negative
  // early-return spelling this file's siblings sometimes use is INVISIBLE to the
  // engine-gated census — its own decoy list proves `!== true` is not a discovered gate —
  // so a key manifested behind one would red the walker's manifest-without-read arm.
  // ⛔ WHAT IT LIGHTS, EXACTLY ONE THING AT THIS CAR: `resolveCoupVerdict` gains a
  // MULTIPLICATIVE factor on the incumbent's political `share`, carrying how much of the
  // settlement would rise (D10's `participation01`) and what that mass is worth against
  // the loyal side. It is deliberately NOT the sixth ADDITIVE adj F3 chartered: the five
  // live adjs sum to 0.815 against a declared budget of 0.82, and that budget's own
  // instrument reserves a raise to the owner's tuning signature. The additive slot stays
  // open and unclaimed.
  // ⚠ IT IS DOMESTIC, which is why it is a third key rather than a rider on the seat key
  // (A1.2.12): a town's own people deciding whether to come out is a fact about that town,
  // and folding it into a key whose certification row promises a FOREIGN counterpart would
  // make a purely domestic same-seed shift ride the wrong promise.
