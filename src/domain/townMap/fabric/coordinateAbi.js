/**
 * domain/townMap/fabric/coordinateAbi.js — MF-T2B · §287.5 / SPEC §10.4 ·
 * ⭐⭐⭐ THE VERSIONED INTEGER COORDINATE ABI.
 *
 * Ported from the sealed W3 sandbox tip's `coordinateAbi.js` (SHA-256
 * `123f3c17ebd42da5223216f0029617706db49b3de7ab602acd6edcf72a2c8404`), re-pointed at this
 * tree's own `./exactGeometry.js` closure. ODQ §303.5 rules that D1's versioned ABI wins on
 * the integer wall and that the ported record shapes re-parameterize onto it.
 *
 ...
 * `TOPOLOGY_PLACES = 6` and `q6` already fix the TEXT a receipt is hashed over — but the
 * quantum is *a SERIALIZATION RULE, NOT A STORAGE RULE*, so what the program publishes today
 * is a decimal STRING and what it decides from is still a float. A string is not an integer:
 * `"1.500000"` and `"1.5"` are different bytes for one coordinate, and nothing declared which
 * one is canonical, at what version, under whose rounding.
 *
 * ⭐⭐⭐ THIS MODULE PUBLISHES THE INTEGER AND VERSIONS IT, AND IT DOES NOT RE-ROUND.
 *
 * ⛔⛔ THE ONE DECISION THAT MATTERS. The obvious spelling of the integer quantum is
 * `Math.round(v * 1e6)`. It is WRONG in two independent ways and both are measurable:
 *
 *   (1) THE TIE RULE DIVERGES ON NEGATIVE COORDINATES. `Number.prototype.toFixed` strips the
 *       sign FIRST and then breaks a tie by taking the LARGER magnitude — round-half-AWAY-
 *       FROM-ZERO. `Math.round` breaks a tie toward `+Infinity`. On `x = -1/128` — an exactly
 *       representable double whose sixth decimal place is an exact tie — `q6` says
 *       `-0.007813` and `Math.round(x * 1e6)` says `-7812`. The fabric's coordinates run
 *       negative, so this is not a corner case; it is a wrong answer on real ground.
 ...
 *
 * ⭐ THE WRAP: the integer is READ OUT OF `q6`'s OWN TEXT — sign, digits, decimal point
 * removed. By construction it cannot round differently from the six-decimal serializer, so
 * the existing fixed-precision topology keys reconcile to this ABI exactly, and the
 * reconciliation is an identity rather than an approximation.
 *
 * ⚠ WHAT IS NOT CLAIMED. Nothing here quantizes STORED geometry, and no consumer in the
 * generation path imports this module. Quantizing the STORED legality geometry moves every
 * pixel in the corpus and owes its own equivalence proof — handed off, not taken here.
 *
 * PURITY: pure. `+ - * /`, string slicing and `Number()`. No clock read, no RNG, no locale, no
 * trig. ⚠ The vocabulary is deliberately named in the ABSTRACT here: the determinism companion
 * scans this file's RAW TEXT for the forbidden identifiers, and a scan that a docstring can
 * trigger is a scan someone eventually widens to excuse the docstring. Prose describes the rule;
 * the pin reads the bytes.
 */

import { TOPOLOGY_PLACES, q6 } from './exactGeometry.js';
 ...
 * fabric publishes no height (SPEC §10.16's status override says so). Declaring the quantum
 * now means the massing members inherit a versioned rule instead of minting a second one;
 * `heightQ` below REFUSES rather than guessing, so no lane can read an unexercised path as a
 * working one.
 */
export const HEIGHT_QUANTUM = Object.freeze({ numerator: 1, denominator: 1000000 });

/** Largest magnitude a world coordinate may carry and still be a safe integer after
 *  quantizing. `Number.MAX_SAFE_INTEGER / 1e6`, floored — the bound §10.4 orders proved over
 *  the largest map. */
export const MAX_WORLD_UNITS = Math.floor(Number.MAX_SAFE_INTEGER / GEOMETRY_QUANTUM.denominator);

/**
 * The angle-table size the sandbox spells `TRIG_N`. ⚠ JUDGMENT, vetoable: the table itself
 * (`trigTable.js`, 270 effective lines and two 1024-entry arrays) is NOT in this member's
 * closure and nothing in this tranche reads it, so the size is declared locally with an
 * equality pin rather than dragging a third leaf in for one integer. The table ports with the
 * geometry that reads it.
 */
export const ANGLE_TABLE_SIZE = 1024;

/**
 * ⭐ THE PUBLISHED ABI RECORD (SPEC §10.4's `CoordinateAbi`, minus the artifact-identity
 * fields that need a `ProvenanceRef`/`UnitRegistry` this era does not have — those are named
 * ABSENT rather than faked, because a `contentHash` field carrying a placeholder is a worse
 * lie than a missing one). `worldUnitsPerMapUnit` is 1: the fabric's leaf frame IS its map
 * frame today, and inventing a scale factor nobody measured would be a free parameter.
 */
export const COORDINATE_ABI = Object.freeze({
  ...
  angleTableSize: ANGLE_TABLE_SIZE,
  axisX: 'EAST',
  axisY: 'NORTH',
  axisZ: 'UP',
   ...
   *  fabric draws in a y-DOWN view frame, while §10.4's world convention is +Y north. The two
   *  are one sign flip on Y and NOTHING in this member applies it: applying it would move
   *  every published coordinate. The flip is a named migration owed by the wave that first
   *  publishes a world-framed artifact (ODQ §303.6). */
  viewFrameYAxis: 'DOWN',
  worldFrameYAxis: 'NORTH_UP',
  viewToWorldYFlipApplied: false,
});

/** The exact tie rule, published as data so a test can assert it rather than a comment claim
 *  it. */
...
/**
 * ⭐⭐⭐ THE QUANTUM ITSELF. `null` for a non-finite input — never `NaN`, which compares
 * unequal to itself and would make an identity unstable against its own value.
...
/** ⛔ The height door, held shut on purpose. No height is published in the plan era, so a
 *  caller asking for one is asking about an artifact that does not exist.
 *  @returns {never} */
export function heightQ() {
  throw new Error('COORDINATE_ABI: heightQ is declared and UNEXERCISED — no height artifact '
    + 'exists in the plan era (SPEC §10.16 status override); the massing wave owns its first '
    + 'publication');
}
...
/** One point at ABI precision.
 *  @param {readonly number[]|null|undefined} p @returns {[number,number]|null} */
 ...
 *  @param {ReadonlyArray<readonly number[]>|null|undefined} poly
 ...
 *
 * The ABI has ONE zero, because an integer has one zero. This predicate names the exception
 * in the direction that is true: the ABI is canonical and the legacy text is not. It is
 * exported so the exception can be COUNTED over the corpus rather than argued about — a
 * latent defect and a live one are different findings, and only a census tells them apart.
 * @param {number} v @returns {boolean}
 */
export function isNegativeZeroText(v) { return q6(v) === '-0.000000'; }

/**
 * ⭐⭐ THE RECONCILIATION, AS AN EXECUTABLE FUNCTION RATHER THAN A CLAIM. Does the integer
 * this ABI publishes reproduce the six-decimal text the program has been hashing? It is an
 * identity by construction everywhere except the negative-zero spelling above; this is the
 * arm that proves the construction was not broken by a later edit.
 * @param {number} v @returns {boolean}
 */
 ...
 *  @param {ReadonlyArray<readonly number[]>|null|undefined} poly @returns {string} */
