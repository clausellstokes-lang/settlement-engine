 ...
 *  v3 is the MAP-FABRIC layout law (the surveyor's-folio street/parcel fabric),
 *  reachable by opt-in redraw and — once §109 accepts it — by the mint dial below.
 *
 *  ⛔ THIS ARRAY IS THE ONLY VOCABULARY. Every read and every merge op in this file
 *  tests MEMBERSHIP against it (`isLayoutLawVersion`), never a literal. The version
 *  before this one shipped a hardcoded `=== 2` in BOTH `readLayoutLawVersion` and
 *  `withLayoutLawVersion`, so widening this array alone was a NO-OP and a v3 blob
 *  was silently stripped on the way to storage — storable, unrenderable, discarded.
 *  A new version is added HERE and nowhere else; the render router in
 *  townMapModel.js is the only other place a version is dispatched on.
 *  @type {ReadonlyArray<number>} */
export const LAYOUT_LAW_VERSIONS = Object.freeze([1, 2, 3]);
...
/** Is `v` one of the known layout-law versions? The ONE membership test — every
 * read and merge op in this file routes through it, so widening
 * `LAYOUT_LAW_VERSIONS` is sufficient AND necessary to admit a new version.
 * Fail-closed: a string '3', a float, null, NaN and an unknown integer are all
 * NOT members (the persisted value is an integer or it is not a version).
 * @param {unknown} v
 * @returns {boolean} */
export function isLayoutLawVersion(v) {
  if (!Number.isInteger(v)) return false;
  for (const known of LAYOUT_LAW_VERSIONS) if (known === v) return true;
  return false;
}

/** The chosen layout-law version — a MEMBERSHIP test over `LAYOUT_LAW_VERSIONS`.
 * Absent / unknown / non-integer ⇒ the dormant v1 default (so every pre-v2
 * settlement and every version-explicit golden stays byte-identical, and an
 * unknown future value still coerces DOWN to v1 rather than throwing).
 *
 * ⛔ LIFECYCLE NOTE (the bug this shape exists to make impossible): the previous
 * spelling was `Number(edits.layoutLawVersion) === 2 ? 2 : DEFAULT`, which is a
 * hardcoded constant wearing a list's clothes. `normalizeMapEdits` calls this
 * function to decide what to KEEP, so any version outside the literal was dropped
 * on the way to storage — the write survived the render path (the model router
 * read the raw blob) and ghosted the persist path. Membership closes both.
 * @param {MapEdits | null | undefined} edits
 * @returns {number} */
export function readLayoutLawVersion(edits) {
  const raw = edits ? edits.layoutLawVersion : undefined;
  return isLayoutLawVersion(raw) ? Number(raw) : DEFAULT_LAYOUT_LAW_VERSION;
}
...
export function normalizeMapEdits(edits) {
  ...
  // layoutLawVersion: kept ONLY for a NON-DEFAULT member of LAYOUT_LAW_VERSIONS (the
  // default v1 ⇒ omitted ⇒ byte-identical to no-edit, the dormancy law — exactly the
  // styleLens pattern). This is what pins the versioning law: an absent marker renders
  // v1 unchanged; only an explicit known version flips. A non-member coerces to v1 and
  // is therefore DROPPED here, which is the fail-closed direction.
 ...
 * v1 pin still nudges the same building under v2 and under v3. Selecting v1 clears the
 * key ⇒ byte-identical dormancy (a redraw back to the original arrangement loses
 * nothing). A value outside `LAYOUT_LAW_VERSIONS` coerces to the v1 default.
 *
 * ⛔ THIS IS THE SECOND LIFECYCLE PATH, and it carried the SAME hardcoded `=== 2`.
 * `readLayoutLawVersion` governs read+normalize; THIS op governs the user-facing
 * opt-in redraw. Curing only the reader would have left the redraw button unable to
 * select any version it did not already know — the write ghosting a different path.
 * Both now route through `isLayoutLawVersion`.
...
export function withLayoutLawVersion(edits, version) {
  const base = normalizeMapEdits(edits) || {};
  // STRICT, NOT TRUNCATING. An earlier spelling coerced 3.5 to 3 before the membership
  // test, which silently ACCEPTS a caller that is already wrong. The reader and the merge
  // op now answer the same question the same way, so there is exactly one definition of
  // "a layout-law version" in this file and a caller cannot reach a version by rounding
  // into it.
  const v = isLayoutLawVersion(version) ? Number(version) : DEFAULT_LAYOUT_LAW_VERSION;
