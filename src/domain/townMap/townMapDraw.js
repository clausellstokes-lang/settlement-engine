...
// THE PRIMITIVE DRAW-OP VOCABULARY (VIEW space) + its SVG adapter — the shared leaf.
import { VIEW, drawListToSvg } from '../drawOpsSvg.js';

export { EXPORT_PALETTE };
// The primitive op vocabulary + the SVG adapter are the SHARED leaf both this surface and
// the retained realm renderer sit on (domain/drawOpsSvg.js, §725/§748). Re-exported here
// so every existing `townMapDraw.js` consumer of `drawListToSvg` / `DrawOp` is unmoved.
export { drawListToSvg };

/** @typedef {import('../drawOpsSvg.js').PolyOp} PolyOp */
/** @typedef {import('../drawOpsSvg.js').LineOp} LineOp */
/** @typedef {import('../drawOpsSvg.js').CircleOp} CircleOp */
/** @typedef {import('../drawOpsSvg.js').RectOp} RectOp */
/** @typedef {import('../drawOpsSvg.js').PathOp} PathOp */
