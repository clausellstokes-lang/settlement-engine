...
import { buildTownLayoutV3 } from './townLayoutV3.js';
import { readLayoutLawVersion } from './mapEdits.js';
...
export function buildTownMapModel(settlement, mapEdits = null) {
  ...
  // ── VERSIONING LAW (task #38; extended for v3 by the mf family) ─────────────
  // The layout-law version is read through `readLayoutLawVersion` — the ONE
  // membership test over `LAYOUT_LAW_VERSIONS` — so the router, the normalizer and
  // the opt-in redraw all agree on the vocabulary by construction. v2 selects the
  // semantic urban-planning engine; v3 selects the MAP-FABRIC layout law. Both are
  // SIBLING generations that emit this exact TownMapModel shape, so every lens /
  // export / hover / pin inherits them. Absent / 1 / anything unknown renders the v1
  // path BELOW, byte-for-byte unchanged — so every pre-v2 settlement and every
  // version-explicit golden stays identical (the v2 lane landed free; v3 lands free
  // for the same structural reason). New settlements mint whatever
  // NEW_SETTLEMENT_LAYOUT_LAW_VERSION says; existing ones never pass through create.
  const lawVersion = readLayoutLawVersion(mapEdits);
  if (lawVersion === 3) {
    const model = buildTownLayoutV3(
