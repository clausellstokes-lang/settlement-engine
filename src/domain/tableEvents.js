 ...
 * live session-ledger's typed events + the 'table-event' pendingEdits kind). That
 * module is NOT on this branch's base, so this leaf MIRRORS its shapes — the closed
 * vocabulary (TABLE_EVENT_KINDS), the named magnitude bands (MAGNITUDE_BANDS), and
 * the source:'table' provenance — so the import flow builds against the SAME
 * contract. AT FOLD: replace the three mirrored constants below with imports from
 * tableLedger.js (they must remain value-identical — a parity pin guards the set),
 * and, if V-F's apply path should also consume backfilled history, wire the store
 * action (importTableEvents) to it there. Nothing else in the import flow changes.
 *
