...
// MOVED VERBATIM to ./spatialLedgerAccess.js and RE-EXPORTED here, so no consumer
// import site moved. WHY: these four accessors are property reads, but this module
// is the 1,080-line / ~53 kB frozen-digest reader that "never reaches first paint",
// and importing ANY symbol from it pulls the whole module into the importer's chunk.
// warCoalitionLedger.js needed only getSpatialLedger and IS reached statically from
// the store, so the geography rode the critical path (main.jsx -> store/index.js ->
// campaignSlice.js -> worldState.js -> warCoalitionLedger.js -> HERE). The house
// leaf extraction (deityConstants / stablePart / userRouteIdentity) moves the
// FUNCTIONS, not a chunk pin. First-paint-reachable readers import the leaf directly;
// everyone else may keep reading them from here. See that file for the namespace
// contract and the dormancy law.
export {
  hasSpatialLedger,
  getSpatialLedger,
  setSpatialLedger,
  dropSpatialLedger,
} from './spatialLedgerAccess.js';
