/**
 * domain/display/hegemonyRead.js — THE UNNAMED EMPIRE, made DM-readable
 * (ambition-fit-2 / DESIGN_SIM_DEPTH_R2 D4(a), the frozen DESIGN_COHESION_WEAVE
 * §F.3b design). Hegemony is A PATTERN, NEVER AN ENTITY: the engine mints no
 * empire object, no imperial ledger, no name. This read DERIVES the pattern each
 * call from the existing treaty topology — a center C holding ≥K subordinate ties
 * (tribute / compelled-alliance / puppet-seat / occupation) forms a sphere. When
 * the edges dissolve the "empire" simply stops matching — no lifecycle, no death
 * event, no persistence, no undo burden (per §F.3b's own pins: ZERO persisted
 * state).
 *
 *   • Legibility (design §F.3b LEGIBILITY): a RealmDashboard cluster line + the
 *     brief's COUNT ("seven towns pay tribute to Thornwall"). Descriptive ALWAYS
 *     ("Thornwall and its tributaries"); christened only by the DM — the engine
 *     builds the bones, THE DM BAPTIZES. A DM-authored canon label is adopted via
 *     the injected `canonLabelFor` resolver (its PERSISTENCE is a separate store
 *     lane — deliberately out of this display module's fence; seam below).
 *   • The reason half (`fear_of_dominance`, DESIGN_SIM_DEPTH_R2 D4(b)) is
 *     W-R2-DEPTH's; it CONSUMES this same selector for the believed-strength-share
 *     input. This module exports the selector + its tuning cleanly for that reuse
 *     and adds NO reason logic here.
 *
 * PRESENTATION ONLY. Pure; no store, no rng, no wall clock; INERT-NOT-CRASH on
 * absent/garbage ledgers; every list codepoint-sorted (same input ⇒ identical
 * output; zero writes). Lazy-only (rides the realm-dashboard / inspector chunk) so
 * budget-free — byte-inert to the engine and its goldens.
 */

import { compareCodepoint } from '../deterministicSort.js';
import { getSpatialLedger } from '../spatial/distanceRead.js';

/** Tuning — exported so the W-R2-DEPTH reason half reads the SAME threshold. */
 ...
 * compelled-ally / puppet-seat / garrison-protectorate). JUDGMENT — vetoable.
 */
