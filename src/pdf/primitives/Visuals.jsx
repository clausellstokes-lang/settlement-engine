          ...
          {list.map((s, i) => {
            ...
            return (
              <View key={`leg-${i}`} style={{ flexDirection: 'row', alignItems: 'center', marginRight: 4 }}>
                ...
                />
                <Text style={{ ...type.caption, fontSize: 7.5, color: palette.muted }}>
                  {s.name}
...
export function ChainRow({ resource, processing, output, status, tone }) {
  ...
  return (
    <View
      ...
      wrap={false}
    >
      <Text style={{ ...type.body, color: palette.ink, flex: 1.2, fontSize: 9 }}>
        {resource || '—'}
      </Text>
      <Text style={{ color: palette.faint, marginHorizontal: 4, fontSize: 9 }}>▶</Text>
      <Text style={{ ...type.body, color: palette.second, flex: 1.5, fontSize: 9 }}>
  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <Text style={{ ...type.label_em, color: palette.ink, fontSize: 10 }}>{label}</Text>
        <Text style={{ ...type.numeric, color: fg, fontSize: 13 }}>{score ?? '—'}</Text>
      </View>
      <View
        style={{
          height: 4,
        ...
        <View style={{ marginTop: 4 }}>
          {factors.map((f, i) => (
            <View key={`fac-${i}`} style={{ flexDirection: 'row', marginBottom: 1.5 }}>
              <Text style={{ color: fg, marginRight: 4, fontSize: 9 }}>·</Text>
              <Text style={{ ...type.caption, flex: 1, color: palette.second, fontSize: 8 }}>
  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <Text style={{ ...type.label, color: fg, fontSize: 8 }}>{label}</Text>
        <View style={{ flexDirection: 'row', alignItems: 'baseline' }}>
          <Text style={{ ...type.numeric, color: fg, fontSize: 16, marginRight: 4 }}>
            {score ?? '—'}
          </Text>
          {scoreLabel && (
            <Text style={{ ...type.label, color: palette.second, fontSize: 8 }}>{scoreLabel}</Text>
          )}
        </View>
      </View>
      {breakdown?.length > 0 && (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 5, gap: 4 }}>
          {breakdown.map((b, i) => {
            ...
            return (
              ...
              >
                <Text style={{ ...type.pill, fontSize: 7.5, color: fg2 }}>
      ...
      {footer && (
        <Text style={{ ...type.caption, color: palette.muted, marginTop: 4, fontSize: 8 }}>
          {footer}
        </Text>
      )}
    </View>
  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'baseline', marginBottom: 2 }}>
        <Text style={{ ...type.body_em, color: palette.ink, fontSize: 10, flex: 1 }}>{name}</Text>
      ...
      {meta?.length > 0 && (
        <Text style={{ ...type.caption, color: palette.muted, marginBottom: 2, fontSize: 8 }}>
          {meta.filter(m => m && m.value).map(m => `${m.label} ${m.value}`).join('  ·  ')}
        </Text>
      )}
      {description && (
        <Text style={{ ...type.body, fontSize: 9, color: palette.second }}>{description}</Text>
      )}
      {body}
    </View>
  );
