/**
 * successionNarrative.js — the succession/tension narrative composer,
 * extracted from powerGenerator.js to keep that megafile under its size
 * ratchet. genSuccessionNarr turns a settlement `ctx` into gated narrative
 * sentences; resolvePartyPair dedupes the governing/dominant faction pair so
 * a name collision reads as a single party ("X decides") rather than
 * "X and X disagree". Pure: depends only on the ctx it is handed.
 */
