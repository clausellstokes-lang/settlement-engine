...
import { coerceFoundingKind } from '../domain/foundingKind.js';
...
export const genArrivalDetail = (
  ...
  //
  // ⭐ MF-0F — THE TYPED FOUNDING KIND (charter §5.0's missing input, minted here).
  // §5.0's morphology law needs to know whether an AUTHORITY planned this settlement,
  // and until this member there was no typed field that could answer: `settlementReason`
  // is free prose, and §5.0 forbids a prose regex standing in for the missing fact
  // (measured: 0% of 120 live settlements could reach the planned band).
  //
  // The mint is deliberately NOT a parallel derivation running alongside the prose —
  // that would let the two disagree ("founded by imperial decree", kind: organic). The
  // founder pools ALREADY encode the founding character; each entry is now a TYPED pair
  // and the kind is read off the SAME entry the prose came from. One decision, one draw,
  // two projections. Consequences, all load-bearing:
  //   • ZERO NEW RNG DRAWS. `pick` still draws exactly once from a pool of unchanged
  //     length and order, so every other generated field is byte-identical and the
  //     declared same-seed shift is exactly ONE ADDED KEY — provable key-by-key.
  //   • THE RARITY FALLS OUT. `planned` appears in no pool below city (1 of 4 at city,
  //     2 of 4 at metropolis), so organic dominates the corpus in the proportion it
  //     dominated history, without a quota being imposed anywhere.
  //   • NO PROSE PARSING, EVER. The type is attached at the authoring site.
  // The vocabulary is FOUNDING_KINDS in ../domain/foundingKind.js (its §42/§43 home);
  // an absent field reads as `organic`, which is the default morphology anyway, so no
  // already-generated world's map moves.
  const FOUNDERS_BY_TIER = {
    thorp: [
      { text: 'a single extended family who came and never left', kind: 'organic' },
      { text: 'a pair of siblings who disagreed about which direction to keep walking', kind: 'organic' },
      { text: 'a discharged soldier who liked the view and had nowhere better to be', kind: 'organic' },
      { text: 'a healer who stopped to treat a traveller and found the location too useful to leave', kind: 'organic' },
    ],
    hamlet: [
      { text: 'three or four families who agreed to try wintering together and never stopped', kind: 'organic' },
      { text: 'a miller who built a mill and found customers before they found customers', kind: 'organic' },
      { text: 'a retired tradesperson whose skills attracted dependents', kind: 'organic' },
      { text: 'a small religious community whose members drifted into secular life over two generations', kind: 'religious' },
    ],
    village: [
      { text: 'a group of settlers seeking new opportunities', kind: 'organic' },
      { text: 'refugees fleeing war or persecution who found the location defensible', kind: 'refuge' },
      { text: 'a noble granted lands by the crown and obligated to populate them', kind: 'charter' },
      { text: 'escaped serfs seeking a place beyond the reach of their former masters', kind: 'refuge' },
      { text: 'a community of tradespeople who left a larger settlement under contested circumstances', kind: 'organic' },
    ],
    town: [
      { text: 'merchants recognising economic potential before anyone else did', kind: 'organic' },
      { text: 'a noble granted lands by the crown with the resources to develop them', kind: 'charter' },
      { text: 'military veterans given land grants who brought their skills with them', kind: 'military' },
      { text: 'a merchant consortium that needed a waypoint and decided to own it', kind: 'charter' },
      { text: 'religious authorities who wanted a centre for their regional operations', kind: 'religious' },
    ],
    city: [
      { text: 'a powerful noble house that needed a commercial base independent of rivals', kind: 'charter' },
      { text: 'a merchant coalition that grew too large for the town they started in', kind: 'organic' },
      { text: 'a military command that built fortifications and found civilians followed', kind: 'military' },
      { text: 'royal decree and a century of forced investment', kind: 'planned' },
    ],
    metropolis: [
      { text: 'imperial decree and the systematic forced relocation of skilled populations', kind: 'planned' },
      { text: 'a dynastic decision that this crossroads would anchor the realm', kind: 'planned' },
      { text: 'the slow accretion of three smaller settlements that eventually merged under a single administration', kind: 'organic' },
      { text: 'a great trade house that outgrew every other settlement in the region', kind: 'charter' },
    ],
  };
  ...
  // ONE draw, two projections: the prose string and its typed kind (MF-0F above).
  //
  // ⛔ DRAW ORDER IS THE CONTRACT. These two `pick`s are hoisted out of the object
  // literal so the kind can be read off the same entry as the prose — and they are
  // hoisted IN SOURCE ORDER, because an object literal evaluates its values top to
  // bottom and this shared stream is positional. Drawing the founder before the reason
  // (the obvious way to write this) silently re-orders every remaining draw in the
  // history stream and turns a one-key shift into a whole-settlement one.
  const foundingReason = pick(reasonPool);
  const founder = pick(FOUNDERS_BY_TIER[tier] || FOUNDERS_BY_TIER.village);

  return {
    age: null, // filled in by historyGenerator
    reason: foundingReason,
    foundedBy: founder.text,
    kind: coerceFoundingKind(founder.kind),
