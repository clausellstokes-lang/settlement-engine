...
const inputStyle = (mobile) => ({
  fontSize: chromeFontSize(FS.xs, mobile), fontFamily: sans, color: INK, background: '#fff',
...
function TestRow({ row, index, settlements, onChange, onRemove }) {
  ...
  return (
    <div data-testid={`autonomy-test-${index}`} style={{ display: 'flex', alignItems: 'center', gap: SP.xs, flexWrap: 'wrap' }}>
      <select aria-label="Signal" value={row.signalId} style={inputStyle(mobile)}
      ...
      {entry && entry.scope !== 'world' && (
        <select aria-label="Settlement" value={row.settlementId || ''} style={inputStyle(mobile)}
      ...
      {entry && entry.scope === 'pair' && (
        <select aria-label="Other settlement" value={row.otherId || ''} style={inputStyle(mobile)}
      ...
      {entry?.type === 'number' && (
        <>
          <select aria-label="Comparison" value={row.op || 'gte'} style={inputStyle(mobile)} onChange={(ev) => set({ op: ev.target.value })}>
            <option value="gte">≥</option>
            <option value="lte">≤</option>
          </select>
          <input aria-label="Threshold" type="number" value={row.value ?? ''} style={{ ...inputStyle(mobile), width: 72 }}
            onChange={(ev) => set({ value: ev.target.value })} />
        </>
      )}
      {(entry?.type === 'band' || entry?.type === 'state') && (
        <select aria-label="Value" value={(row.values && row.values[0]) || ''} style={inputStyle(mobile)}
      ...
      {entry?.type === 'bool' && (
        <select aria-label="Is" value={String(row.is ?? true)} style={inputStyle(mobile)}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: SP.sm }}>
      ...
      {draftText == null ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: SP.xs, flexWrap: 'wrap' }}>
          <span style={{ fontSize: chromeFontSize(FS.xs, mobile), color: persisted ? BODY : MUTED, fontFamily: sans, flex: 1, minWidth: 120 }}>
          ...
          {savedNote && <span style={{ fontSize: chromeFontSize(FS.xs, mobile), color: GREEN, fontFamily: sans }}>{savedNote}</span>}
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: SP.xs }}>
          ...
          <div style={{ display: 'flex', gap: SP.xs }}>
            ...
            <span style={{ fontSize: chromeFontSize(FS.xs, mobile), color: MUTED, fontFamily: sans, alignSelf: 'center' }}>{(draftText || '').length}/{MAX_INSTRUCTIONS_CHARS}</span>
          </div>
        </div>
      )}
      ...
      {unsupported.length > 0 && (
        <div data-testid="autonomy-unsupported" style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {unsupported.map((u, i) => (
            <span key={i} style={{ fontSize: chromeFontSize(FS.xs, mobile), color: MUTED, fontFamily: sans }}>◇ {u.requested}: {u.reason}</span>
          ))}
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: SP.xs }}>
          ...
          {nudges.map((n, i) => (
            <div key={i} data-testid={`autonomy-nudge-${i}`} style={{
              ...
              <span style={{ fontSize: chromeFontSize(FS.xs, mobile), color: BODY, fontFamily: sans, flex: 1, minWidth: 140 }}>
      ...
      {condition && (
        <p data-testid="autonomy-condition" style={{ margin: 0, fontSize: proseFontSize(FS.xs, mobile), color: wall?.ok ? BODY : MUTED, fontFamily: sans, lineHeight: 1.5 }}>
          {describeStopCondition(condition)}
        </p>
      )}
      {wall && !wall.ok && (
        <p style={{ margin: 0, fontSize: proseFontSize(FS.xs, mobile), color: MUTED, fontFamily: sans }}>Not runnable yet: {wall.errors[0]}</p>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: SP.xs, flexWrap: 'wrap' }}>
        ...
        {rows.length > 1 && !composed && (
          <select aria-label="Combine tests with" value={combinator} style={inputStyle(mobile)} onChange={(ev) => setCombinator(ev.target.value)}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: SP.xs, flexWrap: 'wrap' }}>
        <label htmlFor="autonomy-max-weeks" style={{ fontSize: chromeFontSize(FS.xs, mobile), color: MUTED, fontFamily: sans }}>
          Max weeks{' '}
          <input id="autonomy-max-weeks" aria-label="Max weeks" type="number" min={1} max={AUTONOMOUS_ADVANCE_CAP_WEEKS}
            value={maxWeeks} style={{ ...inputStyle(mobile), width: 64 }}
      ...
      {run && (
        <div data-testid="autonomy-receipt" style={{ borderTop: `1px solid ${BORDER}`, paddingTop: SP.sm, display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {receipt ? (
            <>
              <p style={{ margin: 0, fontSize: proseFontSize(FS.xs, mobile), color: BODY, fontFamily: sans, lineHeight: 1.5 }}>
                {receipt.fired
              ...
              <p style={{ margin: 0, fontSize: proseFontSize(FS.xs, mobile), color: MUTED, fontFamily: sans, lineHeight: 1.5 }}>{receipt.conditionLabel}</p>
              {receipt.evaluations.map((ev, i) => (
                <span key={i} style={{ fontSize: chromeFontSize(FS.xs, mobile), color: ev.pass ? BODY : MUTED, fontFamily: sans }}>
            ...
            <p style={{ margin: 0, fontSize: proseFontSize(FS.xs, mobile), color: MUTED, fontFamily: sans }}>The run failed before it could stop cleanly. The world is unchanged past its last committed week.</p>
          )}
        </div>
      )}
    </div>
 ...
 * The autonomy run's progress readout and its stop RECEIPT. A receipt is an
 * exact account of what the operator asked for and what the world did: the
 * week it stopped on and how many it advanced. Rounding either into a phrase
 * would cost the receipt the only thing it is for.
 *
