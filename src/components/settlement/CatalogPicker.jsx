  ...
  return (
    <div style={{ marginTop:6, border:`1px solid ${BORDER}`, background:CARD_ALT, overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 8px', borderBottom:`1px solid ${BORDER}` }}>
        ...
        <input value={query} onChange={e => setQuery(e.target.value)} placeholder={placeholder || 'Search catalog...'} aria-label={placeholder || 'Search catalog'} autoFocus style={{ flex:1, border:'none', background:'transparent', fontSize:chromeFontSize(FS.xs, mobile), fontFamily:sans, color:INK, outline:'none' }}/>
        ...
        <div style={{ display:'flex', gap:3, padding:'4px 8px', flexWrap:'wrap', borderBottom:`1px solid ${BORDER}` }}>
          {['All', ...categoryFilters].map(c => (
            <Button key={c} variant={catFilter===c ? 'gold' : 'ghost'} size="sm" aria-pressed={catFilter===c} onClick={() => setCatFilter(c)}
              style={{ minHeight:'auto', padding:'1px 7px', fontSize:chromeFontSize(FS.micro, mobile), fontWeight:catFilter===c?700:500, border:`1px solid ${catFilter===c?GOLD:BORDER}`, color:catFilter===c?GOLD:SECOND }}>
              {c}
            </Button>
          ))}
        </div>
      ...
      <div style={{ maxHeight:200, overflowY:'auto', padding:4 }}>
        {filtered.length === 0 ? (
          <div style={{ padding:'8px 6px', fontSize:chromeFontSize(FS.xxs, mobile), color:MUTED, textAlign:'center' }}>No matching items</div>
        ) : filtered.map(item => (
          <button type="button" key={item.id || item.name} onClick={() => handlePick(item)}
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:chromeFontSize(FS.xs, mobile), fontWeight:600, color:INK, display:'flex', alignItems:'center', gap:4 }}>
                {item.name}
                {item.isCustom && <span style={{ fontSize:chromeFontSize(FS.nano, mobile), fontWeight:800, color:swatch['#7C3AED'], background:'rgba(124,58,237,0.12)', padding:'0 4px' }}>Custom</span>}
              </div>
              {item.desc && <div style={{ fontSize:chromeFontSize(FS.micro, mobile), color:MUTED, lineHeight:1.3, marginTop:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item.desc}</div>}
            </div>
            {item.category && <span style={{ fontSize:chromeFontSize(FS.micro, mobile), fontWeight:600, color:SECOND, background:`${SECOND}10`, padding:'1px 5px', flexShrink:0 }}>{item.category}</span>}
            {item.alreadyAdded && <span style={{ fontSize:chromeFontSize(FS.micro, mobile), color:MUTED, fontStyle:'italic' }}>Added</span>}
          </button>
        ))}
        {items.length > 30 && !query && <div style={{ padding:'4px 8px', fontSize:chromeFontSize(FS.micro, mobile), color:MUTED, textAlign:'center' }}>Search to see more...</div>}
      </div>
    </div>
  );
}
