...
export default function PhaseBadge() {
  ...
  const onCanonize = () => {
    if (!window.confirm(
      'Mark this settlement as canon? Future changes will be logged as in-world events with timeline entries.',
    )) return;
    canonize();
  ...
  const onReset = () => {
    if (!window.confirm(
      `Reset to draft and discard ${eventCount} timeline entr${eventCount === 1 ? 'y' : 'ies'}? This cannot be undone.`,
    )) return;
    uncanonize();
  };
