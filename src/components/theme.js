...
import { ARROW_CLEAR } from '../lib/chromeInsets.js';
...
//
// THE HEADER IS NOT HERE ANY MORE (owner orders 2026-09-16: the header is the
// owner's arrow painting). Its height scales with the page, so it is not a
// constant: components/nav/ArrowHeader.jsx writes the painted lengths onto the
// document element before first paint (--sf-header-h, --sf-arrow-hang,
// --sf-arrow-clear, --sf-arrow-barb-clear, --sf-bottom-nav-h; lib/chromeInsets.js
// names them) and every sticky or fixed surface composes them through the var()
// strings re-exported below (HEADER_H, ARROW_HANG, ARROW_CLEAR, ARROW_BARB_CLEAR,
// BOTTOM_NAV_H). The retired headerMobile / headerDesktop / scrollPadDesktop /
// mapShellOffset tokens described bars that no longer exist;
// tests/lint/arrowHeaderRetirement.test.js keeps them gone.
//
//   toolbarHeight: the dossier toolbar that pins under the header (~64px).
//   bottomNav: the bottom nav row (44px tap floor + borders); it shows below
//     1024 px, with five seats on phones and six from 640.
...
//   stickyTop: the sticky-aside breathing gap below the painted arrow.
 ...
 *
 * The header is `position:'sticky', top:0`, and the painted arrow's feather hangs
 * below it on a sticky layer of its own, so a fragment jump (a link click, a
 * translated deep link, or a programmatic `scrollIntoView`) would park the target
 * heading UNDERNEATH the painting unless the target names a scroll margin. DERIVED
 * from the painted lengths rather than copied: the header plus the whole hang
 * (ARROW_CLEAR) plus SP.xxl of air, so a landed heading clears the feather at every
 * width and the landing moves with the arrow's scale.
 *
 * ⚠️ NEVER PIN THE SUM. Every assertion about this value must assert the derivation
 * (`calc(${ARROW_CLEAR} + ${SP.xxl}px)`) and not a number it happens to evaluate to
 * at one width. (V2 directive §1.)
 *
 * It lives here because more than one page family needs it: the About family's two
 * pages, the Compendium's registry and catalog hubs, and the dossier's entity
 * anchors all read this one export, so the estate has ONE anchor offset.
 */
export const ANCHOR_OFFSET = `calc(${ARROW_CLEAR} + ${SP.xxl}px)`;
 ...
 *   HEADER_HEIGHT_VAR     the painted header band's height (ArrowHeader writes it)
