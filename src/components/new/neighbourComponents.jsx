    ...
    <div style={{border:`1px solid ${st.border}`,borderLeft:`3px solid ${st.color}`,borderRadius:8,overflow:'hidden'}}>
      <button onClick={()=>setOpen(v=>!v)} style={{width:'100%',display:'flex',alignItems:'center',gap:10,padding:'10px 14px',background:open?st.bg:'#faf8f4',border:'none',cursor:'pointer',textAlign:'left',WebkitTapHighlightColor:'transparent'}}>
        <div style={{flex:1}}>
          <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
            <span style={{...serif,fontSize:FS.lg,fontWeight:700,color:'#1c1409'}}>{link.neighbourName||link.name}</span>
            ...
            {link.neighbourTier&&<span style={{fontSize:FS.xs,color:'#9c8068'}}>{link.neighbourTier}</span>}
          </div>
          {link.sharedHistory&&<p style={{fontSize:FS.sm,color:'#6b5340',margin:'4px 0 0',lineHeight:1.4}}>{link.sharedHistory}</p>}
        </div>
        <span style={{fontSize:FS.xs,color:'#9c8068',flexShrink:0}}>{open?'▲':'▼'}</span>
      </button>
      {open&&<div style={{padding:'12px 14px',background:'rgba(250,248,244,0.97)',borderTop:`1px solid ${st.border}`}}>
        {link.cause&&<div style={{marginBottom:10}}>
          <div style={{fontSize:FS.xxs,fontWeight:700,color:'#6b5340',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:4}}>Cause</div>
          <p style={{fontSize:FS.sm,color:'#3d2b1a',lineHeight:1.5,margin:0}}>{link.cause}</p>
        </div>}
        {link.diplomaticStatus&&<div style={{marginBottom:10}}>
          <div style={{fontSize:FS.xxs,fontWeight:700,color:'#6b5340',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:4}}>Diplomatic Status</div>
          <p style={{fontSize:FS.sm,color:'#3d2b1a',lineHeight:1.5,margin:0}}>{link.diplomaticStatus}</p>
        </div>}
        {(link.tensions?.length>0||link.opportunities?.length>0)&&<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:10,marginBottom:10}}>
          {link.tensions?.length>0&&<div style={{background:'#fdf4f4',border:'1px solid #e8c0c0',borderRadius:6,padding:'8px 10px'}}>
            <div style={{fontSize:FS.xxs,fontWeight:700,color:'#8b1a1a',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:5}}>Tensions</div>
            {link.tensions.map((t,i)=><div key={i} style={{fontSize:FS.sm,color:'#3d2b1a',marginBottom:3,lineHeight:1.4}}>▸ {t}</div>)}
          </div>}
          {link.opportunities?.length>0&&<div style={{background:'#f0faf2',border:'1px solid #a8d8b0',borderRadius:6,padding:'8px 10px'}}>
            <div style={{fontSize:FS.xxs,fontWeight:700,color:'#1a5a28',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:5}}>Opportunities</div>
            {link.opportunities.map((o,i)=><div key={i} style={{fontSize:FS.sm,color:'#1c1409',marginBottom:3,lineHeight:1.4}}>▸ {o}</div>)}
          </div>}
        </div>}
        {link.npcConnections?.length>0&&<div>
          <div style={{fontSize:FS.xxs,fontWeight:700,color:'#2a3a7a',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:6}}>NPC Connections</div>
          {link.npcConnections.map((conn,i)=>(
            <div key={i} style={{background:'#f0f4ff',border:'1px solid #c0c8e8',borderLeft:'3px solid #2a3a7a',borderRadius:6,padding:'10px 12px',marginBottom:6}}>
              ...
              {conn.description&&<p style={{fontSize:FS.sm,color:'#1c1409',lineHeight:1.5,margin:0}}>{conn.description}</p>}
            </div>
          ))}
        </div>}
        {link.tradeGoods?.length>0&&<div style={{marginTop:8}}>
          <div style={{fontSize:FS.xxs,fontWeight:700,color:'#6b5340',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:5}}>Trade Goods</div>
        ...
        {open&&(()=>{
          ...
          return _isr.length>0 ? <div style={{marginTop:10,borderTop:`1px solid ${_c}20`,paddingTop:8}}>
            ...
            {_isr.map((rx,i)=><div key={i} style={{fontSize:FS.xs,color:'#3d2b1a',marginBottom:4,lineHeight:1.4,paddingLeft:8,borderLeft:`2px solid ${_c}40`}}>
              <strong>{rx.npcName}</strong> ({rx.npcRole}) ↔ <strong style={{color:_c}}>{rx.partnerName}</strong> ({rx.partnerRole})
              {rx.description&&<div style={{fontSize:FS.xxs,color:'#6b5340',marginTop:1,fontStyle:'italic'}}>{rx.description}</div>}
            </div>)}
          </div> : null;
        })()}
      </div>}
