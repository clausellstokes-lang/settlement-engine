    ...
    <div style={{...sans}}>
          ...
          {res.economicStrengths?.length>0&&<div style={{flex:'2 1 200px'}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
              {[...res.economicStrengths].sort((a,b)=>(a||'').localeCompare(b||'')).map((s,i)=>(
                <span key={i} style={{fontSize:FS.xs,fontWeight:600,color:'#1a5a28',background:'#e0f0e4',border:'1px solid #a8d8b0',borderRadius:4,padding:'2px 9px'}}>✓ {s}</span>
              ))}
            </div>
          </div>}
        </div>
          ...
          {unexploited.map((chain,i)=>(
            <div key={i} style={{background:'#fdf8e8',border:'1px solid #e0c060',borderLeft:'3px solid #b8860b',borderRadius:6,padding:'10px 14px'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6,flexWrap:'wrap'}}>
                <span style={{fontSize:FS.micro,fontWeight:800,color:'#7a5010',background:'#f5e8c0',borderRadius:3,padding:'1px 6px',letterSpacing:'0.05em'}}>UNEXPLOITED</span>
                <span style={{fontSize:FS.md,fontWeight:700,color:'#1c1409',textTransform:'capitalize'}}>{chain.rawResource}</span>
              ...
              <div style={{display:'flex',alignItems:'center',gap:4,flexWrap:'wrap',marginBottom:6}}>
                <span style={{fontSize:FS.xs,color:'#3d2b1a',background:'#f0ead8',borderRadius:3,padding:'1px 6px',fontWeight:600,textTransform:'capitalize'}}>{chain.rawResource}</span>
                {(chain.intermediateGoods||[]).map((g,j)=>(
                  <React.Fragment key={j}>
                    <span style={{fontSize:FS.xxs,color:'#9c8068'}}>→</span>
                    <span style={{fontSize:FS.xs,color:'#3d2b1a',background:'#f0ead8',borderRadius:3,padding:'1px 6px'}}>{g}</span>
                  </React.Fragment>
                ))}
                {(chain.finalProducts||[]).slice(0,2).map((g,j)=>(
                  <React.Fragment key={j}>
                    <span style={{fontSize:FS.xxs,color:'#9c8068'}}>→</span>
                    <span style={{fontSize:FS.xs,color:'#1a5a28',background:'#e8f5ec',border:'1px solid #a8d8b0',borderRadius:3,padding:'1px 6px',fontWeight:600}}>{g}</span>
                  </React.Fragment>
                ))}
              </div>
              {chain.processingInstitutions?.length>0&&<div style={{fontSize:FS.xs,color:'#5a3a10'}}>
          ...
          {partExploited.map((chain,i)=>(
            <div key={i} style={{background:'#f4faf4',border:'1px solid #a8d8b0',borderLeft:'3px solid #5a6a1a',borderRadius:6,padding:'10px 14px'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}>
                <span style={{fontSize:FS.micro,fontWeight:800,color:'#3a5a1a',background:'#d8ecd8',borderRadius:3,padding:'1px 6px',letterSpacing:'0.05em'}}>PARTIAL</span>
                <span style={{fontSize:FS.md,fontWeight:700,color:'#1c1409',textTransform:'capitalize'}}>{chain.rawResource}</span>
                <span style={{fontSize:FS.xxs,fontWeight:600,color:valColor(chain.exportValue),background:valBg(chain.exportValue),borderRadius:3,padding:'0 5px'}}>{chain.exportValue} value</span>
              </div>
              <div style={{display:'flex',alignItems:'center',gap:4,flexWrap:'wrap'}}>
                <span style={{fontSize:FS.xs,color:'#3d2b1a',background:'#f0ead8',borderRadius:3,padding:'1px 6px',fontWeight:600,textTransform:'capitalize'}}>{chain.rawResource}</span>
                {(chain.intermediateGoods||[]).map((g,j)=>(
                  <React.Fragment key={j}><span style={{fontSize:FS.xxs,color:'#9c8068'}}>→</span><span style={{fontSize:FS.xs,color:'#3d2b1a',background:'#f0ead8',borderRadius:3,padding:'1px 6px'}}>{g}</span></React.Fragment>
                ))}
              </div>
            </div>
          ))}
          ...
          {fullExploited.map((chain,i)=>(
            <div key={i} style={{background:'#f0faf2',border:'1px solid #a8d8b0',borderLeft:'3px solid #1a5a28',borderRadius:6,padding:'8px 14px'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                <span style={{fontSize:FS.micro,fontWeight:800,color:'#1a5a28',background:'#c8ecd4',borderRadius:3,padding:'1px 6px',letterSpacing:'0.05em'}}>✓ FULLY EXPLOITED</span>
                <span style={{fontSize:FS.sm,fontWeight:700,color:'#1c1409',textTransform:'capitalize'}}>{chain.rawResource}</span>
                <span style={{fontSize:FS.xxs,fontWeight:600,color:valColor(chain.exportValue),background:valBg(chain.exportValue),borderRadius:3,padding:'0 5px'}}>{chain.exportValue} value</span>
              </div>
            </div>
          ))}
        ...
        {(() => {
          ...
          return <>
            {depleted.length>0&&<div style={{marginBottom:8}}>
              <div style={{fontSize:FS.xxs,fontWeight:700,color:'#c05000',letterSpacing:'0.06em',marginBottom:4}}> DEPLETED — consumed locally, export potential reduced</div>
              <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
                {depleted.map((rk,i)=><span key={i} style={{fontSize:FS.xs,color:'#8b3000',background:'#fff3ed',border:'1px solid #e08040',borderRadius:4,padding:'2px 9px',fontWeight:600}}> {fmtKey(rk)}</span>)}
              </div>
            </div>}
            {abundant.length>0&&<div style={{marginBottom:res.availableResources?.length>0?8:0}}>
              <div style={{fontSize:FS.xxs,fontWeight:700,color:'#1a5a28',letterSpacing:'0.06em',marginBottom:4}}>✦ ABUNDANT — full export potential</div>
              <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
                {abundant.map((rk,i)=><span key={i} style={{fontSize:FS.xs,color:'#1a5a28',background:'#f0faf2',border:'1px solid #88c880',borderRadius:4,padding:'2px 9px'}}>{fmtKey(rk)}</span>)}
              </div>
            </div>}
            {res.availableResources?.length>0&&<div style={{paddingTop:6,borderTop:'1px solid #e8dcc8'}}>
              <div style={{fontSize:FS.xxs,fontWeight:700,color:'#9c8068',letterSpacing:'0.06em',marginBottom:4}}>COMMODITIES AVAILABLE</div>
              <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
                {res.availableResources.map((r2,i)=><span key={i} style={{fontSize:FS.xs,color:'#3d2b1a',background:'#f0ead8',border:'1px solid #d8c890',borderRadius:4,padding:'2px 9px',textTransform:'capitalize'}}>{r2.replace(/_/g,' ')}</span>)}
              </div>
            </div>}
          </>;
        })()}
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
          {[...res.exports].sort((a,b)=>{
            ...
            return <div key={i} style={{background:'#f0faf2',border:'1px solid #a8d8b0',borderLeft:`3px solid ${valColor(v)}`,borderRadius:5,padding:'6px 10px',minWidth:0}}>
              <div style={{fontSize:FS.sm,fontWeight:700,color:'#1c1409'}}>{e.product||e.good||e.name}</div>
              {e.reason&&<div style={{fontSize:FS.xxs,color:'#6b5340',marginTop:2}}>{e.reason}</div>}
            </div>;
          })}
        </div>
      </Section>}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:6}}>
          ...
          {(res.gaps||[]).map((g,i)=>{
            ...
            return <div key={i} style={{display:'flex',gap:8,padding:'8px 12px',background:gbg,border:`1px solid ${gc}40`,borderLeft:`3px solid ${gc}`,borderRadius:5}}>
              ...
              <div style={{flex:1}}>
                {chain&&<span style={{fontSize:FS.xs,fontWeight:700,color:'#1c1409',textTransform:'capitalize',marginRight:6}}>{chain}:</span>}
                <span style={{fontSize:FS.sm,color:'#3d2b1a'}}>{impact}</span>
              </div>
            </div>;
          })}
        </div>
      ...
      {res.featureEffects?.length>0&&<Section title="Terrain Effects" collapsible defaultOpen={false}>
        {res.featureEffects.map((e,i)=>(
          <div key={i} style={{padding:'6px 0',borderBottom:'1px solid #f0e8d8',fontSize:FS.sm,color:'#3d2b1a'}}>
