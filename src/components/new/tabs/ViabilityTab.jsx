    ...
    <div style={{...sans}}>
      ...
      <div style={{
        ...
        <div style={{display:'flex',alignItems:'flex-start',gap:12,flexWrap:'wrap'}}>
          <div style={{flex:1}}>
            ...
            {summaryClean&&<p style={{fontSize:FS.md,color:'#3d2b1a',lineHeight:1.55,margin:0}}>{summaryClean}</p>}
            <p style={{fontSize:FS.xs,color:'#6b5340',margin:'6px 0 0',lineHeight:1.4}}>
          ...
          <div style={{display:'flex',gap:6,flexWrap:'wrap',flexShrink:0}}>
            {metrics.criticalIssueCount>0&&<span style={{fontSize:FS.xs,fontWeight:700,color:'#8b1a1a',background:'#fde8e8',border:'1px solid #f0a0a0',borderRadius:4,padding:'3px 9px'}}>{metrics.criticalIssueCount} critical</span>}
      ...
      {s?.defenseProfile?.magicDependency&&(
        <div style={{background:'#f8f0ff',border:'1px solid #c0a0e0',borderLeft:'4px solid #7a3a9a',
          ...
          <div style={{fontSize:FS.sm,fontWeight:700,color:'#5a2a8a',marginBottom:4}}>
            ✦ Magic Dependency Detected
          </div>
          <div style={{fontSize:FS.xs,color:'#6b5340',lineHeight:1.5}}>
          ...
          {(s.economicState?.activeChains||[]).filter(c=>c.magicNote).map((c,i)=>(
            <div key={i} style={{fontSize:FS.xxs,color:'#7a4aaa',marginTop:6,paddingLeft:8,
      ...
      {structViolations.length>0&&<Section title={` Structural Crises (${structViolations.length})`} collapsible defaultOpen accent='#8b1a1a'>
        {structViolations.map((v2,i)=>(
          <div key={i} style={{background:'#fdf0f0',border:'1px solid #e0a0a0',borderLeft:'4px solid #8b1a1a',borderRadius:6,padding:'10px 14px',marginBottom:8}}>
            <div style={{fontSize:FS.xs,fontWeight:700,color:'#8b1a1a',marginBottom:3}}>
              {v2.institution||v2.group}
            </div>
            <p style={{fontSize:FS.md,color:'#5a1a1a',lineHeight:1.5,margin:0}}>{v2.reason}</p>
          </div>
        ))}
      </Section>}
            ...
            {issue.suggestedFixes?.length>0&&<div style={{marginTop:6}}>
              {issue.suggestedFixes.map((fix,j)=>(
                <div key={j} style={{display:'flex',gap:6,marginBottom:2}}>
                  <span style={{color:'#1a5a28',flexShrink:0,fontSize:FS.xs}}>→</span>
                  <span style={{fontSize:FS.xs,color:'#3d2b1a'}}>{fix}</span>
                </div>
              ))}
            </div>}
          </div>;
      ...
      {stressConsequences.length>0&&<Section title={`⚡ Active Stress Effects (${stressConsequences.length})`} collapsible defaultOpen={true} accent='#6b4c2a'>
        <p style={{fontSize:FS.sm,color:'#5a3e28',lineHeight:1.5,margin:'0 0 10px',fontStyle:'italic'}}>
