    ...
    <Section title="DM Compass" accent="#6a2a9a">
      {hooks.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: FS.xxs, fontWeight: 800, color: '#6a2a9a', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
            Adventure hooks
          </div>
          {hooks.map((h, i) => (
      ...
      {redFlags.length > 0 && (
        <div style={{ marginBottom: twist ? 12 : 0 }}>
          <div style={{ fontSize: FS.xxs, fontWeight: 800, color: '#8b1a1a', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
            Red flags
          </div>
          {redFlags.map((r, i) => (
      ...
      {twist && (
        <div>
          <div style={{ fontSize: FS.xxs, fontWeight: 800, color: '#a0762a', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
...
function IdentityMarkersPanel({ markers }) {
  ...
  return (
    <Section title="Identity Markers" accent="#1a5a28">
      <div style={{ fontSize: FS.xxs, color: '#6b5340', marginBottom: 6, fontStyle: 'italic', fontFamily: 'Nunito, sans-serif' }}>
...
function FrictionPointsPanel({ points }) {
  ...
  return (
    <Section title="Friction Points" accent="#a0762a">
      <div style={{ fontSize: FS.xxs, color: '#6b5340', marginBottom: 8, fontStyle: 'italic', fontFamily: 'Nunito, sans-serif' }}>
      ...
      {points.map((p, i) => (
        <div key={i} style={{ display: 'flex', gap: 10, padding: '7px 10px', marginBottom: 5, background: 'rgba(160,118,42,0.06)', border: '1px solid rgba(160,118,42,0.18)', borderRadius: 5 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: FS.xs, fontWeight: 700, color: '#1c1409', fontFamily: 'Nunito, sans-serif', marginBottom: 2 }}>
              {p.who}
            </div>
            <div style={{ fontSize: FS.sm, color: '#3d2b1a', lineHeight: 1.5, fontFamily: 'Georgia, serif' }}>
              {p.what}
            </div>
          </div>
        </div>
    ...
    <Section title="Connections Map" accent="#2a3a7a">
      ...
      {edges.map((e, i) => (
        <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '5px 10px', borderBottom: i < edges.length - 1 ? '1px solid #e0d0b0' : 'none', flexWrap: 'wrap' }}>
          ...
          {e.via && (
            <>
              <span style={{ fontSize: FS.xxs, color: '#9c8068' }}>via</span>
              <span style={{ fontSize: FS.xs, color: '#3d2b1a', fontFamily: 'Nunito, sans-serif' }}>{e.via}</span>
            </>
          )}
...
export default function DMCompassTab({ settlement: s }) {
  ...
  if (!anyContent) {
    ...
    return (
      <div style={{ padding: 24, textAlign: 'center', color: '#9c8068', fontSize: FS.sm, fontFamily: 'Nunito, sans-serif' }}>
