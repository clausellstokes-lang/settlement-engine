...
/**
 * The active-page rule's top row: below every descender, above the shaft's lower edge. On
...
export function layoutArrow({ clientWidth, full, short = false }) {
  ...
  if (full) {
    ...
    for (const [id, span] of Object.entries(NAV_WORD)) {
      underline[id] = rect(span, UNDERLINE_ROW, 2);
