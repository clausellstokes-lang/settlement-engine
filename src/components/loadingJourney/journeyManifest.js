 ...
 *
 * The taste walk selected the `bg` masters: six 5.04s legs (CRF 25-27,
 * ~42 MB, all-keyframe 121/121 I). They live under
 * public/media/journey-legs/bg/. The shorter comparison set was retired.
 */
...
const MEDIA_ROOT = '/media/journey-legs/bg';
...
/**
 * legVideoUrl(n) — URL of travel leg n (1-based). Leg n travels from
 ...
 */
export function legVideoUrl(n) {
  const from = JOURNEY_STAGES[n - 1];
  const to = JOURNEY_STAGES[n];
  return `${MEDIA_ROOT}/leg-${n}-${from}-to-${to}.mp4`;
}

/**
 * stopStillUrl(stopIndex) — URL of the crisp stop still at stopIndex
 ...
 */
export function stopStillUrl(stopIndex) {
  return `${MEDIA_ROOT}/still-${stopIndex}-${JOURNEY_STAGES[stopIndex]}.jpg`;
}
