/**
 * lib/contentPacks.js — export / import of custom-content "packs".
 *
 ...
 *
 * File-based only — NO backend, NO migration, NO server round-trip. The pack is
 * a downloadable/uploadable JSON blob; import commits through the normal
 * `addCustomItem` store action.
 *
 * Two hard guarantees on IMPORT (the test gate):
 *
 *   1. RE-VALIDATION — every deity is re-run through `validateDeity` (the same
 *      check the store applies); an invalid deity is rejected, never silently
 *      written. Other buckets are shape-checked (must be a named object).
 *
